...
async fn build_services(
    ...
    let env = if env::var_os("CARGO_BUILD_JOBS").is_none() {
        vec![(
            ...
            Some(env::var("AETHERNET_DEV_CARGO_JOBS").unwrap_or_else(|_| "2".to_owned())),
        )]
    } else {
        Vec::new()
    };
...
pub fn service_command(spec: &RustServiceSpec) -> Vec<String> {
    if env::var("AETHERNET_DEV_RUST_HOT_RELOAD")
        ...
        .unwrap_or(false)
    {
        ...
        return vec![
            ...
            "-w".to_owned(),
            "aethernet_svc/src".to_owned(),
            "-w".to_owned(),
            "aethernet_svc/Cargo.toml".to_owned(),
            "-x".to_owned(),
...
pub fn service_env(spec: &RustServiceSpec, mode: &str, port: u16) -> Vec<(String, Option<String>)> {
    let mut envs = vec![
        ("AETHERNET_SVC_NAME".to_owned(), Some(spec.name.to_owned())),
        ("AETHERNET_SVC_MODE".to_owned(), Some(mode.to_owned())),
        ("AETHERNET_SVC_PORT".to_owned(), Some(port.to_string())),
        (
            "AETHERNET_SVC_LISTEN_HOST".to_owned(),
            Some("0.0.0.0".to_owned()),
        ),
        ("AETHERNET_SVC_SHARD_COUNT".to_owned(), Some("1".to_owned())),
        (
            "AETHERNET_SVC_NATS_URL".to_owned(),
            Some(
                env::var("AETHERNET_SVC_NATS_URL")
                    .or_else(|_| env::var("AETHERNET_NATS_URL"))
                    .unwrap_or_else(|_| "nats://nats:4222".to_owned()),
            ),
        ),
    ];
    if mode == "shard" {
        envs.push(("AETHERNET_SVC_SHARD_ID".to_owned(), Some("0".to_owned())));
    }
    if spec.name == "unfurl" {
        envs.extend([
            (
                "AETHERNET_MEDIA_PROXY_ENDPOINT".to_owned(),
                Some(
                    env::var("AETHERNET_MEDIA_PROXY_ENDPOINT")
            ...
            (
                ...
                Some(
                    env::var("AETHERNET_MEDIA_PROXY_PUBLIC_ENDPOINT")
                        .or_else(|_| env::var("AETHERNET_MEDIA_ENDPOINT"))
            ...
            (
                ...
                Some(
                    env::var("AETHERNET_STATIC_CDN_ENDPOINT")
                        .or_else(|_| env::var("AETHERNET_PUBLIC_URL"))
...
fn proc_has_managed_service_environment(pid: i32, services: &[RustServiceSpec]) -> bool {
    ...
    let Some(name) = environment.get("AETHERNET_SVC_NAME") else {
        return false;
    };
    let Some(mode) = environment.get("AETHERNET_SVC_MODE") else {
        return false;
    };
    let Some(port) = environment
        .get("AETHERNET_SVC_PORT")
        .and_then(|port| port.parse::<u16>().ok())
    else {
        return false;
    };
    if environment.get("AETHERNET_SVC_LISTEN_HOST") != Some(&"0.0.0.0") {
        return false;
    }
...
mod tests {
    ...
    fn builds_unfurl_env_with_media_endpoints() {
        with_public_env(&[], |env| {
            assert!(
                env.iter()
                    .any(|(key, value)| key == "AETHERNET_MEDIA_PROXY_PUBLIC_ENDPOINT"
                        && value.as_deref() == Some("http://localhost:8088/media"))
            );
            assert!(
                env.iter()
                    .any(|(key, value)| key == "AETHERNET_STATIC_CDN_ENDPOINT"
    ...
    fn cmdline_binary_match_uses_selected_service_binary_suffix() {
        let binaries = BTreeSet::from(["target/debug/aethernet_messages".to_owned()]);
        assert!(cmdline_has_service_binary(
            &["/workspaces/aethernet/target/debug/aethernet_messages".to_owned()],
            &binaries
        ));
        assert!(!cmdline_has_service_binary(
            &["/workspaces/aethernet/target/debug/aethernet_users".to_owned()],
            &binaries
        ));
    }
