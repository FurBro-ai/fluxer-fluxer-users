...
pub async fn run_dev(task_names: &[String], cloudflare_tunnel: bool, low_memory: bool) -> Result<i32> {
    let mut tasks = task_table()?;
    if low_memory {
        apply_low_memory_profile(&mut tasks);
        println!("Aethernet low-memory profile enabled (1 build job, bounded JS heaps, compact workers).");
    }
...
fn set_task_env(tasks: &mut BTreeMap<&'static str, DevTask>, task: &str, name: &str, value: &str) {
    if let Some(spec) = tasks.get_mut(task) {
        spec.env.retain(|(key, _)| key != name);
        spec.env.push((name.to_owned(), Some(value.to_owned())));
    }
}

fn apply_low_memory_profile(tasks: &mut BTreeMap<&'static str, DevTask>) {
    set_task_env(tasks, "services", "AETHERNET_DEV_CARGO_JOBS", "1");
    for task in ["admin", "media", "app-proxy"] {
        set_task_env(tasks, task, "CARGO_BUILD_JOBS", "1");
    }
    for task in ["api", "worker"] {
        set_task_env(tasks, task, "NODE_OPTIONS", "--max-old-space-size=512");
    }
    set_task_env(tasks, "worker", "AETHERNET_API_WORKER_LANE_CONCURRENCY_OVERRIDES", r#"{"realtime":2,"unfurl":2,"lifecycle":1,"batch":2}"#);
    set_task_env(tasks, "app", "NODE_OPTIONS", "--max-old-space-size=1024");
    set_task_env(tasks, "app", "TOKIO_WORKER_THREADS", "1");
    set_task_env(tasks, "app", "RAYON_NUM_THREADS", "1");
    set_task_env(tasks, "app", "AETHERNET_LOW_MEMORY", "true");
    set_task_env(tasks, "app", "AETHERNET_APP_SKIP_CSS_TYPE_WATCHER", "true");
}
...
async fn monitor_object_store_dependency() -> Result<()> {
    ...
    {
        bail!(
            "Dev object store is unreachable and automatic repair failed: {error}\nRun `aethernet-dev media-proxy doctor --repair` inside the devcontainer, then restart `aethernet-dev dev`."
        );
    }
...
pub fn task_table() -> Result<BTreeMap<&'static str, DevTask>> {
    ...
    let api_dir = ROOT.join("aethernet_api");
    let api_tsx = api_dir.join("node_modules/.bin/tsx");
    let app_dir = ROOT.join("aethernet_app");
    let app_dev_server = ROOT.join("tools/ci/run.sh");
    let public_url = public_url();
    let admin_endpoint =
        std::env::var("AETHERNET_ADMIN_ENDPOINT").unwrap_or_else(|_| format!("{public_url}/admin"));
    ...
    insert(DevTask {
        ...
        env: vec![
            (
                "AETHERNET_S3_PUBLIC_ENDPOINT".to_owned(),
                Some(public_url.clone()),
            ),
            (
                "AETHERNET_S3_FORCE_PATH_STYLE".to_owned(),
                Some(
                    std::env::var("AETHERNET_S3_FORCE_PATH_STYLE")
                        .unwrap_or_else(|_| "true".to_owned()),
                ),
            ),
            (
                "AETHERNET_DISABLE_RATE_LIMITS".to_owned(),
                Some("true".to_owned()),
            ),
            (
                "AETHERNET_RELAX_REGISTRATION_RATE_LIMITS".to_owned(),
                Some("true".to_owned()),
            ),
        ],
    });
    ...
    insert(DevTask {
        ...
        env: vec![
            ...
            ("AETHERNET_APP_DEV_PORT".to_owned(), Some(APP_PORT.to_string())),
            (
                "AETHERNET_STATIC_CDN_ENDPOINT".to_owned(),
                Some(public_url.clone()),
            ),
            (
    ...
    insert(DevTask {
        name: "app-proxy",
        args: strings(&["cargo", "run", "-p", "aethernet_app_proxy"]),
        cwd: ROOT.clone(),
        env: vec![
            (
                "DISCOVERY_UPSTREAM_URL".to_owned(),
                Some(format!(
                    "http://{LOOPBACK_HOST}:{DEV_PROXY_PORT}/api/.well-known/aethernet"
                )),
            ),
            (
                "AETHERNET_APP_PROXY_INDEX_UPSTREAM_URL".to_owned(),
                Some(format!("http://{LOOPBACK_HOST}:{APP_PORT}/")),
            ),
            (
                "AETHERNET_APP_PROXY_PORT".to_owned(),
            ...
            (
                "AETHERNET_STATIC_DIR".to_owned(),
                Some("aethernet_static".to_owned()),
            ),
    ...
    insert(DevTask {
        ...
        args: strings(&[
            ...
            "aethernet_admin/src",
            "-w",
            "aethernet_admin/Cargo.toml",
            "-w",
            "aethernet_admin/build.rs",
            "-w",
            "aethernet_admin/openapi-admin.json",
            "-w",
            "aethernet_admin/static",
            "-x",
            "run -p aethernet_admin",
        ]),
        cwd: ROOT.clone(),
        env: vec![
            (
                "AETHERNET_ADMIN_BASE_PATH".to_owned(),
                Some("/admin".to_owned()),
            ),
            ("AETHERNET_ADMIN_ENDPOINT".to_owned(), Some(admin_endpoint)),
            ("AETHERNET_ADMIN_PORT".to_owned(), Some(ADMIN_PORT.to_string())),
            (
                "AETHERNET_API_ENDPOINT".to_owned(),
                Some(format!("{public_url}/api")),
            ),
            ("AETHERNET_APP_ENDPOINT".to_owned(), Some(public_url.clone())),
            (
                "AETHERNET_MEDIA_ENDPOINT".to_owned(),
                Some(format!("{public_url}/media")),
            ),
            ("AETHERNET_STATIC_CDN_ENDPOINT".to_owned(), Some(public_url)),
            ("RELEASE_CHANNEL".to_owned(), Some("canary".to_owned())),
        ],
    });
    insert(DevTask {
...
async fn wait_for_search_backend_if_needed(selected: &[String]) -> Result<()> {
    ...
    let search_url = env
        .get("AETHERNET_SEARCH_URL")
        .filter(|url| !url.trim().is_empty())
        .context("Missing AETHERNET_SEARCH_URL for the selected search backend")?;
...
impl SearchBackend {
    fn from_env(env: &BTreeMap<String, String>) -> Result<Self> {
        match env.get("AETHERNET_SEARCH_ENGINE").map(String::as_str) {
            Some("elasticsearch") => Ok(Self::Elasticsearch),
            Some("meilisearch") => Ok(Self::Meilisearch),
            Some(engine) => bail!(
                "Unsupported AETHERNET_SEARCH_ENGINE value {engine:?}; expected `meilisearch` or `elasticsearch`"
            ),
            None => {
                bail!("Missing AETHERNET_SEARCH_ENGINE; expected `meilisearch` or `elasticsearch`")
            }
        }
    }
...
fn public_url() -> String {
    std::env::var("AETHERNET_PUBLIC_URL")
...
fn object_store_monitor_interval() -> Duration {
    let seconds = std::env::var("AETHERNET_DEV_OBJECT_STORE_MONITOR_INTERVAL_SECS")
        .ok()
...
async fn wait_for_rust_services(processes: &mut [Child]) -> Result<()> {
    let timeout = std::env::var("AETHERNET_DEV_RUST_SERVICE_READY_TIMEOUT")
        .ok()
...
async fn wait_for_gateway(processes: &mut [Child], port: u16) -> Result<()> {
    let timeout = std::env::var("AETHERNET_DEV_GATEWAY_READY_TIMEOUT")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(180);
...
async fn wait_for_api(processes: &mut [Child]) -> Result<()> {
    let timeout = std::env::var("AETHERNET_DEV_API_READY_TIMEOUT")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(180);
...
async fn wait_for_cloudflare_tunnel_routes(
    ...
    let timeout = std::env::var("AETHERNET_CLOUDFLARE_TUNNEL_ROUTE_READY_TIMEOUT")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(300);
...
mod tests {
    ...
    #[test]
    fn low_memory_profile_bounds_expensive_tasks() {
        let mut tasks = task_table().unwrap();
        apply_low_memory_profile(&mut tasks);
        let value = |task: &str, key: &str| {
            tasks[task]
                .env
                .iter()
        ...
        assert_eq!(value("services", "AETHERNET_DEV_CARGO_JOBS"), "1");
        assert_eq!(value("app", "TOKIO_WORKER_THREADS"), "1");
        assert_eq!(value("app", "AETHERNET_LOW_MEMORY"), "true");
        assert_eq!(value("app", "AETHERNET_APP_SKIP_CSS_TYPE_WATCHER"), "true");
        assert_eq!(value("worker", "NODE_OPTIONS"), "--max-old-space-size=512");
    }
    ...
    fn task_table_contains_recursive_binary_tasks() {
        ...
        assert_eq!(
            tasks["gateway"]
                .env
                .iter()
                .find(|(key, _)| key == "AETHERNET_DISABLE_RATE_LIMITS")
                .unwrap()
                .1
                .as_deref(),
            Some("true")
        ...
        assert_eq!(
            tasks["api"]
                .env
                .iter()
                .find(|(key, _)| key == "AETHERNET_S3_PUBLIC_ENDPOINT")
                .unwrap()
                .1
                .as_deref(),
        ...
        assert_eq!(
            tasks["api"]
                .env
                .iter()
                .find(|(key, _)| key == "AETHERNET_S3_FORCE_PATH_STYLE")
                .unwrap()
                .1
                .as_deref(),
            Some("true")
        ...
        assert!(crate::manifest::PROXY_ROUTES.iter().any(|route| {
            route.prefix == "/aethernet-uploads" && route.host == LOOPBACK_HOST && route.port == 8333
        }));
        assert!(crate::manifest::PROXY_ROUTES.iter().any(|route| {
            route.prefix == "/admin"
        ...
        assert_eq!(
            tasks["app"]
                .env
                .iter()
                .find(|(key, _)| key == "AETHERNET_APP_DEV_PORT")
                .unwrap()
                .1
                .as_deref(),
        ...
        assert_eq!(
            tasks["app"]
                .env
                .iter()
                .find(|(key, _)| key == "AETHERNET_STATIC_CDN_ENDPOINT")
                .unwrap()
                .1
                .as_deref(),
            Some(expected_public_url.as_str())
        ...
        assert_eq!(
            tasks["app-proxy"]
                .env
                .iter()
                .find(|(key, _)| key == "AETHERNET_APP_PROXY_INDEX_UPSTREAM_URL")
                .unwrap()
                .1
                .as_deref(),
        ...
        assert!(tasks["media"].args.starts_with(&[
            ...
            "run".to_owned(),
            "-p".to_owned(),
            "aethernet-media-proxy".to_owned()
        ]));
        assert_eq!(
            tasks["admin"].args,
            vec![
                ...
                "-w".to_owned(),
                "aethernet_admin/src".to_owned(),
                "-w".to_owned(),
                "aethernet_admin/Cargo.toml".to_owned(),
                "-w".to_owned(),
                "aethernet_admin/build.rs".to_owned(),
                "-w".to_owned(),
                "aethernet_admin/openapi-admin.json".to_owned(),
                "-w".to_owned(),
                "aethernet_admin/static".to_owned(),
                "-x".to_owned(),
                "run -p aethernet_admin".to_owned()
            ]
        );
        assert_eq!(
            tasks["admin"]
                .env
                .iter()
                .find(|(key, _)| key == "AETHERNET_ADMIN_BASE_PATH")
                .unwrap()
                .1
                .as_deref(),
            Some("/admin")
        ...
        assert_eq!(
            tasks["admin"]
                .env
                .iter()
                .find(|(key, _)| key == "AETHERNET_ADMIN_PORT")
                .unwrap()
                .1
                .as_deref(),
