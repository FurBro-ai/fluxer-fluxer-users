...
pub fn s3_endpoint() -> String {
    env::var("AETHERNET_S3_ENDPOINT").unwrap_or_else(|_| "http://127.0.0.1:8333".to_owned())
}
...
pub fn s3_env() -> Vec<(String, Option<String>)> {
    vec![
        (
            ...
            Some(env::var("AETHERNET_S3_ACCESS_KEY_ID").unwrap_or_else(|_| "aethernet".to_owned())),
        ),
        (
            "AWS_SECRET_ACCESS_KEY".to_owned(),
            Some(
                env::var("AETHERNET_S3_SECRET_ACCESS_KEY")
                    .unwrap_or_else(|_| "aethernet-secret".to_owned()),
            ),
        ),
        (
            "AWS_DEFAULT_REGION".to_owned(),
            Some(env::var("AETHERNET_S3_REGION").unwrap_or_else(|_| "us-east-1".to_owned())),
        ),
    ]
}
...
fn cassandra_backend_enabled() -> bool {
    env::var("AETHERNET_DATABASE_BACKEND").as_deref() == Ok("cassandra")
}
...
mod tests {
    ...
    #[test]
    fn s3_env_uses_aethernet_defaults() {
        let env = s3_env();
        assert!(env.iter().any(
