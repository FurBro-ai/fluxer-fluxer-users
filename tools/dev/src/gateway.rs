...
const GATEWAY_FOREGROUND_EVAL: &str = concat!(
    "case application:ensure_all_started(aethernet_gateway) of ",
...
const GATEWAY_CLUSTER_COOKIE: &str = "aethernet-dev";
...
impl GatewayNode {
    ...
    pub fn erlang_name(&self) -> String {
        let prefix = env::var("AETHERNET_DEV_GATEWAY_CLUSTER_NODE_PREFIX")
            .unwrap_or_else(|_| "aethernet_gateway".to_owned());
...
pub fn setup_gateway_config() -> Result<()> {
    write_gateway_config(
        ...
        &env::var("AETHERNET_ERLANG_NODE_NAME")
            .unwrap_or_else(|_| "aethernet_gateway@127.0.0.1".to_owned()),
        &env::var("AETHERNET_ERLANG_COOKIE").unwrap_or_else(|_| GATEWAY_CLUSTER_COOKIE.to_owned()),
        &env::var("AETHERNET_ERLANG_DIST_PORT").unwrap_or_else(|_| "8081".to_owned()),
    )?;
...
pub fn write_gateway_config(
    ...
    let vm_text = fs::read_to_string(vm_template)?
        .replace("${AETHERNET_ERLANG_NODE_NAME}", node_name)
        .replace("${AETHERNET_ERLANG_COOKIE}", cookie)
        .replace("${AETHERNET_ERLANG_DIST_PORT}", dist_port);
...
pub async fn run_gateway() -> Result<i32> {
    ...
    let node_names = std::collections::HashSet::from([env::var("AETHERNET_ERLANG_NODE_NAME")
        .unwrap_or_else(|_| "aethernet_gateway@127.0.0.1".to_owned())]);
...
pub fn build_gateway_cluster_nodes() -> Result<Vec<GatewayNode>> {
    let replicas = positive_int_env(
        "AETHERNET_DEV_GATEWAY_CLUSTER_REPLICAS",
        GATEWAY_CLUSTER_DEFAULT_REPLICAS,
    );
    if replicas > GATEWAY_CLUSTER_DEFAULT_REPLICAS {
        bail!(
            "AETHERNET_DEV_GATEWAY_CLUSTER_REPLICAS={replicas} exceeds the configured local port table ({GATEWAY_CLUSTER_DEFAULT_REPLICAS})"
        );
    }
    let mut nodes = Vec::new();
    let mut dist_port = positive_int_env(
        "AETHERNET_DEV_GATEWAY_CLUSTER_DIST_PORT_BASE",
        GATEWAY_CLUSTER_DIST_PORT_BASE,
    );
    let http_port_offset = non_negative_int_env("AETHERNET_DEV_GATEWAY_CLUSTER_HTTP_PORT_OFFSET", 0);
...
fn wait_for_cluster_ports_available_sync(nodes: &[GatewayNode]) -> Result<()> {
    ...
    if !conflicts.is_empty() {
        bail!(
            "Gateway cluster port(s) already in use: {}. Stop the conflicting process or set AETHERNET_DEV_GATEWAY_CLUSTER_HTTP_PORT_OFFSET/AETHERNET_DEV_GATEWAY_CLUSTER_DIST_PORT_BASE for an isolated run.",
            conflicts.join(", ")
        );
    }
    Ok(())
...
async fn wait_for_cluster_ports_available(nodes: &[GatewayNode]) -> Result<()> {
    let timeout = env::var("AETHERNET_DEV_GATEWAY_CLUSTER_PORT_WAIT_SECONDS")
        .ok()
...
fn orphaned_gateway_leaders(
    ...
    let current_exe = std::env::current_exe().context("failed to resolve aethernet-dev executable")?;
...
fn gateway_node_env(node: &GatewayNode, static_peers: &str) -> Vec<(String, Option<String>)> {
    vec![
        (
            "AETHERNET_GATEWAY_CLUSTER_ENABLED".to_owned(),
            Some("true".to_owned()),
        ),
        (
            "AETHERNET_GATEWAY_CLUSTER_STATIC_PEERS".to_owned(),
            Some(static_peers.to_owned()),
        ),
        ("AETHERNET_GATEWAY_ROLE".to_owned(), Some(node.role.clone())),
        (
            "AETHERNET_GATEWAY_PORT".to_owned(),
            Some(node.http_port.to_string()),
        ),
        (
            "AETHERNET_ERLANG_NODE_NAME".to_owned(),
            Some(node.erlang_name()),
        ),
        (
            "AETHERNET_ERLANG_DIST_PORT".to_owned(),
            Some(node.dist_port.to_string()),
        ),
        (
            "AETHERNET_ERLANG_COOKIE".to_owned(),
            Some(
                env::var("AETHERNET_ERLANG_COOKIE")
...
mod tests {
    ...
    fn node_env_contains_role_port_and_static_peers() {
        ...
        assert!(
            env.iter()
                .any(|(key, value)| key == "AETHERNET_GATEWAY_ROLE"
                    && value.as_deref() == Some("calls"))
        );
        assert!(
            env.iter().any(
                |(key, value)| key == "AETHERNET_GATEWAY_PORT" && value.as_deref() == Some("8811")
            )
        );
        assert!(
            env.iter()
                .any(|(key, value)| key == "AETHERNET_GATEWAY_CLUSTER_STATIC_PEERS"
    ...
    fn cmdline_gateway_node_detection_matches_exact_name_argument() {
        let node_names =
            std::collections::HashSet::from([String::from("aethernet_gateway_websocket_1@127.0.0.1")]);

        assert!(cmdline_has_gateway_node(
            &strings(&[
                "/usr/local/bin/beam.smp",
                "-name",
                "aethernet_gateway_websocket_1@127.0.0.1"
            ]),
            &node_names
        ));
        assert!(!cmdline_has_gateway_node(
