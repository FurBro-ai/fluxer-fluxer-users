...
const LOCAL_ENV_START: &str = "# BEGIN aethernet-dev public URL";
const LOCAL_ENV_END: &str = "# END aethernet-dev public URL";
...
pub fn public_url_env(public_url: &str) -> Result<Vec<(String, String)>> {
    ...
    Ok(vec![
        ("AETHERNET_BASE_DOMAIN".to_owned(), parsed.host.clone()),
        ("AETHERNET_PUBLIC_SCHEME".to_owned(), parsed.scheme.clone()),
        (
            "AETHERNET_PUBLIC_PORT".to_owned(),
            parsed.public_port.to_string(),
        ),
        ("AETHERNET_STATIC_CDN_DOMAIN".to_owned(), parsed.host.clone()),
        ("AETHERNET_PUBLIC_URL".to_owned(), base.to_owned()),
        ("AETHERNET_API_ENDPOINT".to_owned(), format!("{base}/api")),
        (
            "AETHERNET_API_CLIENT_ENDPOINT".to_owned(),
            format!("{base}/api"),
        ),
        ("AETHERNET_APP_ENDPOINT".to_owned(), base.to_owned()),
        (
            "AETHERNET_GATEWAY_ENDPOINT".to_owned(),
            format!("{gateway_base}/gateway"),
        ),
        ("AETHERNET_MEDIA_ENDPOINT".to_owned(), format!("{base}/media")),
        ("AETHERNET_S3_PUBLIC_ENDPOINT".to_owned(), base.to_owned()),
        ("AETHERNET_S3_FORCE_PATH_STYLE".to_owned(), "true".to_owned()),
        ("AETHERNET_STATIC_CDN_ENDPOINT".to_owned(), base.to_owned()),
        ("AETHERNET_ADMIN_ENDPOINT".to_owned(), format!("{base}/admin")),
        (
            "AETHERNET_MARKETING_ENDPOINT".to_owned(),
            std::env::var("AETHERNET_MARKETING_ENDPOINT")
                .unwrap_or_else(|_| "https://aethernet.app".to_owned()),
        ),
        (
            "AETHERNET_MEDIA_PROXY_PUBLIC_ENDPOINT".to_owned(),
            format!("{base}/media"),
        ),
        (
            "AETHERNET_LIVEKIT_URL".to_owned(),
            format!("{gateway_base}/livekit"),
        ),
        (
            "AETHERNET_LIVEKIT_WEBHOOK_URL".to_owned(),
            format!("{base}/api/webhooks/livekit"),
        ),
        (
            "AETHERNET_MEDIA_PROXY_UPLOAD_RELAY_ENDPOINT".to_owned(),
            format!("{base}/media"),
        ),
        (
            "AETHERNET_GATEWAY_MEDIA_PROXY_ENDPOINT".to_owned(),
            format!("{base}/media"),
        ),
        (
            "AETHERNET_GATEWAY_STATIC_CDN_ENDPOINT".to_owned(),
            base.to_owned(),
        ),
        (
            "AETHERNET_ADMIN_OAUTH_REDIRECT_URI".to_owned(),
            format!("{base}/admin/oauth2_callback"),
        ),
        ("AETHERNET_PASSKEY_RP_ID".to_owned(), parsed.host.clone()),
        (
            "AETHERNET_PASSKEY_ADDITIONAL_ALLOWED_ORIGINS".to_owned(),
...
pub fn public_url_env_text(public_url: &str) -> Result<String> {
    ...
    text.push('\n');
    text.push_str("# Generated by `aethernet-dev tunnel configure`; safe to delete.\n");
    for (key, value) in public_url_env(public_url)? {
        text.push_str(&key);
        text.push('=');
        text.push_str(&value);
...
pub fn resolve_cloudflare_public_url(public_url_arg: Option<&str>) -> Result<String> {
    ...
    for key in [
        "AETHERNET_CLOUDFLARE_TUNNEL_PUBLIC_URL",
    ...
    if let Ok(public_url) = std::env::var("AETHERNET_PUBLIC_URL") {
        let public_url = public_url.trim();
...
async fn wait_cloudflare_tunnel_origin() -> Result<()> {
    let timeout = std::env::var("AETHERNET_CLOUDFLARE_TUNNEL_ORIGIN_READY_TIMEOUT")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(30);
    if let Err(error) = wait_tcp("Aethernet dev proxy", LOOPBACK_HOST, DEV_PROXY_PORT, timeout).await {
        bail!(
...
fn resolve_cloudflare_token(
    ...
    for key in [
        "AETHERNET_CLOUDFLARE_TUNNEL_TOKEN",
    ...
    bail!(
        "Missing Cloudflare tunnel token. Set AETHERNET_CLOUDFLARE_TUNNEL_TOKEN or write {}.",
        path.display()
    );
}
...
fn resolve_cloudflared_binary() -> Option<PathBuf> {
    if let Ok(binary) = std::env::var("AETHERNET_CLOUDFLARED_BIN")
...
mod tests {
    ...
    fn derives_https_public_url_env() {
        ...
        assert!(env.iter().any(|(key, value)| {
            key == "AETHERNET_GATEWAY_ENDPOINT" && value == "wss://dev.example.test/gateway"
        }));
        assert!(env.iter().any(|(key, value)| {
            key == "AETHERNET_LIVEKIT_URL" && value == "wss://dev.example.test/livekit"
        }));
        assert!(env.iter().any(|(key, value)| {
            key == "AETHERNET_MEDIA_PROXY_UPLOAD_RELAY_ENDPOINT"
                && value == "https://dev.example.test/media"
        }));
        assert!(env.iter().any(|(key, value)| {
            key == "AETHERNET_S3_PUBLIC_ENDPOINT" && value == "https://dev.example.test"
        }));
        assert!(
            env.iter()
                .any(|(key, value)| { key == "AETHERNET_PUBLIC_PORT" && value == "443" })
        );
    }
    ...
    fn replaces_existing_generated_block() {
        let existing =
            "A=1\n\n# BEGIN aethernet-dev public URL\nOLD=1\n# END aethernet-dev public URL\n\nB=2\n";
