...
pub fn config_from_env() -> Result<CassandraConfig> {
    let hosts = env::var("AETHERNET_CASSANDRA_HOSTS").unwrap_or_else(|_| "cassandra".to_owned());
    let host = hosts
        .split_once(',')
        .map(|(first, _)| first)
        .unwrap_or(&hosts)
    ...
    Ok(CassandraConfig {
        ...
        port: env::var("AETHERNET_CASSANDRA_PORT")
            .unwrap_or_else(|_| "9042".to_owned())
            .parse()
            .context("invalid AETHERNET_CASSANDRA_PORT")?,
        keyspace: env::var("AETHERNET_CASSANDRA_KEYSPACE").unwrap_or_else(|_| "aethernet".to_owned()),
        local_dc: env::var("AETHERNET_CASSANDRA_LOCAL_DC")
            .unwrap_or_else(|_| "datacenter1".to_owned()),
        username: env::var("AETHERNET_CASSANDRA_USERNAME")
            .ok()
            .filter(|value| !value.is_empty()),
        password: env::var("AETHERNET_CASSANDRA_PASSWORD")
            .ok()
            .filter(|value| !value.is_empty()),
    })
}
...
pub fn render_diff(diff: &SchemaDiff) -> String {
    let mut lines = vec![
        "-- Generated by aethernet-dev cassandra diff".to_owned(),
...
mod tests {
    ...
    fn renders_target_schema_with_semicolons() {
        let rendered = render_target_schema("aethernet");
        assert!(rendered.starts_with("CREATE TYPE IF NOT EXISTS aethernet."));
        assert!(rendered.contains("CREATE TABLE IF NOT EXISTS aethernet.messages"));
        assert!(rendered.ends_with(";\n"));
    }

    #[test]
    fn diff_creates_keyspace_and_missing_schema() {
        let diff = create_diff(&ExistingSchema::default(), "aethernet");
        assert!(diff.statements[0].starts_with("CREATE KEYSPACE IF NOT EXISTS aethernet"));
        assert!(
            diff.statements
                .iter()
                .any(|statement| statement.starts_with("CREATE TYPE IF NOT EXISTS aethernet."))
        );
        assert!(
            diff.statements
                .iter()
                .any(|statement| statement.starts_with("CREATE TABLE IF NOT EXISTS aethernet."))
        );
    ...
    fn diff_reports_type_mismatches_and_extra_tables() {
        ...
        let diff = create_diff(&existing, "aethernet");
        assert!(
            diff.errors
                .iter()
