...
const CANARY_APP_NAME: &str = "Aethernet Canary";
const CANARY_BUNDLE_ID: &str = "app.aethernet.canary";
const MACOS_DEV_ELECTRON_USAGE_DESCRIPTIONS: &[(&str, &str)] = &[
    (
        "NSMicrophoneUsageDescription",
        "Aethernet needs access to your microphone to enable voice chat features.",
    ),
    (
        "NSCameraUsageDescription",
        "Aethernet needs access to your camera to enable video chat features.",
    ),
    (
        "NSAppleEventsUsageDescription",
        "Aethernet needs access to Apple Events for automation features.",
    ),
    (
        "NSAudioCaptureUsageDescription",
        "Aethernet captures audio from the screen or window you choose to share.",
    ),
    (
        "NSScreenCaptureUsageDescription",
        "Aethernet captures the screen or window you choose to share.",
    ),
];
...
pub fn build_desktop(skip_native: bool) -> Result<()> {
    let env = vec![
        ...
        (
            "AETHERNET_SKIP_NATIVE".to_owned(),
            if skip_native {
                Some("true".to_owned())
            } else {
                None
...
pub fn electron_args(args: &[String]) -> Vec<String> {
    ...
    if cfg!(target_os = "linux")
        ...
        && env::var("AETHERNET_ELECTRON_NO_SANDBOX").as_deref() != Ok("0")
    {
...
fn run_desktop_process(app_url: &str, extra_args: &[String]) -> Result<()> {
    ...
    let mut args = vec![
        format!("--aethernet-app-url={app_url}"),
        "--aethernet-log-renderer-console".to_owned(),
    ];
...
pub async fn resolve_desktop_canary_app_url(app_url: Option<&str>) -> Result<String> {
    ...
    for key in [
        "AETHERNET_DESKTOP_CANARY_APP_URL",
        "AETHERNET_DESKTOP_APP_URL",
...
mod tests {
    ...
    fn dev_electron_binary_path_points_inside_the_dev_bundle() {
        assert!(dev_electron_binary_path().ends_with(
            "aethernet_desktop/node_modules/electron/dist/Electron.app/Contents/MacOS/Electron"
        ));
    }

    #[test]
    fn dev_electron_plist_path_points_to_aethernet_desktop_electron_app() {
        assert!(dev_electron_info_plist_path().ends_with(
            "aethernet_desktop/node_modules/electron/dist/Electron.app/Contents/Info.plist"
        ));
    }
