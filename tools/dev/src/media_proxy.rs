...
const DEV_S3_BUCKETS: &str =
    "aethernet,aethernet-uploads,aethernet-downloads,aethernet-reports,aethernet-harvests,aethernet-static";
const DEV_S3_ACCESS_KEY_ID: &str = "aethernet";
const DEV_S3_SECRET_ACCESS_KEY: &str = "aethernet-secret";
...
pub async fn run_dev_media_doctor(
    ...
    println!("Aethernet media proxy doctor passed.");
    Ok(())
}
...
pub async fn ensure_dev_object_store(repair: bool, repair_timeout_secs: u64) -> Result<()> {
    ...
    match wait_s3_api(5).await {
        ...
        Err(error) => {
            bail!(
                "SeaweedFS S3 is unreachable: {error}\nRun `aethernet-dev media-proxy doctor --repair` to start the local dev object store."
            );
        }
    }
    ensure_s3_buckets()?;
...
pub fn run_rust_stress_smoke() -> Result<()> {
    crate::proc::run(&["cargo", "test", "-p", "aethernet-media-proxy"])?;
    crate::proc::run(&[
        "cargo",
        "bench",
        "-p",
...
fn start_dev_seaweedfs() -> Result<()> {
    ...
    let weed = resolve_weed_binary().ok_or_else(|| {
        anyhow::anyhow!(
            "missing `weed` binary. Install SeaweedFS or set AETHERNET_WEED_BINARY to its executable path."
        )
    })?;
...
fn resolve_weed_binary() -> Option<PathBuf> {
    std::env::var_os("AETHERNET_WEED_BINARY")
        .map(PathBuf::from)
        .filter(|path| path.is_file())
        .or_else(|| which("weed"))
        // The development bootstrap used by local workspaces places downloaded
        // helper binaries here without always exporting that directory in PATH.
        .or_else(|| {
            let path = PathBuf::from("/tmp/aethernet-bin/weed");
