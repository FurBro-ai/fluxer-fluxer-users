...
const SECURITY_RESPONSE_HEADERS: &[(&str, &str)] = &[
...
pub async fn run_proxy(host: &str, port: u16) -> Result<()> {
    ...
    println!("Aethernet dev proxy listening on {}", listener.local_addr()?);
    ...
    println!(
        "Aethernet dev proxy gateway ports: {}",
        gateway_ports
            .iter()
            .map(u16::to_string)
            .collect::<Vec<_>>()
...
async fn proxy_request(
    ...
    if let Some(location) = tunnel_public_redirect_location(&request_head) {
        return secure_response(redirect_response(&request_head, &location).into_response());
    }
    ...
    if is_upgrade_request(&request_head) {
        return secure_response(proxy_upgrade(
            request,
            request_head,
            route,
            target_host,
...
async fn proxy_http(
    ...
    for (name, value) in &parts.headers {
        ...
        if name.as_str().eq_ignore_ascii_case(X_FORWARDED_FOR_HEADER) {
            // Never trust a client-supplied forwarding chain. This proxy is the
            // security boundary used by the local API and rate limiter.
            continue;
        } else {
    ...
    request_builder = request_builder.header(X_FORWARDED_FOR_HEADER, client_addr.ip().to_string());

    let upstream_response = match request_builder.send().await {
        Ok(response) => response,
        Err(error) => {
            eprintln!("dev proxy HTTP request failed for {target_url}: {error:#}");
            return upstream_unavailable_response(route, error);
        }
    };
...
async fn proxy_upgrade(
    ...
    let mut target = match TcpStream::connect((target_host, target_port)).await {
        Ok(target) => target,
        Err(error) => return upstream_unavailable_response(route, error),
    };
...
fn bad_gateway_response(_error: impl std::fmt::Display) -> Response<Body> {
    (
        ...
        "AETHERNET upstream service is unavailable. Check the local service logs and retry.\n",
    )
        .into_response()
}
...
fn upstream_unavailable_response(
    ...
    let mut response = (
        ...
        "AETHERNET voice is unavailable because the local LiveKit service is not running. Start the development infrastructure with `pnpm dev:infra:start`, then retry.\n",
    )
        .into_response();
    response
        .headers_mut()
...
pub fn tunnel_public_redirect_location(request: &RequestHead) -> Option<String> {
    let public_url = env::var("AETHERNET_PUBLIC_URL").ok()?;
...
pub fn route_for_path(path: &str) -> &'static ProxyRoute {
    ...
    let parsed_path = Url::parse(&format!("http://aethernet.local{normalized_path}"))
...
pub fn rewrite_head(
    ...
    for (name, value) in &request.headers {
        if name.eq_ignore_ascii_case("host") {
            ...
            // Discard untrusted client input. A canonical value is added below.
            continue;
        } else {
...
mod tests {
    ...
    #[test]
    fn livekit_connection_errors_return_actionable_service_unavailable() {
        let response = upstream_unavailable_response(
            route_for_path("/livekit/rtc"),
            "connection refused",
        );
    ...
    #[tokio::test]
    async fn proxy_errors_do_not_expose_internal_details() {
        let response = bad_gateway_response("tcp connect failed at 10.0.0.7:9999");
        let body = response.into_body();
        let bytes = axum::body::to_bytes(body, usize::MAX).await.unwrap();
        let text = String::from_utf8(bytes.to_vec()).unwrap();
        assert!(!text.contains("10.0.0.7"));
        assert!(text.contains("upstream service is unavailable"));
    }

    #[test]
    fn adds_missing_security_headers_without_overwriting_upstream_policy() {
        let mut response = Response::new(Body::empty());
        response.headers_mut().insert(
            header::REFERRER_POLICY,
            HeaderValue::from_static("no-referrer"),
        );
        let response = secure_response(response);
    ...
    fn rewrites_host_connection_and_forwarded_for() {
        ...
        assert!(!head.contains("1.1.1.1"));
        assert!(head.contains("X-Forwarded-For: 2.2.2.2\r\n"));
    }
