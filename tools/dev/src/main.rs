...
use aethernet_dev::cassandra::{
    apply_schema, compute_diff, render_target_schema, verify_schema, write_diff_file,
};
use aethernet_dev::desktop::{
    build_desktop, install_desktop, package_desktop, run_desktop, run_desktop_canary,
    typecheck_desktop,
};
use aethernet_dev::env::merge_default_env_with_current;
use aethernet_dev::manifest::{DEV_PROXY_PORT, LOCAL_APP_URL};
use aethernet_dev::paths::{DEV_ENV_FILE, DEV_LOCAL_ENV_FILE, ROOT, ROOT_LOCAL_ENV_FILE};
...
struct DevArgs {
    ...
    /// Reduce build parallelism and runtime memory for small development servers.
    #[arg(long)]
    low_memory: bool,
    tasks: Vec<String>,
}
...
enum MediaProxyCommand {
    ...
    RustStressSmoke,
    SignExternalUrl(aethernet_dev::media_external::SignExternalUrlArgs),
}
...
enum TunnelCommand {
    ...
    Run {
        #[arg(long, env = "AETHERNET_CLOUDFLARE_TUNNEL_TOKEN", hide_env_values = true)]
        token: Option<String>,
        #[arg(long)]
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Command::Bootstrap(args) => {
            aethernet_dev::bootstrap::bootstrap(args.skip_install).await?;
        }
        Command::PostStart => aethernet_dev::bootstrap::post_start().await?,
        Command::Gateway(args) if args.mode == "single" => {
            std::process::exit(aethernet_dev::gateway::run_gateway().await?)
        }
        Command::Gateway(_) => {
            std::process::exit(aethernet_dev::gateway::run_gateway_cluster().await?)
        }
        Command::Build => std::process::exit(aethernet_dev::tasks::run_build()?),
        Command::Knip => std::process::exit(aethernet_dev::tasks::run_knip()?),
        Command::Lint => std::process::exit(aethernet_dev::tasks::run_lint()?),
        Command::Test => std::process::exit(aethernet_dev::tasks::run_test()?),
        Command::Typecheck => std::process::exit(aethernet_dev::tasks::run_typecheck()?),
        Command::Proxy(args) => aethernet_dev::proxy::run_proxy(&args.host, args.port).await?,
        Command::Dev(args) => {
            if args.cloudflare_tunnel {
                aethernet_dev::tunnel::apply_cloudflare_public_url_env(args.public_url.as_deref())?;
            } else if let Some(public_url) = args.public_url.as_deref() {
                aethernet_dev::tunnel::apply_public_url_env(public_url)?;
            }
            std::process::exit(
                aethernet_dev::dev::run_dev(
                    &args.tasks,
                    args.cloudflare_tunnel,
                    args.low_memory,
                )
                .await?,
            )
        ...
        Command::RustServices(args) => {
            std::process::exit(aethernet_dev::rust_services::run_rust_services(&args.services).await?)
        }
        ...
        Command::Cassandra(args) => match args.command {
            ...
            CassandraCommand::TargetSchema => print!("{}", render_target_schema("aethernet")),
        },
        Command::Desktop(args) => match args.command {
            ...
            DesktopCommand::ExecDisclaimed { program, args } => {
                aethernet_dev::disclaim::exec_disclaimed(&program, &args)?
            }
        },
        Command::MediaProxy(args) => match args.command {
            MediaProxyCommand::Doctor {
                ...
                aethernet_dev::media_proxy::run_dev_media_doctor(repair, &base_url, path.as_deref())
                    .await?;
            }
            MediaProxyCommand::RustStressSmoke => {
                aethernet_dev::media_proxy::run_rust_stress_smoke()?;
            }
            MediaProxyCommand::SignExternalUrl(args) => {
                println!(
                    "{}",
                    aethernet_dev::media_external::sign_external_url(
                        &args.secret_key,
                        &args.server_url,
                        &args.upstream
                    )?
        ...
        Command::Tunnel(args) => match args.command {
            TunnelCommand::Configure { public_url, token } => {
                aethernet_dev::tunnel::write_cloudflare_public_url_file(&public_url)?;
                if let Some(token) = token {
                    aethernet_dev::tunnel::write_cloudflare_token_file(&token)?;
                }
            }
            TunnelCommand::PrintEnv { public_url } => {
                print!("{}", aethernet_dev::tunnel::public_url_env_text(&public_url)?);
            }
            TunnelCommand::Run { token, token_file } => std::process::exit(
                aethernet_dev::tunnel::run_cloudflare_tunnel(token, token_file).await?,
            ),
        },
    }
    Ok(())
