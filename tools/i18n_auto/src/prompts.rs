...
const GLOBAL_SYSTEM_PROMPT: &str = r#"You localize product UI copy for Aethernet, a modern consumer chat app.
...
- Keep Aethernet's product vocabulary and naming stable unless persisted locale guidance explicitly says otherwise.
...
- Keep Aethernet's exact wording, sentence structure, tone, and product vocabulary unless a spelling, punctuation, date, number, measurement, or grammatical locale difference requires a change.
...
pub fn build_system_prompt(locale: &str) -> String {
    [
        ...
        Some(format!(
            "Produce exactly one {} localization for Aethernet.",
            display_name(locale)
        )),
