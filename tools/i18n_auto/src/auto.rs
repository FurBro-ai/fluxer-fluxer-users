...
const ENV_KEYS: &[&str] = &[
    "AETHERNET_AUTO_I18N",
    "AETHERNET_AUTO_I18N_LOCALE_CONCURRENCY",
    "AETHERNET_AUTO_I18N_PROGRESS_INTERVAL",
    "AETHERNET_AUTO_I18N_REQUEST_TIMEOUT",
    "AETHERNET_AUTO_I18N_STRING_CONCURRENCY",
    "I18N_LLM_MODEL",
    "OPENROUTER_API_KEY",
...
pub fn auto_main(args: &[String]) -> Result<u8> {
    ...
    let aethernet_auto_i18n = env_value("AETHERNET_AUTO_I18N", &env_overrides, "");
    ...
    let explicitly_disabled = matches!(
        aethernet_auto_i18n.to_lowercase().as_str(),
        "0" | "false" | "no" | "off"
    );
    if explicitly_disabled && !bypass_run_gate {
        eprintln!("i18n:auto skipped: AETHERNET_AUTO_I18N=0 disables automatic translations.");
        return Ok(0);
    }
    let openrouter_is_available =
        is_openrouter_available(&openrouter_base_url, &openrouter_api_key);
    let should_run = aethernet_auto_i18n == "1" || (!explicitly_disabled && openrouter_is_available);
    if !should_run && !bypass_run_gate {
        eprintln!(
            "i18n:auto skipped: OpenRouter is unavailable. Set OPENROUTER_API_KEY, or set AETHERNET_AUTO_I18N=1 to attempt translations with {} at {}.",
    ...
    if should_run && aethernet_auto_i18n != "1" && openrouter_is_available {
        println!(
            "i18n:auto detected OpenRouter availability at {openrouter_base_url}; running without AETHERNET_AUTO_I18N=1."
        );
    }

    let mut runner_env = env_overrides;
    runner_env.insert(
        "AETHERNET_AUTO_I18N".to_string(),
        if should_run {
            "1".to_string()
        } else {
            aethernet_auto_i18n
...
mod tests {
    ...
    fn parses_simple_export_lines() {
        ...
        assert_eq!(
            parse_export_line("export AETHERNET_AUTO_I18N=1"),
            Some(("AETHERNET_AUTO_I18N".to_string(), "1".to_string()))
        );
