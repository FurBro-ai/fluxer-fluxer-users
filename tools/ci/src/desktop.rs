...
const PUBLIC_DL_BASE: &str = "https://api.aethernet.app/dl";
...
const PLATFORMS: &[Platform] = &[
    ...
    Platform {
        ...
        desktop_variant: DEFAULT_DESKTOP_VARIANT,
        os: "aethernet-desktop-macos-arm64",
...
fn calver_env_from_process() -> CalverEnv {
    CalverEnv {
        ...
        aethernet_build_version: trim_option(env::var("AETHERNET_BUILD_VERSION").ok()),
        aethernet_build_date: trim_option(env::var("AETHERNET_BUILD_DATE").ok()),
    }
}
...
fn resolve_desktop_dir() -> Result<PathBuf> {
    ...
    if cwd.file_name().and_then(|value| value.to_str()) == Some("aethernet_desktop") {
        return Ok(cwd);
    }
    if cwd.join("aethernet_desktop").is_dir() {
        return Ok(cwd.join("aethernet_desktop"));
    }
    Err(anyhow!(
        "Could not resolve aethernet_desktop directory from {}",
        cwd.display()
    ))
}
...
fn desktop_dist_dir() -> PathBuf {
    workdir().join("aethernet_desktop").join("dist-electron")
}
...
async fn install_linux_deps_step() -> Result<()> {
    let apt_conf = runner_temp().join("99aethernet-ci-network");
    fs::write(
        &apt_conf,
        r#"Acquire::Retries "6";
...
"#,
    ...
    run_command(CommandSpec::new("sudo").args([
        ...
        "/etc/apt/apt.conf.d/99aethernet-ci-network",
    ]))?;
...
fn build_electron_main_step() -> Result<()> {
    run_command(
        pnpm_command()?
            ...
            .env("AETHERNET_DESKTOP_PRODUCTION", "true"),
    )
}
...
fn validate_macos_signing_env() -> Result<PathBuf> {
    ...
    let keychain = require_home()?.join("Library/Keychains/aethernet-build.keychain-db");
    ensure!(
        keychain.exists(),
...
fn macos_native_runtime_targets(electron_arch: &str) -> Vec<(String, &'static str)> {
    ...
    [
        "@aethernet/webauthn/webauthn",
        "@aethernet/mac-app-audio/mac-app-audio",
        "@aethernet/mac-clipboard/mac-clipboard",
        "@aethernet/mac-sysctl/mac-sysctl",
        "@aethernet/mac-tcc/mac-tcc",
        "@aethernet/macos-input-hook/macos-input-hook",
        "@aethernet/platform-info/platform-info",
    ]
    .into_iter()
    .map(|prefix| {
        (
...
fn windows_package_config(build_channel: &str, arch: &str) -> WindowsPackageConfig {
    ...
    let pack_title = if canary { "Aethernet Canary" } else { "Aethernet" };
    WindowsPackageConfig {
        pack_id: if canary {
            "aethernet_desktop_canary"
        } else {
            "aethernet_desktop"
        },
        pack_title,
        artifact_prefix: if canary { "Aethernet-Canary" } else { "Aethernet" },
        icon_dir: if canary {
            "icons-canary"
        } else {
            "icons-stable"
...
fn remove_velopack_portable_archives(output_dir: &Path) -> Result<()> {
    for path in collect_files(output_dir)? {
        ...
        println!(
            "Removed Velopack portable archive {}. Aethernet publishes its own portable ZIP built from the signed application tree.",
            path.display()
        );
    }
    Ok(())
...
const AETHERNET_WINDOWS_SIGNER_COMMON_NAME: &str = "Aethernet Platform AB";
...
const FORBIDDEN_WINDOWS_GAME_CAPTURE_ARTIFACT_PREFIXES: &[&str] = &[
    "aethernet-game-hook.",
    "aethernet-inject-helper.",
    "aethernet-vulkan-layer.",
    "aethernet_game_hook.",
    "aethernet_inject_helper.",
    "aethernet_vulkan_layer.",
];
...
fn assert_aethernet_signed(row: &SignatureRow) -> Result<()> {
    ensure!(
        row.status == "Valid",
    ...
    let common_name = certificate_common_name(subject)
        ...
        common_name == AETHERNET_WINDOWS_SIGNER_COMMON_NAME,
        "Signer CN is '{common_name}', expected '{}' (thumbprint {})",
        AETHERNET_WINDOWS_SIGNER_COMMON_NAME,
...
fn assert_signed_by_known_publisher(row: &SignatureRow, relative: &str) -> Result<()> {
    match assert_aethernet_signed(row) {
        Ok(()) => Ok(()),
        Err(aethernet_error) => assert_third_party_signed(row, relative)
            .map_err(|third_party_error| anyhow!("{aethernet_error}; {third_party_error}")),
    }
}
...
fn handoff_artifact_name(
    ...
    format!("aethernet-desktop-{build_channel}-{platform}-{arch}{variant_suffix}{signed_suffix}")
}
...
fn prepare_release_assets_step() -> Result<()> {
    ...
    let descriptor = DesktopReleaseDescriptor {
        ...
        channel: channel.clone(),
        version: version.clone(),
        release_tag: format!("aethernet-desktop-{channel}@{version}"),
        source_sha,
...
fn parse_artifact_dir_name(base: &str, channel: &str) -> Option<ArtifactIdentity> {
    let prefix = format!("aethernet-desktop-{channel}-");
...
mod tests {
    ...
    fn every_uploaded_desktop_object_carries_a_cache_instruction() {
        ...
        fs::write(root.join("arm64").join("Aethernet-1.2.3-arm64.dmg"), "dmg").unwrap();
    ...
    fn the_stored_lifetime_follows_the_key_not_the_upload_batch() {
        assert_eq!(
            desktop_object_cache_control("desktop/stable/darwin/arm64/Aethernet-1.2.3-arm64.dmg"),
            VERSIONED_ARTIFACT_CACHE_CONTROL
        );
        assert_eq!(
            desktop_object_cache_control(
                "desktop/stable/darwin/arm64/Aethernet-1.2.3-arm64.dmg.sha256"
            ),
            VERSIONED_ARTIFACT_CACHE_CONTROL
        );
        assert_eq!(
        ...
        assert_eq!(
            desktop_object_cache_control("desktop-test/canary/linux/x64/Aethernet-1.2.3.AppImage"),
            MUTABLE_DOWNLOAD_CACHE_CONTROL,
    ...
    fn resolves_explicit_calver_with_precedence() {
        let calver_env = CalverEnv {
            ...
            aethernet_build_version: Some("2026.521.2".to_string()),
            aethernet_build_date: Some("2026-05-22T03:04:05Z".to_string()),
        };
        assert_eq!(
    ...
    fn resolves_generated_calver_from_date_override() {
        let calver_env = CalverEnv {
            aethernet_build_date: Some("2026-05-20T01:02:03Z".to_string()),
            ..CalverEnv::default()
        };
        assert_eq!(
    ...
    fn upload_plan_splits_payload_metadata_without_s3() {
        let temp = tempfile::tempdir().unwrap();
        let root = temp.path();
        write_file(&root.join("canary/linux/x64/Aethernet.AppImage"), "app");
        ...
        assert_eq!(binaries, vec!["desktop/canary/linux/x64/Aethernet.AppImage"]);
        assert_eq!(
            metadata,
            vec![
    ...
    const PAYLOAD_BINARIES: &[&str] = &[
        "canary/darwin/arm64/Aethernet-Canary-2026.908.173325-mac-universal.dmg",
        "canary/darwin/arm64/Aethernet-Canary-2026.908.173325-mac-universal.dmg.sha256",
        "canary/darwin/arm64/Aethernet-Canary-2026.908.173325-mac-universal.zip",
        "canary/darwin/arm64/Aethernet-Canary-2026.908.173325-mac-universal.zip.blockmap",
        "canary/darwin/arm64/Aethernet-Canary-2026.908.173325-mac-universal.zip.sha256",
        "canary/darwin/x64/Aethernet-Canary-2026.908.173325-mac-universal.zip",
        "canary/darwin/x64/Aethernet-Canary-2026.908.173325-mac-universal.zip.sha256",
        "canary/linux/x64/Aethernet-Canary-2026.908.173325-linux-amd64.deb",
        "canary/linux/x64/Aethernet-Canary-2026.908.173325-linux-amd64.deb.sha256",
        "canary/linux/x64/Aethernet-Canary-2026.908.173325-linux-x64.tar.gz",
        "canary/linux/x64/Aethernet-Canary-2026.908.173325-linux-x64.tar.gz.sha256",
        "canary/linux/x64/Aethernet-Canary-2026.908.173325-linux-x86_64.AppImage",
        "canary/linux/x64/Aethernet-Canary-2026.908.173325-linux-x86_64.AppImage.sha256",
        "canary/linux/x64/Aethernet-Canary-2026.908.173325-linux-x86_64.rpm",
        "canary/win32/x64/Aethernet-Canary-2026.908.173325-portable-win-x64.zip",
        "canary/win32/x64/Aethernet-Canary-2026.908.173325-portable-win-x64.zip.sha256",
        "canary/win32/x64/Aethernet-Canary-2026.908.173325-win-x64-full.nupkg",
        "canary/win32/x64/Aethernet-Canary-2026.908.173325-win-x64-full.nupkg.sha256",
        "canary/win32/x64/Aethernet-Canary-2026.908.173325-win-x64.exe",
        "canary/win32/x64/Aethernet-Canary-2026.908.173325-win-x64.exe.sha256",
        "canary/win32/x64/windows-game-capture/Aethernet-Canary-2026.908.173325-win-x64-full.nupkg",
        "canary/win32/x64/windows-game-capture/Aethernet-Canary-2026.908.173325-win-x64.exe",
    ];
    ...
    fn the_payload_and_cache_classifiers_agree_on_every_release_feed_filename() {
        for name in [
            ...
            "latest-linux.yml",
            "latest-linux.yaml",
            "Aethernet-Canary-2026.908.173325-win-x64.exe",
            "Aethernet-Canary-2026.908.173325-win-x64-full.nupkg",
            "Aethernet-Canary-2026.908.173325-mac-universal.dmg",
            "Aethernet-Canary-2026.908.173325-mac-universal.zip.blockmap",
            "Aethernet-Canary-2026.908.173325-linux-x86_64.AppImage.sha256",
        ] {
    ...
    fn update_feed_handoff_round_trip_guard_restores_the_release_feed_keys() {
        ...
        write_file(
            &payload_root
                .join("canary/darwin/arm64/Aethernet-Canary-2026.908.173325-mac-universal.zip"),
            "zip",
        );
    ...
    fn parses_handoff_artifact_dir_names() {
        assert_eq!(
            parse_artifact_dir_name("aethernet-desktop-canary-windows-arm64", "canary").unwrap(),
            ArtifactIdentity {
        ...
        assert_eq!(
            parse_artifact_dir_name(
                "aethernet-desktop-canary-windows-x64-windows-game-capture-signed",
                "canary",
            )
            .unwrap(),
            ArtifactIdentity {
        ...
        assert!(parse_artifact_dir_name("aethernet-desktop-stable-linux-x64", "canary").is_none());
        assert_eq!(
            parse_artifact_dir_name("aethernet-desktop-canary-windows-x64-signed", "canary").unwrap(),
            ArtifactIdentity {
                platform: "windows".to_string(),
                arch: "x64".to_string(),
                desktop_variant: DEFAULT_DESKTOP_VARIANT.to_string(),
    ...
    fn handoff_artifact_name_only_marks_signed_windows_uploads() {
        assert_eq!(
            ...
            "aethernet-desktop-canary-windows-x64-signed"
        );
        assert_eq!(
            handoff_artifact_name("canary", "linux", "x64", DEFAULT_DESKTOP_VARIANT, true),
            "aethernet-desktop-canary-linux-x64"
        );
        assert_eq!(
            ...
            "aethernet-desktop-stable-windows-arm64-windows-game-capture"
        );
        assert_eq!(
            handoff_artifact_name("stable", "windows", "arm64", DEFAULT_DESKTOP_VARIANT, false),
            "aethernet-desktop-stable-windows-arm64"
        );
    }
...
export const CHANNEL_DISPLAY_NAME = BUILD_CHANNEL;\n"
    ...
    fn payload_artifact_dirs_prefer_signed_windows_artifacts() {
        ...
        fs::create_dir_all(artifacts.join("aethernet-desktop-canary-windows-x64")).unwrap();
        fs::create_dir_all(artifacts.join("aethernet-desktop-canary-windows-x64-signed")).unwrap();
        fs::create_dir_all(
            artifacts.join("aethernet-desktop-canary-windows-x64-windows-game-capture"),
        )
        .unwrap();
        fs::create_dir_all(
            artifacts.join("aethernet-desktop-canary-windows-x64-windows-game-capture-signed"),
        )
        .unwrap();
        fs::create_dir_all(artifacts.join("aethernet-desktop-canary-linux-x64")).unwrap();
        ...
        assert_eq!(
            ...
            vec![
                (
                    "aethernet-desktop-canary-linux-x64".to_string(),
                    ArtifactIdentity {
                ...
                (
                    "aethernet-desktop-canary-windows-x64-signed".to_string(),
                    ArtifactIdentity {
                        platform: "windows".to_string(),
                        arch: "x64".to_string(),
                        desktop_variant: DEFAULT_DESKTOP_VARIANT.to_string(),
                ...
                (
                    "aethernet-desktop-canary-windows-x64-windows-game-capture-signed".to_string(),
                    ArtifactIdentity {
                        platform: "windows".to_string(),
                        arch: "x64".to_string(),
                        desktop_variant: WINDOWS_GAME_CAPTURE_DESKTOP_VARIANT.to_string(),
    ...
    fn desktop_manifest_uses_checksum_detail_when_present() {
        let temp = tempfile::tempdir().unwrap();
        let root = temp.path();
        write_file(&root.join("Aethernet-2026.520.1-x64.AppImage"), "app");
        write_file(
            &root.join("Aethernet-2026.520.1-x64.AppImage.sha256"),
            "abc123\n",
        );
        write_file(&root.join("Aethernet-2026.520.1-x64.deb"), "deb");
        ...
        assert_eq!(
            ...
            Some(&DesktopManifestFile::Detail {
                filename: "Aethernet-2026.520.1-x64.AppImage".to_string(),
        ...
        assert_eq!(
            ...
            Some(&DesktopManifestFile::Name(
                "Aethernet-2026.520.1-x64.deb".to_string()
            ))
        );
    }
    ...
    fn macos_releases_json_points_at_zip_filename() {
        ...
        let manifest = DesktopManifest {
            ...
            files: BTreeMap::from([(
                "zip".to_string(),
                DesktopManifestFile::Name("Aethernet-2026.520.1-arm64.zip".to_string()),
            )]),
        };
        ...
        assert_eq!(
            ...
            "https://api.aethernet.app/dl/desktop-test/canary/darwin/arm64/Aethernet-2026.520.1-arm64.zip"
        );
    ...
    fn velopack_path_lengths_include_install_prefix_and_sort_descending() {
        ...
        let entries =
            velopack_path_lengths(&archive_path, Path::new(r"C:\Users\a\AppData\Local\Aethernet"))
                .unwrap();
    ...
    fn windows_package_config_tracks_channel_and_arch() {
        ...
        assert_eq!(stable.pack_id, "aethernet_desktop");
        assert_eq!(stable.runtime, "win-x64");
        assert_eq!(stable.main_exe, "Aethernet.exe");

        let canary = windows_package_config("canary", "arm64");
        assert_eq!(canary.pack_id, "aethernet_desktop_canary");
        assert_eq!(canary.runtime, "win-arm64");
        assert_eq!(canary.main_exe, "Aethernet Canary.exe");
    }
    ...
    fn velopack_portable_archives_are_removed_from_the_release_directory() {
        ...
        write_file(
            &output_dir.join("aethernet_desktop_canary-2026.810.1-Portable.zip"),
            "velopack",
        );
        write_file(
            &output_dir.join("aethernet_desktop_canary-2026.810.1-full.nupkg"),
            "payload",
        );
        write_file(
            &output_dir.join("Aethernet Canary-2026.810.1-win-arm64.exe"),
            "setup",
        );
        ...
        assert!(remaining.contains("aethernet_desktop_canary-2026.810.1-full.nupkg"));
        assert!(remaining.contains("Aethernet Canary-2026.810.1-win-arm64.exe"));
    ...
    fn percent_encoded_archive_names_match_their_decoded_inventory_entry() {
        assert_eq!(
            percent_decode_archive_name("Aethernet%20Canary.exe"),
            "Aethernet Canary.exe"
        );
        assert_eq!(percent_decode_archive_name("Aethernet.exe"), "Aethernet.exe");
        assert_eq!(
    ...
    fn canary_nupkg_inventory_accepts_percent_encoded_main_executable() {
        ...
        let files = expected_windows_pe_inventory("arm64", "Aethernet Canary.exe")
            .into_iter()
            .map(|name| {
                if name == "Aethernet Canary.exe" {
                    return root.join("Aethernet%20Canary.exe");
                }
                root.join(name)
            })
            .collect::<Vec<_>>();
        assert_expected_windows_pe_inventory(&root, &files, "arm64", "Aethernet Canary.exe").unwrap();
    }

    #[test]
    fn known_optional_windows_pe_inventory_never_repeats_a_required_binary() {
        for arch in ["x64", "arm64"] {
            for main_exe in ["Aethernet.exe", "Aethernet Canary.exe"] {
                assert_eq!(
