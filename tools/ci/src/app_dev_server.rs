...
impl AppDevServer {
    ...
    async fn run(&mut self) -> Result<()> {
        ...
        self.clean_dist()?;
        let mut css_type_watcher = if env_flag("AETHERNET_APP_SKIP_CSS_TYPE_WATCHER") {
            println!("Skipping CSS type watcher (low-memory profile)");
            None
        } else {
            Some(self.start_css_type_watcher()?)
        };
        let rspack_result = self.run_rspack(&mut shutdown_rx).await;
        if let Some(watcher) = css_type_watcher.as_mut() {
            terminate_child(watcher).await;
        }
        rspack_result
    }
    ...
    fn start_css_type_watcher(&self) -> Result<Child> {
        // Go through the app's ESM wrapper instead of invoking tcm's legacy
        // CommonJS CLI. The latter is incompatible with Node.js 26 because it
        // requires yargs/yargs from a package whose scope is ESM.
        let tsx = self.project_root.join("node_modules/.bin/tsx");
        println!("+ {} scripts/GenerateCssTypes.ts --watch", tsx.display());
        Command::new(tsx)
            .args(["scripts/GenerateCssTypes.ts", "--watch"])
...
async fn listen_for_shutdown(shutdown_tx: watch::Sender<bool>) {
    ...
    println!("\nReceived {signal}, shutting down aethernet app dev server...");
