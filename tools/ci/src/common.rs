...
pub(crate) struct CalverEnv {
    ...
    pub(crate) aethernet_build_version: Option<String>,
    pub(crate) aethernet_build_date: Option<String>,
}
...
pub(crate) fn resolve_calver(calver_env: &CalverEnv, now: DateTime<Utc>) -> Result<String> {
    if let Some(version) = calver_env
        .build_version
        .as_deref()
        .or(calver_env.aethernet_build_version.as_deref())
    {
...
fn parse_instant(calver_env: &CalverEnv, now: DateTime<Utc>) -> Result<DateTime<Utc>> {
    let Some(override_value) = calver_env.aethernet_build_date.as_deref() else {
        return Ok(now);
    };
    ...
    if let Ok(date) = NaiveDate::parse_from_str(override_value, "%Y-%m-%d") {
        return date
            ...
            .ok_or_else(|| anyhow!("Invalid AETHERNET_BUILD_DATE: {override_value}"));
    }
    ...
    bail!("Invalid AETHERNET_BUILD_DATE: {override_value}")
}
