...
fn build_markdown_parser_wasm(app_dir: &Path) -> Result<()> {
    ...
    let wasm_path = target_dir.join("wasm32-unknown-unknown/release/aethernet_markdown_parser.wasm");
    let wasm =
...
pub(crate) fn resolve_app_dir() -> Result<PathBuf> {
    ...
    if cwd.file_name().and_then(|value| value.to_str()) == Some("aethernet_app") {
        return Ok(cwd);
    }
    if cwd.join("aethernet_app").is_dir() {
        return Ok(cwd.join("aethernet_app"));
    }
    Err(anyhow!(
        "Could not resolve aethernet_app directory from {}",
        cwd.display()
    ))
}
...
fn libfluxcore_readme_content() -> &'static str {
    "<!-- SPDX-License-Identifier: AGPL-3.0-or-later -->\n\
# libfluxcore\n\n\
Rust WebAssembly helpers and JavaScript codec wrappers for Aethernet media processing.\n"
}
