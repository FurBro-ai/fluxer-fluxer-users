...
const DEFAULT_PUBLIC_ASSET_BASE_URL: &str = "https://aethernetstatic.com";
const DEFAULT_APP_PROXY_TIME_FREEZE_ENABLED: &str = "true";
const DEFAULT_APP_PROXY_BUNDLE_LOCAL_ASSETS: &str = "false";
const DEFAULT_STATIC_BUCKET: &str = "aethernet-static";
...
fn image_repo() -> Result<String> {
    match env::var("IMAGE_REPO") {
        Ok(value) => Ok(value),
        Err(_) => Ok(format!("ghcr.io/{}/aethernet-app-proxy", ghcr_owner()?)),
    }
}
...
fn bake_command(targets: &[&str]) -> Result<CommandSpec> {
    ...
    Ok(CommandSpec::new("docker")
        .args(["buildx", "bake", "-f", "aethernet_app_proxy/docker-bake.hcl"])
        ...
        .env(
            ...
            env::var("AETHERNET_APP_PROXY_TIME_FREEZE_ENABLED")
...
fn default_app_proxy_cache_ref() -> String {
    let owner = ghcr_owner().unwrap_or_else(|_| "aethernetapp".to_string());
    format!("type=registry,ref=ghcr.io/{owner}/aethernet-app-proxy:buildcache-amd64")
}
...
mod tests {
    ...
    fn build_command_sets_bake_environment() {
        let command = CommandSpec::new("docker")
            .args(["buildx", "bake", "-f", "aethernet_app_proxy/docker-bake.hcl"])
            .env("IMAGE_REPO", "ghcr.io/example/aethernet-app-proxy")
        ...
        assert_eq!(
            ...
            vec![
                ...
                OsString::from("bake"),
                OsString::from("-f"),
                OsString::from("aethernet_app_proxy/docker-bake.hcl"),
            ]
        );
        ...
        assert!(command.env.contains(&(
            OsString::from("AETHERNET_APP_PROXY_TIME_FREEZE_ENABLED"),
    ...
    fn dockerfile_guards_the_trim_on_an_absolute_asset_base_url() {
        let dockerfile = include_str!("../../../aethernet_app_proxy/Dockerfile");
        let trim = dockerfile
    ...
    fn dockerfile_workspace_manifest_keeps_the_release_profile() {
        ...
        assert!(
            ...
            "aethernet_svc runs every request on its own tokio task, so a panicking handler must unwind instead of aborting the pod"
        );
    }
    ...
    fn referenced_assets_collects_absolute_and_root_relative_urls() {
        let index = concat!(
            "<script src=\"https://aethernetstatic.com/assets/a.js\"></script>",
    ...
    fn asset_url_joins_the_base_and_key_once() {
        assert_eq!(
            asset_url("https://aethernetstatic.com", "assets/a.js"),
            "https://aethernetstatic.com/assets/a.js"
        );
        assert_eq!(
            asset_url("https://aethernetstatic.com/", "assets/a.js"),
            "https://aethernetstatic.com/assets/a.js"
    ...
    fn assets_image_ref_names_the_canonical_image() {
        assert_eq!(
            assets_image_ref("ghcr.io/example/aethernet-app-proxy", "2026.520.1"),
            "ghcr.io/example/aethernet-app-proxy:2026.520.1-assets"
        );
    }

    #[test]
    fn extract_commands_pin_the_image_platform() {
        let pull = pull_command("ghcr.io/example/aethernet-app-proxy:1-assets", AMD64_PLATFORM);
        ...
        assert_eq!(
            ...
            vec![
                ...
                OsString::from("ghcr.io/example/aethernet-app-proxy:1-assets"),
            ]
        );

        let create = create_command("ghcr.io/example/aethernet-app-proxy:1-arm64", ARM64_PLATFORM);
        assert_eq!(
            create.args,
            vec![
                ...
                OsString::from("ghcr.io/example/aethernet-app-proxy:1-arm64"),
            ]
        );
    ...
    fn bake_publishes_the_canonical_asset_image() {
        let bake = include_str!("../../../aethernet_app_proxy/docker-bake.hcl");
        for entry in [
    ...
    fn self_hosted_workflow_keeps_assets_same_origin() {
        ...
        for entry in [
            ...
            assert!(
                ...
                "a self-hosted dist that inherits the hosted CDN default would ship index.html pointing at aethernetstatic.com, so the workflow must set {entry}"
            );
        }
    }
