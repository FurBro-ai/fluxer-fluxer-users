    ...
    match args.step {
        ...
        CiStep::Test => {
            ...
            run_command(with_test_env(
                CommandSpec::new("pnpm")
                    .args(["--filter", "aethernet_api", "test"])
                    .current_dir(root),
            ))
        }
        CiStep::Knip => {
            ...
            run_aethernet_app_script(&root, "i18n:compile")?;
            run_command(
                CommandSpec::new("pnpm")
                    .args(["exec", "knip"])
                    .current_dir(root),
        ...
        CiStep::GatewayFmt => {
            run_gateway_step(&root.join("aethernet_gateway"), GatewayStep::FmtCheck, "test")
        }
        CiStep::GatewayCompile => {
            run_gateway_step(&root.join("aethernet_gateway"), GatewayStep::Compile, "test")
        }
        CiStep::GatewayDialyzer => {
            run_gateway_step(&root.join("aethernet_gateway"), GatewayStep::Dialyzer, "test")
        }
        CiStep::GatewayEunit => {
            run_gateway_step(&root.join("aethernet_gateway"), GatewayStep::Eunit, "test")
        }
    }
}
...
fn ensure_desktop_build_channel_file(root: &Path) -> Result<()> {
    ...
    write_build_channel_file(&root.join("aethernet_desktop"), &channel)
}
...
fn run_app_test_artifact_generators(root: &Path, wasm: AppWasm) -> Result<()> {
    match (wasm, missing_app_wasm_artifact(root)) {
        (AppWasm::ReuseIfPresent, None) => {
            println!("Reusing restored aethernet_app wasm artifacts");
        }
        (AppWasm::ReuseIfPresent, Some(missing)) => {
            println!(
                "Rebuilding aethernet_app wasm artifacts: {} is missing",
                missing.display()
            );
            run_aethernet_app_script(root, "wasm:codegen")?;
        }
        (AppWasm::Build, _) => run_aethernet_app_script(root, "wasm:codegen")?,
    }
    run_aethernet_app_script(root, "generate:masks")
}
...
fn run_aethernet_app_script(root: &Path, script: &str) -> Result<()> {
    run_command(
        CommandSpec::new("pnpm")
            .args(["--filter", "aethernet_app", script])
            .current_dir(root),
    )
}
...
fn generator_commands(for_typecheck: bool) -> Vec<CommandSpec> {
    let mut commands =
        vec![CommandSpec::new("pnpm").args(["--filter", "@aethernet/schema", "generate"])];
    if for_typecheck {
        ...
        commands.push(CommandSpec::new("pnpm").args(["--filter", "aethernet_app", "i18n:compile"]));
    }
    commands
}
...
fn with_test_env(spec: CommandSpec) -> CommandSpec {
    let nats_url = env::var("AETHERNET_NATS_URL").unwrap_or_else(|_| default_test_nats_url());
    spec.env("AETHERNET_NATS_URL", &nats_url)
        .env(
            "AETHERNET_NATS_CORE_URL",
            env::var("AETHERNET_NATS_CORE_URL").unwrap_or_else(|_| nats_url.clone()),
        )
        .env(
            "AETHERNET_NATS_JETSTREAM_URL",
            env::var("AETHERNET_NATS_JETSTREAM_URL").unwrap_or_else(|_| nats_url.clone()),
        )
}
...
mod tests {
    ...
    fn with_test_env_sets_all_nats_urls_and_leaves_worker_counts_to_vitest() {
        ...
        assert_eq!(
            env.get(&OsString::from("AETHERNET_NATS_URL")),
            Some(&default_nats_url)
        );
        assert_eq!(
            env.get(&OsString::from("AETHERNET_NATS_CORE_URL")),
            Some(&default_nats_url)
        );
        assert_eq!(
            env.get(&OsString::from("AETHERNET_NATS_JETSTREAM_URL")),
            Some(&default_nats_url)
        );
    ...
    fn workspace_tests_exclude_desktop_and_leave_concurrency_to_pnpm() {
        ...
        assert!(args.windows(2).any(|pair| pair
            == [
                OsString::from("--filter"),
                OsString::from("!aethernet_desktop")
            ]));
    }
    ...
    fn missing_app_wasm_artifact_reports_the_first_absent_output() {
        ...
        assert_eq!(
            ...
            Some(root.join("aethernet_app/pkgs/libfluxcore/libfluxcore.js"))
        );
    ...
    fn image_dockerfiles_carry_the_release_label_block() {
        ...
        for (name, title, dockerfile) in [
            (
                "aethernet_admin",
                "aethernet-admin",
                include_str!("../../../aethernet_admin/Dockerfile"),
            ),
            (
                "aethernet_api",
                "aethernet-api",
                include_str!("../../../aethernet_api/Dockerfile"),
            ),
            (
                "aethernet_app_proxy",
                "aethernet-app-proxy",
                include_str!("../../../aethernet_app_proxy/Dockerfile"),
            ),
            (
                "aethernet_docs",
                "aethernet-docs",
                include_str!("../../../aethernet_docs/Dockerfile"),
            ),
            (
                "aethernet_gateway",
                "aethernet-gateway",
                include_str!("../../../aethernet_gateway/Dockerfile"),
            ),
            (
                "aethernet_gifs",
                "aethernet-gifs",
                include_str!("../../../aethernet_gifs/Dockerfile"),
            ),
            (
                "aethernet_media_proxy",
                "aethernet-media-proxy",
                include_str!("../../../aethernet_media_proxy/Dockerfile"),
            ),
            (
                "aethernet_messages",
                "aethernet-messages",
                include_str!("../../../aethernet_messages/Dockerfile"),
            ),
            (
                "aethernet_recon",
                "aethernet-recon",
                include_str!("../../../aethernet_recon/Dockerfile"),
            ),
            (
                "aethernet_snowflakes",
                "aethernet-snowflakes",
                include_str!("../../../aethernet_snowflakes/Dockerfile"),
            ),
            (
                "aethernet_static",
                "aethernet-static",
                include_str!("../../../aethernet_static/Dockerfile"),
            ),
            (
                "aethernet_unfurl",
                "aethernet-unfurl",
                include_str!("../../../aethernet_unfurl/Dockerfile"),
            ),
            (
                "aethernet_users",
                "aethernet-users",
                include_str!("../../../aethernet_users/Dockerfile"),
            ),
        ] {
            assert!(
