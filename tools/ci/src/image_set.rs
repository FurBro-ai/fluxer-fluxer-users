...
const DEFAULT_REGISTRY: &str = "ghcr.io/aethernetapp";
const DEFAULT_MOVING_TAGS: &str = "v1,latest";
const DEFAULT_FROM_TAG: &str = "v1";
const DEFAULT_OUT_DIR: &str = "release-out";
const RELEASE_COMPONENT: &str = "aethernet-release";
const COMPOSE_IMAGE_PREFIX: &str =
    "${AETHERNET_REGISTRY:-ghcr.io/${AETHERNET_REGISTRY_OWNER:-aethernetapp}}";
...
const COMPONENTS: &[Component] = &[
    ...
    Component {
        image: "aethernet-app-proxy",
        services: &[],
    },
    Component {
        image: "aethernet-app-proxy-self-hosted",
        services: &["app-proxy"],
    },
    Component {
        image: "aethernet-docs",
        services: &[],
    },
    Component {
        image: "aethernet-gateway",
        services: &["gateway"],
    },
    Component {
        image: "aethernet-gifs",
        services: &["gifs", "gifs-shard"],
    },
    Component {
        image: "aethernet-media-proxy",
        services: &["media-proxy"],
    },
    Component {
        image: "aethernet-messages",
        services: &["messages", "messages-shard"],
    },
    Component {
        image: "aethernet-recon",
        services: &[],
    },
    Component {
        image: "aethernet-snowflakes",
        services: &["snowflakes", "snowflakes-shard"],
    },
    Component {
        image: "aethernet-static",
        services: &["static-proxy"],
    },
    Component {
        image: "aethernet-unfurl",
        services: &["unfurl", "unfurl-shard"],
    },
    Component {
        image: "aethernet-users",
...
fn workflow_suffix(image: &str) -> &str {
    image.strip_prefix("aethernet-").unwrap_or(image)
}
...
mod tests {
    ...
    const COMPOSE_IMAGE_TAG: &str = "${AETHERNET_IMAGE_TAG:-v1}";
    ...
    fn components_cover_every_self_hosting_service() {
        ...
        for line in compose.lines() {
            ...
            if !reference.starts_with("${AETHERNET_REGISTRY") {
                continue;
            }
            let image = compose_component(reference)
                .unwrap_or_else(|| panic!("Unexpected Aethernet image line: {trimmed}"));
            let service = service.expect("a Aethernet image line must follow a service header");
    ...
    fn manifest_rejects_bad_digest() {
        ...
        assert!(error.contains("aethernet-admin"), "{error}");
    ...
    fn manifest_requires_source_sha_unless_allowed() {
        ...
        let error = build_manifest(RELEASE_VERSION, DEFAULT_REGISTRY, &resolved, false)
            .unwrap_err()
            .to_string();
        assert!(error.contains("aethernet-admin@2026.830.191141"), "{error}");
        ...
        assert_eq!(manifest.release_tag, "aethernet-release@2026.901.120000");
    ...
    fn compose_override_pins_every_service_by_digest() {
        ...
        let api = manifest
            .components
            .iter()
            .find(|entry| entry.component == "aethernet-api")
            .unwrap();
        for service in ["api", "worker"] {
            assert!(
                rendered.contains(&format!(
                    "  {service}:\n    image: {COMPOSE_IMAGE_PREFIX}/aethernet-api@${{AETHERNET_API_IMAGE_DIGEST:-{}}}\n",
                    api.digest
                )),
                "{rendered}"
            );
        ...
        let app_proxy = manifest
            .components
            .iter()
            .find(|entry| entry.component == "aethernet-app-proxy-self-hosted")
            .unwrap();
        assert!(
            rendered.contains(&format!(
                "  app-proxy:\n    image: {COMPOSE_IMAGE_PREFIX}/aethernet-app-proxy-self-hosted@${{AETHERNET_APP_PROXY_SELF_HOSTED_IMAGE_DIGEST:-{}}}\n",
                app_proxy.digest
            )),
            "{rendered}"
        );
        assert!(!rendered.contains("aethernet-docs"), "{rendered}");
        assert!(
            rendered.starts_with(
                "# aethernet-release@2026.901.120000 image set\n# docker compose -f docker-compose.yml -f aethernet-release-2026.901.120000.yml up -d\nservices:\n"
            ),
            "{rendered}"
        );
    }
    ...
    fn promote_command_targets_the_digest() {
        ...
        let spec = promote_command(
            "ghcr.io/aethernetapp/aethernet-api",
            &digest,
        ...
        assert_eq!(
            ...
            [
                ...
                "create",
                "-t",
                "ghcr.io/aethernetapp/aethernet-api:v1",
                "-t",
                "ghcr.io/aethernetapp/aethernet-api:latest",
                &format!("ghcr.io/aethernetapp/aethernet-api@{digest}"),
            ]
        );
    }
    ...
    fn unpublished_moving_tags_are_reported_without_blocking_the_promote() {
        ...
        let complete = unpublished_moving_tags("ghcr.io/aethernetapp", "aethernet-docs", &tags, |_| true);
        assert!(complete.is_empty());

        let bootstrapping =
            unpublished_moving_tags("ghcr.io/aethernetapp", "aethernet-docs", &tags, |reference| {
                !reference.starts_with("ghcr.io/aethernetapp/aethernet-recon:")
            });
        assert_eq!(
            bootstrapping,
            [
                "aethernet-recon has no published v1 image, so v1 stays an incomplete set until the build-recon workflow runs",
                "aethernet-recon has no published latest image, so latest stays an incomplete set until the build-recon workflow runs",
            ]
        );
    }
    ...
    fn unpublished_moving_tags_never_reports_the_component_being_promoted() {
        ...
        let warnings =
            unpublished_moving_tags("ghcr.io/aethernetapp", "aethernet-recon", &tags, |reference| {
                !reference.starts_with("ghcr.io/aethernetapp/aethernet-recon:")
            });
    ...
    fn parse_component_versions_rejects_junk() {
        let pins = parse_component_versions(&["aethernet-api=2026.830.191141".to_string()]).unwrap();
        assert_eq!(pins["aethernet-api"], "2026.830.191141");

        assert!(parse_component_versions(&["aethernet-api".to_string()]).is_err());
        assert!(parse_component_versions(&["aethernet-nope=2026.830.191141".to_string()]).is_err());
        assert!(parse_component_versions(&["aethernet-api=v1".to_string()]).is_err());
        assert!(
            parse_component_versions(&[
                "aethernet-api=2026.830.191141".to_string(),
                "aethernet-api=2026.830.191142".to_string(),
            ])
            .is_err()
        );
    }
    ...
    fn digest_env_name_matches_component() {
        assert_eq!(digest_env_name("aethernet-api"), "AETHERNET_API_IMAGE_DIGEST");
        assert_eq!(
            digest_env_name("aethernet-app-proxy-self-hosted"),
            "AETHERNET_APP_PROXY_SELF_HOSTED_IMAGE_DIGEST"
        );
    }
    ...
    fn validate_registry_rejects_image_references() {
        ...
        validate_registry("registry.example.com:5000/aethernet").unwrap();
        assert!(validate_registry("").is_err());
        assert!(validate_registry("ghcr.io/aethernetapp/").is_err());
        assert!(validate_registry("ghcr.io/aethernetapp/aethernet-api:v1").is_err());
        assert!(validate_registry("ghcr.io/ aethernetapp").is_err());
    }
    ...
    fn oldest_common_commit_picks_the_commit_every_image_contains() {
        ...
        let candidates = [
            ("aethernet-api", history[2]),
            ("aethernet-gateway", history[0]),
            ("aethernet-users", history[1]),
        ];
        assert_eq!(
        ...
        let single = [("aethernet-api", history[2])];
        assert_eq!(
    ...
    fn oldest_common_commit_refuses_divergent_component_builds() {
        let candidates = [("aethernet-api", "aa"), ("aethernet-gateway", "bb")];
        let error =
            oldest_common_commit(&candidates, |_: &str, _: &str| Ok(CommitCompare::Diverged))
                .unwrap_err()
                .to_string();
        assert!(error.contains("aethernet-api"), "{error}");
        assert!(error.contains("aethernet-gateway"), "{error}");
    ...
    fn release_filenames_are_stable() {
        assert_eq!(
            ...
            "aethernet-release-2026.901.120000.json"
        );
        assert_eq!(
            compose_filename(RELEASE_VERSION),
            "aethernet-release-2026.901.120000.yml"
        );
    }
}
