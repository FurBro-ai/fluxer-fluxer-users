...
pub(crate) fn run_gateway_step(
    ...
    match step {
        ...
        GatewayStep::Compile => {
            run_rebar(gateway_dir, ["get-deps"])?;
            patch_ezstd_for_modern_otp(gateway_dir)?;
            run_rebar(gateway_dir, ["compile"])
        }
...
fn patch_ezstd_for_modern_otp(gateway_dir: &Path) -> Result<()> {
    let build_dir = gateway_dir.join("_build");
    if !build_dir.is_dir() {
        return Ok(());
    }
    for profile in fs::read_dir(&build_dir)
        .with_context(|| format!("Failed to read {}", build_dir.display()))?
    {
        let nif_mk = profile?.path().join("lib/ezstd/c_src/nif.mk");
        if !nif_mk.is_file() {
            continue;
        }
        let source = fs::read_to_string(&nif_mk)
            .with_context(|| format!("Failed to read {}", nif_mk.display()))?;
        let patched = source.replace("erl -noshell -s init stop -eval", "erl -noshell -eval");
        let patched = patched.replace(
            r#"ERTS_INCLUDE_DIR ?= ~s/erts-~s/include/~n\" \"#,
            r#"ERTS_INCLUDE_DIR ?= ~s/erts-~s/include/~n\" ++ \"#,
        );
        let patched = patched.replacen(
            r#"ERL_INTERFACE_INCLUDE_DIR ?= ~s~n\" \"#,
            r#"ERL_INTERFACE_INCLUDE_DIR ?= ~s~n\" ++ \"#,
            1,
        );
        if patched != source {
            fs::write(&nif_mk, patched)
                .with_context(|| format!("Failed to patch {}", nif_mk.display()))?;
            println!("Patched ezstd string concatenation for modern Erlang/OTP");
        }
    }
    Ok(())
}
...
fn build_gateway_nifs(gateway_dir: &Path) -> Result<()> {
    let profile =
        NifBuildProfile::from_env_value(env::var("AETHERNET_GATEWAY_NIF_PROFILE").ok().as_deref());
...
fn resolve_gateway_dir() -> Result<PathBuf> {
    ...
    if cwd.join("rebar.config").is_file()
        && cwd.file_name().and_then(|value| value.to_str()) == Some("aethernet_gateway")
    {
        return Ok(cwd);
    }
    if cwd.join("aethernet_gateway/rebar.config").is_file() {
        return Ok(cwd.join("aethernet_gateway"));
    }
    Err(anyhow!(
        "Could not resolve aethernet_gateway directory from {}",
        cwd.display()
    ))
}
...
mod tests {
    ...
    fn gateway_nif_build_plan_matches_legacy_artifact_layout() {
        let gateway_dir = Path::new("/repo/aethernet_gateway");
        ...
        assert_eq!(
            ...
            GatewayNifBuild {
                ...
                native_dir: PathBuf::from(
                    "/repo/aethernet_gateway/native/push_markdown_plaintext_nif"
                ),
                cargo_args: vec![OsString::from("build"), OsString::from("--release")],
                artifact_path: PathBuf::from(
                    "/repo/aethernet_gateway/native/push_markdown_plaintext_nif/target/release/libpush_markdown_plaintext_nif.so"
                ),
                output_path: PathBuf::from(
                    "/repo/aethernet_gateway/priv/push_markdown_plaintext_nif.so"
                ),
            }
        );
    }
    ...
    fn debug_build_plan_omits_release_arg_and_uses_debug_target_dir() {
        let build = gateway_nif_build(
            Path::new("/repo/aethernet_gateway"),
            NifBuildProfile::Debug,
            "lib",
            "dylib",
        ...
        assert_eq!(
            build.artifact_path,
            PathBuf::from(
                "/repo/aethernet_gateway/native/guild_member_list_oset_nif/target/debug/libguild_member_list_oset_nif.dylib"
            )
        );
        assert_eq!(
            build.output_path,
            PathBuf::from("/repo/aethernet_gateway/priv/guild_member_list_oset_nif.so")
        );
    }
    ...
    fn rebar_command_runs_in_gateway_dir_and_sets_skip_env_for_compile() {
        let command = rebar_command(Path::new("/repo/aethernet_gateway"), ["compile"]);

        assert_eq!(command.program, OsString::from("rebar3"));
        assert_eq!(command.args, vec![OsString::from("compile")]);
        assert_eq!(command.cwd, Some(PathBuf::from("/repo/aethernet_gateway")));
    ...
    fn rebar_command_keeps_project_plugins_for_fmt() {
        let command = rebar_command(Path::new("/repo/aethernet_gateway"), ["fmt", "--check"]);
