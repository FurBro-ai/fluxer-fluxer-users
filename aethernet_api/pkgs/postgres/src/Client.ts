...
class PostgresClient implements IPostgresClient {
	...
	private async openPool(): Promise<void> {
		const pool = new pg.Pool({
			...
			database: this.config.url ? undefined : (this.config.database ?? 'aethernet'),
			user: this.config.url ? undefined : (this.config.username ?? 'aethernet'),
			password: this.config.url ? undefined : (this.config.password ?? 'aethernet'),
	...
	kvTable(): string {
		return this.config.kvTable ?? 'aethernet_kv';
	}
