...
import {createMockLogger} from '@aethernet/logger/src/mock';
...
describe('TwilioSmsProvider', () => {
	...
	it('can start verification with auto channel, device IP, and programmable rate limits', async () => {
		...
		const result = await provider.startVerificationWithResult('+15551234567', {
			...
			rateLimits: {
				aethernet_user_id: '123',
				aethernet_phone_prefix: '+1555',
			},
		});
		const request = getCapturedRequest(capturedRequest);
		...
		expect(request.body).toContain('RateLimits%5Baethernet_user_id%5D=123');
		expect(request.body).toContain('RateLimits%5Baethernet_phone_prefix%5D=%2B1555');
	});
