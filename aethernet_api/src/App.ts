...
import {createServer, setupGracefulShutdown} from '@aethernet/hono/src/Server';
...
async function main(): Promise<void> {
	...
	Logger.info({port: Config.port}, `Starting Aethernet API on port ${Config.port}`);
...
main().catch((err) => {
	Logger.fatal({error: err}, 'Failed to start Aethernet API');
	process.exit(1);
});
