...
const APNS_CATEGORY_MESSAGE = 'AETHERNET_MESSAGE';
...
function buildApnsPayload(payload: Record<string, unknown>): Record<string, unknown> {
	...
	const title = optionalString(notification.title) ?? optionalString(payload.title) ?? 'Aethernet';
	...
	const threadId =
		optionalString(data.notification_tag) ?? (channelId ? `channel:${channelId}` : undefined) ?? 'aethernet-message';
