		...
		await withS3Config(
			{
				...
				forcePathStyle: true,
				region: 'us-east-1',
				accessKeyId: 'aethernet',
				secretAccessKey: 'aethernet-secret',
				buckets: {uploads: 'aethernet-uploads'},
			},
			async () => {
				const service = new StorageService();
				const uploadUrl = await service.getPresignedUploadURL({
					bucket: 'aethernet-uploads',
				...
				expect(decodeURIComponent(url.pathname)).toBe(
					'/aethernet-uploads/stream_previews/1511582191061041152:1511582191061041156:talpa-pentatonic.jpg',
				);
			},
		);
	});
...
describe('StorageService.readObject', () => {
	it('returns the whole object when it fits inside the ceiling', async () => {
		...
		const data = await service.readObject('aethernet-uploads', 'stream_previews/preview.jpg', 5);
	...
	it('rejects an object larger than the ceiling instead of truncating it', async () => {
		...
		await expect(service.readObject('aethernet-uploads', 'stream_previews/preview.jpg', 5)).rejects.toThrow(
	...
	it('reads the whole object when no ceiling is given', async () => {
		const service = new StorageService();
		stubObjectBody(service, [new Uint8Array([1, 2, 3]), new Uint8Array([4, 5, 6])]);
		const data = await service.readObject('aethernet-uploads', 'stream_previews/preview.jpg');
...
describe('StorageService.readObject', () => {
	it('refuses an object whose declared length is already over the cap', async () => {
		...
		await expect(service.readObject('aethernet-downloads', 'desktop/stable/manifest.json', 32)).rejects.toThrow(
	...
	it('returns every byte of an object that exactly fills the cap', async () => {
		...
		await expect(service.readObject('aethernet-downloads', 'desktop/stable/manifest.json', 32)).resolves.toEqual(
	...
	it('rejects rather than hanging when the object store resets the body mid-transfer', async () => {
		...
		const read = service.readObject('aethernet-downloads', 'desktop/stable/manifest.json', 1024);
...
describe('StorageService.listObjects', () => {
	it('returns every page of a truncated listing', async () => {
		...
		const objects = await service.listObjects({bucket: 'aethernet-downloads', prefix: 'desktop/'});
	...
	it('throws a typed overflow error when the prefix outgrows the requested cap', async () => {
		...
		await expect(
			service.listObjects({bucket: 'aethernet-downloads', prefix: 'desktop/', maxObjects: 2}),
	...
	it('throws rather than returning a partial listing when the continuation token is missing', async () => {
		...
		await expect(service.listObjects({bucket: 'aethernet-downloads', prefix: 'desktop/'})).rejects.toThrow(
