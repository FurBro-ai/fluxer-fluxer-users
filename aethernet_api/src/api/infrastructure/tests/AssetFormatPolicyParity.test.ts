...
import {ASSET_FORMAT_POLICY} from '@aethernet/constants/src/AssetFormatPolicy';
...
describe('AssetFormatPolicy parity', () => {
	it('matches the media proxy extension MIME table', () => {
		const rustSource = fs.readFileSync(path.join(REPO_ROOT, 'aethernet_media_proxy/src/constants.rs'), 'utf8');
