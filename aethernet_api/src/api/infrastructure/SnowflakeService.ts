...
export class SnowflakeService implements ISnowflakeService {
	...
	constructor({
		...
		this.batchSize = requireIntegerInRange(
			'AETHERNET_SNOWFLAKE_SERVICE_BATCH_SIZE',
			batchSize,
			1,
			MAX_REMOTE_BATCH_SIZE,
		);
		this.lowWatermark = requireIntegerInRange(
			'AETHERNET_SNOWFLAKE_SERVICE_LOW_WATERMARK',
			lowWatermark,
			0,
			this.batchSize - 1,
		);
		this.requestTimeoutMs = requireIntegerInRange(
			'AETHERNET_SNOWFLAKE_SERVICE_REQUEST_TIMEOUT_MS',
			requestTimeoutMs,
			1,
			60_000,
		);
		this.maxBufferAgeMs = requireIntegerInRange(
			'AETHERNET_SNOWFLAKE_SERVICE_MAX_BUFFER_AGE_MS',
			maxBufferAgeMs,
			1,
			60_000,
		);
