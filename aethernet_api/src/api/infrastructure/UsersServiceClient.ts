...
const USERS_SERVICE_SUBJECT = process.env.AETHERNET_USERS_SERVICE_SUBJECT || 'svc.users';
...
export class NatsUsersServiceClient implements IUsersServiceClient {
	...
	constructor(
		...
		requireIntegerInRange('AETHERNET_USERS_SERVICE_TIMEOUT_MS', requestTimeoutMs, 1, MAX_REQUEST_TIMEOUT_MS);
		requireIntegerInRange(
			'AETHERNET_USERS_SERVICE_INFLIGHT_MAX_ENTRIES',
			maxInflightEntries,
			0,
...
export function createUsersServiceClient(): IUsersServiceClient {
	...
	const manager = new NatsConnectionManager({
		...
		name: process.env.AETHERNET_USERS_SERVICE_NATS_CLIENT_NAME || 'aethernet-api-users',
	});
	usersServiceClient = new NatsUsersServiceClient(
		manager,
		readOptionalIntegerEnv('AETHERNET_USERS_SERVICE_TIMEOUT_MS'),
		USERS_SERVICE_SUBJECT,
		readOptionalIntegerEnv('AETHERNET_USERS_SERVICE_INFLIGHT_MAX_ENTRIES'),
	);
