...
describe('resolveLivekitEndpoint', () => {
	it('keeps the public port when the instance does not serve on the default port', () => {
		...
		expect(resolveLivekitEndpoint('', 'https://aethernet.example.com:8443/api')).toBe(
			'wss://aethernet.example.com:8443/livekit',
		);
	});

	it('omits the port when the public endpoint uses the default port for its scheme', () => {
		expect(resolveLivekitEndpoint(undefined, 'https://aethernet.example.com/api')).toBe(
			'wss://aethernet.example.com/livekit',
		);
		expect(resolveLivekitEndpoint(undefined, 'https://aethernet.example.com:443/api')).toBe(
			'wss://aethernet.example.com/livekit',
		);
		expect(resolveLivekitEndpoint(undefined, 'http://aethernet.example.com:80/api')).toBe(
			'ws://aethernet.example.com/livekit',
		);
	});
	...
	it('maps https to wss and http to ws', () => {
		...
		expect(resolveLivekitEndpoint(undefined, 'http://aethernet.example.com/api')).toBe(
			'ws://aethernet.example.com/livekit',
		);
	});
