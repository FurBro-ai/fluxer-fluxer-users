...
function budgetEnabled(): boolean {
	return process.env.AETHERNET_IPINFO_BUDGET_ENABLED !== '0';
}

function burstConfigFor(priority: IpInfoLookupPriority): {maxTokens: number; refillPerMin: number} {
	if (priority === 'critical') {
		return {
			maxTokens: positiveNumberFromEnv('AETHERNET_IPINFO_BUDGET_CRITICAL_BURST', 60),
			refillPerMin: positiveNumberFromEnv('AETHERNET_IPINFO_BUDGET_CRITICAL_REFILL_PER_MIN', 60),
		};
	}
	if (priority === 'background') {
		return {
			maxTokens: positiveNumberFromEnv('AETHERNET_IPINFO_BUDGET_BACKGROUND_BURST', 120),
			refillPerMin: positiveNumberFromEnv('AETHERNET_IPINFO_BUDGET_BACKGROUND_REFILL_PER_MIN', 30),
		};
	}
	return {
		maxTokens: positiveNumberFromEnv('AETHERNET_IPINFO_BUDGET_STANDARD_BURST', 240),
		refillPerMin: positiveNumberFromEnv('AETHERNET_IPINFO_BUDGET_STANDARD_REFILL_PER_MIN', 120),
	};
}

function monthlyCeilingFor(priority: IpInfoLookupPriority): number {
	const monthlyMax = positiveNumberFromEnv('AETHERNET_IPINFO_BUDGET_MONTHLY_MAX', 140000);
	if (priority === 'critical') return monthlyMax;
	const percent =
		priority === 'background'
			? positiveNumberFromEnv('AETHERNET_IPINFO_BUDGET_BACKGROUND_MONTHLY_PCT', 60)
			: positiveNumberFromEnv('AETHERNET_IPINFO_BUDGET_STANDARD_MONTHLY_PCT', 90);
