...
afterEach(() => {
	...
	delete process.env.AETHERNET_RISK_IPINFO_PRESCREEN_ENABLED;
	delete process.env.AETHERNET_RISK_IPINFO_PRESCREEN_ALLOW_ASNS;
});
...
describe('ipInfoPrescreenOptionsFromEnv', () => {
	...
	it('treats only 1 and true as enabled', () => {
		process.env.AETHERNET_RISK_IPINFO_PRESCREEN_ENABLED = '1';
		expect(ipInfoPrescreenOptionsFromEnv().enabled).toBe(true);
		process.env.AETHERNET_RISK_IPINFO_PRESCREEN_ENABLED = 'TRUE';
		expect(ipInfoPrescreenOptionsFromEnv().enabled).toBe(true);
		process.env.AETHERNET_RISK_IPINFO_PRESCREEN_ENABLED = 'yes';
		expect(ipInfoPrescreenOptionsFromEnv().enabled).toBe(false);
		process.env.AETHERNET_RISK_IPINFO_PRESCREEN_ENABLED = '0';
		expect(ipInfoPrescreenOptionsFromEnv().enabled).toBe(false);
	});

	it('parses a comma separated allowlist and drops non numeric entries', () => {
		process.env.AETHERNET_RISK_IPINFO_PRESCREEN_ALLOW_ASNS = ' 64500, 64501 ,,notanasn,64502x,-3,64503 ';
	...
	it('parses an empty allowlist from an empty or whitespace value', () => {
		process.env.AETHERNET_RISK_IPINFO_PRESCREEN_ALLOW_ASNS = '';
