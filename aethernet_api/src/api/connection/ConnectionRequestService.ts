...
import {
	...
	type ConnectionType,
	ConnectionTypes,
} from '@aethernet/constants/src/ConnectionConstants';
import type {
	ConnectionResponse,
...
} from '@aethernet/schema/src/domains/connection/ConnectionSchemas';
	...
	private generateVerificationInstructions(connectionType: ConnectionType, identifier: string): string {
		switch (connectionType) {
			case ConnectionTypes.DOMAIN:
				return `Add a DNS TXT record at _aethernet.${identifier} with the value aethernet-verification=<token>, or serve the token at https://${identifier}/.well-known/aethernet-verification`;
			default:
