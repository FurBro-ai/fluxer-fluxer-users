...
import {buildDiscoveryResponse, type DiscoveryStaticInput} from '@aethernet/instance_bootstrap/src/BuildDiscovery';
import type {InstanceAppPublic} from '@aethernet/instance_bootstrap/src/Types';
import {WellKnownAethernetResponse} from '@aethernet/schema/src/domains/instance/InstanceSchemas';
...
export function InstanceController(app: Hono<HonoEnv>) {
	app.get(
		'/.well-known/aethernet',
		RateLimitMiddleware(RateLimitConfigs.INSTANCE_INFO),
		OpenAPI({
			operationId: 'get_well_known_aethernet',
			summary: 'Get instance discovery document',
			responseSchema: WellKnownAethernetResponse,
			statusCode: 200,
			security: [],
			tags: ['Instance'],
			description:
				'Returns the instance discovery document including API endpoints, feature flags, and limits. This is the canonical discovery endpoint for all Aethernet clients.',
		}),
		async (ctx) => {
