...
export function parseDesktopReleaseDescriptor(value: unknown): DesktopReleaseDescriptor | null {
	...
	const expectedTag = `aethernet-desktop-${value.channel}@${value.version}`;
	const expectedStoragePrefix = `desktop/${value.channel}/`;
	const expectedReleasePrefix = `${value.channel === 'canary' ? 'Aethernet-Canary' : 'Aethernet'}-${value.version}-`;
