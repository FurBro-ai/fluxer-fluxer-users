...
describe('presigned download redirects', () => {
	it('signs the requested object and returns its URL', async () => {
		...
		const url = await service.getPresignedDownloadRedirect({
			key: 'desktop/canary/darwin/universal/Aethernet.dmg',
			filename: 'Aethernet.dmg',
			expiresIn: 900,
		});
		expect(url).toBe('https://storage.example.test/desktop/canary/darwin/universal/Aethernet.dmg?signed=1');
		expect(presignCalls).toHaveLength(1);
		expect(presignCalls[0]?.key).toBe('desktop/canary/darwin/universal/Aethernet.dmg');
	...
	it('preserves the download filename and content type through the redirect', async () => {
		...
		expect(presignCalls[0]?.responseContentDisposition).toBe(
			`attachment; filename="${encodeURIComponent('Aethernet Canary.dmg')}"`,
		);
	});
