...
import type {
	...
	DesktopFormat,
	DesktopPlatform,
} from '@aethernet/schema/src/domains/download/DownloadSchemas';
...
const GITHUB_RELEASE_DOWNLOAD_BASE_URL = 'https://github.com/aethernetapp/aethernet/releases/download';
...
export class DownloadService {
	...
	async resolveGitHubDesktopRelease(key: string): Promise<GitHubDesktopReleaseResolution> {
		...
		if (
			...
			descriptor.release_tag !== `aethernet-desktop-${scope.channel}@${manifest.version}`
		) {
			throw new Error(
	...
	private async readDesktopReleaseState(channel: DesktopChannel, version: string): Promise<DesktopReleaseState> {
		...
		if (
			...
			readiness.release_tag !== `aethernet-desktop-${channel}@${version}` ||
	...
	private buildPossibleFilenames(
		...
		for (const archSuffix of this.getArchTokens(archMap[arch as 'x64' | 'arm64'])) {
			...
			if (format === 'setup') {
				filenames.add(`aethernet-${channel}-${version}-${archSuffix}-setup${ext}`);
				filenames.add(`Aethernet-${channel}-${version}-${archSuffix}-Setup${ext}`);
				filenames.add(`aethernet-${version}-${archSuffix}-setup${ext}`);
				filenames.add(`Aethernet-${version}-${archSuffix}-Setup${ext}`);
			} else if (format === 'portable') {
				filenames.add(`aethernet-${channel}-${version}-portable-${archSuffix}${ext}`);
				filenames.add(`Aethernet-${version}-portable-${archSuffix}${ext}`);
			} else {
				filenames.add(`aethernet-${channel}-${version}-${archSuffix}${ext}`);
				filenames.add(`aethernet-${version}-${archSuffix}${ext}`);
				filenames.add(`Aethernet-${channel}-${version}-${archSuffix}${ext}`);
				filenames.add(`Aethernet-${version}-${archSuffix}${ext}`);
			}
		}
	...
	private getModernProductNames(channel: DesktopChannel): Array<string> {
		return channel === 'canary' ? ['Aethernet-Canary', 'Aethernet Canary'] : ['Aethernet'];
	}
