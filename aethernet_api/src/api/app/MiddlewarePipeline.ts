...
import {applyMiddlewareStack} from '@aethernet/hono/src/middleware/MiddlewareStack';
import {createInfoRequestLogger, requestLogger} from '@aethernet/hono/src/middleware/RequestLogger';
import {resolveClientIpHeaderName} from '@aethernet/ip_utils/src/ClientIp';
...
export function configureMiddleware(routes: HonoApp, options: MiddlewarePipelineOptions): void {
	...
	applyMiddlewareStack(routes, {
		...
		cors: {
			...
			exposedHeaders: [HttpHeaders.X_AETHERNET_VERSION, HttpHeaders.ETAG],
		},
		skipLogger: true,
	...
	if (nodeEnv === 'production') {
		routes.use('*', async (ctx, next) => {
			...
			if (ctx.req.method !== 'GET' && (host === 'web.aethernet.app' || host === 'web.canary.aethernet.app')) {
