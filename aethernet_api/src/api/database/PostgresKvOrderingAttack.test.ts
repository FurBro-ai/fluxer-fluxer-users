...
const CONTAINER = `aethernet-kvorder-${process.pid.toString(36)}-${Date.now().toString(36)}`;
...
suite('PostgresKvQueryExecutor ordering and limit attack', () => {
	...
	beforeAll(async () => {
		...
		for (let attempt = 0; attempt < 180 && !ready; attempt += 1) {
			...
			try {
				await initPostgres({url: `postgres://aethernet:aethernet@127.0.0.1:${port}/aethernet`, maxConnections: 6});
