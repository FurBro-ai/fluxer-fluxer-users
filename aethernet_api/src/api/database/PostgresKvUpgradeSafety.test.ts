...
const CONTAINER = `aethernet-kvupgrade-${process.pid.toString(36)}-${Date.now().toString(36)}`;
const KV = 'kv_upgrade';
...
if (dockerUp) {
	beforeAll(async () => {
		...
		startDockerContainer([
			...
			CONTAINER,
			'-e',
			'POSTGRES_USER=aethernet',
			'-e',
			'POSTGRES_PASSWORD=aethernet',
			'-e',
			'POSTGRES_DB=aethernet',
			'-p',
			`127.0.0.1:${PORT}:5432`,
			POSTGRES_IMAGE,
			'-c',
		...
		for (let attempt = 0; attempt < 180; attempt += 1) {
			await sleep(500);
			const probe = spawnSync('docker', ['exec', CONTAINER, 'pg_isready', '-U', 'aethernet', '-d', 'aethernet'], {
				stdio: 'ignore',
			});
			if (probe.status !== 0) continue;
			try {
				await initPostgres({url: `postgres://aethernet:aethernet@127.0.0.1:${PORT}/aethernet`, maxConnections: 8});
...
SELECT 'push_subscriptions',
	format('{"__aethernet_type":"bigint","value":"%s"}%s{"__aethernet_type":"bigint","value":"%s"}', u, chr(31), s),
	format('{"__aethernet_type":"bigint","value":"%s"}%s{"__aethernet_type":"bigint","value":"%s"}', u, chr(31), s),
	jsonb_build_object(
		'user_id', jsonb_build_object('__aethernet_type', 'bigint', 'value', u::text),
		'subscription_id', jsonb_build_object('__aethernet_type', 'bigint', 'value', s::text),
