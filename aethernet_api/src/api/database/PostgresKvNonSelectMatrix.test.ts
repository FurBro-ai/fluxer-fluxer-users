...
const CONTAINER = `aethernet-kvm-${process.pid.toString(36)}-${Date.now().toString(36)}`;
...
describe.skipIf(!dockerAvailable)('postgres kv non-select matrix', () => {
	...
	beforeAll(async () => {
		...
		for (let attempt = 0; attempt < 180 && !ready; attempt += 1) {
			await sleep(500);
			if (
				spawnSync('docker', ['exec', CONTAINER, 'pg_isready', '-U', 'aethernet', '-d', 'aethernet'], {stdio: 'ignore'})
					.status !== 0
			)
				continue;
			try {
