...
import type {LookupUserRequest} from '@aethernet/schema/src/domains/admin/AdminUserSchemas';
...
export class AdminUserLookupService {
	...
	async lookupUser(data: LookupUserRequest, acls: ReadonlySet<string>) {
		...
		const aethernetTagMatch = query.match(/^(.+)#(\d{1,4})$/);
		if (aethernetTagMatch) {
			const username = aethernetTagMatch[1];
			const discriminator = parseInt(aethernetTagMatch[2], 10);
