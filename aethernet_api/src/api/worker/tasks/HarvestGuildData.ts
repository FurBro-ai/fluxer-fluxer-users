...
import {GUILD_TEXT_BASED_CHANNEL_TYPES} from '@aethernet/constants/src/ChannelConstants';
...
const harvestGuildData: ArchiveTaskHandler = async (payload, helpers, attempt) => {
	...
	let fileSize: bigint;
	try {
		await using tmpDir = await fs.promises.mkdtempDisposable(path.join(os.tmpdir(), 'aethernet-guild-archive-'));
