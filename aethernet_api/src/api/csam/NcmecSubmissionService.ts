...
import {DeletionReasons} from '@aethernet/constants/src/Core';
import {CATEGORY_CHILD_SAFETY} from '@aethernet/constants/src/ReportCategories';
...
import {NcmecAlreadySubmittedError} from '@aethernet/errors/src/domains/moderation/NcmecAlreadySubmittedError';
import {NcmecSubmissionFailedError} from '@aethernet/errors/src/domains/moderation/NcmecSubmissionFailedError';
import {UnknownReportError} from '@aethernet/errors/src/domains/moderation/UnknownReportError';
import type {NcmecSubmissionStatus} from '@aethernet/schema/src/domains/admin/AdminSchemas';
...
function buildAdditionalInfo(args: {
	...
	const lines = [
		'Aethernet admin attachment report.',
...
function describeBestEffortUploadIpSource(trace: AttachmentUploadTraceByAttachmentRow): string {
	if (trace.completion_ip && trace.completed_at) {
		return 'multipart completion callback observed by Aethernet API';
	}
	if (trace.upload_mode === 'form_data') {
		return 'attachment upload request handled directly by Aethernet API';
	}
	return 'presigned upload authorization request observed by Aethernet API';
}
