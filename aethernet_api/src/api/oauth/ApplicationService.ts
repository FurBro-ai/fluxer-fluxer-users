...
import {UnclaimedAccountCannotCreateApplicationsError} from '@aethernet/errors/src/domains/oauth/UnclaimedAccountCannotCreateApplicationsError';
import {UnknownApplicationError} from '@aethernet/errors/src/domains/oauth/UnknownApplicationError';
import type {BotProfileUpdateRequest} from '@aethernet/schema/src/domains/oauth/OAuthSchemas';
...
import {enforceAethernetTagChangeRateLimit} from '../user/AethernetTagChangeRateLimit';
	...
	async updateBotProfile(
		...
		if (usernameChanged) {
			...
			if (result.username !== botUser.username || result.discriminator !== botUser.discriminator) {
				await enforceAethernetTagChangeRateLimit({
