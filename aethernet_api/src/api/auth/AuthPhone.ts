...
import {PHONE_ADD_CLEARABLE_FLAGS, UserFlags} from '@aethernet/constants/src/UserConstants';
import type {AethernetError} from '@aethernet/errors/src/AethernetError';
import {BotUserAuthEndpointAccessDeniedError} from '@aethernet/errors/src/domains/auth/BotUserAuthEndpointAccessDeniedError';
import {InvalidPhoneNumberError} from '@aethernet/errors/src/domains/auth/InvalidPhoneNumberError';
import {InvalidPhoneVerificationCodeError} from '@aethernet/errors/src/domains/auth/InvalidPhoneVerificationCodeError';
import {PhoneAlreadyUsedError} from '@aethernet/errors/src/domains/auth/PhoneAlreadyUsedError';
import {PhoneCountryNotSupportedError} from '@aethernet/errors/src/domains/auth/PhoneCountryNotSupportedError';
import {PhoneInboundVerificationRequiredError} from '@aethernet/errors/src/domains/auth/PhoneInboundVerificationRequiredError';
import {PhoneLookupUnavailableError} from '@aethernet/errors/src/domains/auth/PhoneLookupUnavailableError';
import {PhoneNumberNotInServiceError} from '@aethernet/errors/src/domains/auth/PhoneNumberNotInServiceError';
import {PhoneNumberNotMobileError} from '@aethernet/errors/src/domains/auth/PhoneNumberNotMobileError';
import {PhoneVerificationNeedsReviewError} from '@aethernet/errors/src/domains/auth/PhoneVerificationNeedsReviewError';
import {PhoneVerificationRequiredError} from '@aethernet/errors/src/domains/auth/PhoneVerificationRequiredError';
import {SmsVerificationUnavailableError} from '@aethernet/errors/src/domains/auth/SmsVerificationUnavailableError';
import {CaptchaVerificationRequiredError} from '@aethernet/errors/src/domains/core/CaptchaVerificationRequiredError';
import {RateLimitError} from '@aethernet/errors/src/domains/core/RateLimitError';
import {UnknownUserError} from '@aethernet/errors/src/domains/user/UnknownUserError';
import {PHONE_E164_REGEX} from '@aethernet/schema/src/primitives/UserValidators';
...
export async function startInboundPhoneChallenge(ctx: ApiContext, userId: UserID): Promise<IssuedChallenge> {
	...
	if (!inboundSmsChallenge || !ourNumber) {
		Logger.warn(
			...
			'Inbound SMS challenge requested but AETHERNET_SMS_INBOUND_CHALLENGE_NUMBER is unset',
		);
...
function buildVerificationStartOptions(
	...
	const rateLimits: Record<string, string> = {
		aethernet_phone_prefix: getRateLimitPhonePrefix(phone),
	};
	if (userId) {
		rateLimits.aethernet_user_id = userId.toString();
	}
	if (options.clientIp) {
		rateLimits.aethernet_client_ip = options.clientIp;
	}
	return {
		rateLimits,
	};
...
export function errorForPhoneRejectReason(reason: PhoneAttemptRejectReason): AethernetError {
	switch (reason) {
		case 'invalid_format':
