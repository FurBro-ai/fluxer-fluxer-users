...
import {AETHERNET_USER_AGENT} from '@aethernet/constants/src/Core';
...
import type {ForgotPasswordRequest, ResetPasswordRequest} from '@aethernet/schema/src/domains/auth/AuthSchemas';
...
export async function isPasswordPwned(_ctx: ApiContext, password: string): Promise<boolean> {
	...
	try {
		const response = await fetch(`https://api.pwnedpasswords.com/range/${hashPrefix}`, {
			headers: {
				'User-Agent': AETHERNET_USER_AGENT,
				'Add-Padding': 'true',
			},
