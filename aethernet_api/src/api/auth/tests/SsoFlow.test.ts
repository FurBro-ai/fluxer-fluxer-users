	...
	describe('complete SSO flow', () => {
		...
		it('uses the requested mobile SSO redirect URI without changing the post-login redirect', async () => {
			const startData = await createBuilderWithoutAuth<SsoStartResponse>(harness)
				...
				.body({
					redirect_to: '/me',
					redirect_uri: 'aethernet://auth/sso/callback',
				})
				.execute();
			expect(startData.redirect_uri).toBe('aethernet://auth/sso/callback');
			expect(getAuthorizationUrlParam(startData.authorization_url, 'redirect_uri')).toBe(
				'aethernet://auth/sso/callback',
			);
