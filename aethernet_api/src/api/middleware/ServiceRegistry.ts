...
export function createSnowflakeService(): SnowflakeService {
	const connectionManager = new NatsConnectionManager({
		...
		name: process.env.AETHERNET_SNOWFLAKE_SERVICE_NATS_CLIENT_NAME || 'aethernet-api-snowflakes',
	});
	return new SnowflakeService({
		connectionManager,
		subject: process.env.AETHERNET_SNOWFLAKE_SERVICE_SUBJECT || undefined,
		batchSize: readOptionalIntegerEnv('AETHERNET_SNOWFLAKE_SERVICE_BATCH_SIZE'),
		lowWatermark: readOptionalIntegerEnv('AETHERNET_SNOWFLAKE_SERVICE_LOW_WATERMARK'),
		maxBufferAgeMs: readOptionalIntegerEnv('AETHERNET_SNOWFLAKE_SERVICE_MAX_BUFFER_AGE_MS'),
		requestTimeoutMs: readOptionalIntegerEnv('AETHERNET_SNOWFLAKE_SERVICE_REQUEST_TIMEOUT_MS'),
	});
}
