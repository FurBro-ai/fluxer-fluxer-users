...
import {getSameIpDecisionKey, type IpAddressFamily} from '@aethernet/ip_utils/src/IpAddress';
...
class IpBanCache {
	...
	private readonly refreshSubscription = new RefreshSubscription({
		...
		periodicIntervalMs: () => {
			const intervalMs = Number(process.env.AETHERNET_IP_BAN_REFRESH_INTERVAL_MS ?? '300000');
