...
} from '@aethernet/schema/src/domains/gif/GifSchemas';
...
const GIF_SERVICE_SUBJECT = process.env.AETHERNET_GIF_SERVICE_SUBJECT || 'svc.gifs';
...
class NatsGifProvider implements IGifProvider {
	...
	constructor(
		...
		requireIntegerInRange('AETHERNET_GIF_SERVICE_TIMEOUT_MS', requestTimeoutMs, 1, MAX_REQUEST_TIMEOUT_MS);
		requireIntegerInRange(
			'AETHERNET_GIF_SERVICE_REGISTER_SHARE_TIMEOUT_MS',
...
export function createNatsGifProvider(apiKeyResolver: GifApiKeyResolver): NatsGifProvider {
	const manager = new NatsConnectionManager({
		...
		name: process.env.AETHERNET_GIF_SERVICE_NATS_CLIENT_NAME || 'aethernet-api-gifs',
	});
	const provider = new NatsGifProvider(
		manager,
		apiKeyResolver,
		readOptionalIntegerEnv('AETHERNET_GIF_SERVICE_TIMEOUT_MS'),
		readOptionalIntegerEnv('AETHERNET_GIF_SERVICE_REGISTER_SHARE_TIMEOUT_MS'),
	);
