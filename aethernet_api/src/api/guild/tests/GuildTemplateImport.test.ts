...
import {SystemChannelFlags} from '@aethernet/constants/src/GuildConstants';
...
describe('Guild Template Import', () => {
	...
	test('maps supported competitor-only channel types to Aethernet equivalents and ignores unsupported ones', async () => {
		const account = await createTestAccount(harness);
		const guild = await createBuilder<GuildResponse>(harness, account.token)
			.post('/guilds')
			.body({
