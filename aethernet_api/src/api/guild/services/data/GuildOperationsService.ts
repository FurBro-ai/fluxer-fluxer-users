...
import {AuditLogActionType} from '@aethernet/constants/src/AuditLogActionType';
import {
	ALL_PERMISSIONS,
	ChannelTypes,
	DEFAULT_PERMISSIONS,
	Permissions,
} from '@aethernet/constants/src/ChannelConstants';
import {DiscoveryApplicationStatus} from '@aethernet/constants/src/DiscoveryConstants';
import {
	ContentWarningLevel,
	GuildFeatures,
	GuildNSFWLevel,
	...
	SystemChannelFlags,
} from '@aethernet/constants/src/GuildConstants';
import {
	MAX_GUILD_CHANNELS,
	MAX_GUILD_ROLES,
	VOICE_CHANNEL_BITRATE_DEFAULT,
	VOICE_CHANNEL_CONNECTION_LIMIT_DEFAULT,
} from '@aethernet/constants/src/LimitConstants';
import {DEFAULT_GUILD_FOLDER_ICON} from '@aethernet/constants/src/UserConstants';
import {ValidationErrorCodes} from '@aethernet/constants/src/ValidationErrorCodes';
import {AccessDeniedError} from '@aethernet/errors/src/domains/core/AccessDeniedError';
import {InputValidationError} from '@aethernet/errors/src/domains/core/InputValidationError';
import {MissingPermissionsError} from '@aethernet/errors/src/domains/core/MissingPermissionsError';
import {BotsCannotCreateGuildsError} from '@aethernet/errors/src/domains/guild/BotsCannotCreateGuildsError';
import {GuildTemplateInvalidError} from '@aethernet/errors/src/domains/guild/GuildTemplateInvalidError';
import {MaxGuildsError} from '@aethernet/errors/src/domains/guild/MaxGuildsError';
import {UnclaimedAccountCannotCreateGuildsError} from '@aethernet/errors/src/domains/guild/UnclaimedAccountCannotCreateGuildsError';
import {UnknownGuildError} from '@aethernet/errors/src/domains/guild/UnknownGuildError';
import {DEFAULT_STOCK_LIMITS} from '@aethernet/limits/src/LimitDefaults';
import type {GuildCreateRequest, GuildUpdateRequest} from '@aethernet/schema/src/domains/guild/GuildRequestSchemas';
import type {GuildPartialResponse, GuildResponse} from '@aethernet/schema/src/domains/guild/GuildResponseSchemas';
import type {
	TemplateChannel,
	TemplateRole,
	TemplateSerializedGuild,
} from '@aethernet/schema/src/domains/guild/GuildTemplateSchemas';
import {extractTimestamp} from '@aethernet/snowflake/src/SnowflakeUtils';
...
export class GuildOperationsService {
	...
	private async buildTemplateEntities(
		...
		for (const channel of template.channels) {
			...
			const aethernetType = this.mapOtherPlatformTemplateChannelTypeToAethernet(channel.type);
			if (aethernetType == null) {
			...
			channelTypeMap.set(channelKey, aethernetType);
			mappedChannels.push({channel, channelId, aethernetType});
		}
		...
		for (const mappedChannel of mappedChannels) {
			const {channel, channelId, aethernetType} = mappedChannel;
			...
			const isVoice = aethernetType === ChannelTypes.GUILD_VOICE;
			...
			if (aethernetType === ChannelTypes.GUILD_TEXT && !firstTextChannelId) {
	...
	private mapOtherPlatformTemplateChannelTypeToAethernet(channelType: number): number | null {
		if (
