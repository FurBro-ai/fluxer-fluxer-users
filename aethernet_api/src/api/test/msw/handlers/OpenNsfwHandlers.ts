...
export function createOpenNsfwHandlers(config: OpenNsfwMockConfig = {}) {
	...
	return [
		http.post('http://opennsfw2.aethernet.svc.cluster.local:8000/predict/image', () => {
			return HttpResponse.json({
				nsfw_probability: nsfwProbability,
			});
		}),
		http.post('http://opennsfw2.aethernet.svc.cluster.local:8000/predict/images', async ({request}) => {
