...
import {resolveDownloadsProvider} from '@aethernet/config/src/S3DownloadsProvider';
import {parseIpAddress} from '@aethernet/ip_utils/src/IpAddress';
...
function normalizeIpBanExemptIps(values: Array<string>): Array<string> {
	...
	for (const value of values) {
		...
		if (!parsed) {
			throw new Error(`AETHERNET_API_IP_BAN_EXEMPT_IPS contains an invalid IP address: ${value}`);
		}
...
export function buildAPIConfigFromMaster(master: MasterConfig): APIConfig {
	...
	if (uploadRelaySecretBase64.length === 0) {
		throw new Error('AETHERNET_MEDIA_PROXY_UPLOAD_RELAY_SECRET_BASE64 is required for the API');
	}
	...
	return {
		...
		postgres: {
			...
			database: postgresSource?.database ?? 'aethernet',
			username: postgresSource?.username ?? 'aethernet',
			password: postgresSource?.password ?? 'aethernet',
			ssl: postgresSource?.ssl ?? false,
			sslCa: postgresSource?.ssl_ca ?? '',
			maxConnections: postgresSource?.max_connections ?? 20,
			kvTable: postgresSource?.kv_table ?? 'aethernet_kv',
		...
		push: {
			...
			apns: {
				...
				apps: mapPushProviderApps(master.integrations.push.apns.apps, 'AETHERNET_PUSH_APNS_APPS'),
			},
			fcm: {
				...
				apps: mapPushProviderApps(master.integrations.push.fcm.apps, 'AETHERNET_PUSH_FCM_APPS'),
			},
		},
		worker: {
