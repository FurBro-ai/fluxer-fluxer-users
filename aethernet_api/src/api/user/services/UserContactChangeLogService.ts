...
interface ContactChange {
	field: 'email' | 'has_verified_phone' | 'aethernet_tag';
...
export class UserContactChangeLogService {
	...
	async recordDiff(params: RecordDiffParams): Promise<void> {
		...
		const changes: Array<ContactChange> = [
			...
			{
				field: 'aethernet_tag',
				oldValue: this.buildAethernetTag(oldUser),
				newValue: this.buildAethernetTag(newUser),
			},
		];
		await awaitAll(
			changes
	...
	private buildAethernetTag(user: User | null): string | null {
		if (!user) return null;
