...
import {SudoModeRequiredError} from '@aethernet/errors/src/domains/auth/SudoModeRequiredError';
...
import type {UserUpdateRequest} from '@aethernet/schema/src/domains/user/UserRequestSchemas';
...
import {enforceAethernetTagChangeRateLimit} from '../AethernetTagChangeRateLimit';
...
export class UserAccountSecurityService {
	...
	async processSecurityUpdates(params: {
		...
		await this.enforceAethernetTagChangeRateLimit({
			user,
	...
	private async enforceAethernetTagChangeRateLimit(params: {
		user: User;
		nextUsername: string;
