...
import {RICH_EMBED_DESCRIPTION_MAX_LENGTH} from '@aethernet/schema/src/domains/message/MessageRequestSchemas';
import {InstatusWebhook} from '@aethernet/schema/src/domains/webhook/InstatusWebhookSchemas';
import {describe, expect, it} from 'vitest';
import {transformInstatusWebhook} from '../transformers/InstatusTransformer';

function createMeta(): InstatusWebhook['meta'] {
	return {unsubscribe: 'https://aethernetstatus.com/unsubscribe?id=1&token=abc', documentation: ''};
}
...
describe('Instatus transformer', () => {
	describe('incidents', () => {
		it('transforms an investigating incident', () => {
			const payload: InstatusWebhook = {
				meta: {unsubscribe: 'https://aethernetstatus.com/unsubscribe?id=1&token=abc', documentation: ''},
				page: createPage(),
				incident: {
					id: 'inc_1',
					name: 'Elevated API latency',
					url: 'https://aethernetstatus.com/incident/inc_1',
			...
			expect(embed?.url).toBe('https://aethernetstatus.com/incident/inc_1');
		...
		it('transforms an in-progress maintenance', () => {
			const payload: InstatusWebhook = {
				...
				maintenance: {
					...
					url: 'https://aethernetstatus.com/maintenance/m_1',
					status: 'INPROGRESS',
					maintenance_updates: [
						{
			...
			const embed = transformInstatusWebhook(payload);
			expect(embed?.title).toBe('Database upgrade');
			expect(embed?.url).toBe('https://aethernetstatus.com/maintenance/m_1');
			expect(embed?.color).toBe(0x3b82f6);
	...
	describe('component updates', () => {
		it('transforms a component going down', () => {
			...
			const embed = transformInstatusWebhook(payload);
			expect(embed?.title).toBe('API - major outage');
			expect(embed?.url).toBe('https://aethernetstatus.com');
			expect(embed?.color).toBe(0xe23c39);
