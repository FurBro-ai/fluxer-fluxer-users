...
impl UnfurlShard {
    pub fn new(lifetime: Duration) -> Self {
        ...
        let media_proxy_endpoint = std::env::var("AETHERNET_MEDIA_PROXY_ENDPOINT")
            .ok()
            .filter(|v| !v.is_empty());
        let media_proxy_secret = std::env::var("AETHERNET_MEDIA_PROXY_SECRET_KEY")
            .ok()
            .filter(|v| !v.is_empty());
        let media_proxy_public_endpoint = std::env::var("AETHERNET_MEDIA_PROXY_PUBLIC_ENDPOINT")
            .ok()
            .filter(|v| !v.is_empty())
            .map(|v| aethernet_common::config::normalize_public_endpoint_from_env(&v));
        let static_cdn_endpoint = aethernet_common::config::normalize_public_endpoint_from_env(
            &std::env::var("AETHERNET_UNFURL_STATIC_CDN_ENDPOINT")
                .or_else(|_| std::env::var("AETHERNET_STATIC_CDN_ENDPOINT"))
                .unwrap_or_default(),
        );
        let (media_proxy_endpoint, media_proxy_secret) = match (
            ...
            (Some(endpoint), None) => {
                panic!(
                    "media proxy endpoint configured without AETHERNET_MEDIA_PROXY_SECRET_KEY: {endpoint}"
                );
            }
            (None, Some(_)) => {
                panic!(
                    "AETHERNET_MEDIA_PROXY_SECRET_KEY configured without AETHERNET_MEDIA_PROXY_ENDPOINT"
                );
            }
            (None, None) => {
                panic!(
                    "unfurl shard requires AETHERNET_MEDIA_PROXY_ENDPOINT and AETHERNET_MEDIA_PROXY_SECRET_KEY"
                );
            }
        };
        tracing::info!(
...
fn base_http_client_builder() -> reqwest::ClientBuilder {
    reqwest::Client::builder()
        .user_agent("Mozilla/5.0 (compatible; Aethernetbot/1.0; +https://aethernet.app)")
...
mod tests {
    ...
    fn a_non_default_public_port_reaches_the_public_media_and_static_endpoints() {
        let shard = shard_from_env(&[
            ("AETHERNET_MEDIA_PROXY_ENDPOINT", "http://media-proxy:8080"),
            ("AETHERNET_MEDIA_PROXY_SECRET_KEY", "secret"),
            (
                "AETHERNET_MEDIA_PROXY_PUBLIC_ENDPOINT",
                "http://aethernet.example/media",
            ),
            ("AETHERNET_STATIC_CDN_ENDPOINT", "http://aethernet.example"),
            ("AETHERNET_BASE_DOMAIN", "aethernet.example"),
            ("AETHERNET_PUBLIC_PORT", "19080"),
        ]);

        assert_eq!(shard.static_cdn_endpoint, "http://aethernet.example:19080");
        assert!(
            shard
                .media_proxy
                .external_proxy_url("https://img.example.net/a.png")
                .expect("proxy url")
                .starts_with("http://aethernet.example:19080/media/external/")
        );
    }
    ...
    fn a_default_public_port_leaves_the_public_media_and_static_endpoints_alone() {
        ...
        assert_eq!(shard.static_cdn_endpoint, "https://aethernet.example");
        assert!(
            shard
                .media_proxy
                .external_proxy_url("https://img.example.net/a.png")
                .expect("proxy url")
                .starts_with("https://aethernet.example/media/external/")
        );
    }
