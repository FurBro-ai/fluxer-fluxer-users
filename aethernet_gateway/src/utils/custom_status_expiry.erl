...
enabled() ->
    application:get_env(aethernet_gateway, ?ENABLED_KEY, false) =:= true.
...
disabled_by_default_test() ->
    application:unset_env(aethernet_gateway, ?ENABLED_KEY),
    Status = status(?LIVE_EXPIRES_AT),
...
with_expiry_enabled(Fun) ->
    Previous = application:get_env(aethernet_gateway, ?ENABLED_KEY),
    application:set_env(aethernet_gateway, ?ENABLED_KEY, true),
    try
        Fun()
    after
...
restore_expiry_env({ok, Value}) ->
    application:set_env(aethernet_gateway, ?ENABLED_KEY, Value).
