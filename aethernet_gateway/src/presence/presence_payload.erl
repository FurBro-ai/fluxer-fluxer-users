...
build_leaves_expired_status_alone_while_disabled_test() ->
    application:unset_env(aethernet_gateway, custom_status_expiry_enabled),
...
with_expiry_enabled(Fun) ->
    ...
    Previous = application:get_env(aethernet_gateway, Key),
    application:set_env(aethernet_gateway, Key, true),
    try
        Fun()
    after
...
restore_expiry_env(Key, undefined) ->
    application:unset_env(aethernet_gateway, Key);
restore_expiry_env(Key, {ok, Value}) ->
    application:set_env(aethernet_gateway, Key, Value).
-endif.
