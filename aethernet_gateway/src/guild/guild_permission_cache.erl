...
members_ets_enabled() ->
    case application:get_env(aethernet_gateway, permission_cache_members_ets, true) of
        false -> false;
        _ -> true
    end.
...
strip_members_memo_enabled() ->
    case application:get_env(aethernet_gateway, permission_cache_strip_memo_enabled, true) of
        false -> false;
        _ -> true
    end.
...
memo_disabled_matches_reference_and_releases_the_memo_test() ->
    ...
    Expected = reference_strip_members(Members),
    ?assertEqual(Expected, memoised_strip_members(Members)),
    application:set_env(aethernet_gateway, permission_cache_strip_memo_enabled, false),
    try
        ?assertEqual(Expected, memoised_strip_members(Members)),
        ?assertEqual(undefined, erlang:get(?STRIPPED_MEMBERS_MEMO))
    after
        application:unset_env(aethernet_gateway, permission_cache_strip_memo_enabled),
        memo_reset()
    end.
