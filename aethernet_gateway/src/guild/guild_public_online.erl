...
classify_restricted(RestrictedIds) ->
    case application:get_env(aethernet_gateway, ?ALL_PERMS_VIEWER_KEY, false) of
...
with_all_perms_viewer(Fun) ->
    Previous = application:get_env(aethernet_gateway, ?ALL_PERMS_VIEWER_KEY),
    application:set_env(aethernet_gateway, ?ALL_PERMS_VIEWER_KEY, true),
    try
        Fun()
    after
...
restore_all_perms_viewer(undefined) ->
    application:unset_env(aethernet_gateway, ?ALL_PERMS_VIEWER_KEY);
restore_all_perms_viewer({ok, Value}) ->
    application:set_env(aethernet_gateway, ?ALL_PERMS_VIEWER_KEY, Value).
