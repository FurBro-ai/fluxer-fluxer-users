...
interval_ms() ->
    case application:get_env(aethernet_gateway, guild_presence_reconcile_interval_ms) of
...
interval_clamps_below_minimum_test() ->
    application:set_env(aethernet_gateway, guild_presence_reconcile_interval_ms, 10),
    try
        ?assertEqual(?DEFAULT_INTERVAL_MS, interval_ms())
    after
        application:unset_env(aethernet_gateway, guild_presence_reconcile_interval_ms)
    end.

interval_uses_configured_value_test() ->
    application:set_env(aethernet_gateway, guild_presence_reconcile_interval_ms, 12000),
    try
