...
push_concurrency_limit() ->
    case application:get_env(aethernet_gateway, ?CONCURRENCY_LIMIT_KEY, undefined) of
...
mentions_respect_active_session() ->
    application:get_env(aethernet_gateway, push_mentions_respect_active_session, true) =:= true.
...
presence_eligibility_enabled() ->
    application:get_env(aethernet_gateway, push_presence_eligibility, true) =:= true.
...
compact_scan_includes_mentioned_user_when_bypass_is_re_enabled_test() ->
    Tab = ets:new(test_members, [set, public]),
    application:set_env(aethernet_gateway, push_mentions_respect_active_session, false),
    try
        true = ets:insert(Tab, {1, #{<<"roles">> => []}}),
        true = ets:insert(Tab, {3, #{<<"roles">> => [<<"200">>]}}),
        State = compact_scan_state(Tab),
    ...
    after
        application:unset_env(aethernet_gateway, push_mentions_respect_active_session),
        ets:delete(Tab)
    end.
...
inflight_pid_without_a_slot_still_counts_against_the_limit_test() ->
    ...
    ok = application:set_env(aethernet_gateway, ?CONCURRENCY_LIMIT_KEY, 1),
    try
...
bounded_push_concurrency_spawns_second_worker_under_limit_test() ->
    reset_push_worker_state(),
    ok = application:set_env(aethernet_gateway, ?CONCURRENCY_LIMIT_KEY, 3),
    Blocker = blocking_push_worker(),
    put(push_inflight, Blocker),
    put(push_inflight_workers, [{1, Blocker, erlang:monotonic_time(millisecond)}]),
    try
...
bounded_push_concurrency_counts_drops_at_limit_test() ->
    reset_push_worker_state(),
    ok = application:set_env(aethernet_gateway, ?CONCURRENCY_LIMIT_KEY, 2),
...
push_concurrency_limit_defaults_and_clamps_test() ->
    ...
    try
        ...
        ok = application:set_env(aethernet_gateway, ?CONCURRENCY_LIMIT_KEY, 0),
        ?assertEqual(?DEFAULT_PUSH_CONCURRENCY, push_concurrency_limit()),
        ok = application:set_env(aethernet_gateway, ?CONCURRENCY_LIMIT_KEY, not_a_number),
        ?assertEqual(?DEFAULT_PUSH_CONCURRENCY, push_concurrency_limit()),
        ok = application:set_env(aethernet_gateway, ?CONCURRENCY_LIMIT_KEY, 1000),
        ?assertEqual(?MAX_PUSH_CONCURRENCY, push_concurrency_limit()),
        ok = application:set_env(aethernet_gateway, ?CONCURRENCY_LIMIT_KEY, 6),
...
reset_push_worker_state() ->
    ...
    ok = application:unset_env(aethernet_gateway, ?CONCURRENCY_LIMIT_KEY).
