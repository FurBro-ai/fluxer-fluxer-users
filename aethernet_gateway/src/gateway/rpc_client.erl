...
-define(RPC_AUTH_HEADER, <<"x-aethernet-rpc-auth">>).
...
-spec rpc_url() -> binary().
rpc_url() ->
    case normalize_optional_binary(aethernet_gateway_env:get(api_rpc_endpoint)) of
        undefined ->
            BaseUrl = trim_trailing_slash(
                ensure_binary(aethernet_gateway_env:get(api_internal_url))
            ),
...
rpc_headers(Request) ->
    ClientIpHeader = normalize_client_ip_header(aethernet_gateway_env:get(client_ip_header)),
    AuthToken = ensure_binary(aethernet_gateway_env:get(rpc_auth_token)),
    [
