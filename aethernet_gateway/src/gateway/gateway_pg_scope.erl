...
-define(PRESENCE_SCOPE, aethernet_presence).
...
presence_scope_test() ->
    ?assertEqual(aethernet_presence, presence_scope()).
...
start_link_reports_scope_status_test() ->
    ...
    ?assertMatch(#{scope := aethernet_presence, pg_pid := PgPid} when is_pid(PgPid), Status),
