...
do_connect_slot(Idx, Connecting, State) ->
    NatsUrl = aethernet_gateway_env:get(nats_core_url),
    AuthToken = aethernet_gateway_env:get(nats_auth_token),
...
pool_size() ->
    case aethernet_gateway_env:get(nats_pool_size) of
...
max_handlers() ->
    case aethernet_gateway_env:get(gateway_nats_rpc_max_handlers) of
        Value when is_integer(Value), Value > 0 -> Value;
