...
-module(aethernet_gateway_config).
-typing([eqwalizer]).
...
env_config() ->
    #{
        <<"env">> => env_binary("AETHERNET_ENV", <<"development">>),
...
env_internal_config() ->
    #{
        <<"api">> => env_binary("AETHERNET_INTERNAL_API_ENDPOINT", <<"http://127.0.0.1:8080">>)
    }.

-spec env_public_config() -> map().
env_public_config() ->
    #{
        <<"base_domain">> => env_optional_binary("AETHERNET_BASE_DOMAIN"),
        <<"scheme">> => env_optional_binary("AETHERNET_PUBLIC_SCHEME"),
        <<"port">> => env_optional_binary("AETHERNET_PUBLIC_PORT")
    }.
...
env_proxy_config() ->
    #{
        ...
        <<"trust_client_ip_header">> => env_bool("AETHERNET_TRUST_CLIENT_IP_HEADER", false)
    }.
...
env_gateway_base_config() ->
    #{
        <<"port">> => env_int("AETHERNET_GATEWAY_PORT", 8771),
        <<"gateway_role">> => env_optional_binary("AETHERNET_GATEWAY_ROLE"),
        <<"rpc_auth_token">> => env_binary("AETHERNET_GATEWAY_RPC_AUTH_TOKEN", <<>>),
        <<"push_enabled">> => env_bool("AETHERNET_GATEWAY_PUSH_ENABLED", true),
        <<"logger_level">> => env_binary("AETHERNET_GATEWAY_LOGGER_LEVEL", <<"info">>),
        <<"api_rpc_endpoint">> => env_optional_binary("AETHERNET_GATEWAY_API_RPC_ENDPOINT"),
        <<"cluster_enabled">> => env_bool("AETHERNET_GATEWAY_CLUSTER_ENABLED", false),
        ...
        <<"cluster_discovery_poll_interval_ms">> => env_int(
            "AETHERNET_GATEWAY_CLUSTER_DISCOVERY_POLL_INTERVAL_MS", 5000
        ),
        <<"cluster_static_peers">> => env_optional_binary(
            "AETHERNET_GATEWAY_CLUSTER_STATIC_PEERS"
        ),
        <<"media_proxy_endpoint">> => env_binary(
            "AETHERNET_GATEWAY_MEDIA_PROXY_ENDPOINT", <<"http://localhost:8088/media">>
        ),
        <<"static_cdn_endpoint">> => env_binary(
            "AETHERNET_GATEWAY_STATIC_CDN_ENDPOINT", <<"http://localhost:8088">>
        ),
        <<"presence_push_buffer_max_entries">> => env_int(
            "AETHERNET_GATEWAY_PRESENCE_PUSH_BUFFER_MAX_ENTRIES", 128
        ),
        <<"presence_push_buffer_max_bytes">> => env_int(
            "AETHERNET_GATEWAY_PRESENCE_PUSH_BUFFER_MAX_BYTES", 1048576
        ),
        <<"shutdown_drain_wait_ms">> => env_int("AETHERNET_GATEWAY_SHUTDOWN_DRAIN_WAIT_MS", 5000),
        <<"gateway_http_rpc_max_concurrency">> => env_int(
            "AETHERNET_GATEWAY_HTTP_RPC_MAX_CONCURRENCY", 512
        ),
        <<"gateway_nats_rpc_max_handlers">> => env_int(
            "AETHERNET_GATEWAY_NATS_RPC_MAX_HANDLERS", 512
        ),
        <<"gateway_http_failure_threshold">> => env_int(
            "AETHERNET_GATEWAY_HTTP_FAILURE_THRESHOLD", 6
        ),
        <<"gateway_http_recovery_timeout_ms">> => env_int(
            "AETHERNET_GATEWAY_HTTP_RECOVERY_TIMEOUT_MS", 15000
        )
    }.
...
env_nats_config() ->
    #{
        <<"core_url">> => env_string("AETHERNET_NATS_URL", "nats://nats:4222"),
        <<"auth_token">> => env_string("AETHERNET_NATS_AUTH_TOKEN", "")
    }.
...
env_auth_config() ->
    #{
        <<"vapid">> => #{
            <<"email">> => env_binary("AETHERNET_VAPID_EMAIL", <<>>),
            <<"public_key">> => env_optional_binary("AETHERNET_VAPID_PUBLIC_KEY"),
            <<"private_key">> => env_optional_binary("AETHERNET_VAPID_PRIVATE_KEY")
        }
    }.
...
env_apns_config() ->
    #{
        <<"enabled">> => env_bool("AETHERNET_PUSH_APNS_ENABLED", false),
        <<"team_id">> => env_optional_binary("AETHERNET_PUSH_APNS_TEAM_ID"),
        <<"key_id">> => env_optional_binary("AETHERNET_PUSH_APNS_KEY_ID"),
        <<"private_key">> => env_optional_binary("AETHERNET_PUSH_APNS_PRIVATE_KEY"),
        <<"private_key_path">> => env_optional_binary("AETHERNET_PUSH_APNS_PRIVATE_KEY_PATH"),
        <<"default_environment">> => env_binary(
            "AETHERNET_PUSH_APNS_DEFAULT_ENVIRONMENT", <<"production">>
        ),
        <<"apps">> => env_json_list("AETHERNET_PUSH_APNS_APPS", [])
    }.

-spec env_fcm_config() -> map().
env_fcm_config() ->
    #{
        <<"enabled">> => env_bool("AETHERNET_PUSH_FCM_ENABLED", false),
        <<"project_id">> => env_optional_binary("AETHERNET_PUSH_FCM_PROJECT_ID"),
        <<"client_email">> => env_optional_binary("AETHERNET_PUSH_FCM_CLIENT_EMAIL"),
        <<"private_key">> => env_optional_binary("AETHERNET_PUSH_FCM_PRIVATE_KEY"),
        <<"private_key_path">> => env_optional_binary("AETHERNET_PUSH_FCM_PRIVATE_KEY_PATH"),
        <<"service_account_json_path">> => env_optional_binary(
            "AETHERNET_PUSH_FCM_SERVICE_ACCOUNT_JSON_PATH"
        ),
        <<"token_uri">> => env_binary(
            "AETHERNET_PUSH_FCM_TOKEN_URI", <<"https://oauth2.googleapis.com/token">>
        ),
        <<"apps">> => env_json_list("AETHERNET_PUSH_FCM_APPS", [])
    }.

-spec env_telemetry_config() -> map().
env_telemetry_config() ->
    #{
        <<"enabled">> => env_bool("AETHERNET_TELEMETRY_ENABLED", false),
        <<"environment">> => env_string("AETHERNET_ENV", "development")
    }.
