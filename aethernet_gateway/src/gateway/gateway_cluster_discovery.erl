...
-define(DEFAULT_NODE_BASENAME, "aethernet_gateway").
...
poll_empty_dns_answer_keeps_existing_peers_test() ->
    OldPeers = ['aethernet_gateway@127.0.0.2'],
...
init_seeds_existing_peers_from_persistent_term_test() ->
    ...
    {ok, State} = init(#{
        dns_name => undefined,
        node_basename => "aethernet_gateway",
...
discovery_test_state(Resolver, Peers) ->
    #{
        dns_name => "aethernet-gateway-headless.aethernet.svc.cluster.local",
        node_basename => "aethernet_gateway",
        poll_interval_ms => 5000,
        resolver => Resolver,
        peers => Peers,
