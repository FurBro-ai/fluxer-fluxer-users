...
current_role() ->
    normalize_role(
        try aethernet_gateway_env:get(gateway_role) of
            Value -> Value
        catch
            throw:_Reason -> all;
            error:_Reason -> all;
...
cluster_children() ->
    case aethernet_gateway_env:get(cluster_enabled) of
        true ->
            [
...
init_child_ids(Config) ->
    persistent_term:put({aethernet_gateway, runtime_config}, Config),
    try
