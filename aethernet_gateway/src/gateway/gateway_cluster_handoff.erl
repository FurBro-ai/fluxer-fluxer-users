...
-spec drain_async() -> ok.
drain_async() ->
    persistent_term:put({aethernet_gateway, draining}, true),
...
clear_draining() ->
    persistent_term:erase({aethernet_gateway, draining}),
...
drain_notify_role() ->
    Role = aethernet_gateway_sup:current_role(),
    case Role of
        websocket ->
...
undrain_clears_draining_flag_test() ->
    Previous = persistent_term:get({aethernet_gateway, draining}, undefined),
    persistent_term:put({aethernet_gateway, draining}, true),
    ?assertEqual(ok, undrain()),
    ?assertEqual(false, persistent_term:get({aethernet_gateway, draining}, false)),
    restore_persistent_term({aethernet_gateway, draining}, Previous).
...
undrain_is_refused_while_handoff_in_flight_test() ->
    ...
    ?assertEqual(true, persistent_term:get({aethernet_gateway, draining}, false)),
