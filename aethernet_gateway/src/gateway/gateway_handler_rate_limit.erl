...
rate_limits_disabled() ->
    case os:getenv("AETHERNET_DISABLE_RATE_LIMITS") of
        "1" -> true;
        "true" -> true;
        "TRUE" -> true;
        _ -> false
...
check_rate_limit_disabled_by_env_test() ->
    ...
    OldValue = os:getenv("AETHERNET_DISABLE_RATE_LIMITS"),
    os:putenv("AETHERNET_DISABLE_RATE_LIMITS", "true"),
    try
        CastState = eqwalizer:dynamic_cast(State),
        ?assertEqual({ok, CastState}, check_rate_limit(CastState, presence_update))
    after
        restore_env("AETHERNET_DISABLE_RATE_LIMITS", OldValue)
    end.
...
with_rate_limits_enabled(Fun) ->
    OldValue = os:getenv("AETHERNET_DISABLE_RATE_LIMITS"),
    os:unsetenv("AETHERNET_DISABLE_RATE_LIMITS"),
    try
        Fun()
    after
        restore_env("AETHERNET_DISABLE_RATE_LIMITS", OldValue)
