...
-include_lib("aethernet_gateway/include/timeout_config.hrl").
...
determine_shard_count() ->
    case aethernet_gateway_env:get(session_shards) of
...
with_runtime_config(Key, Value, Fun) ->
    case persistent_term:get({aethernet_gateway, runtime_config}, undefined) of
        undefined -> persistent_term:put({aethernet_gateway, runtime_config}, #{});
        _ -> ok
    end,
    Original = aethernet_gateway_env:get(Key),
    aethernet_gateway_env:patch(#{Key => Value}),
    Result = Fun(),
    aethernet_gateway_env:update(fun(Map) -> restore_runtime_config(Key, Original, Map) end),
    Result.
