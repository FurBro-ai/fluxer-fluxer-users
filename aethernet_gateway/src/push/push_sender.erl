...
badge_fetch_batch_size() ->
    case application:get_env(aethernet_gateway, push_badge_fetch_batch_size, undefined) of
...
badge_fetch_budget_ms() ->
    case application:get_env(aethernet_gateway, push_badge_fetch_budget_ms, undefined) of
        Value when is_integer(Value), Value > 0 -> Value;
...
fetch_badge_counts_in_batches_keeps_successful_batches_test() ->
    ...
    application:set_env(aethernet_gateway, push_badge_fetch_batch_size, 2),
    try
    ...
    after
        application:unset_env(aethernet_gateway, push_badge_fetch_batch_size),
...
fetch_badge_count_batches_stops_at_the_wall_clock_budget_test() ->
    ok = meck:new(rpc_client, [passthrough, no_link]),
    application:set_env(aethernet_gateway, push_badge_fetch_budget_ms, 1),
    try
    ...
    after
        application:unset_env(aethernet_gateway, push_badge_fetch_budget_ms),
        meck:unload(rpc_client)
    end.
