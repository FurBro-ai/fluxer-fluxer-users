...
app_pos_integer(Key, Default) ->
    case application:get_env(aethernet_gateway, Key, undefined) of
...
loss_logging_enabled() ->
    application:get_env(aethernet_gateway, push_loss_logging, false) =:= true.
...
maybe_warn_vapid_misconfigured(true) ->
    Public = aethernet_gateway_env:get(vapid_public_key),
    Private = aethernet_gateway_env:get(vapid_private_key),
...
warn_unless_vapid_pair_valid(Public, Private) ->
    try
        ...
        _:Reason ->
            logger:error(
                "Push: AETHERNET_VAPID_PUBLIC_KEY and AETHERNET_VAPID_PRIVATE_KEY are not a "
...
env_boolean(Key) ->
    case aethernet_gateway_env:get(Key) of
        true -> true;
        _ -> false
    end.
...
with_fetch_env(ChunkSize, MaxUsers, Fun) ->
    application:set_env(aethernet_gateway, push_blocked_ids_fetch_chunk, ChunkSize),
    application:set_env(aethernet_gateway, push_blocked_ids_fetch_max_users, MaxUsers),
    try
        Fun()
    after
        application:unset_env(aethernet_gateway, push_blocked_ids_fetch_chunk),
        application:unset_env(aethernet_gateway, push_blocked_ids_fetch_max_users)
    end.
