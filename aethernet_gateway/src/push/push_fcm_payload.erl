...
build_clear_message(DeviceToken, Payload) ->
    ...
    Tag = push_utils:normalize_binary(
        maps:get(
            <<"notification_tag">>, Payload, maps:get(<<"tag">>, Payload, <<"aethernet-message">>)
        ),
        <<"aethernet-message">>
    ),
    Data = maps:merge(
...
build_notification_message(DeviceToken, Payload) ->
    ...
    Tag = push_utils:normalize_binary(
        maps:get(<<"tag">>, Payload, <<"aethernet-message">>), <<"aethernet-message">>
    ),
...
build_android_notification(Tag, ImageUrl) ->
    maybe_put(<<"image">>, ImageUrl, #{
        <<"channel_id">> => <<"aethernet_default_push">>,
        <<"tag">> => Tag,
        <<"click_action">> => <<"AETHERNET_MESSAGE">>
    }).
...
resolve_title(Notification, Payload) ->
    sanitize_text(
        push_utils:normalize_binary(
            maps:get(<<"title">>, Notification, maps:get(<<"title">>, Payload, <<"Aethernet">>)),
            <<"Aethernet">>
        )
    ).
...
build_message_includes_android_chat_notification_fields_test() ->
    ...
    ?assertEqual(<<"aethernet_default_push">>, maps:get(<<"channel_id">>, AndroidNotification)),
