...
delivery_max_workers() ->
    case application:get_env(aethernet_gateway, push_delivery_max_workers, undefined) of
...
delivery_tasks_per_worker() ->
    case application:get_env(aethernet_gateway, push_delivery_tasks_per_worker, undefined) of
        Value when is_integer(Value), Value > 0 -> Value;
...
subscription_fetch_batch_size() ->
    case application:get_env(aethernet_gateway, push_subscription_fetch_batch_size, undefined) of
        Value when is_integer(Value), Value > 0 -> Value;
...
subscription_fetch_budget_ms() ->
    case application:get_env(aethernet_gateway, push_subscription_fetch_budget_ms, undefined) of
        Value when is_integer(Value), Value > 0 -> Value;
...
delivery_concurrency() ->
    case aethernet_gateway_env:get_optional(push_subscription_delivery_concurrency) of
        Value when is_integer(Value), Value > 0 -> Value;
...
bounded_worker_count_scales_with_recipients_and_stays_bounded_test() ->
    application:unset_env(aethernet_gateway, push_delivery_tasks_per_worker),
...
bounded_fanout_isolates_a_raising_task_and_counts_it_test() ->
    ...
    application:set_env(aethernet_gateway, push_delivery_tasks_per_worker, 1),
    try
...
fetch_missing_in_batches_sends_only_successful_batches_test() ->
    ...
    application:set_env(aethernet_gateway, push_subscription_fetch_batch_size, 2),
    try
    ...
    after
        application:unset_env(aethernet_gateway, push_subscription_fetch_batch_size),
