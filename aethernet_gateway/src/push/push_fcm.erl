...
send(UserId, Subscription, Payload) ->
    case aethernet_gateway_env:get(fcm_enabled) of
        true ->
...
resolve_project_id(AppId) ->
    Apps = aethernet_gateway_env:get(fcm_apps),
...
resolve_default_project_id() ->
    case aethernet_gateway_env:get(fcm_project_id) of
        ProjectId when is_binary(ProjectId), byte_size(ProjectId) > 0 -> {ok, ProjectId};
...
resolve_service_account() ->
    JsonPath = aethernet_gateway_env:get(fcm_service_account_json_path),
    maybe
...
build_service_account(Json) ->
    ClientEmail = first_binary([
        get_map_value(Json, <<"client_email">>), aethernet_gateway_env:get(fcm_client_email)
    ]),
    TokenUri = first_binary([
        get_map_value(Json, <<"token_uri">>),
        aethernet_gateway_env:get(fcm_token_uri),
        ?DEFAULT_TOKEN_URI
    ]),
...
resolve_private_key(Json) ->
    case
        first_binary([
            get_map_value(Json, <<"private_key">>), aethernet_gateway_env:get(fcm_private_key)
        ])
    of
...
resolve_private_key_from_file() ->
    case aethernet_gateway_env:get(fcm_private_key_path) of
