...
rpc_url_uses_internal_api_when_endpoint_is_undefined_test() ->
    OldConfig = aethernet_gateway_env:get_map(),
    try
        _ = aethernet_gateway_env:patch(#{
    ...
    after
        _ = aethernet_gateway_env:update(fun(_) -> OldConfig end)
    end.
...
rpc_url_meta_matches_derive_path_test() ->
    OldConfig = aethernet_gateway_env:get_map(),
    Endpoints = [
        <<"http://aethernet-api.aethernet.svc.cluster.local/internal/rpc">>,
        <<"https://api.internal/internal/rpc">>,
        <<"http://aethernet-api.aethernet.svc.cluster.local:8080/internal/rpc">>,
        <<"HTTPS://Aethernet-API.Aethernet.SVC.Cluster.Local:8443/internal/rpc">>
    ],
    try
...
assert_rpc_url_meta_for_endpoint(Endpoint) ->
    _ = aethernet_gateway_env:patch(#{api_rpc_endpoint => Endpoint}),
...
rpc_url_meta_expected_values_test() ->
    ...
    try
        ...
        ?assertEqual(
            {
                <<"https://Aethernet-API.Aethernet.SVC.Cluster.Local:8443/internal/rpc">>,
                <<"aethernet-api.aethernet.svc.cluster.local">>,
                true
            },
