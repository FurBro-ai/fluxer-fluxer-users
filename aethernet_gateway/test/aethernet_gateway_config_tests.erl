...
-module(aethernet_gateway_config_tests).
-typing([eqwalizer]).
-include_lib("eunit/include/eunit.hrl").

cluster_defaults_test() ->
    Config = aethernet_gateway_config:build_config(#{}),
...
cluster_overrides_test() ->
        ...
        <<"services">> => #{
            <<"gateway">> => #{
                ...
                <<"cluster_discovery_dns_name">> =>
                    <<"aethernet-gateway-headless.aethernet.svc.cluster.local">>,
                <<"cluster_discovery_node_basename">> => <<"aethernet_gateway">>,
                <<"cluster_discovery_poll_interval_ms">> => 2500,
                <<"cluster_static_peers">> =>
                    <<"aethernet_gateway_websocket_1@127.0.0.1, aethernet_gateway_sessions_1@127.0.0.1">>
            }
        }
    },
    Config = aethernet_gateway_config:build_config(RawConfig),
    ...
    ?assertEqual(
        [
            list_to_atom("aethernet_gateway_websocket_1@127.0.0.1"),
            list_to_atom("aethernet_gateway_sessions_1@127.0.0.1")
        ],
...
cluster_static_peers_rejects_invalid_node_names_test() ->
    ...
    ?assertError(
        ...
        aethernet_gateway_config:build_config(RawConfig)
    ).
...
presence_push_buffer_env_defaults_test() ->
    with_env("AETHERNET_GATEWAY_PRESENCE_PUSH_BUFFER_MAX_ENTRIES", "7", fun() ->
        with_env("AETHERNET_GATEWAY_PRESENCE_PUSH_BUFFER_MAX_BYTES", "4096", fun() ->
            Config = aethernet_gateway_config:load(),
...
env_only_push_and_http_runtime_config_test() ->
    ApnsAppsJson =
        "[{\"app_id\":\"ios-stable\",\"topic\":\"app.aethernet\",\"environment\":\"production\"}]",
    FcmAppsJson = "[{\"app_id\":\"android-stable\",\"project_id\":\"aethernet-fcm\"}]",
    with_envs(
        [
            {"AETHERNET_GATEWAY_SHUTDOWN_DRAIN_WAIT_MS", "1234"},
            {"AETHERNET_GATEWAY_HTTP_RPC_MAX_CONCURRENCY", "42"},
            {"AETHERNET_GATEWAY_HTTP_FAILURE_THRESHOLD", "9"},
            {"AETHERNET_GATEWAY_HTTP_RECOVERY_TIMEOUT_MS", "6000"},
            {"AETHERNET_PUSH_APNS_ENABLED", "true"},
            {"AETHERNET_PUSH_APNS_TEAM_ID", "TEAMID"},
            {"AETHERNET_PUSH_APNS_KEY_ID", "KEYID"},
            {"AETHERNET_PUSH_APNS_PRIVATE_KEY_PATH", "/etc/aethernet/apns.p8"},
            {"AETHERNET_PUSH_APNS_DEFAULT_ENVIRONMENT", "development"},
            {"AETHERNET_PUSH_APNS_APPS", ApnsAppsJson},
            {"AETHERNET_PUSH_FCM_ENABLED", "true"},
            {"AETHERNET_PUSH_FCM_PROJECT_ID", "aethernet-fcm"},
            {"AETHERNET_PUSH_FCM_SERVICE_ACCOUNT_JSON_PATH", "/etc/aethernet/fcm.json"},
            {"AETHERNET_PUSH_FCM_TOKEN_URI", "https://oauth2.example/token"},
            {"AETHERNET_PUSH_FCM_APPS", FcmAppsJson}
        ],
        fun() ->
            Config = aethernet_gateway_config:load(),
            ...
            ?assertEqual(<<"aethernet-fcm">>, maps:get(fcm_project_id, Config)),
            ?assertEqual(<<"https://oauth2.example/token">>, maps:get(fcm_token_uri, Config)),
            ?assertEqual(
                [#{<<"app_id">> => <<"android-stable">>, <<"project_id">> => <<"aethernet-fcm">>}],
...
rpc_concurrency_keys_are_independent_test() ->
    with_envs(
        [
            {"AETHERNET_GATEWAY_HTTP_RPC_MAX_CONCURRENCY", "128"},
            {"AETHERNET_GATEWAY_NATS_RPC_MAX_HANDLERS", "2048"}
        ],
        fun() ->
            Config = aethernet_gateway_config:load(),
...
env_int_rejects_a_non_integer_value_test() ->
    with_env("AETHERNET_GATEWAY_HTTP_RPC_MAX_CONCURRENCY", "abc", fun() ->
        ?assertError(
            {invalid_integer_env, "AETHERNET_GATEWAY_HTTP_RPC_MAX_CONCURRENCY", "abc"},
            aethernet_gateway_config:load()
        )
    end).

env_int_falls_back_to_the_default_for_an_empty_value_test() ->
    with_env("AETHERNET_GATEWAY_HTTP_RPC_MAX_CONCURRENCY", "", fun() ->
        Config = aethernet_gateway_config:load(),
...
optional_string_test() ->
    ?assertEqual(undefined, aethernet_gateway_config:optional_string(undefined)),
    ?assertEqual("hello", aethernet_gateway_config:optional_string(<<"hello">>)),
    ?assertEqual("", aethernet_gateway_config:optional_string(<<>>)).
...
public_endpoints_env_non_default_port_test() ->
    with_envs(
        [
            ...
            {"AETHERNET_PUBLIC_PORT", "8443"},
            {"AETHERNET_GATEWAY_MEDIA_PROXY_ENDPOINT", "https://aethernet.example/media"},
            {"AETHERNET_GATEWAY_STATIC_CDN_ENDPOINT", "https://aethernet.example"}
        ],
        fun() ->
            Config = aethernet_gateway_config:load(),
            ?assertEqual(
            ...
            ?assertEqual(
                <<"https://aethernet.example:8443">>, maps:get(static_cdn_endpoint, Config)
            )
        end
    ).
...
public_endpoints_env_default_port_test() ->
    with_envs(
        [
            ...
            {"AETHERNET_GATEWAY_MEDIA_PROXY_ENDPOINT", "https://aethernet.example/media"},
            {"AETHERNET_GATEWAY_STATIC_CDN_ENDPOINT", "https://cdn.othercdn.net"}
        ],
        fun() ->
            Config = aethernet_gateway_config:load(),
            ?assertEqual(
                <<"https://aethernet.example/media">>, maps:get(media_proxy_endpoint, Config)
            ),
            ?assertEqual(
