...
apply_discovered_retains_connected_existing_member_missing_from_dns_test() ->
    with_clean_cluster_terms(fun() ->
        ExistingNode = 'aethernet_gateway@127.0.0.2',
        NewNode = 'aethernet_gateway@127.0.0.3',
...
apply_discovered_ignores_unrelated_connected_controller_test() ->
    with_clean_cluster_terms(fun() ->
        ...
        DiscoveredNode = 'aethernet_gateway@127.0.0.4',
        State0 = membership_state(#{
            members => [node()],
            discovered => [],
...
maybe_add_member_rejects_valid_role_node_not_seen_in_discovery_test() ->
    with_clean_cluster_terms(fun() ->
        RemoteNode = 'aethernet_gateway@127.0.0.4',
        State0 = membership_state(#{
            members => [node()],
            discovered => [],
...
maybe_add_member_rejects_unknown_role_node_not_seen_in_dns_test() ->
    with_clean_cluster_terms(fun() ->
        RemoteNode = 'aethernet_gateway@127.0.0.5',
        State0 = membership_state(#{
            members => [node()],
            discovered => [],
            connected_nodes => [RemoteNode],
...
update_members_refreshes_role_for_existing_member_test() ->
    with_clean_cluster_terms(fun() ->
        RemoteNode = 'aethernet_gateway@127.0.0.6',
...
update_members_role_lookups_run_concurrently_test() ->
    with_clean_cluster_terms(fun() ->
        Nodes = [
            'aethernet_gateway@127.0.0.11',
            'aethernet_gateway@127.0.0.12',
            'aethernet_gateway@127.0.0.13',
            'aethernet_gateway@127.0.0.14',
            'aethernet_gateway@127.0.0.15'
        ],
        State0 = base_state(#{
            members => [],
            role_fun => fun(_Node) ->
