...
-module(aethernet_gateway_sup_tests).
-typing([eqwalizer]).
-include_lib("eunit/include/eunit.hrl").

cluster_children_empty_when_flag_unset_test() ->
    persistent_term:put({aethernet_gateway, runtime_config}, #{}),
    ?assertEqual([], aethernet_gateway_sup:cluster_children()),
    persistent_term:erase({aethernet_gateway, runtime_config}).

cluster_children_empty_when_flag_false_test() ->
    persistent_term:put(
        {aethernet_gateway, runtime_config},
...
cluster_children_populated_when_flag_true_test() ->
    ...
    [Discovery, Membership, PgScope] = aethernet_gateway_sup:cluster_children(),
    ?assertMatch(
        #{
...
role_enabled_allows_monolith_and_exact_role_only_test() ->
    ?assert(aethernet_gateway_sup:role_enabled(sessions, all)),
    ?assert(aethernet_gateway_sup:role_enabled(guilds, guilds)),
    ?assertNot(aethernet_gateway_sup:role_enabled(sessions, websocket)),
    ?assertNot(aethernet_gateway_sup:role_enabled(push, guilds)).
...
invalid_gateway_role_falls_back_to_stateless_websocket_test() ->
    ...
    {ok, {_SupFlags, Children}} = aethernet_gateway_sup:init([]),
