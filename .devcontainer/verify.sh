...
stage "wasm: pnpm --filter aethernet_app wasm:codegen" pnpm --filter aethernet_app wasm:codegen

stage "app: typecheck" pnpm --filter aethernet_app typecheck
stage "app: unit tests" pnpm --filter aethernet_app exec vitest run

if [ "$QUICK" -eq 0 ]; then
	stage "desktop: typecheck" pnpm --filter aethernet_desktop typecheck
	stage "app: production build" pnpm --filter aethernet_app build
fi
...
if [ "$QUICK" -eq 0 ]; then
	...
	stage "rust: clippy (servers)" cargo clippy -p aethernet_app_proxy -p aethernet_admin --all-targets -- -D warnings
fi
stage "rust: app proxy tests" cargo test -p aethernet_app_proxy
