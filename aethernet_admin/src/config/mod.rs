...
use aethernet_common::config::normalize_public_endpoint_from_env;
use std::env;
...
impl AdminConfig {
    pub fn from_env() -> anyhow::Result<Self> {
        ...
        let secret_key_base = read_env("AETHERNET_ADMIN_SECRET_KEY_BASE", "");
        anyhow::ensure!(
            !secret_key_base.trim().is_empty(),
            "AETHERNET_ADMIN_SECRET_KEY_BASE is required"
        );

        Ok(Self {
            env: RuntimeEnv::from_env_value(&read_env("AETHERNET_ENV", "development")),
            host: read_env("AETHERNET_ADMIN_HOST", "0.0.0.0"),
            port: read_env("AETHERNET_ADMIN_PORT", "3020")
                .parse()
                .unwrap_or(3020),
            secret_key_base,
            base_path,
            ...
            release_channel: read_env_preferred(
                &["RELEASE_CHANNEL", "AETHERNET_RELEASE_CHANNEL"],
                "stable",
            ),
            self_hosted: read_bool_env(&["AETHERNET_SELF_HOSTED"], false),
            proxy: ProxyConfig {
...
mod tests {
    ...
    fn config_from_env(vars: &[(&str, &str)]) -> AdminConfig {
        ...
        unsafe { env::remove_var("AETHERNET_PUBLIC_PORT") };
        unsafe { env::remove_var("AETHERNET_PUBLIC_ORIGIN") };
        unsafe { env::set_var("AETHERNET_ADMIN_SECRET_KEY_BASE", "test-secret") };
    ...
    fn from_env_uses_defaults() {
        ...
        assert_eq!(
            ...
            "https://admin.aethernet.app/oauth2_callback"
        );
    }
    ...
    fn a_non_default_public_port_reaches_the_public_endpoints() {
        let config = config_from_env(&[
            ...
            ("AETHERNET_ADMIN_ENDPOINT", "http://aethernet.example/admin"),
            ("AETHERNET_APP_ENDPOINT", "http://aethernet.example"),
            ("AETHERNET_MEDIA_ENDPOINT", "http://aethernet.example/media"),
            ("AETHERNET_STATIC_CDN_ENDPOINT", "https://cdn.example.net"),
            (
                "AETHERNET_ADMIN_OAUTH_REDIRECT_URI",
                "http://aethernet.example/admin/oauth2_callback",
            ),
        ]);

        assert_eq!(config.admin_endpoint, "http://aethernet.example:19080/admin");
        assert_eq!(config.media_endpoint, "http://aethernet.example:19080/media");
        assert_eq!(config.web_app_endpoint, "http://aethernet.example:19080");
    ...
    fn a_default_public_port_leaves_the_public_endpoints_alone() {
        let config = config_from_env(&[
            ...
            ("AETHERNET_ADMIN_ENDPOINT", "https://aethernet.example/admin"),
            ("AETHERNET_APP_ENDPOINT", "https://aethernet.example"),
            ("AETHERNET_MEDIA_ENDPOINT", "https://aethernet.example/media"),
            ("AETHERNET_STATIC_CDN_ENDPOINT", "https://aethernet.example"),
            (
                "AETHERNET_ADMIN_OAUTH_REDIRECT_URI",
        ...
        assert_eq!(config.admin_endpoint, "https://aethernet.example/admin");
        assert_eq!(config.media_endpoint, "https://aethernet.example/media");
        assert_eq!(config.web_app_endpoint, "https://aethernet.example");
        assert_eq!(config.static_cdn_endpoint, "https://aethernet.example");
        assert_eq!(
            config.oauth_redirect_uri,
            "https://aethernet.example/admin/oauth2_callback"
        );
    ...
    fn the_oauth_redirect_uri_matches_the_api_derived_admin_endpoint() {
        ...
        let api_admin_endpoint = aethernet_common::config::normalize_public_endpoint(
            "http://aethernet.example/admin",
            "aethernet.example",
            Some(19080),
        );
        assert_eq!(
