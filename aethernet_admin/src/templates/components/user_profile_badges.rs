...
pub fn user_profile_badges(
        ...
        && pt != premium_types::NONE
    {
        let tooltip = if pt == premium_types::LIFETIME {
            match premium_since {
                Some(since) => format!("Aethernet Visionary since {since}"),
                None => "Aethernet Visionary".into(),
            }
        } else {
            match premium_since {
                Some(since) => {
                    format!("Aethernet Plutonium subscriber since {since}")
                }
                None => "Aethernet Plutonium".into(),
            }
        };
        badges.push(BadgeDef {
