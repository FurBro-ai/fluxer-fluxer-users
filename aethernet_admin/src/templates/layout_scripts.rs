...
(function () {
	...
	var key = 'aethernet-admin-sidebar-scroll';
	var restored = false;
	try {
...
(function () {
	if (window.__aethernetAdminActionForms) return;
	window.__aethernetAdminActionForms = true;
...
(function () {
	if (window.__aethernetAdminHtmxScrollPreserver) return;
	window.__aethernetAdminHtmxScrollPreserver = true;
	var pendingKey = '__aethernetAdminPendingHtmxScroll';
	var restoreTimer = 0;
...
(function () {
	if (window.__aethernetAdminToastScript) return;
	window.__aethernetAdminToastScript = true;
	...
	function markToastHandled(detail) {
		if (detail) detail.__aethernetToastHandled = true;
		if (detail && detail.xhr) detail.xhr.__aethernetToastHandled = true;
	}

	function toastWasHandled(detail) {
		return !!(detail && (detail.__aethernetToastHandled || (detail.xhr && detail.xhr.__aethernetToastHandled)));
	}
	...
	function disableMutationControls(detail) {
		...
		mutationControls(detail).forEach(function (control) {
			if (control.disabled || control.dataset.aethernetAdminRequestDisabled === 'true') return;
			control.disabled = true;
			control.dataset.aethernetAdminRequestDisabled = 'true';
			disabled.push(control);
		});
		if (detail) detail.__aethernetDisabledControls = disabled;
		if (detail && detail.xhr) detail.xhr.__aethernetDisabledControls = disabled;
	}

	function restoreMutationControls(detail) {
		var controls = (detail && detail.__aethernetDisabledControls) ||
			(detail && detail.xhr && detail.xhr.__aethernetDisabledControls) ||
			[];
		controls.forEach(function (control) {
			if (control.dataset.aethernetAdminRequestDisabled !== 'true') return;
			control.disabled = false;
			delete control.dataset.aethernetAdminRequestDisabled;
		});
	}
	...
	function parseAdminToastHeader(xhr) {
		...
		var raw = xhr.getResponseHeader('X-Aethernet-Admin-Toast');
		if (!raw) return null;
		try {
