...
mod tests {
    ...
    fn build_sso_update_keeps_repeated_allowed_domains() {
        let form = MultiValueForm::parse(
            b"sso_enabled=true&sso_auto_provision=on&sso_allowed_domains%5B%5D=example.com&sso_allowed_domains%5B%5D=example.org&sso_display_name= Aethernet ",
        );
        ...
        assert_eq!(sso.display_name, Some(Some("Aethernet".to_owned())));
        assert_eq!(
            sso.allowed_domains,
