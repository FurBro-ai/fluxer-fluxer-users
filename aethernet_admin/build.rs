...
const BUNDLED_FAMILIES: &[&str] = &["AethernetSans", "AethernetMono"];
...
fn build_fonts(manifest_dir: &Path, out_dir: &Path) {
    ...
    println!("cargo:rerun-if-changed=../packages/fonts/files/AethernetSans");
    println!("cargo:rerun-if-changed=../packages/fonts/files/AethernetMono");
    ...
    assert_eq!(
        faces.len(),
        EXPECTED_FACE_COUNT,
        "packages/fonts no longer offers the {} Latin-core faces aethernet_admin renders; \
    ...
    stylesheet.push_str(
        "/* @generated by aethernet_admin/build.rs from packages/fonts. Do not edit by hand. */\n\n",
    );
    stylesheet.push_str(&format!(
        "/*\n\
         \x20* IBM Plex is licensed under the SIL Open Font License 1.1.\n\
         \x20* The IBM Plex faces below are Modified Versions renamed to \"Aethernet Sans\", so OFL\n\
         ...
         \x20* directory. That keeps the sheet correct under any AETHERNET_ADMIN_BASE_PATH without\n\
...
fn build_tailwind(manifest_dir: &Path, out_dir: &Path) {
    ...
    let Some(cli) = candidates.iter().find(|c| c.exists()) else {
        panic!(
            "tailwindcss CLI not found. Run `pnpm install` in aethernet_admin/.\n\
             Searched: {:?}",
            candidates
        );
    };
