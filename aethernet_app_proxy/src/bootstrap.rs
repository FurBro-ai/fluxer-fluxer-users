...
pub fn build_bootstrap_script(
    ...
    format!(
        r#"<script nonce="{nonce}">window.__AETHERNET_BOOTSTRAP__={bootstrap_json};window.__AETHERNET_CONFIG__={legacy_json};</script>"#
    )
}
...
pub fn inject_bootstrap(
    ...
    if nonced.contains("<!--{{AETHERNET_BOOTSTRAP}}-->") {
        return nonced.replace("<!--{{AETHERNET_BOOTSTRAP}}-->", script_tag);
    }
    if nonced.contains("{{AETHERNET_BOOTSTRAP}}") {
        return nonced.replace("{{AETHERNET_BOOTSTRAP}}", script_tag);
    }
...
mod tests {
    ...
    const SHIPPED_APP_SHELL: &str = include_str!("../../aethernet_app/index.html");
    ...
    fn shipped_shell_declares_both_static_cdn_socket_pools() {
        assert!(
            ...
            "aethernet_app/index.html has no anonymous preconnect for its module scripts and fonts"
        );
        assert!(
            SHIPPED_APP_SHELL.contains(r#"<link rel="preconnect" href="{{STATIC_CDN_ENDPOINT}}">"#),
            "aethernet_app/index.html lost the credentialed preconnect its stylesheet and icons use"
        );
    }
    ...
    fn shipped_shell_ships_no_unsubstituted_placeholder() {
        ...
        assert!(!result.contains("{{AETHERNET_BOOTSTRAP}}"));
    ...
    #[test]
    fn inject_bootstrap_aethernet_placeholder() {
        let html = "<html><head>{{AETHERNET_BOOTSTRAP}}</head></html>";
    ...
    fn inject_bootstrap_replaces_csp_nonce_placeholder() {
        let html = r#"<html><head><script nonce="{{CSP_NONCE_PLACEHOLDER}}"></script>{{AETHERNET_BOOTSTRAP}}</head></html>"#;
    ...
    fn inject_bootstrap_replaces_static_cdn_endpoint_placeholder() {
        let html = r#"<html><head><link href="{{STATIC_CDN_ENDPOINT}}/web/favicon-32x32.png">{{AETHERNET_BOOTSTRAP}}</head></html>"#;
