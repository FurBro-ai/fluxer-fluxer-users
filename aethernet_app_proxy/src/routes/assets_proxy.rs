...
mod tests {
    ...
    impl LocalAssetDir {
        fn with_asset(name: &str, bytes: &[u8]) -> Self {
            ...
            let root =
                std::env::temp_dir().join(format!("aethernet-local-asset-{pid}-{unique}-{name}"));
    ...
    async fn a_local_asset_download_can_be_resumed() {
        let fixture = LocalAssetDir::with_asset("aethernet-setup.exe", b"installer-payload");
