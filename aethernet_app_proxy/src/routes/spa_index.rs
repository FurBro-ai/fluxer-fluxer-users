...
mod tests {
    ...
    use axum::Router;
    use axum::body::Body;
    use aethernet_common::config::GeoipSourceConfig;
    use aethernet_common::geoip::{GeoipConfig, GeoipResolver};
    use std::sync::Arc;
    ...
    const SHELL_WITH_A_NONCE_HOLE: &str = r#"<!doctype html><html><head><title>Aethernet</title><script nonce="{{CSP_NONCE_PLACEHOLDER}}"></script><script src="/assets/app.js"></script></head><body></body></html>"#;
    ...
    const SHELL_WITH_ENDPOINT_HOLES: &str = r#"<!doctype html><html><head><title>Aethernet</title><link rel="preconnect" href="{{STATIC_CDN_ENDPOINT}}">
...
<link rel="icon" type="image/png" sizes="32x32" href="{{STATIC_CDN_ENDPOINT}}/web/favicon-32x32.png"><link rel="apple-touch-icon" sizes="180x180" href="{{STATIC_CDN_ENDPOINT}}/web/apple-touch-icon.png"><script nonce="{{CSP_NONCE_PLACEHOLDER}}"></script><script src="/assets/app.js"></script></head><body></body></html>"#;
    ...
    async fn the_live_branch_serves_a_rendered_document_and_not_the_raw_shell() {
        ...
        assert!(
            served.contains(&format!(
                r#"<script nonce="{granted_nonce}">window.__AETHERNET_BOOTSTRAP__"#
            )),
        ...
        assert!(
            served.contains("window.__AETHERNET_BOOTSTRAP__"),
    ...
    async fn the_frozen_document_is_served_with_its_asset_urls_untouched() {
        ...
        assert!(
            captured.contains(
                r#"<link rel="stylesheet" href="https://aethernetstatic.com/fonts/ibm-plex.css">"#
            ),
    ...
    async fn the_frozen_branch_serves_a_rendered_document_and_not_the_raw_shell() {
        ...
        assert!(
            !captured.contains("window.__AETHERNET_BOOTSTRAP__"),
    ...
    async fn a_crawl_control_document_is_never_served_with_the_asset_lifetime() {
        let root = std::env::temp_dir().join(format!(
            "aethernet-app-proxy-crawl-control-{}",
            std::process::id()
        ));
