...
use aethernet_common::config::{self as cfg, GeoipS3Config, GeoipSourceConfig};
use reqwest::Url;
use std::env;
use std::fmt;

const DEFAULT_DISCOVERY_UPSTREAM_URL: &str = "http://localhost:8088/api/.well-known/aethernet";
...
impl CspConfig {
    pub fn from_env() -> Self {
        Self {
            extra_default_src: read_csp_sources("AETHERNET_CSP_EXTRA_DEFAULT_SRC"),
            extra_connect_src: read_csp_sources("AETHERNET_CSP_EXTRA_CONNECT_SRC"),
            extra_img_src: read_csp_sources("AETHERNET_CSP_EXTRA_IMG_SRC"),
            extra_media_src: read_csp_sources("AETHERNET_CSP_EXTRA_MEDIA_SRC"),
            extra_font_src: read_csp_sources("AETHERNET_CSP_EXTRA_FONT_SRC"),
            extra_script_src: read_csp_sources("AETHERNET_CSP_EXTRA_SCRIPT_SRC"),
            extra_style_src: read_csp_sources("AETHERNET_CSP_EXTRA_STYLE_SRC"),
            extra_frame_src: read_csp_sources("AETHERNET_CSP_EXTRA_FRAME_SRC"),
            extra_worker_src: read_csp_sources("AETHERNET_CSP_EXTRA_WORKER_SRC"),
            extra_manifest_src: read_csp_sources("AETHERNET_CSP_EXTRA_MANIFEST_SRC"),
            report_uri: read_csp_report_uri("AETHERNET_CSP_REPORT_URI"),
        }
    }
}
...
impl AppProxyConfig {
    pub fn from_env() -> Self {
        ...
        let geoip_source = cfg::parse_geoip_source_config(
            &cfg::read_first_env(&["AETHERNET_GEOIP_DB_PATH", "MAXMIND_DB_PATH"], ""),
            "app_proxy",
        );
        ...
        let s3_public_endpoint = parse_optional_http_endpoint(
            ...
            cfg::non_empty_env("AETHERNET_S3_PUBLIC_ENDPOINT"),
        );
        let s3_uploads_bucket = cfg::read_env("AETHERNET_S3_BUCKET_UPLOADS", "aethernet-uploads");
        let s3_uploads_endpoint = s3_public_endpoint.as_ref().and_then(|endpoint| {
            endpoint
                .with_host_prefix("AETHERNET_S3_BUCKET_UPLOADS", s3_uploads_bucket.trim())
                .inspect_err(warn_invalid)
                .ok()
        });

        Self {
            host: cfg::read_env("AETHERNET_APP_PROXY_HOST", "0.0.0.0"),
            port: parse_env_or_warn(
                "AETHERNET_APP_PROXY_PORT",
                &cfg::read_env("AETHERNET_APP_PROXY_PORT", "8080"),
                8080u16,
            ),
            static_dir: cfg::read_env("AETHERNET_STATIC_DIR", "./static"),
            index_upstream_url: parse_optional_http_url(
                "AETHERNET_APP_PROXY_INDEX_UPSTREAM_URL",
                cfg::non_empty_env("AETHERNET_APP_PROXY_INDEX_UPSTREAM_URL"),
            ),
            static_cdn_endpoint: parse_optional_http_endpoint(
                "AETHERNET_STATIC_CDN_ENDPOINT",
                cfg::non_empty_env("AETHERNET_STATIC_CDN_ENDPOINT"),
            ),
            s3_public_endpoint,
            s3_uploads_endpoint,
            ...
            build_version: cfg::read_env_preferred(
                &["BUILD_VERSION", "AETHERNET_BUILD_VERSION"],
            ...
            trust_client_ip_header: cfg::read_bool_env(
                &["AETHERNET_TRUST_CLIENT_IP_HEADER", "TRUST_CLIENT_IP_HEADER"],
                false,
            ),
...
where
    F: FnMut(&str) -> Option<String>,
{
    if let Some(value) = read_var("AETHERNET_APP_PROXY_TIME_FREEZE_ENABLED") {
        return parse_boolish(&value);
    }

    !read_var("AETHERNET_SELF_HOSTED").is_some_and(|value| parse_boolish(&value))
}
...
fn discovery_url_from_api_endpoint(value: &str) -> Option<String> {
    ...
    if endpoint.starts_with("http://") || endpoint.starts_with("https://") {
        Some(format!("{endpoint}/.well-known/aethernet"))
    } else {
        None
    }
}
...
mod tests {
    ...
    fn a_non_default_public_port_reaches_the_boot_html_api_endpoint() {
        assert_eq!(
            resolve_bootstrap_endpoint_from_pairs(&[
                (
                    "PUBLIC_BOOTSTRAP_API_PUBLIC_ENDPOINT",
                    "http://aethernet.example/api",
                ),
                ("AETHERNET_BASE_DOMAIN", "aethernet.example"),
                ("AETHERNET_PUBLIC_PORT", "19080"),
            ]),
            Some("http://aethernet.example:19080/api".to_owned())
        );
    }
    ...
    fn a_default_public_port_leaves_the_boot_html_api_endpoint_alone() {
        assert_eq!(
            resolve_bootstrap_endpoint_from_pairs(&[
                (
                    "PUBLIC_BOOTSTRAP_API_PUBLIC_ENDPOINT",
                    "https://aethernet.example/api",
                ),
                ("AETHERNET_BASE_DOMAIN", "aethernet.example"),
                ("AETHERNET_PUBLIC_PORT", "443"),
            ]),
            Some("https://aethernet.example/api".to_owned())
        );
    }
    ...
    fn the_boot_html_api_endpoint_keeps_a_port_it_already_carries() {
        assert_eq!(
            resolve_bootstrap_endpoint_from_pairs(&[
                (
                    "PUBLIC_BOOTSTRAP_API_PUBLIC_ENDPOINT",
                    "http://aethernet.example:19080/api",
                ),
                ("AETHERNET_BASE_DOMAIN", "aethernet.example"),
                ("AETHERNET_PUBLIC_PORT", "19080"),
            ]),
    ...
    fn the_boot_html_api_endpoint_is_untouched_without_a_base_domain_and_port() {
        assert_eq!(
            ...
            Some("http://aethernet.example/api".to_owned())
        );
    ...
    fn the_public_origin_supplies_the_boot_html_port() {
        assert_eq!(
            resolve_bootstrap_endpoint_from_pairs(&[
                ...
                ("AETHERNET_PUBLIC_ORIGIN", "https://aethernet.example:19080"),
                ("AETHERNET_BASE_DOMAIN", "aethernet.example"),
                ("AETHERNET_PUBLIC_PORT", "443"),
            ]),
            Some("https://aethernet.example:19080/api".to_owned())
        );
    }
    ...
    fn a_malformed_public_port_is_loud() {
        let error = try_resolve_bootstrap_endpoint_from_pairs(&[
            ...
            ("AETHERNET_BASE_DOMAIN", "aethernet.example"),
            ("AETHERNET_PUBLIC_PORT", "not-a-port"),
        ])
        .expect_err("a malformed port is refused");
        assert!(error.to_string().contains("AETHERNET_PUBLIC_PORT"));
    }
    ...
    fn a_malformed_public_origin_is_loud() {
        let error = try_resolve_bootstrap_endpoint_from_pairs(&[
            ...
            ("AETHERNET_PUBLIC_ORIGIN", "aethernet.example:19080"),
        ])
        ...
        assert_eq!(
            resolve_discovery_from_pairs(&[
                (
                    ...
                    "https://web.canary.aethernet.app/api/.well-known/aethernet",
                ),
                ("AETHERNET_API_ENDPOINT", "https://api.canary.aethernet.app"),
            ]),
            "https://web.canary.aethernet.app/api/.well-known/aethernet"
        );
    }
    ...
    fn discovery_upstream_url_derives_from_existing_api_endpoint() {
        assert_eq!(
            resolve_discovery_from_pairs(&[(
                "AETHERNET_API_ENDPOINT",
                "https://api.canary.aethernet.app/"
            )]),
            "https://api.canary.aethernet.app/.well-known/aethernet"
        );
    }
    ...
    fn explicit_time_freeze_setting_overrides_self_hosted_default() {
        assert!(resolve_time_freeze_from_pairs(&[
            ...
            ("AETHERNET_APP_PROXY_TIME_FREEZE_ENABLED", "true"),
        ]));
        assert!(!resolve_time_freeze_from_pairs(&[
            ("AETHERNET_SELF_HOSTED", "false"),
            ("AETHERNET_APP_PROXY_TIME_FREEZE_ENABLED", "false"),
        ]));
    }
}
