...
fn main() -> anyhow::Result<()> {
    ...
    let csp = Arc::new(
        CompiledCspPolicy::from_config(&config)
            .context("failed to compile the Aethernet app proxy content security policy")?,
    );
    ...
    let runtime = Builder::new_multi_thread()
        .enable_all()
        .build()
        .context("failed to create Aethernet app proxy async runtime")?;

    runtime.block_on(async move {
        let http_client =
            build_http_client().context("failed to build Aethernet app proxy HTTP client")?;
        ...
        let listener = TcpListener::bind(&addr)
            .await
            .with_context(|| format!("failed to bind Aethernet app proxy on {addr}"))?;
        tracing::info!(%addr, "starting Aethernet app proxy");
