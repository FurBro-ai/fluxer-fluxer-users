...
const AETHERNET_EPOCH_NUMBER = Number(AETHERNET_EPOCH);
...
function toClampedTimestamp(timestamp: number): number {
	if (timestamp <= AETHERNET_EPOCH_NUMBER) {
		return AETHERNET_EPOCH_NUMBER;
	}
	return timestamp;
}
...
export function extractTimestampBigInt(snowflake: bigint): number {
	return extractTimestampWithEpoch(snowflake, AETHERNET_EPOCH);
}
...
export function fromTimestampBigInt(timestamp: number): bigint {
	...
	if (clampedTimestamp === AETHERNET_EPOCH_NUMBER) {
		return 0n;
	}
...
export function extractTimestampFromSnowflake(snowflake: string, epoch?: string | bigint): number {
	try {
		const epochBigInt = epoch != null ? (typeof epoch === 'string' ? BigInt(epoch) : epoch) : AETHERNET_EPOCH;
