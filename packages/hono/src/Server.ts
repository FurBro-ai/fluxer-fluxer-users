...
import {applyAethernetVersionHeader} from '@aethernet/hono/src/middleware/VersionHeader';
...
function createVersionedFetch<E extends Env>(app: Hono<E>): NodeFetchCallback {
	return async (request, env) => applyAethernetVersionHeader(await app.fetch(request, env as E['Bindings']));
}
