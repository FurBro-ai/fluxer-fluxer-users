...
import {createMetricsMiddleware} from '@aethernet/hono/src/middleware/Metrics';
...
describe('Metrics Middleware', () => {
	describe('request counting', () => {
		test('increments request counter on each request', async () => {
			...
			expect(body).toContain('aethernet_test_http_requests_total{method="GET",status="2xx"} 2');
		});
		...
		test('tracks different methods separately', async () => {
			...
			const res = await requestMetrics(app);
			const body = await res.text();
			expect(body).toContain('aethernet_test_http_requests_total{method="GET",status="2xx"} 1');
			expect(body).toContain('aethernet_test_http_requests_total{method="POST",status="2xx"} 1');
		});
	...
	describe('error counting', () => {
		test('increments error counter on 5xx responses', async () => {
			...
			const res = await requestMetrics(app);
			const body = await res.text();
			expect(body).toContain('aethernet_test_http_errors_total{method="GET"} 2');
		});
		...
		test('does not count 4xx as errors', async () => {
			...
			const res = await requestMetrics(app);
			const body = await res.text();
			expect(body).not.toContain('aethernet_test_http_errors_total{method="GET"}');
		});
	});
	...
	describe('duration histogram', () => {
		test('records request duration', async () => {
			...
			const res = await requestMetrics(app);
			const body = await res.text();
			expect(body).toContain('aethernet_test_http_request_duration_seconds_count 1');
			expect(body).toContain('aethernet_test_http_request_duration_seconds_sum');
			expect(body).toContain('aethernet_test_http_request_duration_seconds_bucket{le="0.005"}');
			expect(body).toContain('aethernet_test_http_request_duration_seconds_bucket{le="+Inf"} 1');
		});
		...
		test('accumulates across multiple requests', async () => {
			...
			const res = await requestMetrics(app);
			const body = await res.text();
			expect(body).toContain('aethernet_test_http_request_duration_seconds_count 3');
		});
	});
	...
	describe('uptime gauge', () => {
		test('reports uptime in seconds', async () => {
			...
			const res = await requestMetrics(app);
			const body = await res.text();
			expect(body).toContain('# TYPE aethernet_test_uptime_seconds gauge');
			expect(body).toMatch(/aethernet_test_uptime_seconds \d/);
		});
	});
	...
	describe('metrics endpoint', () => {
		...
		test('includes HELP and TYPE annotations', async () => {
			...
			const res = await requestMetrics(app);
			const body = await res.text();
			expect(body).toContain('# HELP aethernet_test_http_requests_total Total HTTP requests');
			expect(body).toContain('# TYPE aethernet_test_http_requests_total counter');
			expect(body).toContain('# HELP aethernet_test_http_request_duration_seconds HTTP request duration in seconds');
			expect(body).toContain('# TYPE aethernet_test_http_request_duration_seconds histogram');
			expect(body).toContain('# HELP aethernet_test_http_errors_total Total HTTP 5xx errors');
			expect(body).toContain('# TYPE aethernet_test_http_errors_total counter');
			expect(body).toContain('# HELP aethernet_test_uptime_seconds Process uptime in seconds');
			expect(body).toContain('# TYPE aethernet_test_uptime_seconds gauge');
		});
		...
		test('renders default counter value when no requests made', async () => {
			...
			const res = await requestMetrics(app);
			const body = await res.text();
			expect(body).toContain('aethernet_test_http_requests_total 0');
			expect(body).toContain('aethernet_test_http_errors_total 0');
		});
	});
	...
	describe('service name prefix', () => {
		test('uses provided service name in metric names', async () => {
			...
			const res = await requestMetrics(app);
			const body = await res.text();
			expect(body).toContain('aethernet_gateway_http_requests_total');
			expect(body).toContain('aethernet_gateway_http_request_duration_seconds');
			expect(body).toContain('aethernet_gateway_http_errors_total');
			expect(body).toContain('aethernet_gateway_uptime_seconds');
		});
	});
});
