...
} from '@aethernet/hono/src/middleware/MiddlewareStack';
import type {RateLimitResult, RateLimitService} from '@aethernet/hono/src/middleware/RateLimit';
import {REQUEST_ID_KEY} from '@aethernet/hono/src/middleware/RequestId';
...
describe('applyMiddlewareStack', () => {
	test('applies version header middleware', async () => {
		...
		expect(response.headers.get(Headers.X_AETHERNET_VERSION)).toBe('dev');
	});
