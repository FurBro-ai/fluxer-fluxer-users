...
import {AppErrorHandler} from '@aethernet/errors/src/domains/core/ErrorHandlers';
import {ServiceUnavailableError} from '@aethernet/errors/src/HttpErrors';
import type {BaseHonoEnv} from '@aethernet/hono_types/src/HonoTypes';
...
vi.mock('@aethernet/logger/src/Logger', async (importOriginal) => {
	const actual = await importOriginal<typeof import('@aethernet/logger/src/Logger')>();
...
describe('AppErrorHandler logging', () => {
	...
	it('logs 5xx AethernetErrors with the underlying cause', async () => {
	...
	it('logs 4xx AethernetErrors at debug rather than error', async () => {
		const app = createApp();
		app.get('/thing', () => {
