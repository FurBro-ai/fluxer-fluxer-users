...
describe('AethernetError', () => {
	it.each([
		{
			name: 'defaults the message to the code',
			create: () => new AethernetError({code: 'BAD_REQUEST', status: 400}),
			status: 400,
			body: {code: 'BAD_REQUEST', message: 'BAD_REQUEST'},
		},
		{
			name: 'uses a custom message',
			create: () => new AethernetError({code: 'SERVER_ERROR', message: 'Custom error message', status: 500}),
			status: 500,
			body: {code: 'SERVER_ERROR', message: 'Custom error message'},
		},
		{
			name: 'preserves an intentionally empty message',
			create: () => new AethernetError({code: 'FORBIDDEN', message: '', status: 403}),
			status: 403,
	...
	it('preserves exception metadata and headers without leaking them into the body', async () => {
		...
		const error = new AethernetError({
			code: 'RATE_LIMITED',
			status: 429,
			cause,
			headers,
		...
		expect(error.name).toBe('AethernetError');
