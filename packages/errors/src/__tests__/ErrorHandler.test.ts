...
import {createErrorHandler, type ErrorHandlerOptions} from '@aethernet/errors/src/ErrorHandler';
...
describe('createErrorHandler', () => {
	describe('AethernetError handling', () => {
		it('should return AethernetError response directly', async () => {
			const app = createTestApp();
			app.get('/test', () => {
				throw new AethernetError({
					code: 'TEST_ERROR',
		...
		it('should include AethernetError data in response', async () => {
			const app = createTestApp();
			app.get('/test', () => {
				throw new AethernetError({
		...
		it('should include AethernetError custom headers', async () => {
			const app = createTestApp();
			app.get('/test', () => {
				throw new AethernetError({
					code: 'RATE_LIMITED',
	...
	describe('logError callback', () => {
		...
		it('should call logError for AethernetError', async () => {
			const logError = vi.fn();
			const app = createTestApp({logError});
			app.get('/test', () => {
				throw new AethernetError({code: 'TEST', status: 400});
			});
			await app.request('/test');
			expect(logError).toHaveBeenCalledTimes(1);
			expect(logError.mock.calls[0][0]).toBeInstanceOf(AethernetError);
		});
	});
	describe('customHandler callback', () => {
		...
		it('should fall back to default handling when customHandler returns undefined', async () => {
			...
			const app = createTestApp({customHandler});
			app.get('/test', () => {
				throw new AethernetError({code: 'FALLBACK', status: 400});
			});
			const response = await app.request('/test');
			expect(response.status).toBe(400);
			const body = (await response.json()) as ErrorResponse;
