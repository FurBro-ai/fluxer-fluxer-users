...
const AETHERNET_TAG_REGEX = /^([^#]{1,32})#([0-9]{4})$/;
const AETHERNET_TAG_TYPE = z
	.string()
	.min(3)
	.max(37)
	.refine((value) => AETHERNET_TAG_REGEX.test(value), 'Aethernet tag must be in the format username#1234')
	.describe('A Aethernet username tag in the format username#1234');
...
const DsaReportBase = z.object({
	...
	reporter_aethernet_tag: z
		.optional(AETHERNET_TAG_TYPE)
		.describe('Aethernet tag of the reporter if they have an account'),
});
const DsaReportMessageRequest = DsaReportBase.extend({
	...
	reported_user_tag: z.optional(AETHERNET_TAG_TYPE).describe('Aethernet tag of the user who sent the message'),
});
...
const DsaReportUserRequest = DsaReportBase.extend({
	...
	user_tag: z.optional(AETHERNET_TAG_TYPE).describe('Aethernet tag of the user being reported'),
