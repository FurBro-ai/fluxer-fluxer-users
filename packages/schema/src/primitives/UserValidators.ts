...
const AETHERNET_TAG_REGEX = /^[a-zA-Z0-9_]+$/;
...
export const UsernameType = withOpenApiType(
	z
		...
		.refine((value) => AETHERNET_TAG_REGEX.test(value), ValidationErrorCodes.USERNAME_INVALID_CHARACTERS)
		.refine((value) => {
		...
		.refine((value) => {
			const lowerValue = value.toLowerCase();
			return !lowerValue.includes('aethernet') && !lowerValue.includes('system message');
