...
use aethernet_markdown_parser::ParserFlags;
use aethernet_markdown_parser::native::{AethernetMdBuffer, aethernet_md_buffer_free, aethernet_md_parse};

fn empty_buffer() -> AethernetMdBuffer {
    AethernetMdBuffer {
...
fn parse(input: &str, flags: u32, tsv: &str) -> Result<String, String> {
    ...
    unsafe { aethernet_md_buffer_free(&raw mut out) };
    assert!(out.data.is_null());
...
fn matches_wasm_abi_output() {
    ...
    let json = aethernet_markdown_parser::parse_markdown_json(input, ParserFlags::ALL, "")
...
fn double_free_is_a_no_op() {
    ...
    unsafe { aethernet_md_buffer_free(&raw mut out) };
    unsafe { aethernet_md_buffer_free(&raw mut out) };
    unsafe { aethernet_md_buffer_free(std::ptr::null_mut()) };
}
