...
import {loadConfig, resetConfig} from '@aethernet/config/src/ConfigLoader';
import {normalizePublicEndpoint} from '@aethernet/config/src/EndpointDerivation';
...
const SECRETS: Record<string, string> = {
	...
	AETHERNET_S3_ACCESS_KEY: 's3-access-key',
	AETHERNET_S3_SECRET_KEY: 's3-secret-key',
	LIVEKIT_API_KEY: 'livekit-api-key',
	LIVEKIT_API_SECRET: 'livekit-api-secret',
	AETHERNET_ERLANG_COOKIE: 'erlang-cookie',
	AETHERNET_SUDO_MODE_SECRET: 'sudo-mode-secret',
	AETHERNET_CONNECTION_INITIATION_SECRET: 'connection-initiation-secret',
	AETHERNET_GATEWAY_RPC_AUTH_TOKEN: 'gateway-rpc-auth-token',
	AETHERNET_MEDIA_PROXY_SECRET_KEY: 'media-proxy-secret-key',
	AETHERNET_MEDIA_PROXY_UPLOAD_RELAY_SECRET_BASE64: 'AAECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8=',
	AETHERNET_ADMIN_SECRET_KEY_BASE: 'admin-secret-key-base',
	AETHERNET_ADMIN_OAUTH_CLIENT_SECRET: 'admin-oauth-client-secret',
...
const PORT_ONLY_ENV: Record<string, string> = {
	...
	AETHERNET_PUBLIC_PORT: PUBLIC_PORT,
};

const DOCUMENTED_RECIPE_ENV: Record<string, string> = {
	...PORT_ONLY_ENV,
	AETHERNET_PUBLIC_ORIGIN: `http://${DOMAIN}:${PUBLIC_PORT}`,
	AETHERNET_HTTP_PORT: PUBLIC_PORT,
};

const ORIGIN_ONLY_ENV: Record<string, string> = {
	...SECRETS,
	AETHERNET_PUBLIC_ORIGIN: `https://${DOMAIN}:${ORIGIN_PORT}`,
	AETHERNET_HTTPS_PORT: ORIGIN_PORT,
};
...
function repairedPublicUrls(service: string, env: Record<string, string>): Array<[string, string]> {
	...
	const publicPort = Number.parseInt(environment.AETHERNET_PUBLIC_PORT ?? '', 10);
	return publicUrlNames(environment).map((name) => [
		`${service}.${name}`,
		normalizePublicEndpoint(
			environment[name],
			environment.AETHERNET_BASE_DOMAIN ?? '',
...
describe('the shipped compose stack expanded on a non-default port', () => {
	test('every service handed a public URL is handed the base domain and port that repair it', () => {
		const starved = serviceNames
			...
			.filter((service) => {
				...
				return (
					publicUrlNames(environment).length > 0 &&
					(!environment.AETHERNET_BASE_DOMAIN || !environment.AETHERNET_PUBLIC_PORT)
				);
			});
	...
	test('the edge listens on its scheme default whatever the public port is', () => {
		...
		expect(environment.AETHERNET_EDGE_SITE_ADDRESS).toBe(`http://${DOMAIN}`);
...
describe('a public origin carrying a port while AETHERNET_PUBLIC_PORT stays standard', () => {
	beforeEach(() => {
		resetConfig();
	});
