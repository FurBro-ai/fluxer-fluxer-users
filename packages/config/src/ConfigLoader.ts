...
import {isConfigObject} from '@aethernet/config/src/config_loader/ConfigObject';
import {buildNamedAethernetEnvOverrides} from '@aethernet/config/src/config_loader/EnvironmentOverrides';
import {
	buildUrl,
...
import type {MasterConfig} from '@aethernet/config/src/MasterConfig';
...
function defaultConfig(): MasterConfig {
	return {
		...
		s3: {
			...
			buckets: {
				cdn: 'aethernet',
				uploads: 'aethernet-uploads',
				downloads: 'aethernet-downloads',
				reports: 'aethernet-reports',
				harvests: 'aethernet-harvests',
			},
		},
		services: {
			api: {
...
function validateUploadRelaySecret(value: string, mode: string): void {
	...
	if (trimmed.length === 0) {
		if (mode === 'upload') {
			throw new Error('AETHERNET_MEDIA_PROXY_UPLOAD_RELAY_SECRET_BASE64 is required in upload mode');
		}
		return;
	}
	if (!/^[A-Za-z0-9+/]+={0,2}$/u.test(trimmed)) {
		throw new Error('AETHERNET_MEDIA_PROXY_UPLOAD_RELAY_SECRET_BASE64 must be base64');
	}
	if (Buffer.from(trimmed, 'base64').length < 32) {
		throw new Error('AETHERNET_MEDIA_PROXY_UPLOAD_RELAY_SECRET_BASE64 must decode to at least 32 bytes');
	}
}
...
function validateVapidConfig(config: MasterConfig): void {
	requireString(config.auth.vapid.public_key, 'AETHERNET_VAPID_PUBLIC_KEY');
	requireString(config.auth.vapid.private_key, 'AETHERNET_VAPID_PRIVATE_KEY');
	...
	if (pub.length !== 65 || pub[0] !== 0x04) {
		throw new Error('AETHERNET_VAPID_PUBLIC_KEY must be the base64url 65-byte uncompressed P-256 point');
	}
	if (priv.length !== 32) {
		throw new Error('AETHERNET_VAPID_PRIVATE_KEY must be the base64url 32-byte P-256 scalar');
	}
	let derived: Buffer;
	try {
		...
		throw new Error('AETHERNET_VAPID_PRIVATE_KEY does not match AETHERNET_VAPID_PUBLIC_KEY');
	}
...
function validatePostgresConfig(config: MasterConfig): void {
	...
	assertIntegerInRange(postgres.port, 'AETHERNET_POSTGRES_PORT', 1, 65535);
	assertIntegerInRange(postgres.max_connections, 'AETHERNET_POSTGRES_MAX_CONNECTIONS', 1, 1000);
	assertBoolean(postgres.ssl, 'AETHERNET_POSTGRES_SSL');
	assertIdentifier(postgres.kv_table, 'AETHERNET_POSTGRES_KV_TABLE');
	assertBoolean(postgres.prepared_statements, 'AETHERNET_POSTGRES_PREPARED_STATEMENTS');
	if (config.env !== 'production' || config.database.backend !== 'postgres') {
		return;
	}
	if (!postgres.url) {
		requireString(postgres.host, 'AETHERNET_POSTGRES_HOST');
		requireString(postgres.database, 'AETHERNET_POSTGRES_DATABASE');
		requireString(postgres.username, 'AETHERNET_POSTGRES_USERNAME');
		requireString(postgres.password, 'AETHERNET_POSTGRES_PASSWORD');
		if (['127.0.0.1', 'localhost'].includes(postgres.host.trim().toLowerCase())) {
			throw new Error('AETHERNET_POSTGRES_HOST must be explicitly configured for production');
		}
		if (postgres.password === 'aethernet') {
			throw new Error('AETHERNET_POSTGRES_PASSWORD must not use the development default in production');
		}
	}
	if (!postgres.ssl && !config.instance.self_hosted) {
		throw new Error('AETHERNET_POSTGRES_SSL must be true in production');
	}
}
...
function validateCaptchaConfig(config: MasterConfig): void {
	...
	if (captcha.provider === 'hcaptcha') {
		requireString(captcha.hcaptcha?.site_key, 'AETHERNET_CAPTCHA_HCAPTCHA_SITE_KEY');
		requireString(captcha.hcaptcha?.secret_key, 'AETHERNET_CAPTCHA_HCAPTCHA_SECRET_KEY');
		return;
	}
	if (captcha.provider === 'turnstile') {
		requireString(captcha.turnstile?.site_key, 'AETHERNET_CAPTCHA_TURNSTILE_SITE_KEY');
		requireString(captcha.turnstile?.secret_key, 'AETHERNET_CAPTCHA_TURNSTILE_SECRET_KEY');
		return;
	}
	throw new Error('AETHERNET_CAPTCHA_PROVIDER must be hcaptcha or turnstile when AETHERNET_CAPTCHA_ENABLED is true');
}
...
function validateApiWorkerConfig(config: MasterConfig): void {
	...
	if (worker.mode !== undefined) {
		assertOneOf(worker.mode, ['all_lanes', 'single_lane', 'single_task'], 'AETHERNET_API_WORKER_MODE');
	}
	if (worker.lane !== undefined) {
		assertOneOf(worker.lane, ['realtime', 'unfurl', 'lifecycle', 'batch'], 'AETHERNET_API_WORKER_LANE');
	}
	if (worker.mode === 'single_task') {
		requireString(worker.task, 'AETHERNET_API_WORKER_TASK');
	}
}
...
function validatePublicEndpoints(endpoints: DerivedEndpoints): void {
	for (const [key, value] of Object.entries(endpoints)) {
		const envName = `AETHERNET_${key.toUpperCase()}_ENDPOINT`;
...
function normalizeConfig(config: MasterConfig): MasterConfig {
	assertOneOf(config.env, ['development', 'production', 'test'], 'AETHERNET_ENV');
	assertOneOf(config.domain.public_scheme, ['http', 'https'], 'AETHERNET_PUBLIC_SCHEME');
	assertOneOf(config.domain.internal_scheme, ['http', 'https'], 'AETHERNET_INTERNAL_SCHEME');
	assertOneOf(config.database.backend, ['postgres', 'cassandra'], 'AETHERNET_DATABASE_BACKEND');
	assertOneOf(config.internal.kv_provider, ['redis'], 'AETHERNET_KV_PROVIDER');
	assertOneOf(config.internal.kv_mode, ['standalone', 'cluster'], 'AETHERNET_KV_MODE');
	assertOneOf(config.integrations.email.provider, ['smtp', 'none'], 'AETHERNET_EMAIL_PROVIDER');
	assertOneOf(config.integrations.captcha.provider, ['hcaptcha', 'turnstile', 'none'], 'AETHERNET_CAPTCHA_PROVIDER');
	assertOneOf(config.integrations.search.engine, ['elasticsearch', 'meilisearch'], 'AETHERNET_SEARCH_ENGINE');
	assertOneOf(
	...
	assertIntegerInRange(config.services.api.max_inflight_requests, 'AETHERNET_API_MAX_INFLIGHT_REQUESTS', 1, 100_000);
	assertIntegerInRange(config.services.api.headers_timeout_ms, 'AETHERNET_API_HEADERS_TIMEOUT_MS', 1_000, 3_600_000);
	assertIntegerInRange(config.services.api.request_timeout_ms, 'AETHERNET_API_REQUEST_TIMEOUT_MS', 1_000, 3_600_000);
	assertIntegerInRange(config.domain.public_port, 'AETHERNET_PUBLIC_PORT', 1, 65_535);
	requireString(config.domain.base_domain, 'AETHERNET_BASE_DOMAIN');
	for (const key of ['base_domain', 'static_cdn_domain', 'invite_domain', 'gift_domain'] as const) {
		validateDomain(config.domain[key], `AETHERNET_${key.toUpperCase()}`);
	}
	requireString(config.auth.sudo_mode_secret, 'AETHERNET_SUDO_MODE_SECRET');
	requireString(config.auth.connection_initiation_secret, 'AETHERNET_CONNECTION_INITIATION_SECRET');
	validateVapidConfig(config);
	requireString(config.s3?.access_key_id, 'AETHERNET_S3_ACCESS_KEY_ID');
	requireString(config.s3?.secret_access_key, 'AETHERNET_S3_SECRET_ACCESS_KEY');
	requireString(config.services.media_proxy.secret_key, 'AETHERNET_MEDIA_PROXY_SECRET_KEY');
	validateUploadRelaySecret(config.services.media_proxy.upload_relay.secret_base64, config.services.media_proxy.mode);
	requireString(config.services.admin.secret_key_base, 'AETHERNET_ADMIN_SECRET_KEY_BASE');
	requireString(config.services.admin.oauth_client_secret, 'AETHERNET_ADMIN_OAUTH_CLIENT_SECRET');
	requireString(config.services.gateway.rpc_auth_token, 'AETHERNET_GATEWAY_RPC_AUTH_TOKEN');
	return config;
}
...
function normalizePasskeyOrigin(origin: string, index: number): string {
	...
	throw new Error(
		`AETHERNET_PASSKEY_ADDITIONAL_ALLOWED_ORIGINS entry ${index + 1} must be an HTTP(S) origin or a canonical Android signing-certificate origin`,
	);
}
...
export async function loadConfig(): Promise<MasterConfig> {
	...
	const overrides = buildNamedAethernetEnvOverrides(process.env);
	...
	normalizePasskeys(withPublicPort, process.env.AETHERNET_PASSKEY_ADDITIONAL_ALLOWED_ORIGINS === undefined);
