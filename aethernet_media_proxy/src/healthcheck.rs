...
pub async fn run() -> anyhow::Result<()> {
    let addr = target(
        env::var("AETHERNET_MEDIA_PROXY_HOST").ok().as_deref(),
        env::var("AETHERNET_MEDIA_PROXY_PORT").ok().as_deref(),
    )?;
    probe(addr).await
}
...
fn target(host: Option<&str>, port: Option<&str>) -> anyhow::Result<SocketAddr> {
    ...
    let ip = host
        .parse::<IpAddr>()
        .with_context(|| format!("AETHERNET_MEDIA_PROXY_HOST is not an IP address: {host}"))?;
    let ip = match ip {
    ...
    let port = match port.map(str::trim).filter(|port| !port.is_empty()) {
        Some(port) => port
            .parse::<u16>()
            .with_context(|| format!("AETHERNET_MEDIA_PROXY_PORT is not a port number: {port}"))?,
        None => 8080,
    };
