...
fn read_bucket_defaults_to_cdn_bucket_and_can_be_overridden() {
    let cfg = Config::load_from_iter(env_with(&[("AETHERNET_S3_BUCKET_CDN", "aethernet")])).unwrap();
    assert_eq!("aethernet", cfg.storage.s3_read_bucket);

    let cfg = Config::load_from_iter(env_with(&[
        ("AETHERNET_S3_BUCKET_CDN", "aethernet"),
        ("AETHERNET_S3_READ_BUCKET", "aethernet-static"),
    ]))
    .unwrap();
    assert_eq!("aethernet-static", cfg.storage.s3_read_bucket);
}

#[test]
fn read_bucket_style_inherits_force_path_style() {
    let cfg = Config::load_from_iter(env_with(&[("AETHERNET_S3_FORCE_PATH_STYLE", "false")])).unwrap();
    assert_eq!(BucketStyle::VirtualHosted, cfg.storage.s3_read_bucket_style);

    let cfg = Config::load_from_iter(env_with(&[
        ("AETHERNET_S3_FORCE_PATH_STYLE", "false"),
        ("AETHERNET_S3_READ_BUCKET_STYLE", "root"),
    ]))
    .unwrap();
...
fn read_bucket_style_parses_all_values_case_insensitively() {
    for (raw, expected) in [
        ...
        let cfg =
            Config::load_from_iter(env_with(&[("AETHERNET_S3_READ_BUCKET_STYLE", raw)])).unwrap();
...
fn read_bucket_style_rejects_unknown_value() {
    let err =
        Config::load_from_iter(env_with(&[("AETHERNET_S3_READ_BUCKET_STYLE", "cdn")])).unwrap_err();
    assert!(err.to_string().contains("AETHERNET_S3_READ_BUCKET_STYLE"));
}

#[test]
fn read_signed_parses_boolean() {
    let cfg = Config::load_from_iter(env_with(&[("AETHERNET_S3_READ_SIGNED", "true")])).unwrap();
    assert!(cfg.storage.s3_read_signed);

    let err = Config::load_from_iter(env_with(&[("AETHERNET_S3_READ_SIGNED", "maybe")])).unwrap_err();
    assert!(err.to_string().contains("AETHERNET_S3_READ_SIGNED"));
}
...
fn read_endpoint_is_validated_at_startup() {
    ...
    for (bad, expected) in [
        ...
        let err =
            Config::load_from_iter(env_with(&[("AETHERNET_S3_READ_ENDPOINT", bad)])).unwrap_err();
        let message = err.to_string();
        assert!(
            message.contains("AETHERNET_S3_READ_ENDPOINT") && message.contains(expected),
            "bad={bad} err={message}"
        );
    }
}
