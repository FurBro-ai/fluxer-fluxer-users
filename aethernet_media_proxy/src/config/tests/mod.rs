...
fn base_env() -> Vec<(&'static str, &'static str)> {
    vec![("AETHERNET_MEDIA_PROXY_SECRET_KEY", "secret")]
}
...
fn with_shared_runtime_env(release: &[(&str, &str)]) -> Vec<(String, String)> {
    [
        ...
        ("AETHERNET_MEDIA_PROXY_SECRET_KEY", "shared-runtime-secret"),
        ("AETHERNET_S3_ENDPOINT", "https://ewr1.vultrobjects.com"),
        ("AETHERNET_S3_REGION", "ewr1"),
        ("AETHERNET_S3_ACCESS_KEY_ID", "AKIAIOSFODNN7EXAMPLE"),
        (
...
fn requires_secret_key() {
    ...
    assert!(err.to_string().contains("AETHERNET_MEDIA_PROXY_SECRET_KEY"));
}
...
fn canonical_media_proxy_env_overrides_apply() {
    let cfg = Config::load_from_iter([
        ("AETHERNET_MEDIA_PROXY_SECRET_KEY", "secret"),
        ("AETHERNET_MEDIA_PROXY_STORAGE_BACKEND", "s3"),
        ("AETHERNET_MEDIA_PROXY_STORAGE_ROOT", "/srv/aethernet-media"),
        ("AETHERNET_MEDIA_PROXY_READ_ONLY", "true"),
        ("AETHERNET_S3_FORCE_PATH_STYLE", "false"),
        ("AETHERNET_S3_SESSION_TOKEN", "token"),
        ("AETHERNET_MEDIA_PROXY_MAX_NATIVE_TRANSFORMS", "3"),
        ("AETHERNET_MEDIA_PROXY_WORKER_QUEUE_CAPACITY", "24"),
        ("AETHERNET_MEDIA_PROXY_TRANSFORM_TIMEOUT_MS", "2000"),
        ("AETHERNET_NSFW_SERVICE_ENDPOINT", "http://nsfw:8000"),
        ("AETHERNET_MEDIA_PROXY_NSFW_THRESHOLD", "0.7"),
    ])
    .unwrap();

    assert_eq!(StorageBackend::S3, cfg.storage.backend);
    assert_eq!("/srv/aethernet-media", cfg.storage.root);
...
fn upload_mode_requires_relay_secret() {
    let err = Config::load_from_iter([
        ("AETHERNET_MEDIA_PROXY_SECRET_KEY", "secret"),
        ("AETHERNET_MEDIA_PROXY_MODE", "upload"),
    ])
    .unwrap_err();
    assert!(
        err.to_string()
            .contains("AETHERNET_MEDIA_PROXY_UPLOAD_RELAY_SECRET_BASE64")
    );
}
...
fn rejects_a_body_limit_above_the_spool_budget() {
    let err = Config::load_from_iter([
        ("AETHERNET_MEDIA_PROXY_SECRET_KEY", "secret"),
        ("AETHERNET_MEDIA_PROXY_UPLOAD_RELAY_MAX_BODY_BYTES", "2"),
        ("AETHERNET_MEDIA_PROXY_UPLOAD_RELAY_SPOOL_MAX_TOTAL_BYTES", "1"),
    ])
    .unwrap_err();
    assert!(
        err.to_string()
            .contains("must not exceed AETHERNET_MEDIA_PROXY_UPLOAD_RELAY_SPOOL_MAX_TOTAL_BYTES")
    );
}
...
fn rejects_a_zero_spool_budget() {
    let err = Config::load_from_iter([
        ("AETHERNET_MEDIA_PROXY_SECRET_KEY", "secret"),
        ("AETHERNET_MEDIA_PROXY_UPLOAD_RELAY_SPOOL_MAX_TOTAL_BYTES", "0"),
    ])
    .unwrap_err();
    assert!(
        err.to_string()
            .contains("AETHERNET_MEDIA_PROXY_UPLOAD_RELAY_MAX_BODY_BYTES")
    );
}
...
fn accepts_a_body_limit_equal_to_the_spool_budget() {
    let cfg = Config::load_from_iter([
        ("AETHERNET_MEDIA_PROXY_SECRET_KEY", "secret"),
        ("AETHERNET_MEDIA_PROXY_UPLOAD_RELAY_MAX_BODY_BYTES", "4096"),
        (
...
fn rejects_invalid_mode_env() {
    let err = Config::load_from_iter([
        ("AETHERNET_MEDIA_PROXY_SECRET_KEY", "secret"),
        ("AETHERNET_MEDIA_PROXY_MODE", "worker"),
    ])
    .unwrap_err();
    assert!(err.to_string().contains("AETHERNET_MEDIA_PROXY_MODE"));
}
...
fn rejects_invalid_storage_backend_env() {
    let err = Config::load_from_iter([
        ("AETHERNET_MEDIA_PROXY_SECRET_KEY", "secret"),
        ("AETHERNET_MEDIA_PROXY_STORAGE_BACKEND", "filesystem"),
    ])
    .unwrap_err();
    assert!(
        err.to_string()
            .contains("AETHERNET_MEDIA_PROXY_STORAGE_BACKEND")
    );
}
...
fn rejects_invalid_bool_env() {
    let err = Config::load_from_iter([
        ("AETHERNET_MEDIA_PROXY_SECRET_KEY", "secret"),
        ("AETHERNET_MEDIA_PROXY_READ_ONLY", "maybe"),
    ])
    .unwrap_err();
    assert!(err.to_string().contains("AETHERNET_MEDIA_PROXY_READ_ONLY"));
}
...
fn rejects_invalid_number_env() {
    let err = Config::load_from_iter([
        ("AETHERNET_MEDIA_PROXY_SECRET_KEY", "secret"),
        ("AETHERNET_MEDIA_PROXY_WORKER_QUEUE_CAPACITY", "many"),
    ])
    .unwrap_err();
    assert!(
        err.to_string()
            .contains("AETHERNET_MEDIA_PROXY_WORKER_QUEUE_CAPACITY")
    );
}
...
fn rejects_out_of_range_number_env() {
    let err = Config::load_from_iter([
        ("AETHERNET_MEDIA_PROXY_SECRET_KEY", "secret"),
        ("AETHERNET_MEDIA_PROXY_TRANSFORM_TIMEOUT_MS", "999999"),
    ])
    .unwrap_err();
    assert!(
        err.to_string()
            .contains("AETHERNET_MEDIA_PROXY_TRANSFORM_TIMEOUT_MS")
    );
}
...
fn production_media_proxy_release_env_loads() {
    let cfg = Config::load_from_iter(with_shared_runtime_env(&[
        ...
        (
            ...
            "http://int.flx-nyc-misc1.srv.aethernet.dev:8000",
        ),
        ("AETHERNET_MEDIA_PROXY_MODE", "mp"),
        ("AETHERNET_MEDIA_PROXY_NSFW_THRESHOLD", "0.95"),
        ("AETHERNET_MEDIA_PROXY_STORAGE_BACKEND", "s3"),
        ("AETHERNET_MEDIA_PROXY_READ_ONLY", "true"),
        ("AETHERNET_MEDIA_PROXY_MAX_NATIVE_TRANSFORMS", "4"),
        ("AETHERNET_MEDIA_PROXY_WORKER_QUEUE_CAPACITY", "128"),
        ("AETHERNET_MEDIA_PROXY_TRANSFORM_TIMEOUT_MS", "30000"),
        ("AETHERNET_MEDIA_PROXY_MAX_ENCODE_FRAMES", "4096"),
        ("AETHERNET_MEDIA_PROXY_MAX_ENCODE_DURATION_MS", "30000"),
        ("AETHERNET_MEDIA_PROXY_TRANSFORM_CACHE_BYTES", "1073741824"),
        (
            "AETHERNET_MEDIA_PROXY_TRANSFORM_CACHE_MAX_ENTRY_BYTES",
            "134217728",
        ),
        ("AETHERNET_MEDIA_PROXY_TRANSFORM_CACHE_TTL_MS", "1800000"),
        ("AETHERNET_MEDIA_PROXY_SOCKET_IO_TIMEOUT_MS", "30000"),
    ]))
    .unwrap();
...
fn production_static_proxy_release_env_loads() {
    let cfg = Config::load_from_iter(with_shared_runtime_env(&[
        ("RELEASE_CHANNEL", "canary"),
        ("AETHERNET_MEDIA_PROXY_MODE", "static"),
        ("AETHERNET_MEDIA_PROXY_STORAGE_BACKEND", "s3"),
        ("AETHERNET_MEDIA_PROXY_READ_ONLY", "true"),
        ("AETHERNET_MEDIA_PROXY_SOCKET_IO_TIMEOUT_MS", "30000"),
    ]))
...
fn production_uploads_release_env_loads() {
    ...
    let cfg = Config::load_from_iter(with_shared_runtime_env(&[
        ...
        ("AETHERNET_MEDIA_PROXY_MODE", "upload"),
        ("AETHERNET_MEDIA_PROXY_STORAGE_BACKEND", "s3"),
        ("AETHERNET_MEDIA_PROXY_READ_ONLY", "false"),
        ("AETHERNET_MEDIA_PROXY_SOCKET_IO_TIMEOUT_MS", "300000"),
        ("AETHERNET_MEDIA_PROXY_UPLOAD_RELAY_S3_TIMEOUT_MS", "900000"),
        (
...
fn a_transform_cache_entry_ceiling_above_the_capacity_still_boots() {
    let cfg = Config::load_from_iter([
        ("AETHERNET_MEDIA_PROXY_SECRET_KEY", "secret"),
        ("AETHERNET_MEDIA_PROXY_TRANSFORM_CACHE_BYTES", "1048576"),
        (
            "AETHERNET_MEDIA_PROXY_TRANSFORM_CACHE_MAX_ENTRY_BYTES",
            "2097152",
        ),
...
fn both_transform_cache_env_keys_accept_their_whole_range() {
    let cfg = Config::load_from_iter([
        ("AETHERNET_MEDIA_PROXY_SECRET_KEY", "secret"),
        ("AETHERNET_MEDIA_PROXY_TRANSFORM_CACHE_BYTES", "4294967296"),
        (
            "AETHERNET_MEDIA_PROXY_TRANSFORM_CACHE_MAX_ENTRY_BYTES",
            "536870912",
        ),
    ...
    let cfg = Config::load_from_iter([
        ("AETHERNET_MEDIA_PROXY_SECRET_KEY", "secret"),
        ("AETHERNET_MEDIA_PROXY_TRANSFORM_CACHE_BYTES", "0"),
        ("AETHERNET_MEDIA_PROXY_TRANSFORM_CACHE_MAX_ENTRY_BYTES", "0"),
    ])
    .unwrap();
...
fn every_deployment_mode_and_storage_backend_variant_parses() {
    ...
    for (raw, expected) in [
        ...
        let cfg = Config::load_from_iter([
            ("AETHERNET_MEDIA_PROXY_SECRET_KEY", "secret"),
            ("AETHERNET_MEDIA_PROXY_MODE", raw),
            (
                "AETHERNET_MEDIA_PROXY_UPLOAD_RELAY_SECRET_BASE64",
                relay_secret.as_str(),
            ),
    ...
    for (raw, expected) in [
        ...
        let cfg = Config::load_from_iter([
            ("AETHERNET_MEDIA_PROXY_SECRET_KEY", "secret"),
            ("AETHERNET_MEDIA_PROXY_STORAGE_BACKEND", raw),
        ])
        .unwrap();
...
fn upload_relay_spool_and_bunny_ip_gate_keys_apply() {
    let cfg = Config::load_from_iter([
        ...
        ("AETHERNET_MEDIA_PROXY_BUNNY_IP_GATE_ENABLED", "yes"),
        (
            "AETHERNET_MEDIA_PROXY_BUNNY_IP_GATE_TRUSTED_PROXIES",
            "10.0.0.1, 2001:db8::1 ,",
        ),
        ("AETHERNET_MEDIA_PROXY_BUNNY_IP_GATE_REFRESH_SECS", "900"),
    ])
    .unwrap();
...
fn rejects_invalid_bunny_ip_gate_trusted_proxies() {
    ...
    assert!(
        err.to_string()
            .contains("AETHERNET_MEDIA_PROXY_BUNNY_IP_GATE_TRUSTED_PROXIES")
    );
}
...
fn debug_output_never_reveals_a_secret() {
    ...
    let cfg = Config::load_from_iter([
        ("AETHERNET_MEDIA_PROXY_SECRET_KEY", "signing-secret-material"),
        ("AETHERNET_MEDIA_PROXY_MODE", "upload"),
        (
            "AETHERNET_MEDIA_PROXY_UPLOAD_RELAY_SECRET_BASE64",
            relay_secret.as_str(),
