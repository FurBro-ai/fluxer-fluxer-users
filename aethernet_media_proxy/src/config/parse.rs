...
pub(super) fn parse_mode_env(raw: Option<&str>) -> anyhow::Result<Option<DeploymentMode>> {
    ...
    parse_mode(raw).map(Some).ok_or_else(|| {
        anyhow::anyhow!("AETHERNET_MEDIA_PROXY_MODE must be one of: mp, static, upload")
    })
}
...
pub(super) fn parse_bucket_style(raw: Option<&str>) -> anyhow::Result<Option<BucketStyle>> {
    ...
    match raw.to_ascii_lowercase().as_str() {
        ...
        _ => Err(anyhow::anyhow!(
            "AETHERNET_S3_READ_BUCKET_STYLE must be one of: path, virtual, root"
        )),
    }
}
...
pub(super) fn validate_read_endpoint(endpoint: &str) -> anyhow::Result<()> {
    let parsed = url::Url::parse(endpoint)
        .map_err(|err| anyhow::anyhow!("AETHERNET_S3_READ_ENDPOINT is not a valid URL: {err}"))?;
    anyhow::ensure!(
        matches!(parsed.scheme(), "http" | "https"),
        "AETHERNET_S3_READ_ENDPOINT must be an http or https URL"
    );
    anyhow::ensure!(
        parsed.host_str().is_some_and(|host| !host.is_empty()),
        "AETHERNET_S3_READ_ENDPOINT must include a host"
    );
    anyhow::ensure!(
        parsed.username().is_empty() && parsed.password().is_none(),
        "AETHERNET_S3_READ_ENDPOINT must not contain credentials"
    );
    anyhow::ensure!(
        parsed.query().is_none() && parsed.fragment().is_none(),
        "AETHERNET_S3_READ_ENDPOINT must not contain a query string or fragment"
    );
    Ok(())
}
...
pub(super) fn parse_storage_backend(raw: Option<&str>) -> anyhow::Result<Option<StorageBackend>> {
    ...
    match raw.to_ascii_lowercase().as_str() {
        ...
        _ => Err(anyhow::anyhow!(
            "AETHERNET_MEDIA_PROXY_STORAGE_BACKEND must be one of: local, s3"
        )),
    }
}
...
pub(super) fn decode_upload_relay_secret(
    ...
    let decoded = general_purpose::STANDARD.decode(raw).map_err(|_| {
        anyhow::anyhow!("AETHERNET_MEDIA_PROXY_UPLOAD_RELAY_SECRET_BASE64 must be base64")
    })?;
    anyhow::ensure!(
        decoded.len() >= 32,
