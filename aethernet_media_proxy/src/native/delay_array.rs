...
use super::{NativeStatus, VipsImageHandle, aethernet_free_int_array, aethernet_vips_read_delays_ms};
use libc::c_int;
...
impl VipsDelayArray {
    pub fn read(image: &VipsImageHandle<'_>, n_pages: c_int) -> Result<Self, NativeStatus> {
        ...
        let status = NativeStatus::from_code(unsafe {
            aethernet_vips_read_delays_ms(image.as_ptr(), n_pages, &mut out_ptr, &mut out_len)
        });
        ...
        let len = match usize::try_from(out_len) {
            ...
            _ => {
                unsafe { aethernet_free_int_array(ptr.as_ptr()) };
                return Err(NativeStatus::CodecFailure);
            }
        };
...
impl Drop for VipsDelayArray {
    fn drop(&mut self) {
        unsafe { aethernet_free_int_array(self.ptr.as_ptr()) };
    }
}
