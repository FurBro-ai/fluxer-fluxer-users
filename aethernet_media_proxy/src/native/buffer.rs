...
use super::{aethernet_av_free, aethernet_vips_free, aethernet_webp_free};
...
impl NativeAllocator {
    fn free(self, ptr: NonNull<c_void>) {
        match self {
            Self::AV => unsafe { aethernet_av_free(ptr.as_ptr()) },
            Self::Vips => unsafe { aethernet_vips_free(ptr.as_ptr()) },
            Self::WebP => unsafe { aethernet_webp_free(ptr.as_ptr()) },
        }
    }
}
