...
use super::{AethernetNSFWFrameOut, aethernet_nsfw_frames_free};
...
pub struct NSFWFrameOutput {
    slots: [AethernetNSFWFrameOut; MAX_NSFW_FRAME_OUTPUTS],
    expected: usize,
}
...
impl NSFWFrameOutput {
    pub fn new(expected: usize) -> Self {
        ...
        Self {
            slots: std::array::from_fn(|_| AethernetNSFWFrameOut::empty()),
            expected,
        }
    }

    pub fn as_mut_ptr(&mut self) -> *mut AethernetNSFWFrameOut {
...
impl Drop for NSFWFrameOutput {
    fn drop(&mut self) {
        unsafe { aethernet_nsfw_frames_free(self.slots.as_mut_ptr(), self.expected) };
    }
}
