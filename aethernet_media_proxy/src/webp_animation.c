...
#define AETHERNET_WEBP_MAX_FRAME_DURATION_MS ((1 << 24) - 1)
#define AETHERNET_WEBP_MAX_LOOP_COUNT (1 << 16)
#define AETHERNET_WEBP_ANIMATION_HEADER_BOUND ((size_t)44)
#define AETHERNET_WEBP_ANIMATION_FRAME_OVERHEAD_BOUND ((size_t)24)
#define AETHERNET_WEBP_WRITER_INITIAL_CAPACITY ((size_t)4096)
#define AETHERNET_WEBP_WRITER_MAX_GROWTH_STEPS (sizeof(size_t) * CHAR_BIT)
#define AETHERNET_WEBP_KEYFRAME_INTERVAL ((size_t)4)
#define AETHERNET_WEBP_FRAME_SCAN_DEADLINE_ROWS 64
#define AETHERNET_WEBP_SCALE_DEADLINE_ROWS 64

enum aethernet_webp_writer_failure {
    AETHERNET_WEBP_WRITER_OK = 0,
    AETHERNET_WEBP_WRITER_LIMIT = 1,
    AETHERNET_WEBP_WRITER_ALLOC = 2,
    AETHERNET_WEBP_WRITER_INTERNAL = 3
};

enum aethernet_webp_animation_encoder_state {
    AETHERNET_WEBP_ANIMATION_ENCODER_OPEN = 0,
    AETHERNET_WEBP_ANIMATION_ENCODER_FINISHED = 1
};

struct aethernet_webp_bounded_writer {
    uint8_t *data;
    size_t len;
    size_t capacity;
    size_t max_size;
    size_t allocation_bound;
    enum aethernet_webp_writer_failure failure;
};

struct aethernet_webp_frame_plan {
    const uint32_t *argb;
    int argb_stride;
    int x_offset;
    int y_offset;
...
struct aethernet_webp_animation_encoder {
    ...
    enum aethernet_webp_animation_encoder_state state;
    struct aethernet_webp_bounded_writer frame_writer;
};

static int aethernet_webp_animation_settings_status(
    const struct aethernet_webp_animation_encoder_settings *settings
) {
    if (settings == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (settings->config == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (!WebPValidateConfig(settings->config)) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
    if (settings->canvas_width <= 0) return AETHERNET_NATIVE_STATUS_INVALID_DIMENSIONS;
    if (settings->canvas_width > WEBP_MAX_DIMENSION) {
        return AETHERNET_NATIVE_STATUS_INVALID_DIMENSIONS;
    }
    if (settings->canvas_height <= 0) return AETHERNET_NATIVE_STATUS_INVALID_DIMENSIONS;
    if (settings->canvas_height > WEBP_MAX_DIMENSION) {
        return AETHERNET_NATIVE_STATUS_INVALID_DIMENSIONS;
    }
    if (settings->loop_count < 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (settings->loop_count >= AETHERNET_WEBP_MAX_LOOP_COUNT) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
    if (settings->full_canvas_frames < 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (settings->full_canvas_frames > 1) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (settings->deadline_monotonic_ms < 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (settings->pixel_layout != AETHERNET_WEBP_PIXEL_LAYOUT_RGBA) {
        if (settings->pixel_layout != AETHERNET_WEBP_PIXEL_LAYOUT_BGRA) {
            return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
        }
    }
    if (settings->max_output_size < AETHERNET_WEBP_ANIMATION_HEADER_BOUND) {
        return AETHERNET_NATIVE_STATUS_OUTPUT_LIMIT_EXCEEDED;
    }
    return AETHERNET_NATIVE_STATUS_OK;
}

static int aethernet_webp_canvas_allocation_size(
...
static void aethernet_webp_bounded_writer_init(
    struct aethernet_webp_bounded_writer *writer, size_t allocation_bound
) {
    assert(writer != NULL);
...
static void aethernet_webp_bounded_writer_reset(
    struct aethernet_webp_bounded_writer *writer, size_t max_size
) {
    assert(writer != NULL);
    assert(max_size > 0);
    ...
    writer->failure = AETHERNET_WEBP_WRITER_OK;
}

static void aethernet_webp_bounded_writer_clear(
    struct aethernet_webp_bounded_writer *writer
) {
...
static int aethernet_webp_bounded_writer_next_capacity(
    const struct aethernet_webp_bounded_writer *writer,
    size_t required,
    ...
    if (capacity == 0) {
        capacity = AETHERNET_WEBP_WRITER_INITIAL_CAPACITY;
        if (capacity > writer->max_size) capacity = writer->max_size;
    }
    for (size_t step = 0; capacity < required; step++) {
        if (step >= AETHERNET_WEBP_WRITER_MAX_GROWTH_STEPS) return -1;
...
static int aethernet_webp_bounded_writer_reserve(
    struct aethernet_webp_bounded_writer *writer, size_t required
) {
    assert(writer != NULL);
    assert(required > writer->capacity);
    size_t next_capacity = 0;
    if (aethernet_webp_bounded_writer_next_capacity(
            writer, required, &next_capacity) != 0) {
        writer->failure = AETHERNET_WEBP_WRITER_INTERNAL;
        return -1;
    }
    uint8_t *next = (uint8_t *)WebPMalloc(next_capacity);
    if (next == NULL) {
        writer->failure = AETHERNET_WEBP_WRITER_ALLOC;
        return -1;
    }
...
static int aethernet_webp_bounded_write(
    const uint8_t *data, size_t data_size, const WebPPicture *picture
) {
    if (picture == NULL) return 0;
    if (picture->custom_ptr == NULL) return 0;
    struct aethernet_webp_bounded_writer *writer =
        (struct aethernet_webp_bounded_writer *)picture->custom_ptr;
    if (writer->failure != AETHERNET_WEBP_WRITER_OK) return 0;
    ...
    if (data_size > writer->max_size - writer->len) {
        writer->failure = AETHERNET_WEBP_WRITER_LIMIT;
        return 0;
    }
    size_t required = writer->len + data_size;
    if (required > writer->capacity) {
        if (aethernet_webp_bounded_writer_reserve(writer, required) != 0) return 0;
    }
...
static int aethernet_webp_frame_output_budget(
    const struct aethernet_webp_animation_encoder *encoder,
    size_t *frame_budget
) {
    ...
    if (remaining <= AETHERNET_WEBP_ANIMATION_FRAME_OVERHEAD_BOUND) {
        return AETHERNET_NATIVE_STATUS_OUTPUT_LIMIT_EXCEEDED;
    }
    *frame_budget = remaining - AETHERNET_WEBP_ANIMATION_FRAME_OVERHEAD_BOUND;
    return 0;
}

static int aethernet_webp_next_container_bound(
    const struct aethernet_webp_animation_encoder *encoder,
    size_t frame_size,
    size_t *next_bound
) {
    ...
    size_t remaining = encoder->max_output_size - encoder->container_size_bound;
    if (AETHERNET_WEBP_ANIMATION_FRAME_OVERHEAD_BOUND > remaining) {
        return AETHERNET_NATIVE_STATUS_OUTPUT_LIMIT_EXCEEDED;
    }
    remaining -= AETHERNET_WEBP_ANIMATION_FRAME_OVERHEAD_BOUND;
    if (frame_size > remaining) return AETHERNET_NATIVE_STATUS_OUTPUT_LIMIT_EXCEEDED;
    *next_bound = encoder->container_size_bound;
    *next_bound += AETHERNET_WEBP_ANIMATION_FRAME_OVERHEAD_BOUND;
    *next_bound += frame_size;
    return 0;
}

static int aethernet_webp_deadline_status_at(long long deadline_monotonic_ms) {
    int status = aethernet_monotonic_deadline_status(deadline_monotonic_ms);
    return status == AETHERNET_DEADLINE_PENDING
        ? AETHERNET_NATIVE_STATUS_OK
        : AETHERNET_NATIVE_STATUS_DEADLINE_EXCEEDED;
}

static int aethernet_webp_deadline_status(
    const struct aethernet_webp_animation_encoder *encoder
) {
    assert(encoder != NULL);
    return aethernet_webp_deadline_status_at(encoder->deadline_monotonic_ms);
}

static int aethernet_webp_encode_progress(
    int percent,
    ...
    struct aethernet_webp_animation_encoder *encoder =
        (struct aethernet_webp_animation_encoder *)picture->user_data;
    if (aethernet_webp_deadline_status(encoder) == AETHERNET_NATIVE_STATUS_OK) {
        return 1;
    }
...
static uint32_t aethernet_webp_argb_pixel(
    const uint8_t *pixel, enum aethernet_webp_pixel_layout layout
) {
    assert(pixel != NULL);
    ...
    if (layout == AETHERNET_WEBP_PIXEL_LAYOUT_BGRA) {
        red = pixel[2];
        blue = pixel[0];
    }
    if (alpha == 0) return 0;
...
static int aethernet_webp_frame_is_keyframe(
    const struct aethernet_webp_animation_encoder *encoder
) {
    assert(encoder != NULL);
    if (encoder->frame_count == 0) return 1;
    if (encoder->full_canvas_frames != 0) return 1;
    return encoder->frame_count % AETHERNET_WEBP_KEYFRAME_INTERVAL == 0;
}

static int aethernet_webp_prepare_frame_plan(
    struct aethernet_webp_animation_encoder *encoder,
    const uint8_t *pixels,
    size_t stride,
    struct aethernet_webp_frame_plan *plan
) {
    assert(encoder != NULL);
    assert(pixels != NULL);
    assert(plan != NULL);
    memset(plan, 0, sizeof(*plan));
    int keyframe = aethernet_webp_frame_is_keyframe(encoder);
    int left = encoder->canvas_width;
    int top = encoder->canvas_height;
    int right = -1;
    int bottom = -1;
    for (int y = 0; y < encoder->canvas_height; y++) {
        if (y % AETHERNET_WEBP_FRAME_SCAN_DEADLINE_ROWS == 0 &&
            aethernet_webp_deadline_status(encoder) !=
                AETHERNET_NATIVE_STATUS_OK) {
            return AETHERNET_NATIVE_STATUS_DEADLINE_EXCEEDED;
        }
        ...
        for (int x = 0; x < encoder->canvas_width; x++) {
            uint32_t pixel = aethernet_webp_argb_pixel(
    ...
    if (keyframe) {
        ...
        return aethernet_webp_deadline_status(encoder);
    }
    if (right >= 0) {
        left &= ~1;
        top &= ~1;
...
static int aethernet_webp_encode_frame_candidate(
    struct aethernet_webp_animation_encoder *encoder,
    const struct aethernet_webp_frame_plan *plan,
    size_t frame_budget
) {
    assert(encoder != NULL);
    assert(plan != NULL);
    struct aethernet_webp_bounded_writer *writer = &encoder->frame_writer;
    WebPPicture picture;
    if (!WebPPictureInit(&picture)) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    aethernet_webp_bounded_writer_reset(writer, frame_budget);
    ...
    picture.writer = aethernet_webp_bounded_write;
    picture.custom_ptr = writer;
    picture.progress_hook = aethernet_webp_encode_progress;
    ...
    if (!encoded) {
        int status = AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
        if (writer->failure == AETHERNET_WEBP_WRITER_LIMIT) {
            status = AETHERNET_NATIVE_STATUS_OUTPUT_LIMIT_EXCEEDED;
        } else if (writer->failure == AETHERNET_WEBP_WRITER_ALLOC) {
            status = AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
        } else if (encode_error == VP8_ENC_ERROR_OUT_OF_MEMORY) {
            status = AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
        } else if (encode_error == VP8_ENC_ERROR_USER_ABORT &&
                   encoder->encode_deadline_exceeded != 0) {
            status = AETHERNET_NATIVE_STATUS_DEADLINE_EXCEEDED;
        }
        return status;
    }
    if (writer->failure != AETHERNET_WEBP_WRITER_OK) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
    if (writer->data == NULL) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
...
static int aethernet_webp_push_frame_candidate(
    struct aethernet_webp_animation_encoder *encoder,
    const struct aethernet_webp_frame_plan *plan,
    int duration_ms,
    const struct aethernet_webp_bounded_writer *writer
    ...
    assert(writer != NULL);
    size_t next_bound = 0;
    int bound_status = aethernet_webp_next_container_bound(
    ...
    if (push_status == WEBP_MUX_MEMORY_ERROR) {
        return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
    }
    if (push_status != WEBP_MUX_OK) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    encoder->container_size_bound = next_bound;
    return AETHERNET_NATIVE_STATUS_OK;
}

static int aethernet_webp_animation_mux_create(
    struct aethernet_webp_animation_encoder *encoder, int loop_count
) {
    assert(encoder != NULL);
    encoder->mux = WebPMuxNew();
    if (encoder->mux == NULL) return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
    ...
    if (canvas_status != WEBP_MUX_OK) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
...
int aethernet_webp_animation_encoder_create(
    ...
    if (out_encoder == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    *out_encoder = NULL;
    int settings_status = aethernet_webp_animation_settings_status(settings);
    if (settings_status != AETHERNET_NATIVE_STATUS_OK) return settings_status;
    if (aethernet_monotonic_deadline_status(settings->deadline_monotonic_ms) !=
        AETHERNET_DEADLINE_PENDING) {
        return AETHERNET_NATIVE_STATUS_DEADLINE_EXCEEDED;
    }
    size_t canvas_bytes = 0;
    if (aethernet_webp_canvas_allocation_size(
            settings->canvas_width, settings->canvas_height,
            &canvas_bytes) != 0) return AETHERNET_NATIVE_STATUS_INVALID_DIMENSIONS;
    struct aethernet_webp_animation_encoder *encoder =
        (struct aethernet_webp_animation_encoder *)calloc(1, sizeof(*encoder));
    if (encoder == NULL) return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
    ...
    encoder->container_size_bound = AETHERNET_WEBP_ANIMATION_HEADER_BOUND;
    aethernet_webp_bounded_writer_init(
    ...
    if (encoder->current_canvas == NULL ||
        ...
        aethernet_webp_animation_encoder_delete(encoder);
        return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
    }
    int mux_status = aethernet_webp_animation_mux_create(
        encoder, settings->loop_count);
    if (mux_status != AETHERNET_NATIVE_STATUS_OK) {
        aethernet_webp_animation_encoder_delete(encoder);
        return mux_status;
    }
    if (aethernet_webp_deadline_status(encoder) != AETHERNET_NATIVE_STATUS_OK) {
...
static int aethernet_webp_input_stride_valid(
    const struct aethernet_webp_animation_encoder *encoder, size_t stride
) {
    assert(encoder != NULL);
...
int aethernet_webp_animation_encoder_add(
    ...
    if (encoder == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (pixels == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (encoder->state != AETHERNET_WEBP_ANIMATION_ENCODER_OPEN) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
    if (!aethernet_webp_input_stride_valid(encoder, stride)) {
        return AETHERNET_NATIVE_STATUS_INVALID_DIMENSIONS;
    }
    if (duration_ms <= 0) return AETHERNET_NATIVE_STATUS_WORK_LIMIT_EXCEEDED;
    if (duration_ms > AETHERNET_WEBP_MAX_FRAME_DURATION_MS) {
    ...
    struct aethernet_webp_frame_plan plan;
    int plan_status = aethernet_webp_prepare_frame_plan(
        encoder, pixels, stride, &plan);
    if (plan_status != AETHERNET_NATIVE_STATUS_OK) return plan_status;
    size_t frame_budget = 0;
    int budget_status = aethernet_webp_frame_output_budget(encoder, &frame_budget);
    if (budget_status != 0) return budget_status;
    int encode_status = aethernet_webp_encode_frame_candidate(
        encoder, &plan, frame_budget);
    if (encode_status != 0) return encode_status;
    if (aethernet_webp_deadline_status(encoder) != AETHERNET_NATIVE_STATUS_OK) {
        return AETHERNET_NATIVE_STATUS_DEADLINE_EXCEEDED;
    }
    int push_status = aethernet_webp_push_frame_candidate(
...
int aethernet_webp_animation_encoder_finish(
    ...
    if (encoder->frame_count == 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (encoder->container_size_bound > encoder->max_output_size) {
        return AETHERNET_NATIVE_STATUS_OUTPUT_LIMIT_EXCEEDED;
    }
    if (aethernet_webp_deadline_status(encoder) != AETHERNET_NATIVE_STATUS_OK) {
    ...
    *out_size = output.size;
    encoder->state = AETHERNET_WEBP_ANIMATION_ENCODER_FINISHED;
    return AETHERNET_NATIVE_STATUS_OK;
}
...
void aethernet_webp_animation_encoder_delete(
    ...
    aethernet_webp_bounded_writer_clear(&encoder->frame_writer);
...
struct aethernet_webp_direct_request {
    const void *data;
    size_t len;
    int max_width;
    int max_height;
...
struct aethernet_webp_direct_state {
...
static int webp_direct_request_valid(const struct aethernet_webp_direct_request *request) {
...
static int fit_webp_direct_dimensions(
    ...
    const struct aethernet_webp_direct_request *request,
    int *target_width,
    int *target_height
) {
...
static int scale_webp_bgra_frame(
    const struct aethernet_webp_direct_state *state,
    uint8_t *source_bgra,
    uint8_t **out_bgra
) {
    assert(state != NULL);
    ...
    if (state->source_width == state->target_width) {
        if (state->source_height == state->target_height) {
            *out_bgra = source_bgra;
            return aethernet_webp_deadline_status_at(state->deadline_monotonic_ms);
        }
    }
    if (state->scaled_bgra == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    for (int clip_y = 0; clip_y < state->target_height;
         clip_y += AETHERNET_WEBP_SCALE_DEADLINE_ROWS) {
        int deadline_status = aethernet_webp_deadline_status_at(
            state->deadline_monotonic_ms);
        if (deadline_status != AETHERNET_NATIVE_STATUS_OK) return deadline_status;
        int clip_height = state->target_height - clip_y;
        if (clip_height > AETHERNET_WEBP_SCALE_DEADLINE_ROWS) {
            clip_height = AETHERNET_WEBP_SCALE_DEADLINE_ROWS;
        }
        ...
        if (rc > 0) return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
        if (rc < 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
    int deadline_status = aethernet_webp_deadline_status_at(
        state->deadline_monotonic_ms);
    if (deadline_status != AETHERNET_NATIVE_STATUS_OK) return deadline_status;
...
static int transform_webp_animation_frame(
    struct aethernet_webp_direct_state *state,
    ...
    if (source_bgra == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (next_source_timestamp <= *source_timestamp) {
        return AETHERNET_NATIVE_STATUS_WORK_LIMIT_EXCEEDED;
    }
    int delay = next_source_timestamp - *source_timestamp;
    if (delay > AETHERNET_WEBP_MAX_FRAME_DURATION_MS) {
        return AETHERNET_NATIVE_STATUS_WORK_LIMIT_EXCEEDED;
    }
    ...
    if (scale_status != AETHERNET_NATIVE_STATUS_OK) return scale_status;
    assert(output_bgra != NULL);
    int add_status = aethernet_webp_animation_encoder_add(
        state->encoder,
        output_bgra,
...
static int transform_webp_animation_frames(struct aethernet_webp_direct_state *state) {
    ...
    while (WebPAnimDecoderHasMoreFrames(state->decoder)) {
        ...
        if (aethernet_monotonic_deadline_status(state->deadline_monotonic_ms) != AETHERNET_DEADLINE_PENDING) {
            return AETHERNET_NATIVE_STATUS_DEADLINE_EXCEEDED;
        }
    ...
    if (frames_added == 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (frames_added > state->source_frame_count) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
...
static WebPAnimDecoder *new_webp_direct_decoder(
    ...
    if (aethernet_monotonic_deadline_status(
    ...
    if (info->loop_count >= (uint32_t)AETHERNET_WEBP_MAX_LOOP_COUNT) {
        WebPAnimDecoderDelete(decoder);
        return NULL;
    }
    if (!webp_direct_metadata_supported(decoder)) {
        WebPAnimDecoderDelete(decoder);
        *status = AETHERNET_NATIVE_STATUS_UNSUPPORTED;
        return NULL;
    }
    return decoder;
}
...
static int new_webp_direct_encoder(
    ...
    WebPConfig config;
    int config_rc = aethernet_configure_webp_encoder(
        &config,
        request->quality,
        request->lossless,
        request->effort,
    ...
    if (config_rc != AETHERNET_NATIVE_STATUS_OK) return config_rc;
    config.thread_level = request->thread_level;
    if (!WebPValidateConfig(&config)) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    struct aethernet_webp_animation_encoder_settings settings = {
        .config = &config,
        .canvas_width = target_width,
        .canvas_height = target_height,
        .loop_count = loop_count,
        .full_canvas_frames = 0,
        .pixel_layout = AETHERNET_WEBP_PIXEL_LAYOUT_BGRA,
        .deadline_monotonic_ms = request->limits->deadline_monotonic_ms,
        .max_output_size = request->max_output_size,
    };
    return aethernet_webp_animation_encoder_create(&settings, out_encoder);
}

static int run_webp_direct_transform(struct aethernet_webp_direct_request *request) {
    if (request == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    ...
    if (!webp_direct_request_valid(request)) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    WebPAnimInfo info;
    int status = AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    ...
    struct aethernet_webp_animation_encoder *encoder = NULL;
    ...
    WebPAnimDecoderDelete(decoder);
    if (rc == 0) {
        rc = aethernet_webp_animation_encoder_finish(
...
int aethernet_webp_transform_animated(
    ...
    struct aethernet_webp_direct_request request = {
        .data = webp_data,
        .len = webp_len,
        .max_width = max_width,
