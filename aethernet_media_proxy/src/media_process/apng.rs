...
fn encode_png_strip(
    ...
    let rc = unsafe {
        native::aethernet_vips_extract_area(image.as_ptr(), &mut strip_raw, 0, top, width, page_height)
    };
    ...
    let rc = unsafe { native::aethernet_vips_image_to_rgba(strip.as_ptr(), &mut rgba_raw) };
