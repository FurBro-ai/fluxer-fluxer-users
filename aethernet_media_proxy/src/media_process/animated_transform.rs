...
pub(super) fn try_decode_heif_primary_still(
    ...
    let mut facts = native::AethernetHEIFPrimaryStillDecodeFacts::empty();
    let rc = unsafe {
...
pub(super) fn decode_heif_sequence(
    ...
    let rc = unsafe {
        native::aethernet_ffmpeg_decode_heif_sequence(
            input.as_ptr().cast(),
            input.len(),
            runtime.config().ffmpeg_decoder_threads(),
            0,
...
pub(super) fn resize_animated_gif_with_ffmpeg(
    ...
    let rc = unsafe {
        native::aethernet_ffmpeg_resize_gif(
            ...
            // animation longer than AETHERNET_MEDIA_PROXY_MAX_ENCODE_DURATION_MS.
            native_animated_frame_limit(media_limits),
            c_int::MAX,
            media_limits.animated_total_pixels(),
