...
fn probe_loaded_image_dims(
    ...
    let mut width = unsafe { native::aethernet_vips_image_get_width(image.as_ptr()) as u32 };
    let mut height = unsafe { native::aethernet_vips_image_get_height(image.as_ptr()) as u32 };
    if unsafe { native::aethernet_vips_image_get_orientation_swap(image.as_ptr()) } != 0 {
...
pub(crate) fn load_image<'source>(
    ...
    let raw = unsafe {
        native::aethernet_vips_image_new_from_buffer(
            input.as_ptr().cast(),
            input.len(),
            options.as_ptr(),
        )
