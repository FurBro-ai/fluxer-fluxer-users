...
pub(super) fn decode_rgba(bytes: &[u8]) -> (u32, u32, Vec<u8>) {
    ...
    let width = unsafe { native::aethernet_vips_image_get_width(image.as_ptr()) as u32 };
    let height = unsafe { native::aethernet_vips_image_get_height(image.as_ptr()) as u32 };
    let mut rgba_ptr: *mut c_void = ptr::null_mut();
    let mut rgba_size: size_t = 0;
    let status = NativeStatus::from_code(unsafe {
        native::aethernet_vips_extract_rgba(image.as_ptr(), 0, &mut rgba_ptr, &mut rgba_size)
    });
