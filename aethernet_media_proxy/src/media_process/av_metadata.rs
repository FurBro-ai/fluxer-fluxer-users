...
impl AVMetadataFrame {
    ...
    fn borrow_as_vips_image(
        ...
        let raw = unsafe {
            native::aethernet_vips_image_new_from_memory(
                buffer.as_ptr().cast(),
                buffer.len(),
                width,
                height,
                4,
                native::aethernet_vips_format_uchar,
            )
        };
...
pub fn probe_av_metadata(
    ...
    let mut output = native::AethernetAVMetadataOut::empty();
    let rc = unsafe {
        native::aethernet_av_metadata(
            input.as_ptr().cast(),
            input.len(),
...
fn av_metadata_frame(
    output: &native::AethernetAVMetadataOut,
