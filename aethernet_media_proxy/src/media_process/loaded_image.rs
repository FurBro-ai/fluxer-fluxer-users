...
pub(crate) fn page_height(
    ...
    let has_page_height =
        unsafe { native::aethernet_vips_image_has_field(image.as_ptr(), field.as_ptr()) } != 0;
    if !has_page_height {
        return Ok(None);
    }
    let mut page_height = 0;
    let rc = unsafe {
        native::aethernet_vips_image_get_int(image.as_ptr(), field.as_ptr(), &mut page_height)
    };
    if rc != 0 {
        clear_vips_error();
    ...
    let total_height = unsafe { native::aethernet_vips_image_get_height(image.as_ptr()) };
...
pub(crate) fn validate_vips_image(
    ...
    let width = unsafe { native::aethernet_vips_image_get_width(image.as_ptr()) };
    let height = unsafe { native::aethernet_vips_image_get_height(image.as_ptr()) };
    ...
    let bands = unsafe { native::aethernet_vips_image_get_bands(image.as_ptr()) };
    if bands <= 0 || bands > VIPS_IMAGE_BANDS_MAX {
        return Err(MediaError::InvalidImageDimensions);
    }
    let format = unsafe { native::aethernet_vips_image_get_format(image.as_ptr()) };
    let supported_format = unsafe {
        format == native::aethernet_vips_format_uchar
            || format == native::aethernet_vips_format_ushort
            || format == native::aethernet_vips_format_float
    };
    if !supported_format {
...
pub(crate) fn normalize_vips_image_to_uchar<'source>(
    ...
    let format = unsafe { native::aethernet_vips_image_get_format(image.as_ptr()) };
    if format == unsafe { native::aethernet_vips_format_uchar } {
        return Ok(image);
    }
    let mut normalized_raw = ptr::null_mut();
    let rc = unsafe { native::aethernet_vips_image_to_rgba(image.as_ptr(), &mut normalized_raw) };
    ...
    let normalized_format = unsafe { native::aethernet_vips_image_get_format(normalized.as_ptr()) };
    if normalized_format != unsafe { native::aethernet_vips_format_uchar } {
...
pub(crate) fn resize_loaded_image<'source>(
    ...
    let source_width = unsafe { native::aethernet_vips_image_get_width(image.as_ptr()) };
    let source_height = unsafe { native::aethernet_vips_image_get_height(image.as_ptr()) };
    ...
    let scaled_width = unsafe { native::aethernet_vips_image_get_width(current.as_ptr()) };
    let scaled_height = unsafe { native::aethernet_vips_image_get_height(current.as_ptr()) };
    ...
    drop(current);
    if had_page_height {
        unsafe { native::aethernet_vips_set_page_height(cropped.as_ptr(), final_height) };
    }
...
pub(crate) fn resize_loaded_image_by_scale<'source>(
    ...
    if let Some(old_page_height) = old_page_height {
        let old_total_height = unsafe { native::aethernet_vips_image_get_height(image.as_ptr()) };
    ...
    if old_page_height.is_some() {
        let new_total_height = unsafe { native::aethernet_vips_image_get_height(resized.as_ptr()) };
        if new_total_height <= 0 {
            return Err(MediaError::InvalidImageDimensions);
        }
        unsafe { native::aethernet_vips_set_page_height(resized.as_ptr(), new_total_height) };
    }
...
fn resize_loaded_animation_pages_by_scale<'source>(
    ...
    for page_index in 0..page_count {
        ...
        drop(page);
        let page_width = unsafe { native::aethernet_vips_image_get_width(resized_page.as_ptr()) };
        let page_height = unsafe { native::aethernet_vips_image_get_height(resized_page.as_ptr()) };
    ...
    let rc = unsafe {
        native::aethernet_vips_join_animation_pages(
            image.as_ptr(),
    ...
    let joined_width = unsafe { native::aethernet_vips_image_get_width(joined.as_ptr()) };
    let joined_total_height = unsafe { native::aethernet_vips_image_get_height(joined.as_ptr()) };
...
fn extract_vips_image_area<'source>(
    ...
    let rc = unsafe {
        native::aethernet_vips_extract_area(
            image.as_ptr(),
            &mut extracted_raw,
            left,
            top,
...
fn resize_vips_image<'source>(
    ...
    let rc = unsafe { native::aethernet_vips_resize(image.as_ptr(), &mut resized_raw, scale) };
