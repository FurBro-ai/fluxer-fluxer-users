...
pub fn transform_image(
    ...
    if should_use_resize_path(options, animated_probe) {
        ...
        if rc != 0 || raw.is_null() {
            ...
            clear_vips_error();
            tracing::error!(
                target: "aethernet_media_proxy::transform_debug",
    ...
    let status = NativeStatus::from_code(unsafe {
        native::aethernet_vips_autorot(loaded.as_ptr(), 0, &mut oriented_raw)
    });
...
fn fit_inside_scale(image: &VipsImageHandle<'_>, width: u32, height: u32) -> f64 {
    let image_width = unsafe { native::aethernet_vips_image_get_width(image.as_ptr()) };
    let image_height = unsafe { native::aethernet_vips_image_get_height(image.as_ptr()) };
