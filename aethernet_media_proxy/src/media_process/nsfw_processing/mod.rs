...
fn extract_native_animated_frames_for_nsfw(
    ...
    let status_code = unsafe {
        match media_type {
            MediaType::APNG => native::aethernet_ffmpeg_extract_apng_frames_for_nsfw(
                input.as_ptr().cast(),
                input.len(),
            ...
            MediaType::GIF => native::aethernet_ffmpeg_extract_gif_frames_for_nsfw(
                input.as_ptr().cast(),
                input.len(),
                runtime.config().ffmpeg_decoder_threads(),
                deadline,
            ...
            MediaType::WebP => native::aethernet_webp_extract_frames_for_nsfw(
                input.as_ptr().cast(),
                input.len(),
...
fn extract_vips_animation_frames_for_nsfw(
    ...
    let total_height = unsafe { native::aethernet_vips_image_get_height(loaded.as_ptr()) };
    let width = unsafe { native::aethernet_vips_image_get_width(loaded.as_ptr()) };
    ...
    for idx in indices {
        ...
        let rc = unsafe {
            native::aethernet_vips_extract_area(loaded.as_ptr(), &mut sub_raw, 0, top, width, page_h)
        };
