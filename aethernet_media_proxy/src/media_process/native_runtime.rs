...
impl VipsRuntime {
    fn initialize(config: NativeRuntimeConfig) -> Result<Self, MediaError> {
        let argv0 = CString::new("aethernet-media-proxy").expect("static string has no NUL");
        let rc = unsafe { native::aethernet_vips_init(argv0.as_ptr()) };
        ...
        unsafe { native::aethernet_vips_tune_for_server(config.vips_threads_per_pipeline) };
        Ok(Self { config })
    }
...
pub(crate) fn last_vips_error() -> String {
    let buffer = unsafe { native::aethernet_vips_error_buffer() };
    if buffer.is_null() {
        return String::new();
    }
