...
fn apply_gif_animation_loop_count(
    ...
    let status = NativeStatus::from_code(unsafe {
        native::aethernet_vips_set_animation_loop_count(image.as_ptr(), loop_count)
    });
...
pub(super) fn resolve_animation_loop_count(
    ...
    let rc = unsafe {
        native::aethernet_vips_image_get_int(image.as_ptr(), field.as_ptr(), &mut loop_count)
    };
    if rc != 0 {
        clear_vips_error();
        return 0;
...
fn encode_animated_webp(request: AnimatedWebPEncodeRequest<'_, '_>) -> Result<Vec<u8>, MediaError> {
    ...
    let rc = unsafe {
        native::aethernet_webp_encode_animated(
            image.as_ptr(),
            q as c_int,
