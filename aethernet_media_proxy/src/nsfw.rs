...
mod tests {
    ...
    async fn a_four_frame_request_is_rejected_before_any_http_call() {
        ...
        assert!(rendered.contains("aethernet_media_proxy_nsfw_calls_failed_total 0"));
        ...
        let rendered = metrics.render();
        assert!(rendered.contains("aethernet_media_proxy_nsfw_calls_failed_total 1"));
        assert!(rendered.contains("aethernet_media_proxy_nsfw_calls_ok_total 0"));
    }
    ...
    async fn a_disabled_client_records_a_disabled_call() {
        ...
        assert!(
            metrics
                .render()
                .contains("aethernet_media_proxy_nsfw_calls_disabled_total 1")
        );
    }
}
