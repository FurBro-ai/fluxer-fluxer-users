...
static int aethernet_emit_rgba_nsfw_frame(
    const uint8_t *rgba,
    int width,
    int height,
    ...
    if (image == NULL) return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
    int rc = ff_fit_frame_image(
        &image, AETHERNET_NSFW_FRAME_MAX_DIMENSION, AETHERNET_NSFW_FRAME_MAX_DIMENSION);
    deadline_status = aethernet_native_deadline_status(deadline_monotonic_ms);
    if (deadline_status != AETHERNET_NATIVE_STATUS_OK) rc = deadline_status;
    void *out_buf = NULL;
    size_t out_size = 0;
    size_t out_capacity = 0;
    if (rc == AETHERNET_NATIVE_STATUS_OK) {
        rc = aethernet_vips_image_write_to_buffer_bounded(
            image, ".jpg[Q=65,strip]", deadline_monotonic_ms,
            max_frame_output_size,
            &out_buf, &out_size, &out_capacity);
    }
    g_object_unref(image);
    if (rc == AETHERNET_NATIVE_STATUS_OK && out_buf != NULL && out_size > 0 &&
...
struct aethernet_webp_nsfw_request {
    const void *data;
    size_t len;
    int thread_level;
    const int *indices;
...
struct aethernet_webp_nsfw_selection {
...
static int aethernet_webp_nsfw_request_valid(
    const struct aethernet_webp_nsfw_request *request
) {
    assert(request != NULL);
    ...
    if (request->count > AETHERNET_MAX_NSFW_SAMPLES) return 0;
...
static int aethernet_webp_nsfw_info_valid(
    const WebPAnimInfo *info,
    const struct aethernet_webp_animation_facts *facts,
    const struct aethernet_webp_nsfw_request *request
) {
    assert(info != NULL);
    assert(facts != NULL);
    ...
    return aethernet_nsfw_animation_selection_valid(
        request->indices, request->count, (int)info->frame_count);
}

static int aethernet_webp_open_nsfw_decoder(
    const struct aethernet_webp_nsfw_request *request,
    ...
    *out_decoder = NULL;
    ...
    status = aethernet_webp_animation_preflight(
    ...
    int info_valid = WebPAnimDecoderGetInfo(decoder, out_info) &&
        aethernet_webp_nsfw_info_valid(out_info, &facts, request);
    status = aethernet_native_deadline_status(request->deadline_monotonic_ms);
    if (status != AETHERNET_NATIVE_STATUS_OK) {
...
static int aethernet_webp_decode_nsfw_selection(
    struct aethernet_webp_nsfw_selection *selection
) {
    ...
    for (int frame = 0; frame < (int)selection->info.frame_count; frame++) {
        ...
        int status = aethernet_emit_rgba_nsfw_frame(
            rgba,
...
int aethernet_webp_extract_frames_for_nsfw(
    ...
    if (n_indices > AETHERNET_MAX_NSFW_SAMPLES) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
    aethernet_nsfw_frames_reset(out_frames, n_indices);
    struct aethernet_webp_nsfw_request request = {
        .data = webp_data,
        .len = webp_len,
    ...
    if (!aethernet_webp_nsfw_request_valid(&request)) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }

    WebPAnimInfo info = {0};
    WebPAnimDecoder *decoder = NULL;
    int status = aethernet_webp_open_nsfw_decoder(
    ...
    status = aethernet_webp_decode_nsfw_selection(&selection);
    WebPAnimDecoderDelete(decoder);
    if (status != AETHERNET_NATIVE_STATUS_OK) {
