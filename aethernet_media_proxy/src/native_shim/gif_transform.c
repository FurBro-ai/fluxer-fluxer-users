...
#define AETHERNET_GIF_AVIO_BUFFER_SIZE (64u * 1024u)
#define AETHERNET_GIF_PROBE_SIZE (5u * 1024u * 1024u)
#define AETHERNET_GIF_ANALYZE_DURATION (5 * AV_TIME_BASE)
...
static int write_encoded_gif_packets(AVFormatContext *out_fmt, AVCodecContext *enc_ctx,
    ...
    if (rc < 0) return aethernet_native_status_from_av_error(rc);
    while (1) {
        ...
        if (rc < 0) {
            av_packet_unref(packet);
            return aethernet_native_status_from_av_error(rc);
        }
...
static int validate_gif_resize_request(
    ...
    *out_expected_frames = 0;
    ...
    if (request->target_width <= 0) return AETHERNET_NATIVE_STATUS_INVALID_DIMENSIONS;
    if (request->target_height <= 0) return AETHERNET_NATIVE_STATUS_INVALID_DIMENSIONS;
    ...
    if (request->max_source_frames <= 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (request->max_encode_frames <= 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (request->max_encode_duration_ms <= 0) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
    if (request->max_total_pixels == 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (request->max_output_size == 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;

    int expected_frames = 0;
    int status = aethernet_validate_complete_gif(
    ...
    status = aethernet_native_deadline_status(request->deadline_monotonic_ms);
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    if ((size_t)expected_frames > SIZE_MAX / sizeof(int)) {
        return AETHERNET_NATIVE_STATUS_INVALID_DIMENSIONS;
    }
    if (request->target_width > AETHERNET_MAX_VIDEO_FRAME_DIMENSION) {
        return AETHERNET_NATIVE_STATUS_WORK_LIMIT_EXCEEDED;
    }
    if (request->target_height > AETHERNET_MAX_VIDEO_FRAME_DIMENSION) {
        return AETHERNET_NATIVE_STATUS_WORK_LIMIT_EXCEEDED;
    }
    ...
    if (expected_frames > INT_MAX / AETHERNET_ANIMATION_PACKETS_PER_FRAME_LIMIT) {
        return AETHERNET_NATIVE_STATUS_WORK_LIMIT_EXCEEDED;
    }
...
static int gif_resize_load_frame_delays(struct gif_resize_context *context) {
    ...
    int status = aethernet_read_gif_frame_delays(
        ...
        &context->loop_count);
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    return aethernet_gif_animation_frame_budget(
        context->frame_delays_cs,
        context->expected_frames,
...
static int gif_resize_open_input(struct gif_resize_context *context) {
    ...
    context->input_avio_buffer = av_malloc(AETHERNET_GIF_AVIO_BUFFER_SIZE);
    if (context->input_avio_buffer == NULL) {
        return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
    }
    context->input_avio = avio_alloc_context(
        context->input_avio_buffer,
        (int)AETHERNET_GIF_AVIO_BUFFER_SIZE,
        0,
        &context->reader,
        ff_mem_read_packet,
        NULL,
    ...
    int status = aethernet_restrict_untrusted_av_context(context->input_format, "gif");
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    ...
    context->input_format->probesize = AETHERNET_GIF_PROBE_SIZE;
    context->input_format->max_analyze_duration = AETHERNET_GIF_ANALYZE_DURATION;
...
static int gif_resize_open_decoder(struct gif_resize_context *context) {
    ...
    if (stream == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (stream->codecpar == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (decoder == NULL) {
        decoder = avcodec_find_decoder(stream->codecpar->codec_id);
    }
    if (decoder == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (!aethernet_video_decoder_allowed(stream, decoder)) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
...
static int gif_resize_open_output_io(struct gif_resize_context *context) {
    ...
    size_t buffer_size = AETHERNET_GIF_AVIO_BUFFER_SIZE;
...
static int gif_resize_open_pipeline(struct gif_resize_context *context) {
    ...
    assert(context->decoder != NULL);
    assert(context->encoder != NULL);
    int status = aethernet_gif_setup_filter_graph(
...
static int gif_resize_drain_filter(
    ...
    while (1) {
        ...
        if (sink_status == AVERROR_EOF) return AETHERNET_NATIVE_STATUS_OK;
        if (sink_status == AVERROR(EAGAIN)) {
            if (require_eof != 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
            return AETHERNET_NATIVE_STATUS_OK;
        }
        if (sink_status < 0) {
            return aethernet_native_status_from_av_error_with_deadline(
...
static int gif_resize_submit_decoded_frame(
    ...
    if (duration_cs <= 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
...
static int gif_resize_decode_packets(struct gif_resize_context *context) {
    ...
    while (context->decode_complete == 0 &&
        ...
        if (aethernet_native_deadline_status(
                context->request.deadline_monotonic_ms) !=
    ...
    if (context->decode_complete != 0) return AETHERNET_NATIVE_STATUS_OK;
...
static int gif_resize_flush_pipeline(struct gif_resize_context *context) {
    ...
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    if (context->next_pts <= 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
...
static int gif_resize_take_output(
    ...
    int deadline_status = aethernet_native_deadline_status(
        context->request.deadline_monotonic_ms);
    if (deadline_status != AETHERNET_NATIVE_STATUS_OK) return deadline_status;
    if (context->output.len == 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (context->output.data == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    ...
    status = aethernet_patch_gif_frame_delays(
        context->output.data,
        context->output.len,
        context->frame_delays_cs,
        context->encode_frames);
    ...
    status = aethernet_read_gif_frame_delays(
        context->output.data,
        context->output.len,
        NULL,
        context->encode_frames,
...
static int gif_resize_resolve_output_error(
    ...
    int output_status = aethernet_native_status_from_av_error_with_deadline(
        context->output.error, context->request.deadline_monotonic_ms);
    if (output_status == AETHERNET_NATIVE_STATUS_DEADLINE_EXCEEDED) {
        return output_status;
    }
    if (output_status == AETHERNET_NATIVE_STATUS_OUTPUT_LIMIT_EXCEEDED) {
        return output_status;
    }
    if (output_status == AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED) {
        return output_status;
    }
    return status;
}
...
int aethernet_ffmpeg_resize_gif(
    ...
    if (out_buf == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (out_size == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (out_capacity == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
