...
#define AETHERNET_HDR_PQ_LUT_SIZE 4096
#define AETHERNET_HDR_HLG_LUT_SIZE 4096
#define AETHERNET_HDR_SRGB_LUT_SIZE 4096
#define AETHERNET_PQ_SDR_TARGET_NORM 0.0203f
#define AETHERNET_HLG_REFERENCE_PEAK_NORM 0.1f

enum aethernet_hdr_gamut {
    AETHERNET_HDR_GAMUT_SRGB = 0,
    AETHERNET_HDR_GAMUT_BT2020 = 1,
    AETHERNET_HDR_GAMUT_DISPLAY_P3 = 2,
};

enum aethernet_hdr_transfer {
    AETHERNET_HDR_TRANSFER_SRGB = 0,
    AETHERNET_HDR_TRANSFER_BT709 = 1,
    AETHERNET_HDR_TRANSFER_BT2020_12 = 2,
    AETHERNET_HDR_TRANSFER_LINEAR = 3,
    AETHERNET_HDR_TRANSFER_PQ = 4,
    AETHERNET_HDR_TRANSFER_HLG = 5,
};

extern float aethernet_pq_lut[AETHERNET_HDR_PQ_LUT_SIZE];
extern float aethernet_hlg_lut[AETHERNET_HDR_HLG_LUT_SIZE];
extern float aethernet_hlg_ootf_scale_lut[AETHERNET_HDR_HLG_LUT_SIZE];
extern float aethernet_pq_tone_scale_lut[AETHERNET_HDR_PQ_LUT_SIZE];
extern float aethernet_hlg_tone_scale_lut[AETHERNET_HDR_HLG_LUT_SIZE];
extern uint8_t aethernet_srgb_lut[AETHERNET_HDR_SRGB_LUT_SIZE];
extern float aethernet_pq_sdr_target_perceptual;
extern float aethernet_hlg_source_peak_perceptual;
extern float aethernet_hlg_sdr_target_perceptual;

int aethernet_hdr_luts_ready(void);
int aethernet_hdr_transfer_is_hdr(int transfer);
int aethernet_hdr_apply_sdr_gamut(uint8_t *rgba, int width, int height, int gamut,
                               int transfer, int deadline_rows,
                               long long deadline_monotonic_ms);
int aethernet_hdr_tone_map_rgba16(const uint8_t *source, size_t source_stride,
                               uint8_t *destination, size_t destination_stride,
                               int width, int height, int bit_depth, int gamut,
                               int transfer, int deadline_rows,
                               long long deadline_monotonic_ms);

static inline uint16_t aethernet_hdr_lut_index(uint16_t code, int bit_depth) {
...
static inline uint16_t aethernet_unit_lut_index(float value) {
    if (value <= 0.0f) return 0;
    if (value >= 1.0f) return AETHERNET_HDR_HLG_LUT_SIZE - 1;
    return (uint16_t)(
        value * (AETHERNET_HDR_HLG_LUT_SIZE - 1) + 0.5f);
}

static inline float aethernet_bt2390_eetf_perceptual(
    float encoded,
    float max_luminance
) {
...
static inline float aethernet_pq_oetf(float luminance) {
...
static inline float aethernet_inverse_pq(float encoded) {
    const float m1 = 0.1593017578125f;
    const float m2 = 78.84375f;
    const float c1 = 0.8359375f;
    const float c2 = 18.8515625f;
...
static inline float aethernet_srgb_oetf(float value) {
...
static inline uint8_t aethernet_quantize8(float value) {
    if (value <= 0.0f) return 0;
...
static inline uint8_t aethernet_srgb_lut_quantize(float value) {
    if (value <= 0.0f) return 0;
    if (value >= 1.0f) return 255;
    size_t index = (size_t)(
        value * (AETHERNET_HDR_SRGB_LUT_SIZE - 1) + 0.5f);
    assert(index < AETHERNET_HDR_SRGB_LUT_SIZE);
    return aethernet_srgb_lut[index];
}

static inline void aethernet_bt2020_to_bt709_linear(
    float red,
    float green,
    float blue,
    float *out_red,
...
static inline void aethernet_display_p3_to_srgb_linear(
    float red,
    float green,
    float blue,
    float *out_red,
...
static inline float aethernet_inverse_srgb(float encoded) {
    if (encoded <= 0.0f) return 0.0f;
...
static inline float aethernet_inverse_bt709(float encoded) {
    if (encoded <= 0.0f) return 0.0f;
    if (encoded >= 1.0f) return 1.0f;
...
static inline float aethernet_inverse_bt2020_12(float encoded) {
    if (encoded <= 0.0f) return 0.0f;
    if (encoded >= 1.0f) return 1.0f;
...
static inline void aethernet_hdr_convert_gamut_linear(
    int gamut,
    float red,
    float green,
    float blue,
    ...
    if (gamut == AETHERNET_HDR_GAMUT_BT2020) {
        aethernet_bt2020_to_bt709_linear(
            red, green, blue, out_red, out_green, out_blue);
        return;
    }
    if (gamut == AETHERNET_HDR_GAMUT_DISPLAY_P3) {
        aethernet_display_p3_to_srgb_linear(
            red, green, blue, out_red, out_green, out_blue);
        return;
    }
    assert(gamut == AETHERNET_HDR_GAMUT_SRGB);
    *out_red = red;
    *out_green = green;
    *out_blue = blue;
}

static inline float aethernet_hdr_linear_luma(
    int gamut,
    float red,
    float green,
    float blue
...
static inline float aethernet_hdr_tone_scale(
    float maximum,
    ...
    float perceptual = aethernet_pq_oetf(maximum) /
                       source_peak_perceptual;
    float mapped_perceptual = aethernet_bt2390_eetf_perceptual(
        perceptual, target_perceptual);
    float mapped = aethernet_inverse_pq(
        mapped_perceptual * source_peak_perceptual);
    return (mapped / maximum) / target_normalized;
}

static inline void aethernet_hdr_pipeline_pixel(
    float red,
    float green,
    float blue,
    float scale,
    ...
    output[0] = aethernet_srgb_lut_quantize(linear_red);
    output[1] = aethernet_srgb_lut_quantize(linear_green);
    output[2] = aethernet_srgb_lut_quantize(linear_blue);
}
