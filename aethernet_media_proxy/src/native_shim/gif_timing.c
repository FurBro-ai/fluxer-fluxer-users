...
static int skip_gif_image_block(const uint8_t *bytes, size_t len,
    ...
    *offset += 9;
    if ((packed & 0x80) != 0) {
        ...
        if (table_bytes > len - *offset) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
        *offset += table_bytes;
    }
    if (*offset >= len) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    uint8_t minimum_code_size = bytes[*offset];
    *offset += 1;
    if (minimum_code_size < 2 || minimum_code_size > 8) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
    return aethernet_skip_gif_sub_blocks_checked(bytes, len, offset, block_count);
}
...
int aethernet_patch_gif_frame_delays(uint8_t *bytes, size_t len,
    ...
    int patched = 0;
    while (offset < len) {
        if (block_count >= AETHERNET_MAX_GIF_STRUCTURE_BLOCKS) {
        ...
        if (introducer == 0x21) {
            ...
            int skip_status = aethernet_skip_gif_sub_blocks_checked(
                bytes, len, &offset, &block_count);
            if (skip_status != AETHERNET_NATIVE_STATUS_OK) return skip_status;
            continue;
        }
        ...
        if (delay < 1 || delay > 65535) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
...
static int read_gif_graphics_control_delay(const uint8_t *bytes, size_t len,
    ...
    int delay = (int)aethernet_gif_read_le16(bytes + *offset + 2);
    if (delay <= 0) {
        delay = AETHERNET_DEFAULT_ANIMATION_FRAME_DELAY_MS / 10;
    } else if (delay * 10 < AETHERNET_MIN_ANIMATION_FRAME_DELAY_MS) {
        delay = AETHERNET_MIN_ANIMATION_FRAME_DELAY_MS / 10;
    }
    *delay_cs = delay;
    *offset += 6;
    return AETHERNET_NATIVE_STATUS_OK;
...
static int read_gif_application_loop(const uint8_t *bytes, size_t len,
    ...
    *offset += application_len;
    if (!recognized) {
        if (application_len == 0) return AETHERNET_NATIVE_STATUS_OK;
        return aethernet_skip_gif_sub_blocks_checked(
            bytes, len, offset, block_count);
    }
    if (*has_loop) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (*offset >= len) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (*block_count >= AETHERNET_MAX_GIF_STRUCTURE_BLOCKS) {
        return AETHERNET_NATIVE_STATUS_WORK_LIMIT_EXCEEDED;
    }
    ...
    if (bytes[*offset] != 1) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    *loop_count = (int)aethernet_gif_read_le16(bytes + *offset + 1);
    *offset += 3;
    if (*offset >= len) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (*block_count >= AETHERNET_MAX_GIF_STRUCTURE_BLOCKS) {
        return AETHERNET_NATIVE_STATUS_WORK_LIMIT_EXCEEDED;
    ...
    (*block_count)++;
    if (bytes[(*offset)++] != 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    *has_loop = 1;
    return AETHERNET_NATIVE_STATUS_OK;
}
...
int aethernet_read_gif_frame_delays(const uint8_t *bytes, size_t len,
    ...
    while (offset < len) {
        ...
        if (introducer == 0x21) {
            ...
            uint8_t label = bytes[offset++];
            if (label == 0xf9) {
                if (has_pending_delay) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
            ...
            int status = aethernet_skip_gif_sub_blocks_checked(
                bytes, len, &offset, &block_count);
            if (status != AETHERNET_NATIVE_STATUS_OK) return status;
            continue;
        }
        ...
        int delay_cs = has_pending_delay
                     ? pending_delay_cs
                     : AETHERNET_DEFAULT_ANIMATION_FRAME_DELAY_MS / 10;
...
int aethernet_validate_gif_animation(const void *gif_data, size_t gif_len,
                                  int max_frames, int max_duration_ms,
                                  size_t max_total_pixels) {
    if (max_duration_ms <= 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    ...
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    return aethernet_read_gif_frame_delays(
