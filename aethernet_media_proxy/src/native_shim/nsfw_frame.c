...
#define AETHERNET_MAX_VIDEO_PACKETS_FOR_NSFW 512
#define AETHERNET_MAX_VIDEO_FRAMES_FOR_NSFW_PER_SAMPLE 512
#define AETHERNET_NSFW_BYTE_SEEK_REWIND (5u * 1024u * 1024u)
...
void aethernet_nsfw_frames_reset(struct aethernet_nsfw_frame_out *frames, size_t n) {
    assert(n <= AETHERNET_MAX_NSFW_SAMPLES);
    if (frames == NULL) return;
    for (size_t i = 0; i < n; i++) {
        frames[i].data = NULL;
        frames[i].len = 0;
...
static int aethernet_seconds_to_pts(
    double seconds,
...
static int aethernet_emit_nsfw_frame(
    AVFrame *frame,
    ...
    struct aethernet_nsfw_frame_out *out
) {
    ...
    int rc = ff_emit_frame_thumbnail(
        ...
        AETHERNET_NSFW_FRAME_MAX_DIMENSION,
        AETHERNET_NSFW_FRAME_MAX_DIMENSION,
        deadline_monotonic_ms,
        max_frame_output_size,
        NULL, NULL,
        &out_buf, &out_size, &out_capacity);
    if (rc != AETHERNET_NATIVE_STATUS_OK) {
...
struct ff_animation_selection {
    ...
    int expected;
    size_t max_frame_output_size;
    struct aethernet_nsfw_frame_out *outputs;
};
...
static int ff_receive_selected_animation_frames(
    ...
    while (1) {
        int deadline_status = aethernet_native_deadline_status(
            context->deadline_monotonic_ms);
        if (deadline_status != AETHERNET_NATIVE_STATUS_OK) {
        ...
        if (receive_rc < 0) {
            return aethernet_native_status_from_av_error_with_deadline(
        ...
        if (frame_index == selection->indices[selection->next]) {
            int emit_rc = aethernet_emit_nsfw_frame(
            ...
            if (emit_rc != AETHERNET_NATIVE_STATUS_OK) {
                av_frame_unref(context->frame);
                return emit_rc;
            }
            selection->next++;
...
int aethernet_nsfw_animation_selection_valid(
    const int *indices,
    size_t count,
    int expected_frames
) {
    if (indices == NULL || count == 0 || count > AETHERNET_MAX_NSFW_SAMPLES) return 0;
...
static int ff_extract_selected_animation_frames(
    ...
    if (deadline_monotonic_ms < 0 ||
        !aethernet_ffmpeg_decoder_threads_valid(decoder_threads)) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
    if (!aethernet_nsfw_animation_selection_valid(
            frame_indices, n_indices, expected_frames)) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
    if (allowed_format == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (out_frames == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (max_frame_output_size == 0) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
    if (expected_frames >
        INT_MAX / AETHERNET_ANIMATION_PACKETS_PER_FRAME_LIMIT) {
        return AETHERNET_NATIVE_STATUS_WORK_LIMIT_EXCEEDED;
    }
    ...
    int status = ff_thumbnail_context_open(
        ...
        deadline_monotonic_ms);
    if (status == AETHERNET_NATIVE_STATUS_OK) {
    ...
    if (status == AETHERNET_NATIVE_STATUS_OK) {
        int packet_limit =
            expected_frames * AETHERNET_ANIMATION_PACKETS_PER_FRAME_LIMIT;
    ...
    if (status == AETHERNET_NATIVE_STATUS_OK) {
        if (selection.next != selection.count) {
            status = AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    ...
    if (status != AETHERNET_NATIVE_STATUS_OK) {
        aethernet_nsfw_frames_free(out_frames, n_indices);
    }
    return status;
}
int aethernet_ffmpeg_extract_apng_frames_for_nsfw(
    ...
    if (n_indices > AETHERNET_MAX_NSFW_SAMPLES) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    aethernet_nsfw_frames_reset(out_frames, n_indices);
    ...
    int rc = aethernet_validate_complete_apng(
        apng_data, apng_len, max_frames, max_total_pixels,
        NULL, NULL, &expected_frames, NULL);
    if (rc != 0) return rc;
    deadline_status = aethernet_native_deadline_status(
        deadline_monotonic_ms);
    if (deadline_status != AETHERNET_NATIVE_STATUS_OK) return deadline_status;
...
int aethernet_ffmpeg_extract_gif_frames_for_nsfw(
    ...
    if (deadline_status != AETHERNET_NATIVE_STATUS_OK) return deadline_status;
    int expected_frames = 0;
    int rc = aethernet_validate_complete_gif(
...
static int aethernet_nsfw_uses_byte_seek(const AVFormatContext *format) {
...
static int aethernet_nsfw_seek(
    ...
    int deadline_status = aethernet_native_deadline_status(
        deadline_monotonic_ms);
    if (deadline_status != AETHERNET_NATIVE_STATUS_OK) return deadline_status;
    if (aethernet_nsfw_uses_byte_seek(format) &&
        ...
        if (isfinite(duration_seconds) && duration_seconds > 0.0) {
            ...
            if (offset > AETHERNET_NSFW_BYTE_SEEK_REWIND) {
                offset -= AETHERNET_NSFW_BYTE_SEEK_REWIND;
            } else {
                offset = 0;
            }
            ...
            if (rc >= 0) return AETHERNET_NATIVE_STATUS_OK;
        }
    }
    int rc = av_seek_frame(format, stream_index, target_pts, AVSEEK_FLAG_BACKWARD);
    deadline_status = aethernet_native_deadline_status(deadline_monotonic_ms);
    if (deadline_status != AETHERNET_NATIVE_STATUS_OK) return deadline_status;
    if (rc >= 0) return AETHERNET_NATIVE_STATUS_OK;
    rc = avformat_seek_file(
        format, stream_index, INT64_MIN, target_pts, target_pts, AVSEEK_FLAG_BACKWARD);
    if (rc >= 0) return aethernet_native_deadline_status(deadline_monotonic_ms);
    return aethernet_native_status_from_av_error_with_deadline(
        rc, deadline_monotonic_ms);
}

struct aethernet_nsfw_target {
    double seconds;
    int64_t pts;
...
static int aethernet_nsfw_target_pts(
    AVStream *stream,
    double seconds,
    int64_t *out_pts
) {
    ...
    if (aethernet_seconds_to_pts(seconds, stream->time_base, &target_pts) != 0) return -1;
...
static int aethernet_nsfw_targets_init(
    AVFormatContext *format,
    AVStream *stream,
    const double *timestamps,
    size_t count,
    long long deadline_monotonic_ms,
    struct aethernet_nsfw_target *targets
) {
    assert(stream != NULL);
    assert(timestamps != NULL);
    assert(targets != NULL);
    assert(count <= AETHERNET_MAX_NSFW_SAMPLES);
    int indexed_seek = !aethernet_nsfw_uses_byte_seek(format);
    for (size_t i = 0; i < count; i++) {
        int deadline_status = aethernet_native_deadline_status(
            deadline_monotonic_ms);
        if (deadline_status != AETHERNET_NATIVE_STATUS_OK) {
        ...
        if (aethernet_nsfw_target_pts(stream, seconds, &targets[i].pts) != 0) {
            return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
        }
...
static size_t aethernet_nsfw_group_limit(
    const struct aethernet_nsfw_target *targets,
    size_t count,
    size_t first
) {
    assert(targets != NULL);
    assert(first < count);
    assert(count <= AETHERNET_MAX_NSFW_SAMPLES);
    const struct aethernet_nsfw_target *origin = &targets[first];
    size_t limit = first + 1;
    while (limit < count) {
        const struct aethernet_nsfw_target *previous = &targets[limit - 1];
        const struct aethernet_nsfw_target *next = &targets[limit];
...
static int aethernet_nsfw_decoder_open(ff_thumbnail_context *context, int decoder_threads) {
    assert(context != NULL);
    assert(aethernet_ffmpeg_decoder_threads_valid(decoder_threads));
    int status = aethernet_native_deadline_status(
        context->deadline_monotonic_ms);
    ...
    if (context->stream_index < 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    ...
    if (width <= 0 || height <= 0 ||
        width > AETHERNET_MAX_VIDEO_FRAME_DIMENSION ||
        height > AETHERNET_MAX_VIDEO_FRAME_DIMENSION ||
        (size_t)width > AETHERNET_MAX_VIDEO_PIXELS / (size_t)height) {
    ...
    if (!aethernet_video_decoder_allowed(context->stream, decoder)) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
    context->decoder = avcodec_alloc_context3(decoder);
    if (context->decoder == NULL) return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
    ...
    context->decoder->max_pixels = (int64_t)AETHERNET_MAX_VIDEO_PIXELS;
    ...
    status = aethernet_native_deadline_status(context->deadline_monotonic_ms);
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
...
enum aethernet_nsfw_receive_result {
    AETHERNET_NSFW_RECEIVE_MORE = 100,
    AETHERNET_NSFW_RECEIVE_EOF = 101,
    AETHERNET_NSFW_RECEIVE_COMPLETE = 102,
    AETHERNET_NSFW_RECEIVE_RESEEK = 103
};

struct aethernet_nsfw_decode_group {
    ff_thumbnail_context *context;
    AVFrame *candidate;
    const struct aethernet_nsfw_target *targets;
    struct aethernet_nsfw_frame_out *outputs;
...
static int aethernet_nsfw_copy_output(
    const struct aethernet_nsfw_frame_out *source,
    long long deadline_monotonic_ms,
    struct aethernet_nsfw_frame_out *destination
) {
    assert(source != NULL);
    assert(destination != NULL);
    if (source->data == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (source->len == 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    int status = aethernet_native_deadline_status(deadline_monotonic_ms);
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    void *copy = g_try_malloc(source->len);
    if (copy == NULL) return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
...
static int aethernet_nsfw_emit_frame_range(
    struct aethernet_nsfw_decode_group *group,
    AVFrame *frame,
    size_t first,
    size_t limit
) {
    ...
    int status = aethernet_emit_nsfw_frame(
    ...
    for (size_t index = first + 1; index < limit; index++) {
        status = aethernet_nsfw_copy_output(
...
static void aethernet_nsfw_keep_candidate(struct aethernet_nsfw_decode_group *group) {
    assert(group != NULL);
...
static int aethernet_nsfw_consume_frame(struct aethernet_nsfw_decode_group *group) {
    assert(group != NULL);
    assert(group->next < group->limit);
    AVFrame *frame = group->context->frame;
    int64_t frame_pts = frame->best_effort_timestamp;
    if (frame_pts == AV_NOPTS_VALUE) {
        int rc = aethernet_emit_nsfw_frame(
            frame, group->context->decoder, group->context->format,
            group->context->stream, group->context->deadline_monotonic_ms,
            group->max_frame_output_size,
            &group->outputs[group->next]);
        av_frame_unref(frame);
        if (rc != AETHERNET_NATIVE_STATUS_OK) return rc;
        group->next++;
        return group->next == group->limit
            ? AETHERNET_NSFW_RECEIVE_COMPLETE
            : AETHERNET_NSFW_RECEIVE_RESEEK;
    }
    if (frame_pts < group->targets[group->next].pts) {
        aethernet_nsfw_keep_candidate(group);
        return AETHERNET_NSFW_RECEIVE_MORE;
    }
    ...
    int rc = aethernet_nsfw_emit_frame_range(group, frame, first, limit);
    if (rc != AETHERNET_NATIVE_STATUS_OK) {
        av_frame_unref(frame);
        return rc;
    }
    ...
    if (group->next == group->limit) {
        av_frame_unref(frame);
        return AETHERNET_NSFW_RECEIVE_COMPLETE;
    }
    aethernet_nsfw_keep_candidate(group);
    return AETHERNET_NSFW_RECEIVE_MORE;
}

static int aethernet_nsfw_receive_frames(struct aethernet_nsfw_decode_group *group) {
    assert(group != NULL);
    while (1) {
        int deadline_status = aethernet_native_deadline_status(
        ...
        if (rc == AVERROR(EAGAIN)) return AETHERNET_NSFW_RECEIVE_MORE;
        if (rc == AVERROR_EOF) return AETHERNET_NSFW_RECEIVE_EOF;
        if (rc < 0) {
            return aethernet_native_status_from_av_error_with_deadline(
                rc, group->context->deadline_monotonic_ms);
        }
        group->decoded_frames++;
        if (group->decoded_frames > AETHERNET_MAX_VIDEO_FRAMES_FOR_NSFW_PER_SAMPLE) {
            av_frame_unref(group->context->frame);
            return AETHERNET_NATIVE_STATUS_WORK_LIMIT_EXCEEDED;
        }
        int consume_rc = aethernet_nsfw_consume_frame(group);
        if (consume_rc != AETHERNET_NSFW_RECEIVE_MORE) return consume_rc;
    }
}

static int aethernet_nsfw_emit_candidate(struct aethernet_nsfw_decode_group *group) {
    assert(group != NULL);
    if (group->candidate->data[0] == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    size_t first = group->next;
    int status = aethernet_nsfw_emit_frame_range(
...
static int aethernet_nsfw_decode_group(struct aethernet_nsfw_decode_group *group) {
    assert(group != NULL);
    assert(group->next < group->limit);
    int read_rc = 0;
    while (group->packets < AETHERNET_MAX_VIDEO_PACKETS_FOR_NSFW) {
        int deadline_status = aethernet_native_deadline_status(
            group->context->deadline_monotonic_ms);
        if (deadline_status != AETHERNET_NATIVE_STATUS_OK) {
            return deadline_status;
        ...
        int receive_rc = aethernet_nsfw_receive_frames(group);
        if (receive_rc == AETHERNET_NSFW_RECEIVE_EOF) {
            return aethernet_nsfw_emit_candidate(group);
        }
        if (receive_rc == AETHERNET_NSFW_RECEIVE_COMPLETE ||
            receive_rc == AETHERNET_NSFW_RECEIVE_RESEEK) {
            return AETHERNET_NATIVE_STATUS_OK;
        }
        if (receive_rc != AETHERNET_NSFW_RECEIVE_MORE) return receive_rc;
    }
    if (group->packets == AETHERNET_MAX_VIDEO_PACKETS_FOR_NSFW && read_rc >= 0) {
        return AETHERNET_NATIVE_STATUS_WORK_LIMIT_EXCEEDED;
    }
    ...
    if (receive_rc == AETHERNET_NSFW_RECEIVE_MORE) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
    return receive_rc;
}

static int aethernet_av_nsfw_request_valid(
    const void *media_data,
    size_t media_len,
    int decoder_threads,
    const double *timestamps,
    ...
    if (timestamp_count > AETHERNET_MAX_NSFW_SAMPLES) return 0;
    if (deadline_monotonic_ms < 0) return 0;
    if (max_frame_output_size == 0) return 0;
    if (!aethernet_ffmpeg_decoder_threads_valid(decoder_threads)) return 0;
...
static int aethernet_nsfw_sample_failure_fatal(int status) {
    return status == AETHERNET_NATIVE_STATUS_DEADLINE_EXCEEDED ||
           status == AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
}

static size_t aethernet_nsfw_produced_frames(
    const struct aethernet_nsfw_frame_out *outputs,
    size_t count
) {
    assert(outputs != NULL);
    size_t produced = 0;
...
static int aethernet_nsfw_decode_targets(
    ff_thumbnail_context *context,
    AVFrame *candidate,
    const struct aethernet_nsfw_target *targets,
    size_t target_count,
    ...
    assert(target_count <= AETHERNET_MAX_NSFW_SAMPLES);
    assert(outputs != NULL);
    size_t next = 0;
    int first_failure = AETHERNET_NATIVE_STATUS_OK;
    while (next < target_count) {
        ...
        size_t limit = aethernet_nsfw_group_limit(
        ...
        int status = aethernet_nsfw_seek(
            context->format,
        ...
        if (status == AETHERNET_NATIVE_STATUS_OK) {
            status = aethernet_nsfw_decode_group(&group);
        }
        if (status == AETHERNET_NATIVE_STATUS_OK && group.next > first) {
            next = group.next;
            continue;
        }
        if (aethernet_nsfw_sample_failure_fatal(status)) return status;
        ...
        if (group.next < limit && outputs[group.next].data == NULL) {
            (void)aethernet_nsfw_emit_candidate(&group);
        }
        if (first_failure == AETHERNET_NATIVE_STATUS_OK) first_failure = status;
        next = limit;
    }
    if (aethernet_nsfw_produced_frames(outputs, target_count) == 0) {
        return first_failure != AETHERNET_NATIVE_STATUS_OK
            ? first_failure
            : AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
    return AETHERNET_NATIVE_STATUS_OK;
}
...
int aethernet_av_extract_frames_for_nsfw(
    ...
    if (n_timestamps > AETHERNET_MAX_NSFW_SAMPLES) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
    aethernet_nsfw_frames_reset(out_frames, n_timestamps);
    if (!aethernet_av_nsfw_request_valid(
    ...
    ff_thumbnail_context context;
    int status = ff_thumbnail_context_open(
        &context, media_data, media_len, AETHERNET_AV_INPUT_FORMATS, 0,
        deadline_monotonic_ms);
    if (status == AETHERNET_NATIVE_STATUS_OK) {
        status = aethernet_nsfw_decoder_open(&context, decoder_threads);
    }
    ...
    struct aethernet_nsfw_target targets[AETHERNET_MAX_NSFW_SAMPLES];
    if (status == AETHERNET_NATIVE_STATUS_OK) {
        status = aethernet_nsfw_targets_init(
            context.format, context.stream, timestamps_secs,
            n_timestamps, deadline_monotonic_ms, targets);
    }
    if (status == AETHERNET_NATIVE_STATUS_OK) {
        status = aethernet_nsfw_decode_targets(
    ...
    ff_thumbnail_context_clear(&context);
    if (status != AETHERNET_NATIVE_STATUS_OK) {
        aethernet_nsfw_frames_free(out_frames, n_timestamps);
    }
    return status;
}
