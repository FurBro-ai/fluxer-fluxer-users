...
int aethernet_skip_gif_sub_blocks_checked(const uint8_t *bytes, size_t len,
    ...
    while (*offset < len) {
        ...
        *offset += 1;
        if (block_len == 0) return AETHERNET_NATIVE_STATUS_OK;
...
struct aethernet_gif_validation_context {
    const uint8_t *bytes;
    size_t len;
    size_t offset;
    size_t image_count;
...
static int aethernet_gif_skip_color_table(
    struct aethernet_gif_validation_context *context,
    uint8_t packed
) {
    assert(context != NULL);
    if ((packed & 0x80) == 0) return AETHERNET_NATIVE_STATUS_OK;
...
static int aethernet_gif_validation_context_init(
    struct aethernet_gif_validation_context *context,
    const uint8_t *bytes,
    size_t len,
    int max_frames,
    ...
    uint16_t canvas_width = aethernet_gif_read_le16(bytes + 6);
    uint16_t canvas_height = aethernet_gif_read_le16(bytes + 8);
    ...
    *context = (struct aethernet_gif_validation_context) {
        .bytes = bytes,
        .len = len,
        .offset = 13,
    ...
    return aethernet_gif_skip_color_table(context, bytes[10]);
}

static int aethernet_gif_validate_image_block(
    struct aethernet_gif_validation_context *context
) {
    assert(context != NULL);
    ...
    uint16_t left = aethernet_gif_read_le16(descriptor);
    uint16_t top = aethernet_gif_read_le16(descriptor + 2);
    uint16_t width = aethernet_gif_read_le16(descriptor + 4);
    uint16_t height = aethernet_gif_read_le16(descriptor + 6);
    ...
    int status = aethernet_gif_skip_color_table(context, packed);
    ...
    status = aethernet_skip_gif_sub_blocks_checked(
...
int aethernet_validate_complete_gif(const uint8_t *bytes, size_t len,
    ...
    if (out_expected_frames == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    *out_expected_frames = 0;
    struct aethernet_gif_validation_context context;
    int status = aethernet_gif_validation_context_init(
        &context, bytes, len, max_frames, max_total_pixels);
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    while (context.offset < context.len) {
        if (context.block_count >= AETHERNET_MAX_GIF_STRUCTURE_BLOCKS) {
            return AETHERNET_NATIVE_STATUS_WORK_LIMIT_EXCEEDED;
        }
        context.block_count++;
        ...
        status = aethernet_gif_validate_image_block(&context);
        if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    }
    return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
}
