...
const int aethernet_vips_format_uchar = VIPS_FORMAT_UCHAR;
const int aethernet_vips_format_ushort = VIPS_FORMAT_USHORT;
const int aethernet_vips_format_float = VIPS_FORMAT_FLOAT;

int aethernet_vips_init(const char *argv0) {
    if (argv0 == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
...
void aethernet_vips_tune_for_server(int per_pipeline_threads) {
    assert(per_pipeline_threads >= 1 && per_pipeline_threads <= AETHERNET_MAX_THREADS_PER_PIPELINE);
...
int aethernet_vips_probe_animated(const void *buf, size_t len, int *width, int *height, int *pages) {
    ...
    *height = 0;
    *pages = 0;
    if (buf == NULL || len == 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
...
static int aethernet_vips_operation_status(int rc, VipsImage **out) {
    assert(out != NULL);
    if (rc == 0 && *out != NULL) return AETHERNET_NATIVE_STATUS_OK;
    if (*out != NULL) {
        g_object_unref(*out);
        *out = NULL;
    }
...
struct aethernet_vips_deadline_guard {
    VipsImage *image;
...
static int aethernet_vips_deadline_guard_start(
    struct aethernet_vips_deadline_guard *guard,
    VipsImage *image,
    long long deadline_monotonic_ms);
static int aethernet_vips_deadline_guard_finish(
    struct aethernet_vips_deadline_guard *guard);
...
int aethernet_vips_image_copy_memory(
    ...
    if (out == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    *out = NULL;
    if (in == NULL || deadline_monotonic_ms < 0) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
    struct aethernet_vips_deadline_guard deadline_guard;
    int status = aethernet_vips_deadline_guard_start(
    ...
    int deadline_status = aethernet_vips_deadline_guard_finish(&deadline_guard);
    if (deadline_status != AETHERNET_NATIVE_STATUS_OK) {
        if (memory != NULL) g_object_unref(memory);
        return deadline_status;
    }
    if (memory == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    *out = memory;
    return AETHERNET_NATIVE_STATUS_OK;
}
...
int aethernet_vips_autorot(
    ...
    *out = NULL;
    ...
    return aethernet_vips_operation_status(rc, out);
}

int aethernet_vips_extract_area(VipsImage *in, VipsImage **out, int left, int top, int width, int height) {
    if (out == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    *out = NULL;
    if (in == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
...
static int aethernet_vips_copy_optional_int_metadata(
    VipsImage *source,
...
static int aethernet_vips_copy_animation_metadata(
    VipsImage *source,
    VipsImage *destination,
    int n_pages,
    int page_height
    ...
    for (size_t i = 0; i < sizeof(int_fields) / sizeof(int_fields[0]); i++) {
        int status = aethernet_vips_copy_optional_int_metadata(
...
int aethernet_vips_join_animation_pages(
    ...
    *out = NULL;
    ...
    if (pages[0] == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    ...
    for (int i = 1; i < n_pages; i++) {
        if (pages[i] == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    ...
    int status = aethernet_vips_operation_status(rc, &joined);
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    ...
    status = aethernet_vips_copy_animation_metadata(
...
static int aethernet_vips_image_work_bounds_status(
    VipsImage *image,
    int max_pages,
    size_t max_total_pixels
) {
    ...
    if (pages <= 0) return AETHERNET_NATIVE_STATUS_INVALID_DIMENSIONS;
    if (pages > max_pages) return AETHERNET_NATIVE_STATUS_WORK_LIMIT_EXCEEDED;
...
struct aethernet_vips_thumbnail_request {
    const void *data;
    size_t len;
    VipsImage **output;
    int width;
...
struct aethernet_vips_center_thumbnail {
    VipsImage *image;
    int width;
    int height;
    int has_page_height;
...
static int aethernet_vips_thumbnail_request_status(
    const struct aethernet_vips_thumbnail_request *request
) {
    ...
    if (request->crop_mode != AETHERNET_THUMB_CROP_NONE &&
        request->crop_mode != AETHERNET_THUMB_CROP_CENTRE) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
    if (request->pages == 1 &&
        request->crop_mode == AETHERNET_THUMB_CROP_CENTRE &&
...
static const char *aethernet_vips_thumbnail_options(
    const struct aethernet_vips_thumbnail_request *request
) {
    assert(request != NULL);
...
static int aethernet_vips_uncropped_thumbnail(
    const struct aethernet_vips_thumbnail_request *request,
    const char *options,
    VipsImage *loaded
) {
    ...
    int status = aethernet_vips_operation_status(result, request->output);
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    status = aethernet_vips_image_work_bounds_status(
...
static int aethernet_vips_center_thumbnail_open(
    struct aethernet_vips_center_thumbnail *context,
    const struct aethernet_vips_thumbnail_request *request,
    VipsImage *loaded
) {
    assert(context != NULL);
    ...
    int status = aethernet_vips_image_work_bounds_status(
...
static int aethernet_vips_center_thumbnail_resize(
    struct aethernet_vips_center_thumbnail *context,
    const struct aethernet_vips_thumbnail_request *request
) {
    assert(context != NULL);
...
static int aethernet_vips_center_thumbnail_publish(
    struct aethernet_vips_center_thumbnail *context,
    const struct aethernet_vips_thumbnail_request *request
) {
    assert(context != NULL);
...
static int aethernet_vips_center_thumbnail_run(
    const struct aethernet_vips_thumbnail_request *request,
    VipsImage *loaded
) {
    struct aethernet_vips_center_thumbnail context = {0};
    int status = aethernet_vips_center_thumbnail_open(
        &context, request, loaded);
    if (status == AETHERNET_NATIVE_STATUS_OK) {
        status = aethernet_vips_center_thumbnail_resize(&context, request);
    }
    if (status == AETHERNET_NATIVE_STATUS_OK) {
        status = aethernet_vips_center_thumbnail_publish(&context, request);
    }
...
int aethernet_vips_thumbnail_buffer_ex(
    ...
    if (deadline_monotonic_ms < 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    int status = aethernet_native_deadline_status(deadline_monotonic_ms);
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    struct aethernet_vips_thumbnail_request request = {
        .data = buf,
        .len = len,
        .output = out,
        .width = width,
    ...
    status = aethernet_vips_thumbnail_request_status(&request);
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    const char *options = aethernet_vips_thumbnail_options(&request);
    VipsImage *loaded = vips_image_new_from_buffer(buf, len, options, NULL);
    if (loaded == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    status = aethernet_vips_image_work_bounds_status(
    ...
    int use_center_crop = n == 1 && crop_mode == AETHERNET_THUMB_CROP_CENTRE;
    if (use_center_crop) {
        status = aethernet_vips_center_thumbnail_run(&request, loaded);
    } else {
        status = aethernet_vips_uncropped_thumbnail(&request, options, loaded);
    }
    int deadline_status = aethernet_native_deadline_status(deadline_monotonic_ms);
    if (deadline_status != AETHERNET_NATIVE_STATUS_OK) {
        if (*out != NULL) {
            g_object_unref(*out);
            *out = NULL;
...
struct aethernet_vips_write_ctx {
    aethernet_vips_write_cb cb;
    void *user_data;
    long long deadline_monotonic_ms;
    int status;
};

static void aethernet_vips_deadline_eval(
    VipsImage *image,
    VipsProgress *progress,
    void *user_data
) {
    (void)progress;
    struct aethernet_vips_deadline_guard *guard = user_data;
    if (guard == NULL || guard->reached) return;
    if (aethernet_native_deadline_status(guard->deadline_monotonic_ms) ==
        AETHERNET_NATIVE_STATUS_OK) return;
    guard->reached = 1;
...
static int aethernet_vips_deadline_guard_start(
    ...
    int status = aethernet_native_deadline_status(deadline_monotonic_ms);
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    if (deadline_monotonic_ms == 0) return AETHERNET_NATIVE_STATUS_OK;
    vips_image_set_progress(image, TRUE);
    guard->eval_handler = g_signal_connect(
        image, "eval", G_CALLBACK(aethernet_vips_deadline_eval), guard);
...
static int aethernet_vips_deadline_guard_finish(
    ...
    if (guard->reached) return AETHERNET_NATIVE_STATUS_DEADLINE_EXCEEDED;
    return aethernet_native_deadline_status(guard->deadline_monotonic_ms);
}

int aethernet_vips_image_write_to_memory_deadline(
    VipsImage *image,
    long long deadline_monotonic_ms,
    ...
    if (buffer == NULL) return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
    if (size == 0) {
        g_free(buffer);
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
...
static gint64 aethernet_vips_target_write_adapter(VipsTargetCustom *target, const void *bytes, gint64 length, void *gp) {
    (void)target;
    struct aethernet_vips_write_ctx *c = gp;
    ...
    if (length < 0) {
        c->status = AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
        return -1;
    }
...
int aethernet_vips_image_write_to_callback(
    ...
    if (target == NULL) {
        aethernet_vips_deadline_guard_finish(&deadline_guard);
        return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
    }

    struct aethernet_vips_write_ctx ctx = {
        .cb = cb,
        .user_data = user_data,
        .deadline_monotonic_ms = deadline_monotonic_ms,
        .status = AETHERNET_NATIVE_STATUS_OK,
    };
    g_signal_connect(target, "write", G_CALLBACK(aethernet_vips_target_write_adapter), &ctx);
    ...
    if (ctx.status != AETHERNET_NATIVE_STATUS_OK) return ctx.status;
    if (rc != 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    return AETHERNET_NATIVE_STATUS_OK;
}

struct aethernet_vips_bounded_buffer {
    uint8_t *data;
    size_t len;
    size_t capacity;
    size_t max_output_size;
...
static int aethernet_vips_bounded_buffer_write(void *user_data, const void *bytes, size_t len) {
    if (user_data == NULL) return -1;
    struct aethernet_vips_bounded_buffer *buffer = user_data;
    if (buffer->status != AETHERNET_NATIVE_STATUS_OK) return -1;
    if (len == 0) return 0;
    if (bytes == NULL) {
        buffer->status = AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
        return -1;
    }
    if (buffer->len > buffer->max_output_size ||
        len > buffer->max_output_size - buffer->len) {
        buffer->status = AETHERNET_NATIVE_STATUS_OUTPUT_LIMIT_EXCEEDED;
        return -1;
    }
    ...
    if (required > buffer->capacity) {
        ...
        if (next == NULL) {
            buffer->status = AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
            return -1;
        }
        buffer->data = next;
...
int aethernet_vips_image_write_to_buffer_bounded(VipsImage *image,
                                               const char *suffix,
                                               long long deadline_monotonic_ms,
                                               size_t max_output_size,
                                               void **out_buf,
    ...
    struct aethernet_vips_bounded_buffer buffer = {
        .max_output_size = max_output_size,
        .status = AETHERNET_NATIVE_STATUS_OK,
    };
    int status = aethernet_vips_image_write_to_callback(
        image, suffix, deadline_monotonic_ms,
        aethernet_vips_bounded_buffer_write, &buffer);
    if (buffer.status != AETHERNET_NATIVE_STATUS_OK) status = buffer.status;
    if (status == AETHERNET_NATIVE_STATUS_OK &&
...
int aethernet_vips_extract_rgba(
    ...
    VipsImage *rgba = NULL;
    int status = aethernet_vips_image_to_rgba(in, &rgba);
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    ...
    void *buf = NULL;
    status = aethernet_vips_image_write_to_memory_deadline(
