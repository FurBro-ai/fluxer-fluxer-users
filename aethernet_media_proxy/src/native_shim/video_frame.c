...
static int ff_frame_to_rgba_image(
    ...
    *out = NULL;
    ...
    if (rgba == NULL) return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
    ...
    if (convert_status != AETHERNET_NATIVE_STATUS_OK) {
...
static int ff_apply_display_transform(
    ...
    if (transform->hscale != 1.0 || transform->vscale != 1.0) {
        ...
        if (projected_width > (double)AETHERNET_MAX_VIDEO_FRAME_DIMENSION ||
            projected_height > (double)AETHERNET_MAX_VIDEO_FRAME_DIMENSION) {
            return AETHERNET_NATIVE_STATUS_INVALID_DIMENSIONS;
        }
...
static int ff_measure_display_geometry(
    ...
    if (projected_width > (double)AETHERNET_MAX_VIDEO_FRAME_DIMENSION ||
        projected_height > (double)AETHERNET_MAX_VIDEO_FRAME_DIMENSION) return -1;
...
int ff_emit_frame_thumbnail(
    ...
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    int write_rc = aethernet_vips_image_write_to_buffer_bounded(
...
int ff_fit_frame_image(VipsImage **image, int max_width, int max_height) {
    if (image == NULL || *image == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (max_width <= 0 || max_height <= 0) return AETHERNET_NATIVE_STATUS_OK;
    ...
    if (scale >= 1.0) return AETHERNET_NATIVE_STATUS_OK;
...
static int ff_emit_frame_rgba(
    ...
    g_object_unref(image);
    if (status == AETHERNET_NATIVE_STATUS_OK && out->rgba != NULL &&
    ...
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    return allocation_failed ? AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED
...
int ff_thumbnail_context_open(
    ...
    if (context == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    ...
    if (context->avio_buffer == NULL) return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
    context->avio = avio_alloc_context(
        context->avio_buffer, 64 * 1024, 0, &context->reader,
        ff_mem_read_packet, NULL, ff_mem_seek);
    if (context->avio == NULL) return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
    context->avio_buffer = NULL;
    context->format = avformat_alloc_context();
    if (context->format == NULL) return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
    int status = aethernet_restrict_untrusted_av_context(context->format, allowed_formats);
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    context->format->pb = context->avio;
    context->format->interrupt_callback.callback = aethernet_ffmpeg_interrupt_deadline;
    ...
    status = aethernet_prepare_untrusted_av_input(context->format);
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    if (!ff_container_headers_are_complete(context->format)) {
        int stream_info_rc = aethernet_find_stream_info_bounded(context->format);
        if (stream_info_rc < 0) {
            return aethernet_native_status_from_av_error_with_deadline(
...
static int ff_thumbnail_context_probe(
    ...
    return out->has_video || out->has_audio
        ...
        : AETHERNET_NATIVE_STATUS_UNSUPPORTED;
}
...
int ff_thumbnail_decoder_open(ff_thumbnail_context *context, int decoder_threads) {
    ...
    if (context->stream_index < 0) return AETHERNET_NATIVE_STATUS_UNSUPPORTED;
    ...
    if (!aethernet_video_decoder_allowed(context->stream, codec)) {
...
static int ff_thumbnail_decode_frame(ff_thumbnail_context *context, int max_packets) {
    ...
    for (;;) {
        ...
        if (receive_rc == 0) return AETHERNET_NATIVE_STATUS_OK;
        if (receive_rc != AVERROR(EAGAIN) && receive_rc != AVERROR_EOF) {
            return aethernet_native_status_from_av_error_with_deadline(
                receive_rc, context->deadline_monotonic_ms);
        }
        if (receive_rc == AVERROR_EOF) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
        if (send_blocked) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
        if (packet_pending) {
        ...
        if (input_exhausted) {
            if (drain_sent) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
...
static int ff_thumbnail_packet_limit(int requested) {
    if (requested <= 0) return AETHERNET_MAX_VIDEO_THUMBNAIL_PACKETS;
    if (requested > AETHERNET_MAX_VIDEO_THUMBNAIL_PACKETS) {
        return AETHERNET_MAX_VIDEO_THUMBNAIL_PACKETS;
    }
    return requested;
}
...
int aethernet_ffmpeg_video_thumbnail_ex(
    ...
    if (!aethernet_ffmpeg_decoder_threads_valid(decoder_threads)) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
    if (max_width < 0 || max_height < 0 ||
    ...
    if (rc != AETHERNET_NATIVE_STATUS_OK && *out_buf != NULL) {
        g_free(*out_buf);
        *out_buf = NULL;
        *out_size = 0;
        *out_capacity = 0;
...
static void ff_av_metadata_discard_rgba(struct aethernet_av_metadata_out *out) {
    if (out == NULL) return;
    if (out->rgba != NULL) g_free(out->rgba);
    out->rgba = NULL;
    out->rgba_size = 0;
...
int aethernet_av_metadata(
    ...
    int rc = ff_thumbnail_context_open(
        &context, media_data, media_len, AETHERNET_AV_INPUT_FORMATS, 0,
        deadline_monotonic_ms);
    if (rc == AETHERNET_NATIVE_STATUS_OK) rc = ff_thumbnail_context_probe(&context, out);
    if (rc == AETHERNET_NATIVE_STATUS_OK && out->has_video && max_width > 0) {
        int frame_rc = ff_thumbnail_decoder_open(&context, decoder_threads);
        if (frame_rc == AETHERNET_NATIVE_STATUS_OK) {
            int64_t frame_count = context.stream->nb_frames;
            if (frame_count < 0) {
                frame_rc = AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
            } else if (frame_count > INT_MAX) {
                frame_rc = AETHERNET_NATIVE_STATUS_WORK_LIMIT_EXCEEDED;
            } else {
