...
#define AETHERNET_HEIF_MAX_AUXILIARY_IMAGES 4096
#define AETHERNET_HEIF_DEADLINE_ROWS 64
#define AETHERNET_HEIF_ICC_PROFILE_BYTES_MAX ((size_t)4 * 1024 * 1024)

static unsigned char aethernet_ascii_lower(unsigned char value) {
    if (value >= 'A' && value <= 'Z') {
...
static int aethernet_ascii_contains_folded(
    const char *haystack,
    const char *needle
) {
    ...
    for (const char *position = haystack; *position != '\0'; position++) {
        ...
        while (index < needle_length && position[index] != '\0' &&
               aethernet_ascii_lower((unsigned char)position[index]) ==
               aethernet_ascii_lower((unsigned char)needle[index])) {
            index++;
        }
...
static int aethernet_heif_aux_type_is_hdr_gain_map(const char *type) {
    if (type == NULL || type[0] == '\0') return 0;
    if (aethernet_ascii_contains_folded(type, "hdrgainmap") ||
        aethernet_ascii_contains_folded(type, "hdr_gain_map") ||
        aethernet_ascii_contains_folded(type, "hdr-gain-map")) {
        return 1;
    }
    if (!aethernet_ascii_contains_folded(type, "gainmap")) return 0;
    return aethernet_ascii_contains_folded(type, "hdr") ||
           aethernet_ascii_contains_folded(type, "21496") ||
           aethernet_ascii_contains_folded(type, "iso");
}

static int aethernet_heif_auxiliary_is_gain_map(
    ...
    *found = 0;
    ...
    if (error.code != heif_error_Ok) {
        if (auxiliary != NULL) heif_image_handle_release(auxiliary);
        return aethernet_native_status_from_heif_error(error);
    }
    if (auxiliary == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    ...
    *found = aethernet_heif_aux_type_is_hdr_gain_map(type);
    heif_image_handle_release_auxiliary_type(auxiliary, &type);
    heif_image_handle_release(auxiliary);
    return AETHERNET_NATIVE_STATUS_OK;
}

int aethernet_heif_detect_hdr_gain_map(
    ...
    *detected = 0;
    ...
    status = aethernet_native_deadline_status(deadline_monotonic_ms);
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    if (count < 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (count > AETHERNET_HEIF_MAX_AUXILIARY_IMAGES) {
        return AETHERNET_NATIVE_STATUS_WORK_LIMIT_EXCEEDED;
    }
    if (count == 0) return AETHERNET_NATIVE_STATUS_OK;
    heif_item_id *identifiers = calloc((size_t)count, sizeof(*identifiers));
    if (identifiers == NULL) return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
    int received = heif_image_handle_get_list_of_auxiliary_image_IDs(
        handle, filter, identifiers, count);
    status = aethernet_native_deadline_status(deadline_monotonic_ms);
    if (status == AETHERNET_NATIVE_STATUS_OK && received != count) {
        status = AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
    for (int index = 0;
         status == AETHERNET_NATIVE_STATUS_OK && index < received;
         index++) {
        status = aethernet_native_deadline_status(deadline_monotonic_ms);
        if (status != AETHERNET_NATIVE_STATUS_OK) break;
        int found = 0;
        status = aethernet_heif_auxiliary_is_gain_map(
...
int aethernet_heif_checked_rgba_size(
    int width,
    int height,
    size_t *out_size
) {
...
struct aethernet_heif_color_profile {
    int transfer;
    int primaries;
    int matrix;
    int nclx_present;
...
static int aethernet_heif_read_color_profile(
    struct heif_image_handle *handle,
    long long deadline_monotonic_ms,
    struct aethernet_heif_color_profile *profile
) {
    assert(handle != NULL);
    assert(profile != NULL);
    ...
    if (profile->icc_size > AETHERNET_HEIF_ICC_PROFILE_BYTES_MAX) {
        return AETHERNET_NATIVE_STATUS_WORK_LIMIT_EXCEEDED;
    }
    ...
    if (nclx == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
...
static int aethernet_heif_apply_icc_profile(
    struct heif_image_handle *handle,
    uint8_t *destination,
    int width,
    int height,
    ...
    assert(profile_size <= AETHERNET_HEIF_ICC_PROFILE_BYTES_MAX);
    int status = aethernet_native_deadline_status(deadline_monotonic_ms);
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    uint8_t *profile_bytes = malloc(profile_size);
    if (profile_bytes == NULL) return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
    struct heif_error error = heif_image_handle_get_raw_color_profile(
        handle, profile_bytes);
    status = aethernet_native_deadline_status(deadline_monotonic_ms);
    if (status == AETHERNET_NATIVE_STATUS_OK && error.code != heif_error_Ok) {
        status = aethernet_native_status_from_heif_error(error);
    }
    ...
    if (status == AETHERNET_NATIVE_STATUS_OK) {
        ...
        if (transform == NULL) status = AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
    if (status == AETHERNET_NATIVE_STATUS_OK) {
        status = aethernet_native_deadline_status(deadline_monotonic_ms);
    }
    ...
    if (status == AETHERNET_NATIVE_STATUS_OK) {
        ...
        if (source_row == NULL) status = AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
    }
    for (int row = 0;
         status == AETHERNET_NATIVE_STATUS_OK && row < height;
         row++) {
        if (row % AETHERNET_HEIF_DEADLINE_ROWS == 0) {
            status = aethernet_native_deadline_status(deadline_monotonic_ms);
            if (status != AETHERNET_NATIVE_STATUS_OK) break;
        }
...
static int aethernet_heif_nclx_gamut(int primaries, int *out_gamut) {
    assert(out_gamut != NULL);
    switch (primaries) {
        case heif_color_primaries_ITU_R_BT_709_5:
            *out_gamut = AETHERNET_HDR_GAMUT_SRGB;
            return AETHERNET_NATIVE_STATUS_OK;
        case heif_color_primaries_ITU_R_BT_2020_2_and_2100_0:
            *out_gamut = AETHERNET_HDR_GAMUT_BT2020;
            return AETHERNET_NATIVE_STATUS_OK;
        case heif_color_primaries_SMPTE_EG_432_1:
            *out_gamut = AETHERNET_HDR_GAMUT_DISPLAY_P3;
            return AETHERNET_NATIVE_STATUS_OK;
        default:
...
static int aethernet_heif_nclx_transfer(int transfer, int *out_transfer) {
    assert(out_transfer != NULL);
    switch (transfer) {
        case heif_transfer_characteristic_IEC_61966_2_1:
            *out_transfer = AETHERNET_HDR_TRANSFER_SRGB;
            return AETHERNET_NATIVE_STATUS_OK;
        ...
        case heif_transfer_characteristic_ITU_R_BT_2020_2_10bit:
            *out_transfer = AETHERNET_HDR_TRANSFER_BT709;
            return AETHERNET_NATIVE_STATUS_OK;
        case heif_transfer_characteristic_ITU_R_BT_2020_2_12bit:
            *out_transfer = AETHERNET_HDR_TRANSFER_BT2020_12;
            return AETHERNET_NATIVE_STATUS_OK;
        case heif_transfer_characteristic_linear:
            *out_transfer = AETHERNET_HDR_TRANSFER_LINEAR;
            return AETHERNET_NATIVE_STATUS_OK;
        default:
            return AETHERNET_NATIVE_STATUS_UNSUPPORTED;
    }
...
static int aethernet_heif_apply_sdr_nclx(
    uint8_t *destination,
    int width,
    int height,
    const struct aethernet_heif_color_profile *profile,
    long long deadline_monotonic_ms
) {
    assert(destination != NULL);
    assert(profile != NULL);
    int gamut = AETHERNET_HDR_GAMUT_SRGB;
    int status = aethernet_heif_nclx_gamut(profile->primaries, &gamut);
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    int transfer = AETHERNET_HDR_TRANSFER_SRGB;
    status = aethernet_heif_nclx_transfer(profile->transfer, &transfer);
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    return aethernet_hdr_apply_sdr_gamut(
        destination, width, height, gamut, transfer,
        AETHERNET_HEIF_DEADLINE_ROWS, deadline_monotonic_ms);
}

static int aethernet_heif_cancel_decoding(void *opaque) {
...
static int aethernet_heif_decode_interleaved(
    struct heif_image_handle *handle,
    ...
    *out_image = NULL;
    ...
    if (options == NULL) return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
    options->progress_user_data = &deadline_monotonic_ms;
    options->cancel_decoding = aethernet_heif_cancel_decoding;
    ...
    if (profile_error.code == heif_error_Ok) {
        ...
        heif_decoding_options_free(options);
        return aethernet_native_status_from_heif_error(profile_error);
    }
    ...
    if (*out_image == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    return AETHERNET_NATIVE_STATUS_OK;
}

static int aethernet_heif_interleaved_plane(
...
static int aethernet_heif_decode_sdr(
    struct heif_image_handle *handle,
    uint8_t *destination,
    int width,
    int height,
    ...
    int status = aethernet_heif_decode_interleaved(
...
static int aethernet_heif_decode_hdr(
    ...
    if (profile->transfer == heif_transfer_characteristic_ITU_R_BT_2100_0_PQ) {
        transfer = AETHERNET_HDR_TRANSFER_PQ;
    ...
    int gamut = AETHERNET_HDR_GAMUT_SRGB;
    if (aethernet_heif_nclx_gamut(profile->primaries, &gamut) !=
        AETHERNET_NATIVE_STATUS_OK) {
    ...
    struct heif_image *image = NULL;
    status = aethernet_heif_decode_interleaved(
    ...
    if (status == AETHERNET_NATIVE_STATUS_OK) {
        status = aethernet_hdr_tone_map_rgba16(
...
int aethernet_heif_decode_to_sdr_rgba8(
    struct heif_image_handle *handle,
    uint8_t *destination,
    ...
    if (aethernet_heif_checked_rgba_size(width, height, &expected_size) != 0 ||
        destination_capacity < expected_size) {
        return AETHERNET_NATIVE_STATUS_INVALID_DIMENSIONS;
    }
    struct aethernet_heif_color_profile profile;
    int status = aethernet_heif_read_color_profile(
    ...
    if (!is_hdr) {
        ...
        return aethernet_heif_decode_sdr(
            handle, destination, width, height,
            deadline_monotonic_ms, &profile);
    }
    status = aethernet_heif_decode_hdr(
        handle, destination, width, height,
        deadline_monotonic_ms, &profile);
    if (status == AETHERNET_NATIVE_STATUS_OK && out_was_hdr != NULL) {
        *out_was_hdr = 1;
    }
    return status;
}
