...
#define AETHERNET_WEBP_MAX_NON_FRAME_CHUNKS ((size_t)64)
...
int aethernet_configure_webp_encoder(
    ...
    if (config == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (!WebPConfigInit(config)) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (lossless) {
...
static int webp_animation_structure_status(
    ...
    *frame_count = 0;
    ...
    size_t chunk_limit = (size_t)max_frames +
                         AETHERNET_WEBP_MAX_NON_FRAME_CHUNKS;
    ...
    while (offset < webp_len) {
        ...
        if (remaining < 8) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
        ...
        if (memcmp(data + offset, "ANMF", 4) == 0) {
            if (chunk_size < 16) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    ...
    if (*frame_count < 2) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
...
static int webp_animation_canvas_status(
    ...
    if (max_total_pixels == 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
...
int aethernet_webp_animation_preflight(
    ...
    int structure_status = webp_animation_structure_status(
        ...
        &frame_count);
    if (structure_status != AETHERNET_NATIVE_STATUS_OK) return structure_status;
    ...
    if (features_status != VP8_STATUS_OK) {
        return features_status == VP8_STATUS_OUT_OF_MEMORY
             ? AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED
             : AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
    if (!features.has_animation) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    int canvas_status = webp_animation_canvas_status(
        (uint32_t)features.width, (uint32_t)features.height,
        max_total_pixels);
    if (canvas_status != AETHERNET_NATIVE_STATUS_OK) return canvas_status;
    size_t frame_pixels =
...
int aethernet_vips_read_delays_ms(VipsImage *image, int n_pages, int **out_delays, int *out_len) {
    ...
    if (out_delays == NULL || out_len == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    int delays_status = aethernet_vips_read_animation_delays_ms(
        image, n_pages, out_delays);
    if (delays_status != AETHERNET_NATIVE_STATUS_OK) return delays_status;
    *out_len = n_pages;
    return AETHERNET_NATIVE_STATUS_OK;
}
...
static int aethernet_webp_add_vips_animation_frame(
    ...
    if (encoder == NULL || image == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    ...
    size_t rgba_size = 0;
    int extract_status = aethernet_vips_extract_animation_rgba_strip(
        strip, scratch, scratch_cap, deadline_monotonic_ms, &rgba_size);
    g_object_unref(strip);
    if (extract_status != AETHERNET_NATIVE_STATUS_OK) return extract_status;
...
static int aethernet_webp_vips_animation_frame_budget(
    const int *delays,
    int n_pages,
    int max_frames,
    int max_duration_ms,
    ...
    *out_frames = 0;
    ...
    for (int i = 0; i < n_pages; i++) {
        ...
        int delay = delays[i];
        if (delay <= 0) return AETHERNET_NATIVE_STATUS_WORK_LIMIT_EXCEEDED;
    ...
    if (frames <= 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    *out_frames = frames;
    return AETHERNET_NATIVE_STATUS_OK;
}

static int aethernet_webp_encode_vips_animation_frames(
    struct aethernet_webp_animation_encoder *encoder,
    VipsImage *image,
    int width,
    int page_height,
    ...
    for (int i = 0; i < n_pages; i++) {
        ...
        int delay = delays[i];
        int add_status = aethernet_webp_add_vips_animation_frame(
...
struct aethernet_webp_vips_encode_request {
    VipsImage *image;
    int quality;
    int lossless;
    int effort;
...
struct aethernet_webp_vips_encode_context {
    WebPConfig config;
    struct aethernet_webp_animation_encoder *encoder;
    int *delays;
    unsigned char *scratch;
...
static int aethernet_webp_vips_encode_request_status(
    const struct aethernet_webp_vips_encode_request *request
) {
...
static int aethernet_webp_vips_encode_geometry(
    struct aethernet_webp_vips_encode_context *context,
    VipsImage *image
) {
    assert(context != NULL);
    assert(image != NULL);
    ...
    if (pixels > AETHERNET_MAX_VIDEO_RGBA_BYTES / 4u) {
        return AETHERNET_NATIVE_STATUS_INVALID_DIMENSIONS;
    }
    context->width = width;
...
static void aethernet_webp_vips_encode_context_clear(
    struct aethernet_webp_vips_encode_context *context
) {
    if (context == NULL) return;
    aethernet_webp_animation_encoder_delete(context->encoder);
    free(context->scratch);
    free(context->delays);
    memset(context, 0, sizeof(*context));
}

static int aethernet_webp_vips_encode_context_open(
    struct aethernet_webp_vips_encode_context *context,
    const struct aethernet_webp_vips_encode_request *request
) {
    assert(context != NULL);
    assert(request != NULL);
    int status = aethernet_webp_vips_encode_geometry(context, request->image);
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    status = aethernet_vips_read_animation_delays_ms(
        request->image, context->n_pages, &context->delays);
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    status = aethernet_webp_vips_animation_frame_budget(
    ...
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    status = aethernet_configure_webp_encoder(
    ...
    struct aethernet_webp_animation_encoder_settings settings = {
        ...
        .pixel_layout = AETHERNET_WEBP_PIXEL_LAYOUT_RGBA,
        .deadline_monotonic_ms = request->limits->deadline_monotonic_ms,
        .max_output_size = request->max_output_size,
    };
    return aethernet_webp_animation_encoder_create(
        &settings, &context->encoder);
}

static int aethernet_webp_vips_encode_context_run(
    struct aethernet_webp_vips_encode_context *context,
    const struct aethernet_webp_vips_encode_request *request
) {
    assert(context != NULL);
    ...
    assert(request != NULL);
    int status = aethernet_webp_encode_vips_animation_frames(
    ...
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    return aethernet_webp_animation_encoder_finish(
...
int aethernet_webp_encode_animated(
    ...
    struct aethernet_webp_vips_encode_request request = {
        .image = image,
        .quality = quality,
        .lossless = lossless,
        .effort = effort,
    ...
    int status = aethernet_webp_vips_encode_request_status(&request);
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    struct aethernet_webp_vips_encode_context context = {0};
    status = aethernet_webp_vips_encode_context_open(&context, &request);
    if (status == AETHERNET_NATIVE_STATUS_OK) {
        status = aethernet_webp_vips_encode_context_run(&context, &request);
    }
    aethernet_webp_vips_encode_context_clear(&context);
    return status;
}
