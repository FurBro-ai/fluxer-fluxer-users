...
static int gif_filter_create_source(
    ...
    int written = snprintf(
        ...
        time_base.num,
        time_base.den);
    if (written < 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    ...
    if (filter == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    int av_status = avfilter_graph_create_filter(
        out_source, filter, "in", arguments, NULL, graph);
    if (av_status < 0) return aethernet_native_status_from_av_error(av_status);
...
static int gif_filter_create_sink(
    ...
    if (sink == NULL) return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
...
static int gif_filter_create_endpoint(
    ...
    if (endpoint == NULL) return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
...
static int gif_filter_connect(
    ...
    if (status == AETHERNET_NATIVE_STATUS_OK) {
        ...
        if (av_status < 0) {
            status = aethernet_native_status_from_av_error(av_status);
        }
    }
...
int aethernet_gif_setup_filter_graph(
    ...
    if (out_graph == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (out_source == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (out_sink == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (source_width <= 0) return AETHERNET_NATIVE_STATUS_INVALID_DIMENSIONS;
    if (source_height <= 0) return AETHERNET_NATIVE_STATUS_INVALID_DIMENSIONS;
    if (target_width <= 0) return AETHERNET_NATIVE_STATUS_INVALID_DIMENSIONS;
    if (target_height <= 0) return AETHERNET_NATIVE_STATUS_INVALID_DIMENSIONS;
    if (frame_time_base.num <= 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (frame_time_base.den <= 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    *out_graph = NULL;
    *out_source = NULL;
    *out_sink = NULL;

    AVFilterGraph *graph = avfilter_graph_alloc();
    if (graph == NULL) return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
    graph->nb_threads = 1;
