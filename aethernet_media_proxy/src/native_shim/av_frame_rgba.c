...
static int ff_frame_color_transfer(const AVFrame *frame, int *out_transfer) {
    ...
    switch (frame->color_trc) {
        ...
        case AVCOL_TRC_SMPTE2084:
            *out_transfer = AETHERNET_HDR_TRANSFER_PQ;
            return 0;
        case AVCOL_TRC_ARIB_STD_B67:
            *out_transfer = AETHERNET_HDR_TRANSFER_HLG;
            return 0;
        default:
            return -1;
    }
...
static int ff_frame_plan_is_passthrough(
    ...
    assert(plan != NULL);
    return plan->gamut == AETHERNET_HDR_GAMUT_SRGB &&
           !aethernet_hdr_transfer_is_hdr(plan->transfer) &&
           plan->transfer != AETHERNET_HDR_TRANSFER_LINEAR;
}
...
static int ff_convert_i420_frame_to_rgba_libyuv(
    ...
    *out_applied = 1;
    for (int row = 0; row < output_height;) {
        ...
        if (rows > AETHERNET_VIDEO_DEADLINE_ROWS) {
            rows = AETHERNET_VIDEO_DEADLINE_ROWS;
        }
        ...
        int convert_rc = convert(
            ...
            output_width, rows);
        if (convert_rc != 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
        row += rows;
    }
...
static int ff_swscale_validate_source_layout(
    ...
    if (chroma_alignment > AETHERNET_VIDEO_DEADLINE_ROWS) {
        return AETHERNET_NATIVE_STATUS_UNSUPPORTED;
    }
...
static int ff_swscale_frame_to_rgba(
    ...
    assert(dst != NULL);
    const AVPixFmtDescriptor *descriptor = av_pix_fmt_desc_get(frame->format);
    if (descriptor == NULL) return AETHERNET_NATIVE_STATUS_UNSUPPORTED;
    ...
    if (layout.is_bayer) {
        if (source_height < 2) return AETHERNET_NATIVE_STATUS_UNSUPPORTED;
    }
    ...
    for (int source_row = 0; source_row < source_height;) {
        ...
        if (source_rows > AETHERNET_VIDEO_DEADLINE_ROWS) {
            source_rows = AETHERNET_VIDEO_DEADLINE_ROWS;
        }
        ...
        if (source_rows <= 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
        ...
        if (scaled_rows < 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
...
static int ff_convert_hdr_frame_to_rgba(
    ...
    assert(dst != NULL);
    assert(plan != NULL);
    if (rgba_size > AETHERNET_MAX_VIDEO_RGBA_BYTES / 2u) {
        return AETHERNET_NATIVE_STATUS_INVALID_DIMENSIONS;
    }
    uint8_t *staging = (uint8_t *)g_try_malloc(rgba_size * 2u);
    if (staging == NULL) return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
    *sws = sws_getCachedContext(*sws,
    ...
    int status = AETHERNET_NATIVE_STATUS_OK;
    if (*sws == NULL) {
    ...
    if (status == AETHERNET_NATIVE_STATUS_OK) {
        status = aethernet_hdr_tone_map_rgba16(
            ...
            plan->gamut, plan->transfer, AETHERNET_VIDEO_DEADLINE_ROWS,
...
int aethernet_av_frame_convert_to_rgba(
    AVFrame *frame,
    int source_width,
    int source_height,
    int output_width,
    ...
    if (aethernet_hdr_transfer_is_hdr(plan.transfer)) {
    ...
    if (!libyuv_applied) {
        *sws = sws_getCachedContext(*sws,
                                    ...
                                    AV_PIX_FMT_RGBA,
                                    SWS_FAST_BILINEAR, NULL, NULL, NULL);
        if (*sws == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    ...
    if (ff_frame_plan_is_passthrough(&plan)) return AETHERNET_NATIVE_STATUS_OK;
    return aethernet_hdr_apply_sdr_gamut(
        dst, output_width, output_height, plan.gamut, plan.transfer,
        AETHERNET_VIDEO_DEADLINE_ROWS, deadline_monotonic_ms);
}
