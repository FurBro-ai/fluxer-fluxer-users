...
static int aethernet_monotonic_ms(long long *out_monotonic_ms) {
...
int aethernet_monotonic_deadline_status(long long deadline_monotonic_ms) {
    ...
    if (deadline_monotonic_ms == 0) return AETHERNET_DEADLINE_PENDING;
    long long now_monotonic_ms = 0;
    if (aethernet_monotonic_ms(&now_monotonic_ms) != 0) {
        return AETHERNET_DEADLINE_CLOCK_FAILED;
    }
    return now_monotonic_ms >= deadline_monotonic_ms
        ? AETHERNET_DEADLINE_REACHED
        : AETHERNET_DEADLINE_PENDING;
}

int aethernet_native_deadline_status(long long deadline_monotonic_ms) {
    assert(deadline_monotonic_ms >= 0);
    return aethernet_monotonic_deadline_status(deadline_monotonic_ms) ==
                   AETHERNET_DEADLINE_PENDING
...
int aethernet_ffmpeg_decoder_threads_valid(int decoder_threads) {
    return decoder_threads >= 1 && decoder_threads <= AETHERNET_MAX_THREADS_PER_PIPELINE;
}

int aethernet_native_status_from_av_error(int error) {
    if (error == AVERROR(ENOMEM)) return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
    if (error == AVERROR(ENOSPC)) return AETHERNET_NATIVE_STATUS_OUTPUT_LIMIT_EXCEEDED;
    if (error == AVERROR(E2BIG)) return AETHERNET_NATIVE_STATUS_WORK_LIMIT_EXCEEDED;
...
int aethernet_native_status_from_av_error_with_deadline(
    ...
    return aethernet_native_status_from_av_error(error);
}
...
static int aethernet_video_codec_id_allowed(enum AVCodecID id) {
    switch (id) {
...
int aethernet_prepare_untrusted_av_input(AVFormatContext *format) {
    if (format == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (format->nb_streams > AETHERNET_MAX_AV_STREAMS) {
        return AETHERNET_NATIVE_STATUS_WORK_LIMIT_EXCEEDED;
    }
    ...
    for (unsigned int i = 0; i < format->nb_streams; i++) {
        ...
        if (type == AVMEDIA_TYPE_VIDEO && !ff_stream_is_attached_picture(stream)) {
            if (id != AV_CODEC_ID_NONE && !aethernet_video_codec_id_allowed(id)) {
                return AETHERNET_NATIVE_STATUS_UNSUPPORTED;
            }
...
static int aethernet_deny_child_av_io(AVFormatContext *format, AVIOContext **io,
...
int aethernet_restrict_untrusted_av_context(AVFormatContext *format,
    ...
    format->io_open = aethernet_deny_child_av_io;
    format->format_whitelist = av_strdup(format_whitelist);
    format->protocol_whitelist = av_strdup("");
    format->codec_whitelist = av_strdup(AETHERNET_ALLOWED_VIDEO_DECODERS);
    format->max_streams = AETHERNET_MAX_AV_STREAMS;
...
int aethernet_find_stream_info_bounded(AVFormatContext *format) {
    ...
    if (stream_count > AETHERNET_MAX_AV_STREAMS) return AVERROR(E2BIG);
    ...
    for (unsigned int i = 0; i < stream_count; i++) {
        ...
        if (av_dict_set_int(&options[i], "max_pixels",
                            (int64_t)AETHERNET_MAX_VIDEO_PIXELS, 0) < 0 ||
        ...
        if (stream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO &&
            av_dict_set(&options[i], "codec_whitelist",
                        AETHERNET_ALLOWED_VIDEO_DECODERS, 0) < 0) {
            rc = AVERROR(ENOMEM);
            break;
        }
    }
...
int aethernet_video_decoder_allowed(const AVStream *stream, const AVCodec *decoder) {
    ...
    return aethernet_video_codec_id_allowed(decoder->id);
}
...
int ff_validate_rgba_geometry(int width, int height, size_t *out_size) {
    ...
    if (rgba_size == 0 || rgba_size > AETHERNET_MAX_VIDEO_RGBA_BYTES) return -1;
...
int ff_find_primary_video_stream(
    ...
    *out_codec = NULL;
    ...
    for (unsigned int i = 0; i < format->nb_streams; i++) {
        ...
        if (!aethernet_video_decoder_allowed(stream, codec)) continue;
