...
#define AETHERNET_ANIMATION_AVIO_BUFFER_SIZE (64u * 1024u)
#define AETHERNET_ANIMATION_PROBE_SIZE (5u * 1024u * 1024u)
#define AETHERNET_ANIMATION_ANALYZE_DURATION (5 * AV_TIME_BASE)
#define AETHERNET_VIPS_RGBA_REGION_ROWS 64
...
static int ff_animation_frame_delay_ms(AVFrame *frame, AVStream *stream,
    ...
    *out_delay = 0;
    ...
    if (duration <= 0) {
        *out_delay = AETHERNET_DEFAULT_ANIMATION_FRAME_DELAY_MS;
    ...
    if (delay < AETHERNET_MIN_ANIMATION_FRAME_DELAY_MS) {
        delay = AETHERNET_MIN_ANIMATION_FRAME_DELAY_MS;
    }
    *out_delay = delay;
    return AETHERNET_NATIVE_STATUS_OK;
}
...
static int ff_animation_reserve_frames(
    ...
    if (context->capacity > context->frames) return AETHERNET_NATIVE_STATUS_OK;
    size_t frame_pixels = frame_bytes / 4u;
    ...
    if (new_pixels == NULL) return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
    context->pixels = new_pixels;
    int *new_delays = realloc(
        context->delays, (size_t)new_capacity * sizeof(int));
    if (new_delays == NULL) return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
...
static int ff_animation_accept_frame(
    ...
    int convert_status = aethernet_av_frame_convert_to_rgba(
        context->frame,
        width,
        height,
        width,
        ...
        context->request.deadline_monotonic_ms,
        destination);
    if (convert_status != AETHERNET_NATIVE_STATUS_OK) return convert_status;
    int delay = 0;
...
static int ff_animation_decode_request_valid(
    ...
    assert(request != NULL);
    if (request->data == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (request->len == 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (request->len > (size_t)INT64_MAX) {
        return AETHERNET_NATIVE_STATUS_WORK_LIMIT_EXCEEDED;
    }
    if (!aethernet_ffmpeg_decoder_threads_valid(request->decoder_threads)) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
    if (request->deadline_monotonic_ms < 0) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
    if (request->max_frames <= 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (request->max_total_pixels == 0) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
    if (request->require_complete < 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (request->require_complete > 1) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
...
static int ff_animation_open_input(
    ...
    context->input_avio_buffer = av_malloc(
        AETHERNET_ANIMATION_AVIO_BUFFER_SIZE);
    ...
    context->input_avio = avio_alloc_context(
        ...
        (int)AETHERNET_ANIMATION_AVIO_BUFFER_SIZE,
        0,
        &context->reader,
        ff_mem_read_packet,
        NULL,
    ...
    int status = aethernet_restrict_untrusted_av_context(
    ...
    status = aethernet_prepare_untrusted_av_input(context->input_format);
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    av_status = aethernet_find_stream_info_bounded(context->input_format);
    if (av_status < 0) {
        return aethernet_native_status_from_av_error_with_deadline(
            av_status, context->request.deadline_monotonic_ms);
    }
    return aethernet_prepare_untrusted_av_input(context->input_format);
}
...
static int ff_animation_validate_stream_limits(
    ...
    if (context->expected_frames == 0) {
        ...
        if (stream_frames < 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    ...
    context->packet_limit =
        packet_frame_limit * AETHERNET_ANIMATION_PACKETS_PER_FRAME_LIMIT;
    return AETHERNET_NATIVE_STATUS_OK;
}
...
static int ff_animation_open_decoder(
    ...
    if (stream_index < 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    ...
    if (!aethernet_video_decoder_allowed(context->input_stream, decoder)) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }
...
static int ff_animation_receive_frames(
    ...
    while (1) {
        ...
        if (receive_status == AVERROR_EOF) return AETHERNET_NATIVE_STATUS_OK;
        if (receive_status == AVERROR(EAGAIN)) {
            if (flushing != 0) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
            return AETHERNET_NATIVE_STATUS_OK;
        }
        if (receive_status < 0) {
            return aethernet_native_status_from_av_error_with_deadline(
...
static int ff_animation_decode_packets(
    ...
    if (context->stopped) return AETHERNET_NATIVE_STATUS_OK;
...
static int ffmpeg_decode_animation_stack(
    ...
    if (out_frame_count == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    *out_frame_count = 0;
    if (decode_pixels != 0) {
...
int aethernet_ffmpeg_decode_apng(
    ...
    if (out == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    *out = NULL;
    if (out_num_plays == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    *out_num_plays = 0;
    int expected_frames = 0;
    uint32_t num_plays = 0;
    int validate_rc = aethernet_validate_complete_apng(
        apng_data, apng_len,
...
int aethernet_ffmpeg_decode_bmp(
    ...
    *out = NULL;
    const AVInputFormat *bmp_format =
        av_find_input_format(AETHERNET_AV_BMP_INPUT_FORMAT);
    if (bmp_format == NULL) return AETHERNET_NATIVE_STATUS_UNSUPPORTED;
    int decoded_frames = 0;
    int status = ffmpeg_decode_animation_stack(
        bmp_data, bmp_len, decoder_threads, deadline_monotonic_ms,
        out, &decoded_frames, 1, max_total_pixels, 1,
        AETHERNET_AV_BMP_INPUT_FORMAT, bmp_format, 1, 1, 1);
    if (status == AETHERNET_NATIVE_STATUS_OK) {
        assert(*out != NULL);
...
int aethernet_ffmpeg_count_heif_sequence_frames(
    ...
    if (out_frame_count == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    *out_frame_count = 0;
    int validation_status = aethernet_heif_validate(
        heif_data, heif_len, deadline_monotonic_ms);
    if (validation_status != AETHERNET_NATIVE_STATUS_OK) return validation_status;
    int status = ffmpeg_decode_animation_stack(
        heif_data, heif_len, decoder_threads, deadline_monotonic_ms,
        NULL, out_frame_count, max_frames, max_total_pixels,
        1, AETHERNET_AV_INPUT_FORMATS, NULL, 0, 2, 0);
    if (status == AETHERNET_NATIVE_STATUS_OK) {
...
int aethernet_ffmpeg_decode_heif_sequence(
    ...
    *out = NULL;
    ...
    int status = ffmpeg_decode_animation_stack(
        ...
        1, AETHERNET_AV_INPUT_FORMATS, NULL, 0, 2, 1);
    if (status == AETHERNET_NATIVE_STATUS_OK) {
        assert(*out != NULL);
        assert(*out_frame_count >= 2);
    } else {
...
static int aethernet_vips_copy_animation_rgba(
    VipsRegion *region,
    uint8_t *destination,
    size_t row_bytes,
    int width,
    ...
    for (int top = 0; top < height; top += AETHERNET_VIPS_RGBA_REGION_ROWS) {
        if (aethernet_monotonic_deadline_status(deadline_monotonic_ms) !=
            AETHERNET_DEADLINE_PENDING) {
            return AETHERNET_NATIVE_STATUS_DEADLINE_EXCEEDED;
        }
        int rows = height - top;
        if (rows > AETHERNET_VIPS_RGBA_REGION_ROWS) rows = AETHERNET_VIPS_RGBA_REGION_ROWS;
...
int aethernet_vips_extract_animation_rgba_strip(
    VipsImage *input,
    uint8_t *destination,
    ...
    if (input == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (destination == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (out_size == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (deadline_monotonic_ms < 0) {
        return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    }

    VipsImage *rgba = NULL;
    int status = aethernet_vips_image_to_rgba(input, &rgba);
    if (status != AETHERNET_NATIVE_STATUS_OK) return status;
    ...
    status = aethernet_vips_copy_animation_rgba(
...
int aethernet_vips_read_animation_delays_ms(VipsImage *image, int n_pages,
                                         int **out_delays) {
    if (out_delays == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    *out_delays = NULL;
    if (image == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    if (n_pages <= 0) return AETHERNET_NATIVE_STATUS_INVALID_DIMENSIONS;
    ...
    if (vips_image_get_typeof(image, "delay") != 0) {
        ...
        if (arr == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
        if (n != n_pages) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
        int *out = (int *)malloc((size_t)n_pages * sizeof(int));
        if (out == NULL) return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
        for (int i = 0; i < n_pages; i++) {
            if (arr[i] <= 0) {
                out[i] = AETHERNET_DEFAULT_ANIMATION_FRAME_DELAY_MS;
            } else if (arr[i] < AETHERNET_MIN_ANIMATION_FRAME_DELAY_MS) {
                out[i] = AETHERNET_MIN_ANIMATION_FRAME_DELAY_MS;
            } else {
                out[i] = arr[i];
            }
        }
    ...
    if (gif_delay > INT_MAX / 10) return AETHERNET_NATIVE_STATUS_WORK_LIMIT_EXCEEDED;
    int *out = (int *)malloc((size_t)n_pages * sizeof(int));
    if (out == NULL) return AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
    int delay_ms = gif_delay > 0 ? gif_delay * 10 : 0;
    if (delay_ms <= 0) {
        delay_ms = AETHERNET_DEFAULT_ANIMATION_FRAME_DELAY_MS;
    } else if (delay_ms < AETHERNET_MIN_ANIMATION_FRAME_DELAY_MS) {
        delay_ms = AETHERNET_MIN_ANIMATION_FRAME_DELAY_MS;
    }
