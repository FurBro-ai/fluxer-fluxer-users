...
static void aethernet_clamp_heif_u64(uint64_t *value, uint64_t limit) {
    if (*value == 0 || *value > limit) *value = limit;
}

static void aethernet_clamp_heif_u32(uint32_t *value, uint32_t limit) {
    if (*value == 0 || *value > limit) *value = limit;
}

static int aethernet_set_heif_security_limits(struct heif_context *ctx,
                                           size_t max_total_pixels) {
    if (ctx == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    ...
    uint64_t pixel_limit = max_total_pixels > 0
                         ...
                         : (uint64_t)AETHERNET_MAX_VIDEO_PIXELS;
    aethernet_clamp_heif_u64(&limits->max_image_size_pixels, pixel_limit);
    aethernet_clamp_heif_u64(&limits->max_number_of_tiles, 4096);
    aethernet_clamp_heif_u32(&limits->max_bayer_pattern_pixels, 4096);
    aethernet_clamp_heif_u32(&limits->max_items, 4096);
    aethernet_clamp_heif_u32(&limits->max_color_profile_size, 4u * 1024u * 1024u);
    aethernet_clamp_heif_u64(&limits->max_memory_block_size, 512u * 1024u * 1024u);
    aethernet_clamp_heif_u32(&limits->max_components, 64);
    aethernet_clamp_heif_u32(&limits->max_iloc_extents_per_item, 4096);
    aethernet_clamp_heif_u32(&limits->max_size_entity_group, 4096);
    aethernet_clamp_heif_u32(&limits->max_children_per_box, 65536);
    return AETHERNET_NATIVE_STATUS_OK;
}

#define AETHERNET_BMFF_MAX_DEPTH 32u
#define AETHERNET_BMFF_MAX_BOXES 65536u
#define AETHERNET_BMFF_MAX_IINF_ENTRIES 16384u
#define AETHERNET_BMFF_MAX_ILOC_ITEMS 4096u
#define AETHERNET_BMFF_MAX_ILOC_EXTENTS_PER_ITEM 4096u
#define AETHERNET_BMFF_MAX_ILOC_EXTENTS_TOTAL 65536u
...
typedef struct {
    const uint8_t *data;
    size_t data_len;
    bmff_frame frames[AETHERNET_BMFF_MAX_DEPTH];
    size_t depth;
    size_t *box_count;
...
static int bmff_cursor_next(bmff_cursor *cursor, bmff_box *out) {
    ...
    while (cursor->depth > 0) {
        ...
        if (*cursor->box_count >= AETHERNET_BMFF_MAX_BOXES) return -1;
        ...
        if (container_kind != 0) {
            ...
            if (child_start < out->payload_end) {
                if (cursor->depth >= AETHERNET_BMFF_MAX_DEPTH) return -1;
...
static int bmff_parse_iinf(const uint8_t *payload, size_t len,
    ...
    if (*iinf_entry_count > AETHERNET_BMFF_MAX_IINF_ENTRIES) return -1;
    if ((size_t)entry_count > AETHERNET_BMFF_MAX_IINF_ENTRIES - *iinf_entry_count) return -1;
    ...
    for (uint32_t index = 0; index < entry_count; index++) {
        ...
        if (*box_count >= AETHERNET_BMFF_MAX_BOXES) return -1;
        bmff_box box;
...
static int bmff_iloc_validation_init(
    ...
    *validation = (bmff_iloc_validation) {
    ...
    return *item_count <= AETHERNET_BMFF_MAX_ILOC_ITEMS ? 0 : -1;
}
...
static int bmff_validate_iloc_item(
    ...
    if (extent_count > AETHERNET_BMFF_MAX_ILOC_EXTENTS_PER_ITEM ||
        (size_t)extent_count > AETHERNET_BMFF_MAX_ILOC_EXTENTS_TOTAL -
...
static int aethernet_heif_validate_with_tmap(const void *buf, size_t len,
                                          long long deadline_monotonic_ms,
                                          int *has_tmap_item) {
    ...
    if (status == 0) return AETHERNET_NATIVE_STATUS_OK;
    if (status == AETHERNET_NATIVE_STATUS_DEADLINE_EXCEEDED) return status;
    return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
}
...
int aethernet_heif_validate(
    ...
    int has_tmap_item = 0;
    return aethernet_heif_validate_with_tmap(
        buf, len, deadline_monotonic_ms, &has_tmap_item);
}
...
static int heif_primary_still_context_open(
    ...
    status = aethernet_set_heif_security_limits(
    ...
    if (context->handle == NULL) return AETHERNET_NATIVE_STATUS_CODEC_FAILURE;
    return AETHERNET_NATIVE_STATUS_OK;
}
...
static int heif_primary_still_geometry(
    ...
    size_t pixels_len = 0;
    if (aethernet_heif_checked_rgba_size(width, height, &pixels_len) != 0) {
        return AETHERNET_NATIVE_STATUS_INVALID_DIMENSIONS;
    }
...
static int decode_heif_primary_still_pixels(
    ...
    if (status == AETHERNET_NATIVE_STATUS_OK) {
        status = aethernet_heif_detect_hdr_gain_map(
            context.handle, deadline_monotonic_ms, &gain_map_detected);
    }
    if (status == AETHERNET_NATIVE_STATUS_OK) {
        pixels = g_try_malloc(pixels_len);
        if (pixels == NULL) status = AETHERNET_NATIVE_STATUS_ALLOCATION_FAILED;
    }
    int hdr_tone_mapped = 0;
    if (status == AETHERNET_NATIVE_STATUS_OK) {
        status = aethernet_heif_decode_to_sdr_rgba8(
...
int aethernet_heif_decode_primary_still(
    ...
    int has_tmap_item = 0;
    int validate_status = aethernet_heif_validate_with_tmap(
        buf, len, deadline_monotonic_ms, &has_tmap_item);
    if (validate_status != AETHERNET_NATIVE_STATUS_OK) return validate_status;

    heif_primary_still_pixels decoded;
    int decode_status = decode_heif_primary_still_pixels(
        buf, len, max_pixels, max_dimension,
        deadline_monotonic_ms, &decoded);
    if (decode_status != AETHERNET_NATIVE_STATUS_OK) return decode_status;
    int deadline_status = aethernet_native_deadline_status(
        deadline_monotonic_ms);
    if (deadline_status != AETHERNET_NATIVE_STATUS_OK) {
