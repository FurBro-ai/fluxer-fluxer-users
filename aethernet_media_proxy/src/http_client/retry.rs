...
mod tests {
    ...
    fn transient_status_classifications_are_counted_once_and_fatal_ones_are_not() {
        ...
        assert_eq!(
            ...
            "aethernet_media_proxy_http_retryable_classifications_total{reason=\"status\"} 1"
        );
    }
    ...
    fn every_retry_decision_reaches_the_injected_metrics_handle() {
        ...
        let rendered = metrics.render();
        assert_eq!(
            counter_line(&rendered, "aethernet_media_proxy_http_retries_total"),
            "aethernet_media_proxy_http_retries_total 1"
        );
        assert_eq!(
            counter_line(&rendered, "aethernet_media_proxy_http_retries_exhausted_total"),
            "aethernet_media_proxy_http_retries_exhausted_total 1"
        );
    }
}
