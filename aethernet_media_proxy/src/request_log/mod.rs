...
mod tests {
    ...
    async fn middleware_records_metrics_and_reads_error_reason() {
        ...
        assert!(rendered.contains("aethernet_media_proxy_requests_5xx_total{kind=\"other\"} 1\n"));
        assert!(rendered.contains("aethernet_media_proxy_request_duration_ms_count 1\n"));
    }
    ...
    async fn trace_public_request_observes_a_non_axum_request() {
        ...
        let rendered = metrics.render();
        assert!(
            rendered.contains("aethernet_media_proxy_requests_2xx_total{kind=\"attachment\"} 1\n")
        );
    }
}
