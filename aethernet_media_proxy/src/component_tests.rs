...
fn thumbnail_status(
    ...
    let status = NativeStatus::from_code(unsafe {
        native::aethernet_vips_thumbnail_buffer_ex(
            input.as_ptr().cast(),
            input.len(),
...
fn blocked_libvips_loaders_cannot_decode_untrusted_input() {
    ...
    let raw = unsafe {
        native::aethernet_vips_image_new_from_buffer(ppm.as_ptr().cast(), ppm.len(), options.as_ptr())
    };
    assert!(raw.is_null());
    unsafe { native::aethernet_vips_error_clear() };
}
...
fn gif_resize_status(
    ...
    let status = NativeStatus::from_code(unsafe {
        native::aethernet_ffmpeg_resize_gif(
            input.as_ptr().cast(),
            input.len(),
            1,
            1,
...
fn webp_direct_transform_status(input: &[u8]) -> (NativeStatus, *mut c_void, size_t) {
    ...
    let mut output_size = usize::MAX;
    let status = NativeStatus::from_code(unsafe {
        native::aethernet_webp_transform_animated(
            input.as_ptr().cast(),
            input.len(),
            16,
            16,
...
fn heif_validate_status(input: &[u8]) -> NativeStatus {
    NativeStatus::from_code(unsafe {
        native::aethernet_heif_validate(input.as_ptr().cast(), input.len(), 0)
    })
}

fn gif_animation_status(input: &[u8]) -> NativeStatus {
    NativeStatus::from_code(unsafe {
        native::aethernet_validate_gif_animation(
            input.as_ptr().cast(),
            input.len(),
            16,
            1_000,
...
fn probe_animated_status(input: &[u8]) -> (NativeStatus, c_int, c_int, c_int) {
    ...
    let status = NativeStatus::from_code(unsafe {
        native::aethernet_vips_probe_animated(
            input.as_ptr().cast(),
            input.len(),
            &mut width,
            &mut height,
...
fn apng_decode_status(input: &[u8]) -> (NativeStatus, *mut native::VipsImage, u32) {
    ...
    let status = NativeStatus::from_code(unsafe {
        native::aethernet_ffmpeg_decode_apng(
            input.as_ptr().cast(),
            input.len(),
            1,
            0,
...
fn bmp_decode_status(input: &[u8]) -> (NativeStatus, *mut native::VipsImage) {
    let mut out = ptr::null_mut();
    let status = NativeStatus::from_code(unsafe {
        native::aethernet_ffmpeg_decode_bmp(
            input.as_ptr().cast(),
            input.len(),
            1,
            0,
...
fn heif_still_decode_status(
    ...
    *mut native::VipsImage,
    native::AethernetHEIFPrimaryStillDecodeFacts,
) {
    let mut out = ptr::null_mut();
    let mut facts = native::AethernetHEIFPrimaryStillDecodeFacts {
        hdr_tone_mapped: c_int::MAX,
        hdr_gain_map_detected: c_int::MAX,
    };
    let status = NativeStatus::from_code(unsafe {
        native::aethernet_heif_decode_primary_still(
            input.as_ptr().cast(),
            input.len(),
            0,
            &mut out,
...
fn webp_nsfw_extract_status(input: &[u8]) -> NativeStatus {
    ...
    let status = NativeStatus::from_code(unsafe {
        native::aethernet_webp_extract_frames_for_nsfw(
            input.as_ptr().cast(),
            input.len(),
            1,
            0,
...
fn nsfw_extract_statuses(input: &[u8], deadline_monotonic_ms: i64) -> [NativeStatus; 3] {
    ...
    let apng = NativeStatus::from_code(unsafe {
        native::aethernet_ffmpeg_extract_apng_frames_for_nsfw(
            input.as_ptr().cast(),
            input.len(),
            1,
            deadline_monotonic_ms,
    ...
    let gif = NativeStatus::from_code(unsafe {
        native::aethernet_ffmpeg_extract_gif_frames_for_nsfw(
            input.as_ptr().cast(),
            input.len(),
            1,
            deadline_monotonic_ms,
    ...
    let av = NativeStatus::from_code(unsafe {
        native::aethernet_av_extract_frames_for_nsfw(
            input.as_ptr().cast(),
            input.len(),
            1,
            deadline_monotonic_ms,
