...
#[derive(Debug, Parser)]
#[command(name = "aethernet-media-proxy", disable_help_subcommand = true)]
pub struct Args {
...
mod tests {
    ...
    fn cli_overrides_env_config() {
        ...
        let cfg = load_config_from_iter(
            ...
            [
                ...
                ("AETHERNET_MEDIA_PROXY_HOST", "0.0.0.0"),
                ("AETHERNET_MEDIA_PROXY_PORT", "8080"),
            ],
        )
        .unwrap();
    ...
    fn cli_parses_healthcheck_subcommand() {
        assert_eq!(
            ...
            Args::try_parse_from(["aethernet-media-proxy", "healthcheck"])
                .unwrap()
                .command
        );
        assert!(
            Args::try_parse_from(["aethernet-media-proxy"])
                .unwrap()
                .command
                .is_none()
        );
    ...
    fn cli_rejects_empty_bind_host() {
        let args = Args::try_parse_from(["aethernet-media-proxy", "--bind-host", ""]).unwrap();
        let err = load_config_from_iter(&args, [("AETHERNET_MEDIA_PROXY_SECRET_KEY", "secret")])
            .unwrap_err();
