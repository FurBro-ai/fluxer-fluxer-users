...
fn upload_relay_test_config(
    ...
    Config::load_from_iter([
        (
            "AETHERNET_MEDIA_PROXY_SECRET_KEY".to_owned(),
            "secret".to_owned(),
        ),
        ("AETHERNET_MEDIA_PROXY_MODE".to_owned(), "upload".to_owned()),
        (
            "AETHERNET_MEDIA_PROXY_STORAGE_BACKEND".to_owned(),
            "local".to_owned(),
        ),
        (
            "AETHERNET_MEDIA_PROXY_STORAGE_ROOT".to_owned(),
            storage_root.display().to_string(),
        ),
        (
            "AETHERNET_MEDIA_PROXY_UPLOAD_RELAY_SECRET_BASE64".to_owned(),
            base64::Engine::encode(&STANDARD, relay_secret),
        ),
        (
            "AETHERNET_MEDIA_PROXY_UPLOAD_RELAY_MAX_BODY_BYTES".to_owned(),
            "4096".to_owned(),
        ),
        (
            "AETHERNET_MEDIA_PROXY_UPLOAD_RELAY_SPOOL_DIR".to_owned(),
            spool_dir.display().to_string(),
        ),
        (
            "AETHERNET_MEDIA_PROXY_UPLOAD_RELAY_SPOOL_MAX_TOTAL_BYTES".to_owned(),
            (1u64 << 30).to_string(),
        ),
    ])
    .unwrap()
...
fn relay_cors_allows_client_context_headers() {
    ...
    assert!(
        allow_headers
            .split(',')
            .any(|name| name.trim().eq_ignore_ascii_case("x-aethernet-features"))
    );
    assert!(
        allow_headers
            .split(',')
