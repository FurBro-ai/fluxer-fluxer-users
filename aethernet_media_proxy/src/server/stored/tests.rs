...
fn asset_test_config(storage_root: &std::path::Path) -> Config {
    Config::load_from_iter([
        ...
        ("AETHERNET_MEDIA_PROXY_MODE".to_owned(), "mp".to_owned()),
        (
...
async fn an_asset_image_transform_is_served_and_then_reused_from_its_content_identity_key() {
    ...
    assert!(
        rendered.contains("aethernet_media_proxy_transform_cache_hits_total 1\n"),
        "the second request reuses the cached transform"
    );
    assert!(rendered.contains("aethernet_media_proxy_transform_cache_misses_total 1\n"));
    assert!(
        rendered.contains("aethernet_media_proxy_storage_hits_total 2\n"),
    ...
    assert!(
        app.metrics
            .render()
            .contains("aethernet_media_proxy_transform_cache_misses_total 2\n"),
