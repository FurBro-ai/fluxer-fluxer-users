...
LIBWEBP_FLOOR=1.1.0
LCMS2_FLOOR=2.8
# Optional: uncommon slide formats (SVS, NDPI, SCN, …) via libvips' openslide
# loader. Off by default because most instances never see these formats.
# Opt in with: AETHERNET_WITH_OPENSLIDE=1 ./install-native-deps.sh
OPENSLIDE_FLOOR=3.4.0
...
verify_openslide_floor() {
    local hint="$1"
    atleast openslide "$OPENSLIDE_FLOOR" "$hint"
    vips --vips-config 2>/dev/null | grep -qi openslide \
        || log "warning: libvips was built without its openslide loader; slide formats stay unsupported despite openslide being present"
}
...
verify_macos_floors() {
    ...
    if [ "${AETHERNET_WITH_OPENSLIDE:-0}" = "1" ]; then
        verify_openslide_floor "run 'brew install openslide'"
    fi
    local prefix
...
verify_linux_floors() {
    ...
    if [ "${AETHERNET_WITH_OPENSLIDE:-0}" = "1" ]; then
        verify_openslide_floor "install libopenslide-dev (apt) or openslide (brew)"
    fi
...
register_library_path() {
    ...
    printf '%s\n' "${PREFIX}/lib" >/etc/ld.so.conf.d/aethernet-media-native.conf
    ldconfig
}
...
if [ "${AETHERNET_WITH_OPENSLIDE:-0}" = "1" ]; then
    log "openslide support enabled (uncommon slide formats via libvips)"
else
    log "openslide support skipped (set AETHERNET_WITH_OPENSLIDE=1 for SVS/NDPI/SCN via libvips)"
fi
