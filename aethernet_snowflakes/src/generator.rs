...
pub const AETHERNET_EPOCH_MS: u64 = 1_420_070_400_000;
...
pub fn parse_snowflake(snowflake: u64) -> SnowflakeParts {
    ...
    SnowflakeParts {
        timestamp_ms: relative_timestamp + AETHERNET_EPOCH_MS,
        worker_id,
        sequence,
    }
}
...
fn to_relative_timestamp(timestamp_ms: u64) -> anyhow::Result<u64> {
    if timestamp_ms < AETHERNET_EPOCH_MS {
        anyhow::bail!("timestamp must be on or after the Aethernet epoch");
    }
    Ok(timestamp_ms - AETHERNET_EPOCH_MS)
}
...
mod tests {
    ...
    fn constants_match_compatible_layout() {
        assert_eq!(AETHERNET_EPOCH_MS, 1_420_070_400_000);
    ...
    fn creates_and_parses_known_snowflake() {
        let snowflake = create_snowflake(AETHERNET_EPOCH_MS + 1_000_000, 3, 77).unwrap();
        let parsed = parse_snowflake(snowflake);
        assert_eq!(parsed.timestamp_ms, AETHERNET_EPOCH_MS + 1_000_000);
    ...
    async fn generator_starts_sequence_at_zero() {
        let clock = ManualClock::new(AETHERNET_EPOCH_MS + 1000);
        ...
        assert_eq!(parsed.timestamp_ms, AETHERNET_EPOCH_MS + 1000);
    ...
    async fn generator_remains_monotonic_when_clock_moves_backwards() {
        let clock = ManualClock::new(AETHERNET_EPOCH_MS + 1002);
        let mut generator = SnowflakeGenerator::with_clock(1, clock.clone()).unwrap();
        let first = generator.generate().await.unwrap();
        clock.set(AETHERNET_EPOCH_MS + 1001);
        let second = generator.generate().await.unwrap();
        assert!(second > first);
        assert_eq!(parse_snowflake(second).timestamp_ms, AETHERNET_EPOCH_MS + 1002);
    }
    ...
    async fn generator_waits_for_next_millisecond_on_sequence_overflow() {
        ...
        tokio::spawn(async move {
            ...
            advance_clock.set(AETHERNET_EPOCH_MS + 1001);
        });
        let id = generator.generate().await.unwrap();
        let parsed = parse_snowflake(id);
        assert_eq!(parsed.timestamp_ms, AETHERNET_EPOCH_MS + 1001);
        assert_eq!(parsed.sequence, 0);
    }

    #[test]
    fn validates_worker_and_sequence_bounds() {
        assert!(create_snowflake(AETHERNET_EPOCH_MS, 1023, 4095).is_ok());
        assert!(create_snowflake(AETHERNET_EPOCH_MS, 1024, 0).is_err());
        assert!(create_snowflake(AETHERNET_EPOCH_MS, 0, 4096).is_err());
        assert!(create_snowflake(AETHERNET_EPOCH_MS - 1, 0, 0).is_err());
    }
}
