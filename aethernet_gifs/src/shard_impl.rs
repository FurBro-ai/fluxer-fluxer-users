...
use aethernet_svc::config::optional_env;
...
impl CacheBudgets {
    fn from_env() -> anyhow::Result<Self> {
        let total = match optional_env("AETHERNET_GIFS_SHARD_CACHE_MAX_BYTES") {
            Some(value) => value.parse::<u64>().map_err(|error| {
                anyhow::anyhow!("invalid AETHERNET_GIFS_SHARD_CACHE_MAX_BYTES: {error}")
            })?,
            None => DEFAULT_SHARD_CACHE_MAX_BYTES,
        };
        if total < MIN_SHARD_CACHE_MAX_BYTES {
            anyhow::bail!(
                "AETHERNET_GIFS_SHARD_CACHE_MAX_BYTES must be at least {MIN_SHARD_CACHE_MAX_BYTES}"
            );
        }
...
impl GifsShard {
    ...
    fn render_cache_metrics<T>(&self, output: &mut String, cache: &ManagedCache<T>) {
        ...
        let _ = writeln!(
            output,
            "aethernet_gifs_shard_cache_budget_bytes{{cache=\"{name}\"}} {}",
            cache.max_weight_bytes
        );
        let _ = writeln!(
            output,
            "aethernet_gifs_shard_cache_entries{{cache=\"{name}\"}} {}",
            cache.cache.entry_count()
        );
        let _ = writeln!(
            output,
            "aethernet_gifs_shard_cache_weighted_size_bytes{{cache=\"{name}\"}} {}",
            cache.cache.weighted_size()
        );
        let _ = writeln!(
            output,
            "aethernet_gifs_shard_cache_hits_total{{cache=\"{name}\"}} {}",
            telemetry.hits_total.load(METRICS_ORDERING)
        );
        let _ = writeln!(
            output,
            "aethernet_gifs_shard_cache_misses_total{{cache=\"{name}\"}} {}",
            telemetry.misses_total.load(METRICS_ORDERING)
        );
        let _ = writeln!(
            output,
            "aethernet_gifs_shard_cache_evictions_total{{cache=\"{name}\",cause=\"size\"}} {}",
            telemetry.size_evictions_total.load(METRICS_ORDERING)
        );
        let _ = writeln!(
            output,
            "aethernet_gifs_shard_cache_evictions_total{{cache=\"{name}\",cause=\"expired\"}} {}",
            telemetry.expired_evictions_total.load(METRICS_ORDERING)
        );
        let _ = writeln!(
            output,
            "aethernet_gifs_shard_cache_refreshes_total{{cache=\"{name}\",result=\"success\"}} {}",
            telemetry.refresh_success_total.load(METRICS_ORDERING)
        );
        let _ = writeln!(
            output,
            "aethernet_gifs_shard_cache_refreshes_total{{cache=\"{name}\",result=\"error\"}} {}",
            telemetry.refresh_error_total.load(METRICS_ORDERING)
        );
        let _ = writeln!(
            output,
            "aethernet_gifs_shard_cache_refreshes_total{{cache=\"{name}\",result=\"superseded\"}} {}",
            telemetry.refresh_superseded_total.load(METRICS_ORDERING)
        );
        let _ = writeln!(
            output,
            "aethernet_gifs_shard_cache_refreshes_total{{cache=\"{name}\",result=\"duplicate\"}} {}",
            telemetry.refresh_duplicate_total.load(METRICS_ORDERING)
        );
        let _ = writeln!(
            output,
            "aethernet_gifs_shard_cache_refreshes_total{{cache=\"{name}\",result=\"saturated\"}} {}",
            telemetry.refresh_saturated_total.load(METRICS_ORDERING)
        );
        let _ = writeln!(
            output,
            "aethernet_gifs_shard_cache_refreshes_in_flight{{cache=\"{name}\"}} {}",
...
impl ShardService for GifsShard {
    ...
    fn render_prometheus_metrics(&self, output: &mut String) {
        let _ = writeln!(output, "# TYPE aethernet_gifs_shard_cache_budget_bytes gauge");
        let _ = writeln!(output, "# TYPE aethernet_gifs_shard_cache_entries gauge");
        let _ = writeln!(
            output,
            "# TYPE aethernet_gifs_shard_cache_weighted_size_bytes gauge"
        );
        let _ = writeln!(output, "# TYPE aethernet_gifs_shard_cache_hits_total counter");
        let _ = writeln!(
            output,
            "# TYPE aethernet_gifs_shard_cache_misses_total counter"
        );
        let _ = writeln!(
            output,
            "# TYPE aethernet_gifs_shard_cache_evictions_total counter"
        );
        let _ = writeln!(
            output,
            "# TYPE aethernet_gifs_shard_cache_refreshes_total counter"
        );
        let _ = writeln!(
            output,
            "# TYPE aethernet_gifs_shard_cache_refreshes_in_flight gauge"
        );
