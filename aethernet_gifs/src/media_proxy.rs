...
impl MediaProxyUrlBuilder {
    ...
    pub fn from_env() -> anyhow::Result<Self> {
        let endpoint = std::env::var("AETHERNET_MEDIA_PROXY_PUBLIC_ENDPOINT")
            .or_else(|_| std::env::var("AETHERNET_MEDIA_ENDPOINT"))
            .unwrap_or_default();
        if endpoint.trim().is_empty() {
            anyhow::bail!(
                "gifs shard requires AETHERNET_MEDIA_PROXY_PUBLIC_ENDPOINT or AETHERNET_MEDIA_ENDPOINT"
            );
        }

        let secret_key = std::env::var("AETHERNET_MEDIA_PROXY_SECRET_KEY").unwrap_or_default();
        if secret_key.trim().is_empty() {
            anyhow::bail!("gifs shard requires AETHERNET_MEDIA_PROXY_SECRET_KEY");
        }

        let endpoint = aethernet_common::config::normalize_public_endpoint_from_env(
    ...
    fn from_env_uses_public_endpoint_when_internal_proxy_endpoint_is_set() -> anyhow::Result<()> {
        with_media_proxy_env(
            &[
                ...
                ("AETHERNET_MEDIA_PROXY_SECRET_KEY", Some("secret")),
            ],
            || {
    ...
    fn from_env_inserts_a_non_default_public_port() -> anyhow::Result<()> {
        with_media_proxy_env(
            &[
                (
                    "AETHERNET_MEDIA_PROXY_PUBLIC_ENDPOINT",
                    Some("http://aethernet.example/media"),
                ),
                ("AETHERNET_MEDIA_PROXY_SECRET_KEY", Some("secret")),
                ("AETHERNET_BASE_DOMAIN", Some("aethernet.example")),
                ("AETHERNET_PUBLIC_PORT", Some("19080")),
            ],
            || {
                let builder = MediaProxyUrlBuilder::from_env()?;

                assert_eq!(builder.endpoint, "http://aethernet.example:19080/media");
                assert_eq!(builder.endpoint_host.as_deref(), Some("aethernet.example"));
                assert!(
                    builder
    ...
    fn from_env_leaves_a_default_public_port_alone() -> anyhow::Result<()> {
        with_media_proxy_env(
            &[
                (
                    "AETHERNET_MEDIA_PROXY_PUBLIC_ENDPOINT",
                    Some("https://aethernet.example/media"),
                ),
                ("AETHERNET_MEDIA_PROXY_SECRET_KEY", Some("secret")),
                ("AETHERNET_BASE_DOMAIN", Some("aethernet.example")),
                ("AETHERNET_PUBLIC_PORT", Some("443")),
            ],
            || {
                let builder = MediaProxyUrlBuilder::from_env()?;

                assert_eq!(builder.endpoint, "https://aethernet.example/media");
                Ok(())
            },
        )
    }
    ...
    fn from_env_rejects_internal_proxy_endpoint_without_public_endpoint() -> anyhow::Result<()> {
        with_media_proxy_env(
            ...
            || {
                ...
                assert!(err.contains("AETHERNET_MEDIA_PROXY_PUBLIC_ENDPOINT"));
                Ok(())
            },
        )
    }
