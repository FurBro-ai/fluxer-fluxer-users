...
pub fn check_coverage_period(
    ...
    if period_ms > hard_cap_ms {
        anyhow::bail!(
            "the derived coverage period is {period_ms} ms, above AETHERNET_RECON_COVERAGE_PERIOD_HARD_CAP_MS ({hard_cap_ms} ms); the room count this process can audit at the configured read rates has been exceeded"
        );
    }
