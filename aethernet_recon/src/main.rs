...
use aethernet_recon::clock::{SharedClock, SystemClock};
use aethernet_recon::config::ReconConfig;
use aethernet_recon::control::{CONTROL_SUBJECT, INSTANCE_SUBJECT, run_control};
use aethernet_recon::gateway::nats::NatsGateway;
use aethernet_recon::metrics::ReconMetrics;
use aethernet_recon::runtime::{
    InstanceIdentity, Shared, boot_assertions, monotonic_now, run_singleton, runtime_tasks,
};
use aethernet_recon::service::{
    GuardedClock, ReconEngine, SharedFleet, build_livekit, connect_topology, internal_endpoint,
    run_census, run_discovery, run_engine,
};
use aethernet_recon::webhook::run_webhook;
use aethernet_svc::config::ServiceConfig;
use aethernet_svc::metrics::ServiceMetrics;
use aethernet_svc::shutdown::{DEFAULT_DRAIN_TIMEOUT, drain_with_timeout, wait_for_shutdown};
...
async fn serve(service: ServiceConfig, recon: ReconConfig) -> anyhow::Result<()> {
    ...
    tasks.spawn(async move {
        aethernet_svc::server::run_http(health_addr, health_serving, health_metrics, health_name).await
    });
