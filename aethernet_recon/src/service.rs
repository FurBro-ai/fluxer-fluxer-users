...
use aethernet_svc::config::{DatabaseBackend, ServiceConfig};
...
pub async fn connect_topology(service: &ServiceConfig) -> anyhow::Result<Arc<dyn TopologyStore>> {
    match service.database_backend {
        DatabaseBackend::Postgres => {
            let config = aethernet_svc::postgres::PostgresConfig::from_service_config(service);
...
async fn connect_scylla_topology(
    ...
    let config = aethernet_svc::scylla::ScyllaConfig::from_service_config(service);
...
async fn connect_scylla_topology(
    ...
    anyhow::bail!(
        "AETHERNET_DATABASE_BACKEND selects cassandra but this binary was built without the scylla feature"
    )
}
