...
use aethernet_svc::config::ServiceConfig;
...
pub fn monotonic_now() -> Millis {
    Millis::new(aethernet_svc::metrics::now_ms().max(0) as u64)
}
...
pub fn assert_single_replica(
    ...
    if service_name != SERVICE_NAME {
        anyhow::bail!(
            "AETHERNET_SVC_NAME must be {SERVICE_NAME} so the control subject and metric namespace agree, got {service_name}"
        );
    }
    if shard_count != 1 {
        anyhow::bail!(
            "AETHERNET_SVC_SHARD_COUNT must be 1 for a single-replica service, got {shard_count}"
        );
    }
    if shard_id != 0 {
        anyhow::bail!("AETHERNET_SVC_SHARD_ID must be 0 for a single-replica service, got {shard_id}");
    }
    Ok(())
}
...
pub fn assert_coverage_budget(config: &ReconConfig) -> anyhow::Result<()> {
    if config.coverage_target_ms > config.coverage_period_hard_cap_ms {
        anyhow::bail!(
            "AETHERNET_RECON_COVERAGE_TARGET_MS ({}) exceeds AETHERNET_RECON_COVERAGE_PERIOD_HARD_CAP_MS ({})",
            config.coverage_target_ms,
            config.coverage_period_hard_cap_ms
        );
    }
    if config.tick_ms == 0 {
        anyhow::bail!("AETHERNET_RECON_TICK_MS must be greater than zero");
    }
    if config.worker_threads == 0 {
        anyhow::bail!("AETHERNET_RECON_WORKER_THREADS must be greater than zero");
    }
    ...
    if derived > config.coverage_period_hard_cap_ms {
        anyhow::bail!(
            "AETHERNET_RECON_EXPECTED_ROOMS ({}) needs a coverage period of {derived} ms at the configured read rates, above AETHERNET_RECON_COVERAGE_PERIOD_HARD_CAP_MS ({}); this process can audit {auditable} rooms within that cap",
