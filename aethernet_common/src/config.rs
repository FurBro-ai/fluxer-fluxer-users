    ...
    F: FnMut(&str) -> Option<String>,
{
    match source {
        GeoipSourceConfig::S3 { .. } => Some(GeoipS3Config {
            endpoint: read_var("AETHERNET_S3_ENDPOINT").unwrap_or_default(),
            region: read_var("AETHERNET_S3_REGION").unwrap_or_default(),
            access_key_id: read_var("AETHERNET_S3_ACCESS_KEY_ID").unwrap_or_default(),
            secret_access_key: read_var("AETHERNET_S3_SECRET_ACCESS_KEY").unwrap_or_default(),
        }),
    ...
    F: FnMut(&str) -> Option<String>,
{
    ...
    let configured_port = match non_empty_var("AETHERNET_PUBLIC_PORT") {
        None => None,
        Some(raw) => Some(
            raw.parse::<u16>()
                .ok()
                ...
                .ok_or_else(|| {
                    anyhow::anyhow!(
                        "AETHERNET_PUBLIC_PORT must be a port between 1 and 65535, got {raw}"
                    )
                })?,
        ),
    };
    let configured_domain =
        non_empty_var("AETHERNET_BASE_DOMAIN").map(|value| canonicalize_domain(&value));
    let Some(origin) = non_empty_var("AETHERNET_PUBLIC_ORIGIN") else {
        return Ok((configured_domain.unwrap_or_default(), configured_port));
    };
    let parsed = parse_public_origin(&origin).ok_or_else(|| {
        anyhow::anyhow!(
            "AETHERNET_PUBLIC_ORIGIN must be a scheme, host and optional port such as https://chat.example.com:8443, got {origin}"
        )
    })?;
...
mod tests {
    ...
    fn parses_s3_geoip_source() {
        ...
        assert_eq!(
            source,
            GeoipSourceConfig::S3 {
                maxmind_db_path: "/tmp/aethernet/geoip/test_svc/city.mmdb".to_owned(),
                maxmind_asn_db_path: Some(
                    "/tmp/aethernet/geoip/test_svc/GeoLite2-ASN.mmdb".to_owned()
                ),
    ...
    fn reads_geoip_s3_config_only_for_s3_source() {
        ...
        let config = read_geoip_s3_config(&source, |name| {
            Some(
                match name {
                    "AETHERNET_S3_ENDPOINT" => "https://s3.example.test",
                    "AETHERNET_S3_REGION" => "ewr1",
                    "AETHERNET_S3_ACCESS_KEY_ID" => "access",
                    "AETHERNET_S3_SECRET_ACCESS_KEY" => "secret",
                    _ => "",
                }
                .to_owned(),
            )
    ...
    fn default_https_install_is_untouched() {
        for url in [
            ...
            "https://aethernet.example/admin/oauth2_callback",
            "wss://aethernet.example/gateway",
        ] {
            assert_eq!(
                url,
                normalize_public_endpoint(url, "aethernet.example", Some(443))
            );
        }
    }
    ...
    fn default_http_install_is_untouched() {
        for url in [
            "http://aethernet.example",
            "http://aethernet.example/media",
            "ws://aethernet.example/gateway",
        ] {
            assert_eq!(
                url,
                normalize_public_endpoint(url, "aethernet.example", Some(80))
            );
        }
    }
    ...
    fn inserts_a_non_default_port_for_the_base_domain() {
        assert_eq!(
            ...
            normalize_public_endpoint("http://aethernet.example/media", "aethernet.example", Some(19080))
        );
        assert_eq!(
            "http://aethernet.example:19080",
            normalize_public_endpoint("http://aethernet.example", "aethernet.example", Some(19080))
        );
        assert_eq!(
            "https://aethernet.example:8443/admin/oauth2_callback",
    ...
    fn a_default_port_for_the_urls_own_scheme_is_never_inserted() {
        assert_eq!(
            "ws://aethernet.example/gateway",
            normalize_public_endpoint("ws://aethernet.example/gateway", "aethernet.example", Some(80))
        );
        assert_eq!(
            "wss://aethernet.example/gateway",
            normalize_public_endpoint("wss://aethernet.example/gateway", "aethernet.example", Some(443))
        );
        assert_eq!(
            "http://aethernet.example:443/media",
            normalize_public_endpoint("http://aethernet.example/media", "aethernet.example", Some(443))
        );
        assert_eq!(
            "https://aethernet.example:80/media",
            normalize_public_endpoint("https://aethernet.example/media", "aethernet.example", Some(80))
        );
    }
    ...
    fn another_host_is_never_touched() {
        for url in [
            ...
            assert_eq!(
                url,
                normalize_public_endpoint(url, "aethernet.example", Some(19080))
            );
        }
        assert_eq!(
            "https://sub.aethernet.example/media",
            normalize_public_endpoint(
                "https://sub.aethernet.example/media",
                "aethernet.example",
                Some(19080)
    ...
    fn an_explicit_port_is_never_touched() {
        for url in [
            ...
            "https://aethernet.example:443/media",
            "http://aethernet.example:80/media",
            "http://user:pass@aethernet.example:19080/media",
        ] {
            assert_eq!(
                url,
                normalize_public_endpoint(url, "aethernet.example", Some(19080))
    ...
    fn is_idempotent() {
        let once =
            normalize_public_endpoint("http://aethernet.example/media", "aethernet.example", Some(19080));
        let twice = normalize_public_endpoint(&once, "aethernet.example", Some(19080));
        assert_eq!("http://aethernet.example:19080/media", once);
    ...
    fn an_unset_port_leaves_every_url_alone() {
        assert_eq!(
            "http://aethernet.example/media",
            normalize_public_endpoint("http://aethernet.example/media", "aethernet.example", None)
        );
        assert_eq!(
            "http://aethernet.example/media",
            normalize_public_endpoint("http://aethernet.example/media", "aethernet.example", Some(0))
        );
    }
    ...
    fn an_empty_base_domain_leaves_every_url_alone() {
        assert_eq!(
            "http://aethernet.example/media",
            normalize_public_endpoint("http://aethernet.example/media", "", Some(19080))
        );
        assert_eq!(
            "http://aethernet.example/media",
            normalize_public_endpoint("http://aethernet.example/media", "   ", Some(19080))
    ...
    fn a_url_that_does_not_parse_is_returned_unchanged() {
        for url in ["", "/api", "aethernet.example/media", "not a url", "://x"] {
            assert_eq!(
                url,
                normalize_public_endpoint(url, "aethernet.example", Some(19080))
            );
    ...
    fn preserves_path_query_fragment_and_trailing_slash() {
        ...
        assert_eq!(
            "http://aethernet.example:19080?a=1",
            normalize_public_endpoint("http://aethernet.example?a=1", "aethernet.example", Some(19080))
        );
        assert_eq!(
            "http://aethernet.example:19080#top",
            normalize_public_endpoint("http://aethernet.example#top", "aethernet.example", Some(19080))
        );
        assert_eq!(
            "http://aethernet.example:19080/media/x.png?v=1#frag",
            normalize_public_endpoint(
                "http://aethernet.example/media/x.png?v=1#frag",
                "aethernet.example",
                Some(19080)
            )
        );
    ...
    fn keeps_credentials_and_ipv6_literals_intact() {
        assert_eq!(
            "http://user:pass@aethernet.example:19080/media",
            normalize_public_endpoint(
                "http://user:pass@aethernet.example/media",
                "aethernet.example",
                Some(19080)
            )
        );
    ...
    fn the_origin_wins_over_the_named_variables() {
        let (domain, port) = resolve_public_domain_and_port(reader(&[
            ...
            ("AETHERNET_BASE_DOMAIN", "other.example.com"),
    ...
    fn a_malformed_public_port_is_loud() {
        for raw in ["abc", "0", "70000", "-1"] {
            let error = resolve_public_domain_and_port(reader(&[
                ...
                ("AETHERNET_PUBLIC_PORT", raw),
            ]))
            .expect_err("a malformed port is refused");
            assert!(
                error.to_string().contains("AETHERNET_PUBLIC_PORT"),
                "port {raw}"
            );
        }
    }
    ...
    fn a_malformed_public_origin_is_loud() {
        ...
        assert!(error.to_string().contains("AETHERNET_PUBLIC_ORIGIN"));
    }
