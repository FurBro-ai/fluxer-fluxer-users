...
describe('parsePastedUrl', () => {
	it('accepts http and https urls', () => {
		expect(parsePastedUrl('https://aethernet.app/download')).toBe('https://aethernet.app/download');
	...
	it('rejects prose that merely contains a url', () => {
		expect(parsePastedUrl('see https://aethernet.app for more')).toBeNull();
	});

	it('rejects angle brackets that would break the masked link', () => {
		expect(parsePastedUrl('https://aethernet.app/<script>')).toBeNull();
	});

	it('rejects empty and oversized input', () => {
		expect(parsePastedUrl('')).toBeNull();
		expect(parsePastedUrl(null)).toBeNull();
		expect(parsePastedUrl(`https://aethernet.app/${'a'.repeat(4000)}`)).toBeNull();
	});
});
...
describe('resolvePastedLinkInsertion', () => {
	it('wraps a selection when a bare url is pasted over it', () => {
		expect(resolvePastedLinkInsertion('https://aethernet.app/download', 'the download page')).toBe(
			'[the download page](<https://aethernet.app/download>)',
		);
	});

	it('wraps a message link the same way a plain url is wrapped', () => {
		const messageLink = 'https://aethernet.app/channels/1234567890/9876543210/1122334455';
	...
	it('declines when the selection cannot be wrapped', () => {
		expect(resolvePastedLinkInsertion('https://aethernet.app', '   ')).toBeNull();
		expect(resolvePastedLinkInsertion('https://aethernet.app', 'https://example.test')).toBeNull();
	});
