...
export const AETHERNET_COMPOSER_CLIPBOARD_MIME = 'application/x-aethernet-composer+json';
...
function writeComposerClipboardSelection(event: ClipboardEvent): boolean {
	...
	if (serialized != null) {
		event.clipboardData.setData(AETHERNET_COMPOSER_CLIPBOARD_MIME, serialized);
	}
	return true;
}
			...
			(event) => {
				...
				const serialized =
					clipboardEvent == null || clipboardEvent.clipboardData == null
						? null
						: clipboardEvent.clipboardData.getData(AETHERNET_COMPOSER_CLIPBOARD_MIME);
