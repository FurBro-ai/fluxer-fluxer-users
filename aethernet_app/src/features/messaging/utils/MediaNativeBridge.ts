...
declare global {
	var aethernetNativeMedia: NativeBridge | undefined;
}

export function getNativeBridge(): NativeBridge | null {
	return globalThis.aethernetNativeMedia ?? null;
}
...
export function __setNativeBridgeForTesting(bridge: NativeBridge | null): void {
	if (bridge === null) {
		delete globalThis.aethernetNativeMedia;
	} else {
		globalThis.aethernetNativeMedia = bridge;
	}
}
