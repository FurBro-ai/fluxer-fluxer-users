...
describe('the cascade the gate is computed from', () => {
	...
	it('names every gated family in the base --font-sans, or the gate could never select them', () => {
		...
		for (const family of ['Aethernet Sans SC', 'Aethernet Sans TC', 'Aethernet Sans JP', 'Aethernet Sans KR']) {
	...
	it('orders the base chain SC before TC before JP, which is the no-signal Han tie-break', () => {
		const chain = /--font-sans:([^;]*);/.exec(variables)?.[1] ?? '';
		expect(chain.indexOf('Aethernet Sans SC')).toBeLessThan(chain.indexOf('Aethernet Sans TC'));
		expect(chain.indexOf('Aethernet Sans TC')).toBeLessThan(chain.indexOf('Aethernet Sans JP'));
	});
	...
	it('hoists the family hanChunkForLanguage picks, for every locale rule that exists', () => {
		const expected: Record<string, string> = {
			ja: 'Aethernet Sans JP',
			ko: 'Aethernet Sans KR',
			'zh-CN': 'Aethernet Sans SC',
			'zh-TW': 'Aethernet Sans TC',
		};
		for (const [tag, family] of Object.entries(expected)) {
			...
			const chain = rule![1];
			const hoisted = /'(Aethernet Sans (?:SC|TC|JP|KR))'/.exec(chain)?.[1];
