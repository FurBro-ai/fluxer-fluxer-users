...
import {RelationshipTypes} from '@aethernet/constants/src/UserConstants';
import {Routes} from '@app/app/Routes';
import styles from '@app/features/app/components/layout/GuildsLayout.module.css';
import {getAethernetButtonBadgeCount} from '@app/features/app/components/layout/sidebar_nav/AethernetButtonBadgeUtils';
...
import {AethernetButtonContextMenu} from '@app/features/ui/action_menu/AethernetButtonContextMenu';
...
const MESSAGE_1_UNREAD_DM_DESCRIPTOR = msg({
	message: '1 unread DM',
	comment: 'Short label in the sidebar navigation aethernet button.',
});
const UNREAD_DMS_DESCRIPTOR = msg({
	message: '{displayedUnreadDmCount} unread DMs',
	comment:
		'Short label in the sidebar navigation aethernet button. Preserve {displayedUnreadDmCount}; it is inserted by code.',
});
...
const FRIEND_REQUESTS_DESCRIPTOR = msg({
	...
	comment:
		'Sidebar Aethernet-home button tooltip showing how many incoming friend requests are pending. Count is interpolated.',
});
export const AethernetButton = observer(() => {
	...
	const badgeCount = getAethernetButtonBadgeCount({
	...
	const handleContextMenu = useCallback((event: React.MouseEvent) => {
		...
		ContextMenuCommands.openFromEvent(event, (props) => (
			<AethernetButtonContextMenu
				onClose={props.onClose}
				data-flx="app.sidebar-nav.aethernet-button.handle-context-menu.aethernet-button-context-menu"
			/>
		));
	}, []);
	...
	return (
		<Tooltip
			...
			data-flx="app.sidebar-nav.aethernet-button.tooltip"
		>
			<FocusRing
				offset={-2}
				focusTarget={buttonRef}
				ringTarget={iconRef}
				data-flx="app.sidebar-nav.aethernet-button.focus-ring"
			>
				<button
					type="button"
					className={clsx(styles.aethernetButton, contextMenuOpen && styles.contextMenuHover)}
					...
					ref={mergedButtonRef}
					data-flx="app.sidebar-nav.aethernet-button.aethernet-button.select"
				>
					<AnimatePresence data-flx="app.sidebar-nav.aethernet-button.animate-presence">
						{(isSelected || shouldShowHoverState) && (
							<div className={styles.guildIndicator} data-flx="app.sidebar-nav.aethernet-button.guild-indicator">
								<motion.span
									...
									data-flx="app.sidebar-nav.aethernet-button.guild-indicator-bar"
								/>
							</div>
						)}
					</AnimatePresence>
					<div className={styles.relative} data-flx="app.sidebar-nav.aethernet-button.relative">
						<motion.div
							ref={iconRef}
							className={clsx(styles.aethernetButtonIcon, isSelected && styles.aethernetButtonIconSelected)}
							animate={{borderRadius: isActive ? '30%' : '50%'}}
							initial={false}
							transition={{duration: 0.07, ease: 'easeOut'}}
							data-flx="app.sidebar-nav.aethernet-button.aethernet-button-icon"
						>
							<ChatCircleIcon
								weight="fill"
								className={styles.messageBubbleIcon}
								data-flx="app.sidebar-nav.aethernet-button.message-bubble-icon"
							/>
						</motion.div>
						<div
							className={clsx(styles.guildBadge, badgeCount > 0 && styles.guildBadgeActive)}
							data-flx="app.sidebar-nav.aethernet-button.guild-badge"
						>
							<MentionBadgeAnimated
								mentionCount={badgeCount}
								size="small"
								data-flx="app.sidebar-nav.aethernet-button.mention-badge-animated"
							/>
						</div>
					</div>
				</button>
