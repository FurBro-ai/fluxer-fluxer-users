...
const AETHERNET_HQ_INVITE_CODE = 'aethernet-hq';
...
export const GuildMembershipCtaNagbar = observer(({isMobile}: {isMobile: boolean}) => {
	...
	const inviteState = Invites.invites.get(AETHERNET_HQ_INVITE_CODE);
	const invite = inviteState?.data ?? null;
	const [isSubmitting, setIsSubmitting] = useState(false);
	useEffect(() => {
		const aethernetHqGuild = Guilds.getGuilds().find((guild) => guild.vanityURLCode === AETHERNET_HQ_INVITE_CODE);
		if (aethernetHqGuild && GuildMembers.getMember(aethernetHqGuild.id, currentUserId ?? '')) {
	...
	const handleJoinGuild = async () => {
		...
		try {
			await InviteCommands.fetchWithCoalescing(AETHERNET_HQ_INVITE_CODE);
		} finally {
			setIsSubmitting(false);
			ModalCommands.push(
				modal(() => (
					<InviteAcceptModal
						code={AETHERNET_HQ_INVITE_CODE}
