...
import * as SnowflakeUtils from '@aethernet/snowflake/src/SnowflakeUtils';
...
import {AethernetButton} from '@app/features/app/components/layout/sidebar_nav/AethernetButton';
...
type GuildNavigationRow =
	| (NavigationRow & {readonly kind: 'aethernet'})
...
interface GuildNavigationVisibility {
	readonly aethernetVisible: boolean;
...
function useGuildNavigationVisibility(): GuildNavigationVisibility {
	...
	const aethernetVisible = !RuntimeConfig.directMessagesDisabled;
	...
	return useMemo(
		...
		[communityActionsVisible, downloadVisible, favoritesVisible, aethernetVisible, helpVisible],
	);
}
...
function createGuildNavigationRows({
	...
	if (visibility.aethernetVisible) {
		rows.push({
			kind: 'aethernet',
			key: 'aethernet',
			focusable: true,
			focusTargetIdentity: 'aethernet',
		});
	}
...
function resolveSelectedGuildNavigationKey({
	...
	if (visibility.aethernetVisible && pathname === Routes.ME) return 'aethernet';
	return null;
}
...
function createGuildRailSkeletonLayout({
	...
	return Object.freeze({
		...
		aethernetVisible: visibility.aethernetVisible,
...
const GuildList = observer(() => {
	...
	const renderGuildNavigationRow = (row: GuildNavigationRow): React.ReactNode => {
		switch (row.kind) {
			case 'aethernet':
				return <AethernetButton data-flx="app.guilds-layout.render-guild-navigation-row.aethernet-button" />;
			case 'favorites':
