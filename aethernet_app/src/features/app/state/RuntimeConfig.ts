...
import {API_CODE_VERSION} from '@aethernet/constants/src/AppConstants';
import type {
	InstanceAppPublic,
	InstanceCaptcha,
	InstanceCommunity,
...
} from '@aethernet/instance_bootstrap/src/Types';
import {expandWireFormat} from '@aethernet/limits/src/LimitDiffer';
import type {LimitConfigSnapshot, LimitConfigWireFormat} from '@aethernet/limits/src/LimitTypes';
import type {InstanceConfigResponse} from '@aethernet/schema/src/domains/admin/AdminSchemas';
import {
...
function getInlinedInstance(): InstanceDiscoveryResponse {
	const bootstrap = typeof window !== 'undefined' ? window.__AETHERNET_BOOTSTRAP__ : undefined;
	if (!bootstrap) {
		throw new Error('window.__AETHERNET_BOOTSTRAP__ is missing — app must be served by aethernet_app_proxy');
	}
...
function upsertDocumentLink(rel: string, href: string): void {
	...
	const selector = `link[rel="${rel}"][data-aethernet-branding="true"]`;
	const existing = document.head.querySelector<HTMLLinkElement>(selector);
	const link = existing ?? document.createElement('link');
	link.rel = rel;
	link.href = href;
	link.dataset.aethernetBranding = 'true';
	if (!existing) {
...
function removeDocumentLink(rel: string): void {
	if (typeof document === 'undefined') return;
	document.head
		.querySelectorAll<HTMLLinkElement>(`link[rel="${rel}"][data-aethernet-branding="true"]`)
		.forEach((link) => {
			link.remove();
		});
}
...
function upsertDocumentMeta(name: string, content: string): void {
	if (typeof document === 'undefined') return;
	const brandedMeta = document.head.querySelector<HTMLMetaElement>(
		`meta[name="${name}"][data-aethernet-branding="true"]`,
	);
	const meta = brandedMeta ?? document.createElement('meta');
	meta.name = name;
	meta.content = content;
	meta.dataset.aethernetBranding = 'true';
...
function removeDocumentMeta(name: string): void {
	if (typeof document === 'undefined') return;
	document.head
		.querySelectorAll<HTMLMetaElement>(`meta[name="${name}"][data-aethernet-branding="true"]`)
		.forEach((meta) => {
			meta.remove();
		});
}
