...
const CANARY_X64 = 'https://api.canary.aethernet.app/dl/desktop/canary/linux/x64';
const STABLE_X64 = 'https://api.aethernet.app/dl/desktop/stable/linux/x64';
...
describe('buildLinuxManualUpdateOptions', () => {
		...
		expect(options).toEqual([
			{
				...
				suggestedName: `AETHERNET-${REPORTED_VERSION}-linux-x86_64.AppImage`,
				sha256: null,
			},
			{
				format: 'deb',
				...
				suggestedName: `AETHERNET-${REPORTED_VERSION}-linux-amd64.deb`,
				sha256: null,
			},
			{
				format: 'rpm',
				...
				suggestedName: `AETHERNET-${REPORTED_VERSION}-linux-x86_64.rpm`,
				sha256: null,
			},
			{
				format: 'tar_gz',
				...
				suggestedName: `AETHERNET-${REPORTED_VERSION}-linux-x64.tar.gz`,
				sha256: null,
			},
		]);
	});
	...
	it('only uses latest links and latest file names when no version is known', () => {
		const options = buildLinuxManualUpdateOptions({
			...
			knownOptions: desktopMainOptions({
				linkBase: STABLE_X64,
				productName: 'Aethernet',
				version: REPORTED_VERSION,
				linkVersion: 'latest',
			}),
		});
		expect(options.map(({url, suggestedName, sha256}) => ({url, suggestedName, sha256}))).toEqual([
			{url: `${STABLE_X64}/latest/appimage`, suggestedName: 'AETHERNET-latest-linux-x86_64.AppImage', sha256: null},
			{url: `${STABLE_X64}/latest/deb`, suggestedName: 'AETHERNET-latest-linux-amd64.deb', sha256: null},
			{url: `${STABLE_X64}/latest/rpm`, suggestedName: 'AETHERNET-latest-linux-x86_64.rpm', sha256: null},
			{url: `${STABLE_X64}/latest/tar_gz`, suggestedName: 'AETHERNET-latest-linux-x64.tar.gz', sha256: null},
		]);
	});
	...
	it('uses arm64 links and file name tokens on arm64 systems', () => {
		const expected = [
			{
				url: `https://api.aethernet.app/dl/desktop/stable/linux/arm64/${REPORTED_VERSION}/appimage`,
				suggestedName: `AETHERNET-${REPORTED_VERSION}-linux-arm64.AppImage`,
			},
			{
				url: `https://api.aethernet.app/dl/desktop/stable/linux/arm64/${REPORTED_VERSION}/deb`,
				suggestedName: `AETHERNET-${REPORTED_VERSION}-linux-arm64.deb`,
			},
			{
				url: `https://api.aethernet.app/dl/desktop/stable/linux/arm64/${REPORTED_VERSION}/rpm`,
				suggestedName: `AETHERNET-${REPORTED_VERSION}-linux-aarch64.rpm`,
			},
			{
				url: `https://api.aethernet.app/dl/desktop/stable/linux/arm64/${REPORTED_VERSION}/tar_gz`,
				suggestedName: `AETHERNET-${REPORTED_VERSION}-linux-arm64.tar.gz`,
			},
		];
		...
		const fromLatestLink = buildLinuxManualUpdateOptions({
			downloadUrl: 'https://api.aethernet.app/dl/desktop/stable/linux/arm64/latest/appimage',
			version: REPORTED_VERSION,
		});
		const fromPinnedLink = buildLinuxManualUpdateOptions({
			downloadUrl: `https://api.aethernet.app/dl/desktop/stable/linux/arm64/${REPORTED_VERSION}/deb`,
			version: REPORTED_VERSION,
		});
	...
	it('builds links on the stable and canary endpoints', () => {
		...
		const configuredCanary = buildLinuxManualUpdateOptions({
			...
			arch: 'x64',
			version: REPORTED_VERSION,
			apiEndpoint: 'https://web.canary.aethernet.app/api/',
		});
		const proxiedCanaryLink = buildLinuxManualUpdateOptions({
			downloadUrl: 'https://web.canary.aethernet.app/api/dl/desktop/canary/linux/x64/latest/deb',
			version: REPORTED_VERSION,
			apiEndpoint: 'https://api.aethernet.app',
		});
		...
		expect(configuredCanary[1].url).toBe(
			`https://web.canary.aethernet.app/api/dl/desktop/canary/linux/x64/${REPORTED_VERSION}/deb`,
		);
	...
	it('never points a prompt for 2026.908.173325 at a link that could serve another build', () => {
		const options = buildLinuxManualUpdateOptions({
			...
			arch: 'x64',
			version: REPORTED_VERSION,
			apiEndpoint: 'https://api.canary.aethernet.app',
			knownOptions: desktopMainOptions({
				linkBase: CANARY_X64,
				productName: 'AETHERNET-Canary',
				version: REPORTED_VERSION,
		...
		expect(options.find((option) => option.format === 'deb')).toMatchObject({
			...
			suggestedName: `AETHERNET-Canary-${REPORTED_VERSION}-linux-amd64.deb`,
	...
	it.each([
		...
		expect(options[1]).toEqual({
			...
			suggestedName: 'AETHERNET-latest-linux-amd64.deb',
			sha256: null,
		});
	});
	...
	it('ignores a download link that is not for Linux', () => {
		const options = buildLinuxManualUpdateOptions({
			downloadUrl: 'https://api.aethernet.app/dl/desktop/stable/darwin/arm64/latest/dmg?test=1',
			channel: 'canary',
			arch: 'x64',
			version: REPORTED_VERSION,
		});
