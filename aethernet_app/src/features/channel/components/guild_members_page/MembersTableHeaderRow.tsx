...
const JOINED_PRODUCT_DESCRIPTOR = msg({
	...
	comment: 'Community members table column header for the date the account joined Aethernet.',
});

export interface MembersTableHeaderRowProps {
	memberSinceActive: boolean;
	joinedAethernetActive: boolean;
	joinMethodActive: boolean;
	rolesActive: boolean;
	onMemberSinceFilterOpen: (event: React.MouseEvent<HTMLButtonElement>) => void;
	onJoinedAethernetFilterOpen: (event: React.MouseEvent<HTMLButtonElement>) => void;
...
export function MembersTableHeaderRow({
	memberSinceActive,
	joinedAethernetActive,
	joinMethodActive,
	rolesActive,
	onMemberSinceFilterOpen,
	onJoinedAethernetFilterOpen,
			...
			>
				...
				>
					<div className={styles.thContent} data-flx="channel.guild-members-page.members-table-view.th-content--2">
						...
						<button
							type="button"
							className={clsx(styles.filterButton, joinedAethernetActive && styles.filterButtonActive)}
							onClick={onJoinedAethernetFilterOpen}
							aria-label={i18n._(FILTER_BY_ACCOUNT_CREATION_DATE_DESCRIPTOR)}
							aria-pressed={joinedAethernetActive}
							aria-haspopup="menu"
							data-flx="channel.guild-members-page.members-table-view.filter-button.joined-aethernet-filter-open"
						>
							<FunnelIcon
								size={remFromPx(12)}
								weight={joinedAethernetActive ? 'fill' : 'bold'}
