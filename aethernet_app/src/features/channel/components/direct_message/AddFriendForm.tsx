...
import {EXAMPLE_AETHERNET_TAG_FULL} from '@app/features/app/config/I18nDisplayConstants';
...
const PLEASE_ENTER_A_VALID_USERNAME_DESCRIPTOR = msg({
	message: 'Enter a valid username ({exampleAethernetTagFull}).',
	comment:
		'Description text in the channel and chat add friend form. Preserve {exampleAethernetTagFull}; it is inserted by code.',
});
...
export const AddFriendForm: React.FC<AddFriendFormProps> = observer(({onSuccess}) => {
	...
	const getErrorMessage = () => {
		...
		if (errorCode === APIErrorCodes.NO_USERS_WITH_AETHERNETTAG_EXIST) {
			return i18n._(NO_USER_FOUND_WITH_THAT_USERNAME_DESCRIPTOR);
		}
		if (errorCode === APIErrorCodes.DISCRIMINATOR_REQUIRED) {
			return i18n._(PLEASE_ENTER_A_VALID_USERNAME_DESCRIPTOR, {exampleAethernetTagFull: EXAMPLE_AETHERNET_TAG_FULL});
		}
	...
	const handleSubmit = (e: React.SubmitEvent<HTMLFormElement>) => {
		...
		if (!username || !discriminator || !/^\d{4}$/.test(discriminator)) {
			...
			setErrorCode(APIErrorCodes.NO_USERS_WITH_AETHERNETTAG_EXIST);
			return;
		}
		setIsLoading(true);
		...
		<form onSubmit={handleSubmit} className={styles.form} data-flx="channel.direct-message.add-friend-form.form.submit">
			...
			>
				<Input
					...
					placeholder={EXAMPLE_AETHERNET_TAG_FULL}
					className={clsx(
						styles.input,
