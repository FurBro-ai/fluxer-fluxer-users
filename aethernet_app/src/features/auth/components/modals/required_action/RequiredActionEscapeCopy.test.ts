...
describe('buildPhoneGateEscapeConfirmCopy', () => {
	...
	it('warns about the remaining email step before the support line, which is always last', () => {
		...
		expect(copy.bodyLines.at(-1)).toBe('You can still email support@aethernet.app for a human review at any time.');
		expect(confirmCopy().bodyLines.at(-1)).toBe(
			'You can still email support@aethernet.app for a human review at any time.',
		);
	});
});
