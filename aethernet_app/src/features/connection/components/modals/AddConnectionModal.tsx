...
import {type ConnectionType, ConnectionTypes} from '@aethernet/constants/src/ConnectionConstants';
import type {ConnectionVerificationResponse} from '@aethernet/schema/src/domains/connection/ConnectionSchemas';
...
export const AddConnectionModal = observer(({defaultType}: AddConnectionModalProps) => {
	...
	const hostRecord = useMemo(
		() => (verificationData?.id ? `_aethernet.${verificationData.id}` : ''),
		[verificationData?.id],
	);
	const dnsValue = useMemo(
		() => (verificationData?.token ? `aethernet-verification=${verificationData.token}` : ''),
		[verificationData?.token],
	);
	const dnsUrl = useMemo(
		() => (verificationData?.id ? `https://${verificationData.id}/.well-known/aethernet-verification` : ''),
		[verificationData?.id],
	);
	...
	const downloadTokenFile = useCallback(() => {
		...
		link.href = blobUrl;
		link.download = 'aethernet-verification';
			...
			{dnsUrl && (
				<div className={styles.tokenCard} data-flx="connection.add-connection-modal.token-card">
					...
					<div className={styles.tokenDownloadRow} data-flx="connection.add-connection-modal.token-download-row">
						...
						>
							<Trans>Download aethernet-verification</Trans>
						</Button>
					</div>
