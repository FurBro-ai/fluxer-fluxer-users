...
export const ReportPage = observer(() => {
	...
	useAethernetDocumentTitle(i18n._(REPORT_ILLEGAL_CONTENT_DESCRIPTOR));
	useLayoutEffect(() => {
		...
		(
			...
			if (error && typeof error === 'object' && 'body' in error && (error as {body?: unknown}).body) {
				...
				const pathMap: Record<string, keyof FormValues> = {
					...
					reporter_aethernet_tag: 'reporterAethernetTag',
	...
	const handleSubmit = useCallback(async () => {
		...
		const reporterAethernetTag = state.formValues.reporterAethernetTag.trim();
		...
		if (reporterAethernetTag) payload.reporter_aethernet_tag = reporterAethernetTag;
