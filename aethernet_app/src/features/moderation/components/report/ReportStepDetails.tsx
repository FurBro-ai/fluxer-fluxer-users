...
const REPORTED_USER_TAG_OPTIONAL_DESCRIPTOR = msg({
	...
	comment:
		'Field label on the details step of the DSA report form (message report). Optional AethernetTag of the user who sent the message.',
});
const USER_ID_OPTIONAL_DESCRIPTOR = msg({
	message: 'User ID (optional)',
	comment:
		'Field label on the details step of the DSA report form (user report). Optional Aethernet user snowflake ID. "ID" is conventional.',
});
const USER_TAG_OPTIONAL_DESCRIPTOR = msg({
	message: 'User tag (optional)',
	comment:
		'Field label on the details step of the DSA report form (user report). Optional AethernetTag of the reported user.',
});
...
const YOUR_USERNAME_OPTIONAL_DESCRIPTOR = msg({
	...
	comment:
		'Field label on the details step of the DSA report form. Optional Aethernet username of the reporter so staff can follow up.',
});
			...
			<div className={styles.cardBody} data-flx="moderation.report.report-step-details.card-body">
				...
				>
					...
					<Input
						...
						type="text"
						value={formValues.reporterAethernetTag}
						onChange={(e) => onFieldChange('reporterAethernetTag', e.target.value)}
						placeholder={EXAMPLE_USERNAME_TAG}
						error={fieldErrors.reporterAethernetTag}
