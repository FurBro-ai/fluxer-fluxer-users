...
import {UserPremiumTypes} from '@aethernet/constants/src/UserConstants';
import {ConfirmModal} from '@app/features/app/components/dialogs/ConfirmModal';
import * as Modal from '@app/features/app/components/dialogs/Modal';
import {EXAMPLE_AETHERNET_TAG, PREMIUM_PRODUCT_NAME} from '@app/features/app/config/I18nDisplayConstants';
...
import styles from '@app/features/user/components/modals/AethernetTagChangeModal.module.css';
import type {User} from '@app/features/user/models/User';
import {isVisionaryDiscriminator0000Blocked} from '@app/features/user/utils/AethernetTagDiscriminatorUtils';
...
const USERNAME_ALREADY_TAKEN_DESCRIPTOR = msg({
	...
	comment: 'Short label in the AethernetTag change modal. Keep it concise.',
});
...
const GET_TO_CUSTOMIZE_YOUR_TAG_OR_KEEP_IT_DESCRIPTOR = msg({
	...
	comment:
		'Description text in the AethernetTag change modal. Preserve {premiumProductName}; it is inserted by code. Keep the tone plain and specific.',
});
...
const CUSTOM_DISCRIMINATORS_ARE_NOT_AVAILABLE_ON_THIS_INSTANCE_DESCRIPTOR = msg({
	...
	comment: 'Error message in the AethernetTag change modal.',
});
const THE_0000_TAG_IS_RESERVED_FOR_VISIONARY_SUBSCRIBERS_DESCRIPTOR = msg({
	...
	comment: 'Description text in the AethernetTag change modal.',
});
...
interface AethernetTagChangeModalProps {
	user: User;
}

export const AethernetTagChangeModal = observer(({user}: AethernetTagChangeModalProps) => {
	...
	const onSubmit = useCallback(
		async (data: FormInputs) => {
			...
			if (!hasCustomDiscriminator && !skipAvailabilityCheckRef.current && !confirmedRerollRef.current) {
				const tagTaken = await UserCommands.checkAethernetTagAvailability({
					username: usernameValue,
					discriminator: normalizedDiscriminator,
				});
				if (tagTaken && !isSameTag) {
					const aethernetTag = `${usernameValue}#${normalizedDiscriminator}`;
					ModalCommands.push(
						modal(() => (
							<ConfirmModal
								...
								description={
									<div
										...
										data-flx="user.aethernet-tag-change-modal.on-submit.confirm-description"
									>
										<p data-flx="user.aethernet-tag-change-modal.on-submit.p">
											<Trans>
												The username{' '}
												<strong data-flx="user.aethernet-tag-change-modal.on-submit.strong">{aethernetTag}</strong> is
												already taken. Continuing will reroll your tag automatically.
											</Trans>
										</p>
										<p
											className={styles.confirmSecondary}
											data-flx="user.aethernet-tag-change-modal.on-submit.confirm-secondary"
										>
								...
								data-flx="user.aethernet-tag-change-modal.on-submit.confirm-modal"
							/>
						)),
					);
					return;
	...
	return (
		<Modal.Root
			...
			data-flx="user.aethernet-tag-change-modal.modal-root"
		>
			<Form
				form={form}
				onSubmit={handleSubmit}
				aria-label={i18n._(CHANGE_USERNAME_FORM_DESCRIPTOR)}
				data-flx="user.aethernet-tag-change-modal.form.submit"
			>
				<Modal.Header
					title={i18n._(CHANGE_YOUR_USERNAME_DESCRIPTOR)}
					data-flx="user.aethernet-tag-change-modal.modal-header"
				/>
				<Modal.Content data-flx="user.aethernet-tag-change-modal.modal-content">
					<Modal.ContentLayout data-flx="user.aethernet-tag-change-modal.modal-content-layout">
						<Modal.Description data-flx="user.aethernet-tag-change-modal.modal-description">
							<Trans>
						...
						<div
							className={styles.aethernetTagContainer}
							data-flx="user.aethernet-tag-change-modal.aethernet-tag-container"
						>
							<span className={styles.aethernetTagLabel} data-flx="user.aethernet-tag-change-modal.aethernet-tag-label">
								<Trans>Username</Trans>
							</span>
							{(form.formState.errors.username || form.formState.errors.discriminator) && (
								<div className={styles.errorBox} role="alert" data-flx="user.aethernet-tag-change-modal.error-box">
									{form.formState.errors.username?.message || form.formState.errors.discriminator?.message}
								</div>
							)}
							<div
								className={styles.aethernetTagInputRow}
								data-flx="user.aethernet-tag-change-modal.aethernet-tag-input-row"
							>
								<div className={styles.usernameInput} data-flx="user.aethernet-tag-change-modal.username-input">
									<Controller
										name="username"
										control={form.control}
										render={({field}) => (
											<Input
												data-flx="user.aethernet-tag-change-modal.input.text"
												{...field}
												ref={usernameRef}
												autoComplete="username"
												aria-label={i18n._(USERNAME_DESCRIPTOR)}
												placeholder={EXAMPLE_AETHERNET_TAG}
												required={true}
												type="text"
											/>
										)}
										data-flx="user.aethernet-tag-change-modal.controller"
									/>
								</div>
								<span className={styles.separator} data-flx="user.aethernet-tag-change-modal.separator">
									#
								</span>
								<div
									className={styles.discriminatorInput}
									data-flx="user.aethernet-tag-change-modal.discriminator-input"
								>
									{!hasCustomDiscriminator ? (
										showPremium ? (
											<Tooltip
												...
												data-flx="user.aethernet-tag-change-modal.tooltip"
											>
												<div
													className={styles.discriminatorInputDisabled}
													data-flx="user.aethernet-tag-change-modal.discriminator-input-disabled"
												>
													<Input
														data-flx="user.aethernet-tag-change-modal.input.set-value.text"
													...
													/>
													<FocusRing offset={-2} data-flx="user.aethernet-tag-change-modal.focus-ring">
														<button
															type="button"
															onClick={() => {
															...
															data-flx="user.aethernet-tag-change-modal.discriminator-overlay.open.button"
														/>
													</FocusRing>
												</div>
											</Tooltip>
											...
											<Tooltip
												...
												data-flx="user.aethernet-tag-change-modal.tooltip--2"
											>
												<div
													className={styles.discriminatorInputDisabled}
													data-flx="user.aethernet-tag-change-modal.discriminator-input-disabled--2"
												>
													<Input
														data-flx="user.aethernet-tag-change-modal.input.set-value.text--2"
														{...form.register('discriminator')}
														aria-label={i18n._(TAG_DESCRIPTOR)}
														maxLength={4}
														placeholder="0000"
										...
										<Input
											data-flx="user.aethernet-tag-change-modal.input.set-value.text--3"
											{...form.register('discriminator', {
												validate: (value) =>
							...
							<div className={styles.validationBox} data-flx="user.aethernet-tag-change-modal.validation-box">
								<UsernameValidationRules
									username={form.watch('username')}
									data-flx="user.aethernet-tag-change-modal.username-validation-rules"
								/>
							</div>
							{shouldShowPremiumDiscriminatorWarning && (
								<WarningAlert
									className={styles.premiumDiscriminatorWarning}
									data-flx="user.aethernet-tag-change-modal.premium-discriminator-warning"
								>
									{hasPendingDiscriminatorChange ? (
										premiumExpiresAtLabel ? (
											<Trans>
												...
												expires on{' '}
												<strong data-flx="user.aethernet-tag-change-modal.strong">{premiumExpiresAtLabel}</strong>,
												unless it renews.
											</Trans>
										) : (
											<Trans>
										...
										<Trans>
											...
											<strong data-flx="user.aethernet-tag-change-modal.strong--2">{premiumExpiresAtLabel}</strong>,
											unless it renews.
										</Trans>
									) : (
										<Trans>
							...
							{!hasCustomDiscriminator && (
								<PlutoniumUpsell
									...
									data-flx="user.aethernet-tag-change-modal.premium-upsell"
								>
				...
				<Modal.Footer data-flx="user.aethernet-tag-change-modal.modal-footer">
					<Button onClick={ModalCommands.pop} variant="secondary" data-flx="user.aethernet-tag-change-modal.button.pop">
						<Trans>Cancel</Trans>
					</Button>
					<Button type="submit" submitting={isSubmitting} data-flx="user.aethernet-tag-change-modal.button.submit">
