...
import type {HarvestStatusResponse} from '@aethernet/schema/src/domains/user/UserHarvestSchemas';
import type {
...
interface AethernetTagAvailabilityResponse {
	taken: boolean;
}
...
export async function checkAethernetTagAvailability({
	username,
	discriminator,
}: {
	username: string;
	...
	try {
		logger.debug(`Checking availability for AethernetTag ${username}#${discriminator}`);
		const response = await http.get<AethernetTagAvailabilityResponse>(Endpoints.USER_CHECK_TAG, {
			query: {username, discriminator},
		});
		return response.body.taken;
	} catch (error) {
		logger.error('Failed to check AethernetTag availability:', error);
		throw error;
	}
}
