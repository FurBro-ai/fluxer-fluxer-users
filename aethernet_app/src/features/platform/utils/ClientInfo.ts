...
export function installAethernetConfigDebugApi(): void {
	const config = getAethernetDebugObject();
	if (!config) {
		return;
	}
	try {
		...
		logger.warn(' Failed to install __AETHERNET_DEBUG__ helpers', error);
	}
}
