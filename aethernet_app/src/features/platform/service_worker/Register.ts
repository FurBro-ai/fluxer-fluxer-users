...
const AETHERNET_SERVICE_WORKER_CACHE_PREFIX = 'aethernet-';
...
async function clearAethernetServiceWorkerCaches(): Promise<Array<string>> {
	...
	await Promise.all(
		cacheNames.map(async (cacheName) => {
			if (!cacheName.startsWith(AETHERNET_SERVICE_WORKER_CACHE_PREFIX)) return;
...
async function unregisterAethernetServiceWorkers({
	cleanupPushState,
	logContext,
	reason,
}: {
	...
	try {
		...
		const deletedCaches = await clearAethernetServiceWorkerCaches();
...
export async function unregisterServiceWorkersForNativeDesktop(reason = 'native-desktop'): Promise<void> {
	await unregisterAethernetServiceWorkers({
