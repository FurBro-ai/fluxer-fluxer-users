...
declare const __AETHERNET_PRECACHE_MANIFEST__: ReadonlyArray<PrecacheEntry>;
declare const __AETHERNET_SW_VERSION__: string;
...
const SERVICE_WORKER_VERSION = typeof __AETHERNET_SW_VERSION__ === 'string' ? __AETHERNET_SW_VERSION__ : 'dev';
const PRECACHE_MANIFEST = typeof __AETHERNET_PRECACHE_MANIFEST__ === 'undefined' ? [] : __AETHERNET_PRECACHE_MANIFEST__;
...
const fetchInstanceConfig = async (): Promise<{
	...
	try {
		const res = await fetch('/.well-known/aethernet', {credentials: 'include'});
