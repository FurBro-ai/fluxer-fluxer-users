...
const WORKER_ORIGIN = 'https://app.aethernet.test';
...
describe('WorkerFetchRouting', () => {
	it('does not intercept expression media URLs', () => {
		expect(getWorkerFetchRoute(request('https://aethernetusercontent.com/emojis/123.webp'), WORKER_ORIGIN)).toBe(
			'ignore',
		);
		expect(getWorkerFetchRoute(request('https://aethernetusercontent.com/stickers/456.webp'), WORKER_ORIGIN)).toBe(
			'ignore',
		);
	...
	it('only recognises metadata served from the worker origin', () => {
		expect(getWorkerFetchRoute(request('https://cdn.aethernet.test/manifest.json'), WORKER_ORIGIN)).toBe('ignore');
		expect(getWorkerFetchRoute(request('https://cdn.aethernet.test/version.json'), WORKER_ORIGIN)).toBe('ignore');
	});
