...
const WORKER_ORIGIN = 'https://self-hosted.aethernet.test';
const PRECACHE_NAME = 'aethernet-precache-test';
const NAVIGATION_CACHE_NAME = 'aethernet-navigation-test';
...
const RENDERED_INDEX_DOCUMENT = [
	'<!doctype html><html lang="en"><head>',
	'<link rel="preconnect" href="https://cdn.aethernet.test">',
	'<script nonce="abc123">window.__AETHERNET_BOOTSTRAP__={"instance":{}};</script>',
	'</head><body><div id="root"></div></body></html>',
].join('');
...
describe('WorkerAppShell', () => {
	...
	it('never serves the app-proxy index template that was fetched outside the bootstrap render', async () => {
		...
		expect(html).toContain('window.__AETHERNET_BOOTSTRAP__');
