...
import {classifyVoiceEngineV2TrackStats} from '@aethernet/voice_engine_v2';
import {
	type PerTrackStats,
	type TransportInfo,
	...
	return new Map<string, unknown>([
		...
		[
			'LC1',
			{
				...
				protocol: 'udp',
				relayProtocol: 'tls',
				url: 'turn:turn.aethernet.example:443?transport=tcp',
				networkType: 'wifi',
			},
		],
...
describe('VoiceEngineV2AppStatsHostAdapter watched screen share rows', () => {
	...
	it('collects the relay protocol of the selected local candidate, which the local protocol never shows', async () => {
		...
		expect(transport?.localCandidateUrlHost).toBe('turn.aethernet.example');
	});
