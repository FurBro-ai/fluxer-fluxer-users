...
import type {
	...
	AethernetSelectProtocolMessage,
...
function sessionUpdatePayload(codecs: Array<AethernetCodecAdvertisement>): Uint8Array {
...
describe('advertising local capabilities over the negotiation topic', () => {
	...
	async function publishAndCaptureSelectProtocol(): Promise<{
		payload: Uint8Array;
		message: AethernetSelectProtocolMessage;
	}> {
		...
		return {payload, message: JSON.parse(new TextDecoder().decode(payload)) as AethernetSelectProtocolMessage};
	}
