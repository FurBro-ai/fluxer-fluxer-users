...
const PROTOCOL_TOPIC = 'aethernet.rtc.codec-negotiation.v1';
...
const VIDEO_CODEC_NAMES: Record<VideoCodec, AethernetVideoCodecName> = {
	av1: 'AV1',
	h265: 'H265',
	h264: 'H264',
	vp9: 'VP9',
...
const NAME_TO_VIDEO_CODEC: Record<AethernetVideoCodecName, VideoCodec> = {
	AV1: 'av1',
	H265: 'h265',
	H264: 'h264',
	VP9: 'vp9',
...
type AethernetVideoCodecName = 'AV1' | 'H265' | 'H264' | 'VP9' | 'VP8';
type AethernetCodecName = 'opus' | AethernetVideoCodecName;
type AethernetCodecType = 'audio' | 'video';
...
export interface AethernetCodecAdvertisement {
	name: AethernetCodecName;
	type: AethernetCodecType;
	payload_type: number;
...
export interface AethernetSelectProtocolMessage {
	...
	d: {
		...
		codecs: Array<AethernetCodecAdvertisement>;
		rtc_connection_id: string | null;
		experiments: Array<string>;
	};
}

export interface AethernetSessionUpdateMessage {
	op: typeof SESSION_UPDATE_OP;
	d: {
		video_codec: AethernetVideoCodecName;
...
export type AethernetCodecNegotiationMessage = AethernetSelectProtocolMessage | AethernetSessionUpdateMessage;
...
type ScreenShareCodecNegotiationMachineEvent =
	| {
			...
			localCodecs: ReadonlyArray<AethernetCodecAdvertisement>;
			remoteCodecs: ReadonlyArray<ReadonlyArray<AethernetCodecAdvertisement>>;
			unknownParticipants: number;
			reason: NegotiationReason;
...
export function buildLocalCodecAdvertisements(): Array<AethernetCodecAdvertisement> {
...
function getDecodeSet(codecs: ReadonlyArray<AethernetCodecAdvertisement>): Set<VideoCodec> {
	const result = new Set<VideoCodec>();
	for (const codec of codecs) {
		if (codec.type !== 'video' || codec.decode !== true) continue;
		const mapped = NAME_TO_VIDEO_CODEC[codec.name as AethernetVideoCodecName];
		if (mapped) result.add(mapped);
	}
	return result;
}

function getEncodeSet(codecs: ReadonlyArray<AethernetCodecAdvertisement>): Set<VideoCodec> {
	const result = new Set<VideoCodec>();
	for (const codec of codecs) {
...
function isAethernetVideoCodecName(value: unknown): value is AethernetVideoCodecName {
	return value === 'AV1' || value === 'H265' || value === 'H264' || value === 'VP9' || value === 'VP8';
}

function isAethernetCodecName(value: unknown): value is AethernetCodecName {
	return value === 'opus' || isAethernetVideoCodecName(value);
}

function isAethernetCodecType(value: unknown): value is AethernetCodecType {
	return value === 'audio' || value === 'video';
}

function isCodecAdvertisement(value: unknown): value is AethernetCodecAdvertisement {
	if (!isObject(value)) return false;
	return (
		isAethernetCodecName(value.name) &&
		isAethernetCodecType(value.type) &&
...
function isCodecAdvertisementList(value: unknown): value is Array<AethernetCodecAdvertisement> {
	return (
		Array.isArray(value) &&
		value.length > 0 &&
...
function isSelectProtocolMessage(value: unknown): value is AethernetSelectProtocolMessage {
...
function isSessionUpdateMessage(value: unknown): value is AethernetSessionUpdateMessage {
	if (!isObject(value) || value.op !== SESSION_UPDATE_OP || !isObject(value.d)) return false;
	return (
		isAethernetVideoCodecName(value.d.video_codec) &&
...
function parseMessage(payload: Uint8Array): AethernetCodecNegotiationMessage | null {
...
class ScreenShareCodecNegotiation {
	...
	private localCodecs: Array<AethernetCodecAdvertisement> = [];
	private remoteCodecsByIdentity = new Map<string, Array<AethernetCodecAdvertisement>>();
	...
	getLocalCodecAdvertisements(): Array<AethernetCodecAdvertisement> {
	...
	private getRemoteCodecInputs(room: Room | null = this.room): {
		knownRemoteCodecs: Array<Array<AethernetCodecAdvertisement>>;
		unknownParticipants: number;
	} {
		const knownRemoteCodecs: Array<Array<AethernetCodecAdvertisement>> = [];
	...
	private async publishBoundLocalCapabilities(
		...
		const message: AethernetSelectProtocolMessage = {
	...
	private async publishSessionUpdate(room: Room, selection: CodecNegotiationSelection): Promise<void> {
		const message: AethernetSessionUpdateMessage = {
	...
	private async publishMessage(room: Room, message: AethernetCodecNegotiationMessage): Promise<void> {
		try {
