...
AppStorage.setItem(
	...
	JSON.stringify({
		...
		screenShareAudioExcludeSources: [{'application.name': 'Aethernet'}],
...
describe('manual screen-share audio sources', () => {
	it('hydrates a profile that still stores the removed opt-in keys without losing anything else', () => {
		...
		expect(VoiceSettings.getScreenShareAudioExcludeSources()).toEqual([{'application.name': 'Aethernet'}]);
	});
	...
	it('hands every Linux user the stored selection with no opt-in left to satisfy', () => {
		...
		expect(VoiceSettings.getEffectiveScreenShareAudioExcludeSources()).toEqual([{'application.name': 'Aethernet'}]);
