...
describe('sharing a video device', () => {
	...
	test('keeps the microphone when the picked applications cannot be routed', async () => {
		platform.current = 'linux';
		voiceSettings.audioSourceMode = 'specific';
		voiceSettings.includeSources = [{'aethernet.display.name': 'mpv'}];
...
describe('sharing a whole display', () => {
	test('captures the desktop mix without Aethernet on Linux when nothing is selected', async () => {
		platform.current = 'linux';
...
describe('sharing a Aethernet-owned window while app audio is enabled', () => {
...
describe('changing the audio sources of a live window share', () => {
	...
	test('never routes audio into a Aethernet-owned window share', async () => {
		platform.current = 'linux';
