...
import type {VoiceEngineV2AudioDeviceRole} from '@aethernet/voice_engine_v2';
...
const INTERNAL_VIRTUAL_AUDIO_DEVICE_LABELS = [
	'aethernet-screen-share',
	'aethernet screen share',
	'aethernet-direct-capture',
	'aethernet direct capture',
];
...
export type VoiceMediaDeviceInfo = MediaDeviceInfo & {
	isDefault?: boolean;
	aethernetVoiceAudioDevice?: VoiceAudioDeviceMetadata;
};
...
function createAudioDeviceInfo(
	...
	if (metadata) {
		shaped.aethernetVoiceAudioDevice = metadata;
	}
	return shaped;
}

export function getVoiceAudioDeviceMetadata(device: MediaDeviceInfo): VoiceAudioDeviceMetadata | null {
	return (device as VoiceMediaDeviceInfo).aethernetVoiceAudioDevice ?? null;
}
