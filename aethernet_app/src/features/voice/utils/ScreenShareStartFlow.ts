...
export async function reconfigureActiveLinuxAppShareAudio(
	...
	if (ActiveScreenShareSource.isOwnWindow()) {
		logger.warn('Refusing to route audio into a Aethernet-owned window share');
...
async function runConfiguredDisplayScreenShare(
	...
	if (electronApi) {
		...
		if (isOwnWindowShare && requestedAudio) {
			logger.warn('Aethernet-owned window audio is excluded from screen share capture; continuing video-only', {
				sourceId,
		...
		if (requestedAppAudioOnLinux && appShareAudioRoute === 'window') {
			...
			if (requestedNativePickerAudioOnLinux && options?.preferredDisplaySurface === 'window') {
				logger.info(
					'Wayland window share cannot identify the shared window; capturing the desktop mix without Aethernet instead',
					{sourceMode},
				);
			}
			try {
