...
async function startSystemNativeAudioFramePump(
	...
	if (!systemAudioExcludesAethernet(availability)) {
...
async function startWindowNativeAudioFramePump(
	...
	try {
		...
		const detail = getNativeAudioErrorDetail(error);
		const refusedSelf = /Refusing to capture native audio from (?:own|Aethernet) process/.test(detail);
		setLastArmFailure({
			platform: electronApi.platform,
			sourceMode: 'specific',
		...
		if (refusedSelf) {
			logger.info('Native window audio frame pump refused for Aethernet-owned process', {targetPid, detail});
		} else {
...
function systemAudioExcludesAethernet(availability: NativeAudioAvailability): boolean {
	if (!availability.available) return false;
...
export async function captureNativeAudioTrackForWindowPid(targetPid: number): Promise<MediaStreamTrack | null> {
	...
	try {
		...
		if (refusedSelf) {
			logger.info('Native window audio capture refused for Aethernet-owned process', {targetPid, detail});
		} else {
