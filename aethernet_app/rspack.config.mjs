...
const isLowMemory = process.env.AETHERNET_LOW_MEMORY === 'true';
...
export default () => {
	...
	const staticCdnEndpoint = envString(
		...
		envString('AETHERNET_STATIC_CDN_ENDPOINT', isProduction ? '' : ''),
	);
	...
	return {
		...
		// Full development source maps retain a very large module graph. The compact
		// profile keeps useful line mappings without paying that memory cost.
		devtool: isProduction ? 'source-map' : isLowMemory ? 'eval-cheap-module-source-map' : 'source-map',
		target: ['web', 'browserslist'],
		lazyCompilation: isLowMemory ? {entries: false, imports: true} : false,
		resolve: {
			alias: {
				...
				'@app': SRC_DIR,
				'@aethernet/voice_engine_v2/bridge': path.join(
					MONOREPO_ROOT,
					'packages',
					'voice_engine_v2',
					'src',
				...
				'@aethernet/voice_engine_v2/runtime': path.join(
					MONOREPO_ROOT,
					'packages',
					'voice_engine_v2',
					'src',
				...
				'@aethernet/voice_engine_v2/testing': path.join(
					MONOREPO_ROOT,
					'packages',
					'voice_engine_v2',
					'src',
				...
				'@aethernet': path.join(MONOREPO_ROOT, 'packages'),
				'@pkgs': PKGS_DIR,
		...
		plugins: [
			...
			new DefinePlugin({
				__AETHERNET_PRECACHE_MANIFEST__: JSON.stringify([]),
				__AETHERNET_SW_VERSION__: JSON.stringify(publicValues.PUBLIC_BUILD_VERSION || 'dev'),
			...
			splitChunks: isProduction
				? {
						...
						cacheGroups: {
							...
							// Heavy optional voice-AI: TTS voices, ONNX denoisers, and the
							// Sapphi-Red worklet. All are dynamic-imported, so keeping them
							// out of the initial bundle saves ~1 MB for text-only users.
							voiceAi: {
								test: /[\\/]node_modules[\\/](mespeak|onnxruntime-web|deepfilternet3-noise-filter|@sapphi-red)[\\/]/,
								name: 'voice-ai',
								priority: 52,
								reuseExistingChunk: true,
								enforce: true,
							...
							vendor: {
								test: (module) => {
									...
									if (/[\\/](@arborium|@phosphor-icons|katex|mespeak|onnxruntime-web|deepfilternet3-noise-filter|@sapphi-red)[\\/]/.test(module.resource)) return false;
								...
								priority: 10,
								reuseExistingChunk: true,
								// Keep the fallback vendor chunk from becoming the new
								// oversized bundle once the optional groups are split out.
								maxSize: 400_000,
							},
						},
					}
		...
		devServer: {
			port: Number(process.env.AETHERNET_APP_DEV_PORT) || DEFAULT_DEV_SERVER_PORT,
			hot: false,
			liveReload: false,
			client: false,
