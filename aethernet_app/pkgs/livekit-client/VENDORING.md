...
4. **Screen share scalability mode** (`src/room/participant/LocalParticipant.ts`)
   ...
   A caller-supplied `scalabilityMode` is preserved for screen shares instead of being forced to `L3T3_KEY`, so VP9 and AV1 screen shares use the browser default unless Aethernet asks for a specific SVC layout.
...
8. **Media publishing defaults** (`src/room/defaults.ts`, `src/room/utils.ts`, `src/room/track/options.ts`)
   ...
   Codec fallback follows actual sender capabilities in the order H.264, VP9, VP8, AV1, HEVC. Advanced codecs are paired with an H.264 backup simulcast, and screen shares default to maintain-resolution with a 4K60-ready bitrate cap. AV1 and HEVC come last because both are opt-in in Aethernet, so a fallback inside `publishTrack` must not land on a codec the user did not enable. Aethernet picks the codec itself before publishing, so this order only applies when the client overrides the request, such as the reconnect republish that runs outside Aethernet's own flows.
