...
const POSTGRES_KV_MIGRATION_TABLE: &str = "__aethernet_schema_migrations";
...
pub async fn connect(config: &PostgresConfig) -> anyhow::Result<Pool> {
    ...
    let mut pg = if let Some(url) = &config.url {
        PgConfig::from_str(url).context("failed to parse AETHERNET_POSTGRES_URL")?
    } else {
...
fn build_tls_connector(ca_pem: Option<&str>) -> anyhow::Result<MakeRustlsConnect> {
    ...
    if let Some(ca_pem) = ca_pem.filter(|value| !value.trim().is_empty()) {
        ...
        for cert in CertificateDer::pem_slice_iter(normalized.as_bytes()) {
            roots.add(cert.context("failed to parse AETHERNET_POSTGRES_SSL_CA certificate")?)?;
        }
        if roots.is_empty() {
            anyhow::bail!("AETHERNET_POSTGRES_SSL_CA did not contain any certificates");
        }
...
fn decode_value(value: Value, date_mode: DecodeDateMode) -> anyhow::Result<Value> {
    match value {
        Value::Array(values) => values
            ...
            .collect::<anyhow::Result<Vec<_>>>()
            .map(Value::Array),
        Value::Object(mut object) => match object.get("__aethernet_type").and_then(Value::as_str) {
            Some("bigint") => {
                let value = object
                    .remove("value")
...
mod tests {
    ...
    fn encodes_row_keys_like_postgres_kv_executor() {
        assert_eq!(
            ...
            r#"{"__aethernet_type":"bigint","value":"42"}"#
        );
        assert_eq!(
            kv_key(&[
                KeyPart::BigInt(10),
            ...
            .unwrap(),
            "{\"__aethernet_type\":\"bigint\",\"value\":\"10\"}\u{001f}416\u{001f}\"wave\""
        );
        assert_eq!(
            kv_key(&[KeyPart::BigInt(5), KeyPart::Bool(false)]).unwrap(),
            "{\"__aethernet_type\":\"bigint\",\"value\":\"5\"}\u{001f}false"
        );
    }
    ...
    fn decodes_tagged_json_values() {
        let decoded = decode_row_dates_as_millis(json!({
            "id": {"__aethernet_type": "bigint", "value": "1509197195776110592"},
            "when": {"__aethernet_type": "date", "value": "2026-06-15T12:34:56.789Z"},
            "birth": {"__aethernet_type": "local_date", "value": "1999-01-02"},
            "bytes": {"__aethernet_type": "buffer", "value": "YWJj"},
            "ids": {"__aethernet_type": "set", "value": [
                {"__aethernet_type": "bigint", "value": "1"},
                {"__aethernet_type": "bigint", "value": "2"}
            ]},
            "metadata": {"__aethernet_type": "map", "value": [
                ["kind", {"__aethernet_type": "bigint", "value": "9"}]
            ]}
        }))
        .unwrap();
    ...
    fn test_postgres_config(kv_table: &str) -> PostgresConfig {
        PostgresConfig {
            ...
            port: 5432,
            database: "aethernet".to_owned(),
            username: "aethernet".to_owned(),
            password: None,
            ssl: false,
            ssl_ca: None,
            max_connections: 1,
    ...
    fn test_kv_client(kv_table: &str) -> KvClient {
        let pg = PgConfig::from_str("postgres://aethernet@127.0.0.1:5432/aethernet").unwrap();
    ...
    fn hoists_kv_statements_for_the_quoted_table() {
        let kv = test_kv_client("aethernet_kv");

        assert_eq!(
            kv.get_row_sql,
            "SELECT row_data FROM \"aethernet_kv\" WHERE table_name = $1 AND row_key = $2 AND (expires_at IS NULL OR expires_at > now()) LIMIT 1"
        );
        assert_eq!(
            kv.get_rows_sql,
            "SELECT row_key, row_data FROM \"aethernet_kv\" WHERE table_name = $1 AND row_key = ANY($2::text[]) AND (expires_at IS NULL OR expires_at > now())"
        );
        assert_eq!(
            kv.get_partition_rows_sql,
            "SELECT row_key, row_data FROM \"aethernet_kv\" WHERE table_name = $1 AND partition_key = $2 AND (expires_at IS NULL OR expires_at > now())"
        );
        assert_eq!(
            kv.get_row_key_prefix_rows_sql,
            "SELECT row_key, row_data FROM \"aethernet_kv\" WHERE table_name = $1 AND row_key COLLATE \"C\" >= $2 AND row_key COLLATE \"C\" < $3 AND (expires_at IS NULL OR expires_at > now())"
        );
        assert_eq!(
            kv.delete_row_sql,
            "DELETE FROM \"aethernet_kv\" WHERE table_name = $1 AND row_key = $2"
        );
    }

    #[test]
    fn carries_the_prepared_statement_switch_onto_the_client() {
        let mut config = test_postgres_config("aethernet_kv");
        ...
        assert!(test_kv_client("aethernet_kv").prepared_statements);
    }
    ...
    fn rejects_unsafe_identifiers() {
        assert!(quote_identifier("aethernet_kv").is_ok());
        assert!(quote_identifier("aethernet-kv").is_err());
