...
impl ServiceConfig {
    ...
    {
        let service_name =
            optional_from(&get, "AETHERNET_SVC_NAME").unwrap_or_else(|| "default".to_owned());

        let database_backend = match optional_from(&get, "AETHERNET_DATABASE_BACKEND")
            ...
            .as_str()
        {
            ...
            other => anyhow::bail!("unsupported AETHERNET_DATABASE_BACKEND: {other}"),
        };

        let mode = match optional_from(&get, "AETHERNET_SVC_MODE").as_deref() {
            None | Some("router") => Mode::Router,
            Some("shard") => Mode::Shard,
            Some(other) => anyhow::bail!("unsupported AETHERNET_SVC_MODE: {other}"),
        };

        let shard_count = optional_from(&get, "AETHERNET_SVC_SHARD_COUNT")
            .map(|v| v.parse::<u32>())
            .transpose()?
            .unwrap_or(1);

        let shard_id = match optional_from(&get, "AETHERNET_SVC_SHARD_ID") {
            Some(v) => v.parse::<u32>()?,
        ...
        if shard_id >= shard_count {
            anyhow::bail!(
                "AETHERNET_SVC_SHARD_ID ({shard_id}) must be less than AETHERNET_SVC_SHARD_COUNT ({shard_count})"
            );
        }

        let listen_host =
            optional_from(&get, "AETHERNET_SVC_LISTEN_HOST").unwrap_or_else(|| "0.0.0.0".to_owned());
        let listen_port = optional_from(&get, "AETHERNET_SVC_PORT")
            .map(|v| v.parse::<u16>())
            .transpose()?
            .unwrap_or(8090);

        let nats_url = optional_from(&get, "AETHERNET_SVC_NATS_URL")
            .unwrap_or_else(|| "nats://127.0.0.1:4222".to_owned());

        let nats_auth_token = optional_from(&get, "AETHERNET_NATS_AUTH_TOKEN");

        let cache_ttl_ms = optional_from(&get, "AETHERNET_SVC_CACHE_TTL_MS")
            .map(|v| v.parse::<u64>())
            .transpose()?
            .unwrap_or(30_000);

        let cache_hard_ttl_ms = optional_from(&get, "AETHERNET_SVC_CACHE_HARD_TTL_MS")
            .map(|v| v.parse::<u64>())
            .transpose()?
            .unwrap_or(600_000)
            .max(cache_ttl_ms);

        let cassandra_port = optional_from(&get, "AETHERNET_CASSANDRA_PORT")
            .map(|v| v.parse::<u16>())
            .transpose()?
            .unwrap_or(9042);

        let default_scylla_hosts = || vec![normalize_host("127.0.0.1", cassandra_port)];
        let scylla_hosts = optional_from(&get, "AETHERNET_CASSANDRA_HOSTS")
            .map(|hosts| {
                parse_hosts(&hosts)
                    .into_iter()
        ...
        let postgres_port = optional_from(&get, "AETHERNET_POSTGRES_PORT")
            .map(|v| v.parse::<u16>())
            .transpose()?
            .unwrap_or(5432);
        let postgres_ssl = optional_from(&get, "AETHERNET_POSTGRES_SSL")
            .map(|v| parse_bool(&v))
            .transpose()?
            .unwrap_or(false);
        let postgres_max_connections = optional_from(&get, "AETHERNET_POSTGRES_MAX_CONNECTIONS")
            .map(|v| v.parse::<usize>())
            .transpose()?
            .unwrap_or(20)
            .max(1);
        let postgres_prepared_statements =
            optional_from(&get, "AETHERNET_POSTGRES_PREPARED_STATEMENTS")
                .map(|v| parse_bool(&v))
                .transpose()?
                .unwrap_or(true);

        let max_concurrent_requests = optional_from(&get, "AETHERNET_SVC_MAX_CONCURRENT_REQUESTS")
            .map(|v| v.parse::<usize>())
            .transpose()?
        ...
        Ok(Self {
            ...
            nats_url,
            nats_auth_token,
            cache_max_entries: optional_from(&get, "AETHERNET_SVC_CACHE_MAX_ENTRIES")
                .map(|v| v.parse::<u64>())
                .transpose()?
                .unwrap_or(100_000),
            ...
            scylla_hosts,
            scylla_keyspace: optional_from(&get, "AETHERNET_CASSANDRA_KEYSPACE")
                .unwrap_or_else(|| "aethernet".to_owned()),
            scylla_username: optional_from(&get, "AETHERNET_CASSANDRA_USERNAME"),
            scylla_password: optional_from(&get, "AETHERNET_CASSANDRA_PASSWORD"),
            postgres_url: optional_from(&get, "AETHERNET_POSTGRES_URL"),
            postgres_host: optional_from(&get, "AETHERNET_POSTGRES_HOST")
                .unwrap_or_else(|| "127.0.0.1".to_owned()),
            postgres_port,
            postgres_database: optional_from(&get, "AETHERNET_POSTGRES_DATABASE")
                .unwrap_or_else(|| "aethernet".to_owned()),
            postgres_username: optional_from(&get, "AETHERNET_POSTGRES_USERNAME")
                .unwrap_or_else(|| "aethernet".to_owned()),
            postgres_password: optional_from(&get, "AETHERNET_POSTGRES_PASSWORD")
                .or_else(|| Some("aethernet".to_owned())),
            postgres_ssl,
            postgres_ssl_ca: optional_from(&get, "AETHERNET_POSTGRES_SSL_CA"),
            postgres_max_connections,
            postgres_kv_table: optional_from(&get, "AETHERNET_POSTGRES_KV_TABLE")
                .unwrap_or_else(|| "aethernet_kv".to_owned()),
...
mod tests {
    ...
    fn extracts_statefulset_ordinal() {
        ...
        assert_eq!(shard_id_from_pod_name("aethernet-user-svc-12"), Some(12));
    ...
    fn reads_canonical_cassandra_env_for_shards() {
        let cfg = config_from_pairs(&[
            ("AETHERNET_SVC_NAME", "messages"),
            ("AETHERNET_SVC_MODE", "shard"),
            ("AETHERNET_SVC_SHARD_COUNT", "2"),
            ("AETHERNET_SVC_SHARD_ID", "1"),
            ("AETHERNET_CASSANDRA_HOSTS", "cass-a,cass-b:9142"),
            ("AETHERNET_CASSANDRA_PORT", "9042"),
            ("AETHERNET_CASSANDRA_KEYSPACE", "aethernet_dev"),
            ("AETHERNET_CASSANDRA_USERNAME", "aethernet"),
            ("AETHERNET_CASSANDRA_PASSWORD", "secret"),
        ]);
        ...
        assert_eq!("aethernet_dev", cfg.scylla_keyspace);
        assert_eq!(Some("aethernet".to_owned()), cfg.scylla_username);
    ...
    fn defaults_database_backend_to_postgres() {
        ...
        assert_eq!("aethernet", cfg.postgres_database);
        assert_eq!("aethernet", cfg.postgres_username);
        assert_eq!(Some("aethernet".to_owned()), cfg.postgres_password);
        assert!(!cfg.postgres_ssl);
        assert_eq!(None, cfg.postgres_ssl_ca);
        assert_eq!(20, cfg.postgres_max_connections);
        assert_eq!("aethernet_kv", cfg.postgres_kv_table);
    ...
    fn reads_postgres_database_env() {
        let cfg = config_from_pairs(&[
            ("AETHERNET_DATABASE_BACKEND", "postgresql"),
            ("AETHERNET_POSTGRES_URL", "postgres://user:pass@db/aethernet"),
            ("AETHERNET_POSTGRES_HOST", "db"),
            ("AETHERNET_POSTGRES_PORT", "6432"),
            ("AETHERNET_POSTGRES_DATABASE", "aethernet_dev"),
            ("AETHERNET_POSTGRES_USERNAME", "app"),
            ("AETHERNET_POSTGRES_PASSWORD", "secret"),
            ("AETHERNET_POSTGRES_SSL", "true"),
            ("AETHERNET_POSTGRES_SSL_CA", "ca-pem"),
            ("AETHERNET_POSTGRES_MAX_CONNECTIONS", "7"),
            ("AETHERNET_POSTGRES_KV_TABLE", "aethernet_kv_dev"),
            ("AETHERNET_POSTGRES_PREPARED_STATEMENTS", "false"),
        ]);

        assert_eq!(DatabaseBackend::Postgres, cfg.database_backend);
        assert_eq!(
            Some("postgres://user:pass@db/aethernet".to_owned()),
            cfg.postgres_url
        );
        assert_eq!("db", cfg.postgres_host);
        assert_eq!(6432, cfg.postgres_port);
        assert_eq!("aethernet_dev", cfg.postgres_database);
        ...
        assert_eq!("aethernet_kv_dev", cfg.postgres_kv_table);
        assert!(!cfg.postgres_prepared_statements);
    }
    ...
    fn rejects_a_non_boolean_prepared_statements_value() {
        let result = ServiceConfig::from_env_reader(|name| {
            (name == "AETHERNET_POSTGRES_PREPARED_STATEMENTS").then(|| "maybe".to_owned())
        });
    ...
    fn raises_concurrency_defaults_for_hot_path_services() {
        assert_eq!(
            ...
            config_from_pairs(&[("AETHERNET_SVC_NAME", "messages")]).max_concurrent_requests
        );
        assert_eq!(
            SNOWFLAKES_MAX_CONCURRENT_REQUESTS,
            config_from_pairs(&[("AETHERNET_SVC_NAME", "snowflakes")]).max_concurrent_requests
        );
        assert_eq!(
            DEFAULT_MAX_CONCURRENT_REQUESTS,
            config_from_pairs(&[("AETHERNET_SVC_NAME", "users")]).max_concurrent_requests
        );
    }
    ...
    fn concurrency_env_override_wins_over_service_default() {
        let cfg = config_from_pairs(&[
            ("AETHERNET_SVC_NAME", "messages"),
            ("AETHERNET_SVC_MAX_CONCURRENT_REQUESTS", "32"),
        ]);
    ...
    fn reads_legacy_cassandra_backend_aliases() {
        let cfg = config_from_pairs(&[("AETHERNET_DATABASE_BACKEND", "scylla")]);
