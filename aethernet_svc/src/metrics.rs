...
impl ServiceMetrics {
    ...
    pub fn render_prometheus(&self, service_name: &str) -> String {
        ...
        render_counter(
            &mut out,
            &format!("aethernet_{service_name}_requests_total"),
            &self.requests_total,
        );
        render_counter(
            &mut out,
            &format!("aethernet_{service_name}_request_errors_total"),
            &self.request_errors_total,
        );
        render_counter(
            &mut out,
            &format!("aethernet_{service_name}_cache_hits_total"),
            &self.cache_hits_total,
        );
        render_counter(
            &mut out,
            &format!("aethernet_{service_name}_cache_misses_total"),
            &self.cache_misses_total,
        );
        render_counter(
            &mut out,
            &format!("aethernet_{service_name}_shard_forwards_total"),
            &self.shard_forwards_total,
        );
        render_histogram(
            &mut out,
            &format!("aethernet_{service_name}_request_duration_ms"),
        ...
        render_gauge(
            &mut out,
            &format!("aethernet_{service_name}_uptime_seconds"),
...
mod tests {
    ...
    fn render_prometheus_produces_valid_output() {
        ...
        assert!(text.contains("# TYPE aethernet_user_svc_requests_total counter"));
        assert!(text.contains("aethernet_user_svc_requests_total 2"));
        assert!(text.contains("# TYPE aethernet_user_svc_request_errors_total counter"));
        assert!(text.contains("aethernet_user_svc_request_errors_total 1"));
        assert!(text.contains("# TYPE aethernet_user_svc_cache_hits_total counter"));
        assert!(text.contains("aethernet_user_svc_cache_hits_total 1"));
        assert!(text.contains("# TYPE aethernet_user_svc_request_duration_ms histogram"));
        assert!(text.contains("aethernet_user_svc_request_duration_ms_bucket{le=\"50\"} 1"));
        assert!(text.contains("# TYPE aethernet_user_svc_uptime_seconds gauge"));
    }
