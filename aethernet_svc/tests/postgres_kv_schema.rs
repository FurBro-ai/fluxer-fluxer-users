...
use aethernet_svc::postgres::{PostgresConfig, connect, ensure_kv_schema};
...
fn postgres_config(port: u16, kv_table: &str, max_connections: usize) -> PostgresConfig {
    PostgresConfig {
        ...
        password: Some("aethernet".to_owned()),
        ssl: false,
        ssl_ca: None,
        max_connections,
...
async fn migration_marker_count(pool: &Pool, kv_table: &str) -> anyhow::Result<i64> {
    ...
    let row = client
        .query_one(
            &format!(
                "SELECT count(*) AS n FROM \"{kv_table}\" WHERE table_name = '__aethernet_schema_migrations' AND row_key = 'messages_partition_key_v1'"
            ),
            &[],
        )
        .await?;
...
async fn concurrent_boots_all_ensure_the_kv_schema() -> anyhow::Result<()> {
    ...
    let port = free_port()?;
    let container = format!("aethernet-kvschema-boot-{}-{port}", std::process::id());
...
async fn boot_survives_a_peer_that_creates_the_table_without_the_schema_lock() -> anyhow::Result<()>
{
    ...
    let port = free_port()?;
    let container = format!("aethernet-kvschema-race-{}-{port}", std::process::id());
    start_postgres(&container, port)?;
    let result = async {
        wait_for_postgres(&container, port).await?;
...
async fn backfills_message_partition_keys_once_and_never_scans_again() -> anyhow::Result<()> {
    ...
    let port = free_port()?;
    let container = format!("aethernet-kvschema-backfill-{}-{port}", std::process::id());
    start_postgres(&container, port)?;
    let result = async {
        wait_for_postgres(&container, port).await?;
        let helper = connect(&postgres_config(port, KV_TABLE_HELPER, 4)).await?;
