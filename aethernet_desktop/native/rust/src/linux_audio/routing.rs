...
mod tests {
    ...
    fn structural_self_exclude_beats_include_rules_across_pid_binary_name_keys() {
        ...
        self_identity.add_binary("aethernet");
        self_identity.add_binary("aethernet.exe");
    ...
    fn self_identity_matches_across_the_four_documented_pipewire_keys() {
        ...
        assert!(self_identity.matches(&map(&[("application.process.binary", "aethernet")])));
        assert!(self_identity.matches(&map(&[("application.name", "aethernet")])));
        assert!(self_identity.matches(&map(&[("node.name", "aethernet")])));
        assert!(self_identity.matches(&map(&[("node.nick", "Aethernet Canary")])));
        assert!(self_identity.matches(&map(&[("node.description", "Aethernet app audio capture",)])));
