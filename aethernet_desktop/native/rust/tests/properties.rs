...
use aethernet_desktop_native::audio::contract::{
...
use aethernet_desktop_native::input::ring::Ring;
use aethernet_desktop_native::linux_audio::routing::{
    MEDIA_CLASS_PLAYBACK_STREAM, RoutingRule, SelfIdentity, map, matches_pattern, should_route_node,
};
use aethernet_desktop_native::linux_evdev::event::{InputEvent, parse_input_event};
use aethernet_desktop_native::linux_portals::pid_payload::parse_shell_eval_pid_payload;
use aethernet_desktop_native::mac_app_audio::process_tree::{
...
fn self_identity_always_beats_user_include_rules() {
    ...
    self_identity.add_binary("aethernet");
    self_identity.add_display_name("Aethernet Canary");
    self_identity.add_display_prefix("Aethernet ");
    let rule = RoutingRule {
        include_when: vec![
            map(&[("application.process.id", "42")]),
            map(&[("application.name", "aethernet")]),
        ],
        ..RoutingRule::default()
    };
    for props in [
        ...
        map(&[
            ...
            (
                "node.description",
                "Aethernet desktop audio tap excluding pid 42",
            ),
        ]),
    ] {
