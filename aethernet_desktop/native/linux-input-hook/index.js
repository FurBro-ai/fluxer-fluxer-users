...
const MODULE_NAME = '@aethernet/linux-input-hook';
const SKIP_NATIVE_PROBE_ENV = 'AETHERNET_LINUX_INPUT_HOOK_SKIP_NATIVE_PROBE';
...
function nativeFileName() {
	if (process.platform !== 'linux') {
		throw new Error(`@aethernet/linux-input-hook is only supported on Linux, got ${process.platform}`);
	}
	switch (process.arch) {
		case 'x64':
