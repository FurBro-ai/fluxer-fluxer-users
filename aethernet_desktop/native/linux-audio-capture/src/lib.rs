...
impl AudioMixRuntimeHandle {
    ...
    fn build(
        ...
        if source_count as usize > aethernet_audio_mix::MAX_MIX_SOURCES {
            return Err(invalid_arg("AudioMixRuntimeHandle exceeds MAX_MIX_SOURCES"));
        }
        let clock: Arc<dyn aethernet_rt_thread::MonotonicClock> =
            Arc::new(aethernet_rt_thread::SystemMonotonicClock::new());
        let mut builder =
    ...
    pub fn source_count_js(&self) -> u32 {
        ...
        assert!(self.source_count as usize <= aethernet_audio_mix::MAX_MIX_SOURCES);
        self.source_count
    }

    #[napi]
    pub fn tick(&self, tick_at_ns: Option<i64>) -> Result<u32> {
        use aethernet_rt_thread::MonotonicClock as _;
        assert!(self.source_count > 0);
        let mut guard = self
            .inner
            .lock()
        ...
        let at_ns: u64 = match tick_at_ns {
            ...
            _ => aethernet_rt_thread::SystemMonotonicClock::new().now_ns(),
        };
        assert!(at_ns > 0);
        let marked = runtime
...
mod js_path_tests {
    ...
    use aethernet_rt_thread::MonotonicClock;
    use std::sync::Arc;
