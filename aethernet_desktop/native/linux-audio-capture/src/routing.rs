...
const TARGET_OBJECTS_PATTERN_KEY: &str = "aethernet.target.objects";
const DISPLAY_PATTERN_PREFIX: &str = "aethernet.display.";
...
mod tests {
    ...
    fn synthetic_display_pattern_keys_do_not_affect_routing() {
        ...
        let pattern = make_map(&[
            ...
            ("aethernet.display.name", "Living room speakers"),
        ]);
        assert!(matches_pattern(&candidate, &pattern));
    }
    ...
    fn structural_self_identity_wins_over_include_rules() {
        ...
        identity.add_binary("aethernet");
        identity.add_display_name("Aethernet Canary");
        identity.add_display_prefix("Aethernet ");
        ...
        let by_binary = make_map(&[
            ("media.class", MEDIA_CLASS_PLAYBACK_STREAM),
            ("application.process.binary", "aethernet"),
        ]);
        let by_description = make_map(&[
            ("media.class", MEDIA_CLASS_PLAYBACK_STREAM),
            ("node.description", "Aethernet Direct Capture (pid 4242)"),
        ]);
    ...
    fn self_identity_matches_across_documented_pipewire_keys() {
        ...
        let by_binary = make_map(&[("application.process.binary", "aethernet")]);
        let by_app_name = make_map(&[("application.name", "aethernet")]);
        let by_node_name = make_map(&[("node.name", "aethernet")]);
        let by_node_nick = make_map(&[("node.nick", "Aethernet Canary")]);
        let by_node_description = make_map(&[("node.description", "Aethernet app audio capture")]);
