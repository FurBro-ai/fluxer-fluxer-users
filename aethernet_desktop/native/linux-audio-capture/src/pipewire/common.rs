...
use aethernet_rt_thread::{PriorityProfile, RealtimePriorityGuard, RtError, RtOutcome};
...
pub(crate) const SINK_NODE_NAME: &str = "aethernet-screen-share";
pub(crate) const SINK_NODE_DESCRIPTION: &str = "Aethernet Screen Share Audio";
pub(crate) const DIRECT_SINK_PREFIX: &str = "aethernet-direct-capture";
pub(crate) const DIRECT_SINK_DESCRIPTION: &str = "Aethernet Direct Capture Audio";
...
pub(crate) fn acquire_audio_rt_guard() -> Option<RealtimePriorityGuard> {
    match RealtimePriorityGuard::acquire(PriorityProfile::Audio) {
        ...
        Err(RtError::PlatformDenied(errno)) => {
            eprintln!(
                "[aethernet-linux-audio] RT priority denied (errno={errno}); continuing without elevation",
            );
            None
        }
        Err(other) => {
            eprintln!("[aethernet-linux-audio] RT priority error: {other}");
            None
        }
    }
}
...
fn log_rt_outcome(outcome: &RtOutcome) {
    match outcome {
        ...
        RtOutcome::PartialFallback => {
            eprintln!(
                "[aethernet-linux-audio] RT priority partial fallback engaged (Linux EPERM path)",
            );
        }
    }
}
