...
use aethernet_screen_frame_bus::NativeScreenFrameSinkHandleRef;
use pipewire as pw;
use pw::channel::{Sender as PwSender, channel as pw_channel};

use aethernet_rt_thread::{MonotonicClock, SystemMonotonicClock};
...
impl PipeWireBridge {
    pub fn open() -> Option<Self> {
        ...
        let handle = thread::Builder::new()
            .name("aethernet-pipewire-bridge".into())
            .spawn(move || {
...
impl PipeWireDirectCapture {
    ...
    pub fn open_with_clock(clock: Arc<dyn MonotonicClock>) -> Option<Self> {
        ...
        let (ready_tx, ready_rx) = std::sync::mpsc::sync_channel::<bool>(1);
        let handle = thread::Builder::new()
            .name("aethernet-pipewire-direct".into())
            .spawn(move || {
...
mod tests {
    ...
    fn direct_stream_props_capture_private_sink_monitor() {
        let props = build_direct_stream_props(
            "aethernet-direct-capture-7-1",
            "aethernet-direct-capture-7-1-stream",
        );
        let dict = props.dict();
        assert_eq!(
            dict.get("node.name"),
            Some("aethernet-direct-capture-7-1-stream")
        );
        ...
        assert_eq!(dict.get("audio.channels"), Some("2"));
        assert_eq!(dict.get("audio.position"), Some("[FL,FR]"));
        assert_eq!(dict.get("target.object"), Some("aethernet-direct-capture-7-1"));
    }
