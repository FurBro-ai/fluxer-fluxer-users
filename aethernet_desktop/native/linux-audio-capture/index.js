...
const MODULE_NAME = '@aethernet/linux-audio-capture';
const SKIP_NATIVE_PROBE_ENV = 'AETHERNET_LINUX_AUDIO_CAPTURE_SKIP_NATIVE_PROBE';
...
function nativeFileName() {
	if (process.platform !== 'linux') {
		throw new Error(`@aethernet/linux-audio-capture is only supported on Linux, got ${process.platform}`);
	}
	switch (process.arch) {
		case 'x64':
