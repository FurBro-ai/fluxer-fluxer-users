...
mod tests {
    ...
    fn request_path_sanitizes_unique_bus_name() {
        assert_eq!(
            request_path(":1.42", "aethernet_fc_open_1"),
            "/org/freedesktop/portal/desktop/request/1_42/aethernet_fc_open_1"
        );
    }

    #[test]
    fn mint_token_is_distinct_and_prefixed() {
        let a = mint_token("aethernet_fc_open");
        let b = mint_token("aethernet_fc_open");
        assert_ne!(a, b);
        assert!(a.starts_with("aethernet_fc_open_"));
    }
}
