...
impl Subscription {
    pub fn configure(
        ...
        let thread = thread::Builder::new()
            .name("aethernet-linux-portals-global-shortcuts".to_string())
            .spawn(move || {
...
async fn create_session(conn: &zbus::Connection) -> Result<String, GlobalShortcutsError> {
    let handle_token = mint_token("aethernet_gs_create");
    let session_handle_token = mint_token("aethernet_gs_session");
...
async fn list_shortcuts(
    ...
    let handle_token = mint_token("aethernet_gs_list");
    let mut stream = request_stream(conn, &handle_token).await?;
    let proxy = global_shortcuts_proxy(conn).await?;
    let mut options: HashMap<&str, Value<'_>> = HashMap::new();
    options.insert("handle_token", Value::new(handle_token.as_str()));
...
async fn bind_shortcuts(
    ...
    let session_path = owned_path(session_handle)?;
    let handle_token = mint_token("aethernet_gs_bind");
    let mut stream = request_stream(conn, &handle_token).await?;
    let proxy = global_shortcuts_proxy(conn).await?;
