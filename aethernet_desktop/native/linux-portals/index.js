...
const MODULE_NAME = '@aethernet/linux-portals';
const SKIP_NATIVE_PROBE_ENV = 'AETHERNET_LINUX_PORTALS_SKIP_NATIVE_PROBE';
...
function nativeFileName() {
	if (process.platform !== 'linux') {
		throw new Error(`@aethernet/linux-portals is only supported on Linux, got ${process.platform}`);
	}
	switch (process.arch) {
		case 'x64':
