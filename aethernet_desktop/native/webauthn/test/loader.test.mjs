...
test('normalization creates spec-shaped client data and Windows transport bits', () => {
	...
	const normalized = webauthnPure.normalizeCreateOptions({
		origin: 'https://web.canary.aethernet.app/channels/@me',
		challenge,
		rp: {id: 'aethernet.app', name: 'Aethernet'},
	...
	assert.equal(normalized.rpId, 'aethernet.app');
	...
	assert.deepEqual(JSON.parse(normalized.clientDataJSON.toString('utf8')), {
		...
		challenge: 'AQIDBA',
		origin: 'https://web.canary.aethernet.app',
		crossOrigin: false,
	});
});
