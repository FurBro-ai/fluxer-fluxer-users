...
const MODULE_NAME = '@aethernet/webauthn';
const SKIP_NATIVE_PROBE_ENV = 'AETHERNET_WEBAUTHN_SKIP_NATIVE_PROBE';
...
function unavailableError() {
	return new Error(
		`@aethernet/webauthn native backend unavailable on ${process.platform}/${process.arch}: ${
...
async function create(options) {
	...
	if (typeof native.create !== 'function') {
		throw new Error(
			`@aethernet/webauthn native backend did not export registration on ${process.platform}/${process.arch}`,
		);
	}
...
async function get(options) {
	...
	if (typeof native.get !== 'function') {
		throw new Error(
			`@aethernet/webauthn native backend did not export authentication on ${process.platform}/${process.arch}`,
		);
	}
