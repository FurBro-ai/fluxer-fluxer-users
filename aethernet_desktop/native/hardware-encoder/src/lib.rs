...
pub fn hardware_encoder_capability() -> HardwareEncoderCapability {
    ...
    let compiled = match platform {
        CapabilityPlatform::Linux => cfg!(aethernet_linux_nvenc),
        CapabilityPlatform::Windows => {
            cfg!(aethernet_windows_nvenc) && cfg!(aethernet_windows_nvenc_encoder)
        }
        _ => false,
    };
    describe_capability(
...
fn describe_videotoolbox_capability(
    ...
    HardwareEncoderCapability {
        ...
        compiled: cfg!(aethernet_macos_videotoolbox),
        runtime: false,
        codecs: Vec::new(),
        zero_copy: false,
        native_inputs: Vec::new(),
...
mod nvenc_probe {
    ...
    fn load_cuda_context_create(cuda: &Library) -> Option<CuCtxCreate> {
        let cuda_version = option_env!("AETHERNET_CUDA_VERSION")
...
mod tests {
    ...
    #[test]
    fn reports_no_supported_codecs_when_guid_query_finds_no_aethernet_codecs() {
        let capability = describe_capability(
            CapabilityPlatform::Linux,
            true,
            Some(NvencRuntimeProbe {
