...
use aethernet_desktop_native::win_process_loopback::audio_contract;
use aethernet_screen_frame_bus::{NativeScreenFrameSinkHandle, NativeScreenFrameSinkHandleRef};
use napi::{
    Env, JsValue,
    bindgen_prelude::{
...
fn classify_windows_process_loopback(
            ...
            match detected {
                Some(build) if build < min_build => format!(
                    "Process loopback activation succeeded on Windows build {build}, \
                     ...
                     Aethernet is enabling it because the runtime API probe passed."
                ),
                Some(build) => format!(
                    "Process loopback activation succeeded on Windows build {build} \
    ...
    match detected {
        None => (
            false,
            format!(
                "@aethernet/win-process-loopback only supports Windows. \
        ...
        Some(build) if build >= min_build => (
            ...
            format!(
                "Windows build {build} meets Microsoft's documented \
                 {MIN_WINDOWS_VERSION_LABEL}, but Aethernet's runtime process-loopback probe \
        ...
        Some(build) => (
            ...
            format!(
                "Microsoft documents process loopback as requiring \
                 ...
                 and Aethernet's runtime process-loopback probe failed. Aethernet must not use \
                 system-wide endpoint loopback because it cannot exclude Aethernet's WebRTC \
...
fn classify_windows_capabilities(
    ...
    let reason = if supported {
        if process_exclude_supported {
            ...
            "Per-process audio loopback include mode is supported; desktop audio uses Aethernet's session mixer fallback."
                .to_owned()
        }
...
pub fn get_backend_info() -> WinProcessLoopbackBackendInfo {
    ...
    let min_build =
        aethernet_desktop_native::win_process_loopback::windows_version::PROCESS_LOOPBACK_MIN_BUILD;
    let (
        supported,
        process_supported,
        system_supported,
...
mod platform {
    ...
    use aethernet_desktop_native::win_process_loopback::{
        audio_contract, process_tree, session_mixer,
    };
    use aethernet_rt_thread::{PriorityProfile, RealtimePriorityGuard};
    ...
    impl Inner {
        pub(crate) fn new(
            ...
            if is_self_or_descendant_pid(pid) && !is_excluding_self_system_scope {
                return Err(Error::new(
                    ...
                    "ProcessLoopback refuses to capture Aethernet's own process tree",
                ));
            }
            Ok(Self {
                pid,
        ...
        pub(crate) fn start(&self) -> Result<()> {
            ...
            let join = match thread::Builder::new()
                .name("aethernet-win-process-loopback".to_string())
                .spawn(move || {
    ...
    fn run_process_loopback_probe() -> ProcessLoopbackRuntimeProbe {
        ...
        let _ = thread::Builder::new()
            .name("aethernet-win-process-loopback-probe".to_owned())
            .spawn(move || {
