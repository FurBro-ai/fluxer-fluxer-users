...
const MODULE_NAME = '@aethernet/win-process-loopback';
...
function getBackendInfo() {
	if (!binding) {
		const reason =
			process.platform !== 'win32'
				? `@aethernet/win-process-loopback only supports Windows, got ${process.platform}. Microsoft's documented process-loopback minimum is Windows build 20348.`
				: `@aethernet/win-process-loopback native binary unavailable: ${loadError?.message ?? 'unknown reason'}`;
		return {
	...
	try {
		...
		return {
			backend: 'win-process-loopback',
			supported: false,
			reason: `@aethernet/win-process-loopback getBackendInfo failed: ${err?.message ?? String(err)}`,
			processSupported: false,
			systemSupported: false,
			systemExcludesSelf: false,
			processIncludeSupported: false,
...
class ProcessLoopback extends EventEmitter {
	constructor(pid, opts = {}) {
		super();
		if (!binding) {
			throw loadError ?? new Error('@aethernet/win-process-loopback binding unavailable');
		}
