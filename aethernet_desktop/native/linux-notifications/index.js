...
const MODULE_NAME = '@aethernet/linux-notifications';
const SKIP_NATIVE_PROBE_ENV = 'AETHERNET_LINUX_NOTIFICATIONS_SKIP_NATIVE_PROBE';
...
function nativeFileName() {
	if (process.platform !== 'linux') {
		throw new Error(`@aethernet/linux-notifications is only supported on Linux, got ${process.platform}`);
	}
	switch (process.arch) {
		case 'x64':
