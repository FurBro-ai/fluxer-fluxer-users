...
const MODULE_NAME = '@aethernet/mac-sysctl';
const SKIP_NATIVE_PROBE_ENV = 'AETHERNET_MAC_SYSCTL_SKIP_NATIVE_PROBE';
...
function nativeFileName() {
	if (process.platform !== 'darwin') {
		throw new Error(`@aethernet/mac-sysctl is only supported on macOS, got ${process.platform}`);
	}
	switch (process.arch) {
		case 'x64':
