...
pub fn write_key_name(buf: &mut [u8; KEY_NAME_BUF], len_out: &mut u8, vk: u16) {
    use aethernet_desktop_native::input::keymap::{fallback_name, windows::vk_to_name};
...
mod platform {
    ...
    fn sample_modifiers() -> (bool, bool, bool, bool) {
        use aethernet_desktop_native::input::modifiers::windows::from_sampled;
        unsafe {
            let m = from_sampled(
    ...
    unsafe extern "system" fn mouse_proc(code: i32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
        use aethernet_desktop_native::input::windows_mouse::{Action, Axis, classify};
        if code >= 0
            && let Some(handle) = current_handle()
        {
    ...
    pub(crate) fn start(handle: Arc<HookHandle>) -> Result<(), String> {
        ...
        *slot = Some(handle.clone());
        ...
        let worker = {
            ...
            match thread::Builder::new()
                .name("aethernet-input-hook-worker".into())
        ...
        *lock_recover(&handle.worker_thread) = Some(worker);
        ...
        let watchdog = {
            let h = handle.clone();
            match thread::Builder::new()
                .name("aethernet-input-hook-watchdog".into())
