...
const MODULE_NAME = '@aethernet/linux-screen-capture';
const SKIP_NATIVE_PROBE_ENV = 'AETHERNET_LINUX_SCREEN_CAPTURE_SKIP_NATIVE_PROBE';
...
function nativeFileName() {
	if (process.platform !== 'linux') {
		throw new Error(`@aethernet/linux-screen-capture is only supported on Linux, got ${process.platform}`);
	}
	switch (process.arch) {
		case 'x64':
		...
		return {
			...
			reason:
				process.platform === 'linux'
					? `@aethernet/linux-screen-capture native binary unavailable: ${loadError?.message ?? 'unknown reason'}`
					: `@aethernet/linux-screen-capture is only supported on Linux, got ${process.platform}`,
...
function getGameCaptureLaunchEnvironment(options = {}) {
	...
	return {
		...
		diagnostics: {
			...
			glCaptureLib,
			licenseBoundary:
				'obs-vkcapture hook assets are separate GPL-covered runtime tools; Aethernet communicates through the OBS-compatible socket protocol.',
		},
	};
}
...
class ScreenCapture extends EventEmitter {
	constructor(options = {}) {
		super();
		if (!binding) {
			throw loadError || new Error('@aethernet/linux-screen-capture binding unavailable');
		}
		...
		try {
			if (this.frameSinkHandle != null) {
				if (typeof this.native.setFrameSinkHandle !== 'function') {
					throw new Error('@aethernet/linux-screen-capture native binding does not support frame sink handles');
				}
				this.native.setFrameSinkHandle(this.frameSinkHandle);
			} else if (this.nativeFrameSinkRequired) {
				throw new Error('@aethernet/linux-screen-capture native frame sink handle is required');
			}
