...
const DESKTOP_ENTRY_ID_ENV: &str = "AETHERNET_LINUX_DESKTOP_ENTRY_ID";
...
fn watch_request(
    ...
    let listener = std::thread::Builder::new()
        .name("aethernet-linux-screen-capture-req".to_string())
        .spawn(move || {
            response_listener(
                conn_for_thread,
                &expected_for_thread,
...
pub fn open_session_and_pick() -> Result<(LiveSession, Vec<StreamInfo>), PortalError> {
    ...
    let create_token = mint_token("aethernet_sc_create");
    let session_token = mint_token("aethernet_sc_session");
    ...
    let select_token = mint_token("aethernet_sc_select");
    ...
    let start_token = mint_token("aethernet_sc_start");
...
mod tests {
    ...
    fn request_path_sanitizes_unique_bus_name() {
        assert_eq!(
            request_path(":1.42", "aethernet_sc_create_1"),
            "/org/freedesktop/portal/desktop/request/1_42/aethernet_sc_create_1"
        );
    }
    ...
    fn session_path_sanitizes_unique_bus_name() {
        assert_eq!(
            session_path(":1.42", "aethernet_sc_session_1"),
            "/org/freedesktop/portal/desktop/session/1_42/aethernet_sc_session_1"
        );
    }

    #[test]
    fn mint_token_is_distinct_and_prefixed() {
        let a = mint_token("aethernet_sc_create");
        let b = mint_token("aethernet_sc_create");
        assert_ne!(a, b);
        assert!(a.starts_with("aethernet_sc_create_"));
    }
    ...
    #[test]
    fn normalize_desktop_entry_app_id_accepts_aethernet_ids() {
        assert_eq!(
            normalize_desktop_entry_app_id("aethernet-canary"),
            Some("aethernet-canary".to_string())
        );
        assert_eq!(
            normalize_desktop_entry_app_id("aethernet-canary.desktop"),
            Some("aethernet-canary".to_string())
        );
        assert_eq!(
            normalize_desktop_entry_app_id("app.aethernet.canary"),
            Some("app.aethernet.canary".to_string())
        );
    }
    ...
    fn normalize_desktop_entry_app_id_rejects_paths_and_empty_values() {
        ...
        assert_eq!(normalize_desktop_entry_app_id("../aethernet-canary"), None);
        assert_eq!(normalize_desktop_entry_app_id("aethernet canary"), None);
    }
