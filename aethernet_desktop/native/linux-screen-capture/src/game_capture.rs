...
const IMPORT_MODE_ENV: &str = "AETHERNET_LINUX_GAME_CAPTURE_IMPORT_MODE";
...
impl GameCaptureVideoStream {
    pub fn open(
        ...
        let handle = thread::Builder::new()
            .name("aethernet-linux-game-capture-vkcapture".to_string())
            .spawn(move || {
                run_server(
                    frame_rate,
                    import_state,
...
impl GameCaptureImportState {
    ...
    fn initial_reason(self) -> String {
        ...
        match self.policy {
            ...
            GameCaptureImportPolicy::Fixed(GameCaptureImportMode::LinearHostMapped) => {
                "Aethernet requests OBS linear host-mapped capture for CPU NV12 conversion".to_string()
            }
            GameCaptureImportPolicy::Fixed(mode) => format!(
                "Aethernet requests OBS {} capture for Linux native GPU import",
                mode.as_str()
            ),
        }
    }
                        ...
                        RecvMessage::Texture(data, fds) => {
                            match map_texture(data, fds, import_state.current, &on_lifecycle) {
                                ...
                                None => {
                                    ...
                                    {
                                        let reason = format!(
                                            "OBS import mode {} was not usable in this Aethernet backend; requesting {}",
                                            from.as_str(),
                                            to.as_str()
                                        );
...
mod tests {
    ...
    fn non_host_mapped_import_forwards_dmabuf_without_mapping() {
        let fd = memfd_with_data("aethernet-zero-copy-dmabuf-test", &[0u8; 16 * 16 * 4]);
    ...
    fn host_mapped_import_reads_cpu_nv12_and_preserves_source_dmabuf_metadata() {
        ...
        let fd = memfd_with_data("aethernet-host-mapped-texture-test", &bgra);
