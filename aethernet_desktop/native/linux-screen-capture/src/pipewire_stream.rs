...
const SCREEN_CAPTURE_DMABUF_ENV: &str = "AETHERNET_SCREEN_CAPTURE_DMABUF";
...
impl VideoFrameData {
    ...
    pub fn into_bus_frame_data(self) -> aethernet_screen_frame_bus::FrameData {
        let len = self.len();
        let data = match self {
            Self::Empty => aethernet_screen_frame_bus::FrameData::from(Vec::new()),
            Self::Owned(v) => aethernet_screen_frame_bus::FrameData::from(v),
...
fn spawn_pw_thread(
    ...
    assert!(pool.bytes_per_buffer() > 0);
    thread::Builder::new()
        .name("aethernet-linux-screen-capture-pw".to_string())
        .spawn(move || {
            run_worker(
                portal_fd,
                node_id,
...
fn spawn_convert_worker(
    ...
    assert!(pool.bytes_per_buffer() > 0);
    thread::Builder::new()
        .name("aethernet-linux-screen-capture-convert".to_string())
        .spawn(move || {
...
fn run_worker(
    ...
    let stream_props = properties! {
        ...
        *keys::NODE_NAME => "aethernet-screen-capture",
    };

    let Ok(stream) = pw::stream::StreamRc::new(core.clone(), "aethernet-screen-capture", stream_props)
    else {
        let _ = ready_tx.send(false);
        return;
    };
...
mod tests {
    ...
    fn convert_worker_returns_slot_after_processing() {
        ...
        let handle = thread::Builder::new()
            .name("aethernet-test-convert".to_string())
            .spawn(move || {
