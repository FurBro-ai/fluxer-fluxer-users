...
Aethernet can launch Linux games with OBS-compatible Vulkan/OpenGL capture enabled.
The Aethernet code in this package does not include obs-vkcapture source code; it only
...
- `default-dmabuf`, `no-modifiers-dmabuf`, and `linear-dmabuf` request GPU
  ...
  descriptors end to end, Aethernet reports them as requested rather than available.
...
If Aethernet ships obs-vkcapture hook binaries under `obs-vkcapture/`, treat those
...
- Do not copy obs-vkcapture implementation code into AGPL-licensed Aethernet modules
