...
use aethernet_encoder_ring::EncoderFrameRate;
#[cfg(target_os = "windows")]
use aethernet_screen_frame_bus::EnqueueOutcome;
#[cfg(target_os = "windows")]
...
pub struct CaptureInner {
    ...
    pub native_frame_sink:
        Mutex<Option<Arc<aethernet_screen_frame_bus::NativeScreenFrameSinkHandleRef>>>,
    #[cfg(target_os = "windows")]
...
impl BusSharedTexture {
    fn into_bus_desc(self) -> aethernet_screen_frame_bus::SharedTextureDesc {
        aethernet_screen_frame_bus::SharedTextureDesc {
            handle: self.handle,
            width: self.width,
            height: self.height,
...
pub(crate) enum FrameSinkRef {
    Native(Arc<aethernet_screen_frame_bus::NativeScreenFrameSinkHandleRef>),
    Bus(Arc<dyn aethernet_screen_frame_bus::ScreenFrameSink>),
}
...
pub(crate) fn resolve_frame_sink(
    ...
    aethernet_screen_frame_bus::get_sink(capture_id).map(FrameSinkRef::Bus)
}
...
pub(crate) fn emit_shared_texture_frame(
    ...
    let outcome = match sink {
        ...
        FrameSinkRef::Bus(sink) => {
            sink.enqueue(aethernet_screen_frame_bus::ScreenFrame::SharedTexture(desc))
        }
    };
...
fn native_frame_sink_for(
    inner: &CaptureInner,
) -> Option<Arc<aethernet_screen_frame_bus::NativeScreenFrameSinkHandleRef>> {
...
fn retain_native_frame_sink_handle(
    value: Unknown<'_>,
) -> Result<Arc<aethernet_screen_frame_bus::NativeScreenFrameSinkHandleRef>> {
    ...
    let handle = unsafe {
        data.cast::<aethernet_screen_frame_bus::NativeScreenFrameSinkHandle>()
            .as_ref()
    }
    .and_then(aethernet_screen_frame_bus::NativeScreenFrameSinkHandle::retain_ref)
    .ok_or_else(|| {
        napi::Error::new(
            Status::InvalidArg,
