...
mod tests {
    ...
    fn obs_high_precision_formats_without_reader_conversion_stay_unsupported() {
        assert_eq!(
            ...
            None,
            "OBS maps RGBA16 UNORM, but Aethernet needs a row converter before claiming support"
        );
        assert_eq!(
            SourceFormat::classify(DXGI_FORMAT_R32G32B32A32_FLOAT, GAME_CAPTURE_FLAG_HDR),
            None,
            "OBS maps RGBA32F, but Aethernet has no reader-side conversion for it yet"
        );
        assert_eq!(
            SourceFormat::classify(
