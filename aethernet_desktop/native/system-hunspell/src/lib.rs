...
use aethernet_desktop_native::system_hunspell::dictionaries::{EnvSnapshot, discover_dictionaries};
use aethernet_desktop_native::system_hunspell::encoding::is_utf8_encoding;
use aethernet_desktop_native::system_hunspell::hashing::hash_file_to_hex;
use napi::Task;
...
mod tests {
    ...
    fn dictionary_fixture_spells_and_accepts_runtime_words() {
        ...
        assert!(!dict.spell("Aethernet").unwrap());
        dict.add("Aethernet").unwrap();
        assert!(dict.spell("Aethernet").unwrap());
        dict.remove("Aethernet").unwrap();
        assert!(!dict.spell("Aethernet").unwrap());
