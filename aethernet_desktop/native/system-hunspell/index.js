...
const MODULE_NAME = '@aethernet/system-hunspell';
const SKIP_NATIVE_PROBE_ENV = 'AETHERNET_SYSTEM_HUNSPELL_SKIP_NATIVE_PROBE';
...
function nativeFileName() {
	if (process.platform !== 'linux') {
		throw new Error(`@aethernet/system-hunspell is only supported on Linux, got ${process.platform}`);
	}
	switch (process.arch) {
		case 'x64':
