...
const MODULE_NAME = '@aethernet/win-toast';
...
function nativeFileName() {
	if (process.platform !== 'win32') {
		throw new Error(`@aethernet/win-toast is only supported on Windows, got ${process.platform}`);
	}
	switch (process.arch) {
		case 'x64':
...
const stubAsync = () => Promise.reject(loadError ?? new Error('@aethernet/win-toast not loaded'));
