...
fn create_tap_description(
    ...
    let name = match scope {
        CaptureScope::Process => format!("Aethernet app audio tap pid {pid}"),
        CaptureScope::System => format!("Aethernet desktop audio tap excluding pid {pid}"),
    };
    unsafe {
...
fn create_aggregate_description(
    ...
    let uid_string = format!("app.aethernet.mac-app-audio.tap.{pid}.{counter}");
    let uid = NSString::from_str(&uid_string);
    let name = NSString::from_str("Aethernet app audio capture");
...
impl Capture {
    pub fn create(
        ...
        DEBUG_COREAUDIO.store(
            getenv_set(c"AETHERNET_MAC_APP_AUDIO_DEBUG_COREAUDIO"),
            Ordering::Release,
        );
...
fn spawn_late_spawn_refresher(
    ...
    let thread = thread::Builder::new()
        .name("aethernet-mac-tap-refresh".into())
        .spawn(move || {
