...
const MODULE_NAME = '@aethernet/mac-app-audio';
...
function nativeFileName() {
	if (process.platform !== 'darwin') {
		throw new Error(`@aethernet/mac-app-audio is only supported on macOS, got ${process.platform}`);
	}
	switch (process.arch) {
		case 'x64':
		...
		return {
			...
			reason:
				process.platform === 'darwin'
					? `@aethernet/mac-app-audio native binary unavailable: ${loadError?.message ?? 'unknown reason'}`
					: `@aethernet/mac-app-audio is only supported on macOS, got ${process.platform}`,
...
class ProcessLoopback extends EventEmitter {
	constructor(pid, options = {}) {
		super();
		if (!binding) {
			throw loadError || new Error('@aethernet/mac-app-audio binding unavailable');
		}
		this.pid = pid;
