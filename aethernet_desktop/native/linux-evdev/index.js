...
const MODULE_NAME = '@aethernet/linux-evdev';
const SKIP_NATIVE_PROBE_ENV = 'AETHERNET_LINUX_EVDEV_SKIP_NATIVE_PROBE';
...
function nativeFileName() {
	if (process.platform !== 'linux') {
		throw new Error(`@aethernet/linux-evdev is only supported on Linux, got ${process.platform}`);
	}
	switch (process.arch) {
		case 'x64':
