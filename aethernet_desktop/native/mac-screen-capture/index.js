...
const MODULE_NAME = '@aethernet/mac-screen-capture';
...
function nativeFileName() {
	if (process.platform !== 'darwin') {
		throw new Error(`@aethernet/mac-screen-capture is only supported on macOS, got ${process.platform}`);
	}
	switch (process.arch) {
		case 'x64':
		...
		return {
			...
			reason:
				process.platform === 'darwin'
					? `@aethernet/mac-screen-capture native binary unavailable: ${loadError?.message ?? 'unknown reason'}`
					: `@aethernet/mac-screen-capture is only supported on macOS, got ${process.platform}`,
...
class ScreenCapture extends EventEmitter {
	constructor(options = {}) {
		super();
		if (!binding) {
			throw loadError || new Error('@aethernet/mac-screen-capture binding unavailable');
		}
		...
		try {
			if (this.frameSinkHandle != null) {
				if (typeof this.native.setFrameSinkHandle !== 'function') {
					throw new Error('@aethernet/mac-screen-capture native binding does not support native frame sink handles');
				}
