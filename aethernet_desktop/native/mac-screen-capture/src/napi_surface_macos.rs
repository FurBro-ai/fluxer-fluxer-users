...
use aethernet_screen_frame_bus::{
...
struct CaptureState {
    ...
    delegate: Option<Retained<AethernetSCKScreenSource>>,
...
define_class!(
    ...
    #[name = "AethernetSCKScreenSource"]
    #[ivars = DelegateIvars]
    pub struct AethernetSCKScreenSource;

    unsafe impl NSObjectProtocol for AethernetSCKScreenSource {}

    unsafe impl SCStreamDelegate for AethernetSCKScreenSource {
    ...
    unsafe impl SCStreamOutput for AethernetSCKScreenSource {
...
impl ScreenCapture {
    ...
    pub async fn start(
        ...
        *self.inner.bus_sink.write() = if self.inner.native_frame_sink.read().is_some() {
        ...
        let stream_name = format!("Aethernet ScreenCapture ({}:{})", source_kind, source_id);
        let nsname = NSString::from_str(&stream_name);
        sck::cfg_set_stream_name_if_available(&cfg, &nsname);

        let delegate = AethernetSCKScreenSource::new(Arc::downgrade(&self.inner));
...
fn build_scstream_sample_queue(
    ...
    let label = format!("com.aethernet.scstream.{}-{}", source_kind, source_id);
