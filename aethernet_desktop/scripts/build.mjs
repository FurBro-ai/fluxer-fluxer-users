...
const isProduction =
	...
	process.env.AETHERNET_DESKTOP_PRODUCTION === 'true' ||
	process.env.GITHUB_ACTIONS === 'true';
const skipNative = process.env.AETHERNET_SKIP_NATIVE === 'true';
...
const pathAliasPlugin = {
	...
	setup(build) {
		...
		build.onResolve({filter: /^@aethernet\/voice_engine_v2(?:\/.*)?$/}, (args) => {
			const packageSrcDir = path.join(ROOT_DIR, '..', 'packages', 'voice_engine_v2', 'src');
			if (args.path === '@aethernet/voice_engine_v2') {
				return {path: path.join(packageSrcDir, 'index.ts')};
			}
			const relativePath = args.path.replace(/^@aethernet\/voice_engine_v2\//, '');
...
function removeStaleWinGameCaptureArtifacts(packageDir, primaryNodeFileName) {
	...
	const stale = fs
		...
		.filter((entry) => {
			...
			return (
				entry.name.startsWith('aethernet-game-hook.') ||
				entry.name.startsWith('aethernet-inject-helper.') ||
				entry.name.startsWith('aethernet-vulkan-layer.') ||
	...
	for (const fileName of stale) {
		...
		console.log(`  Removed stale @aethernet/win-game-capture artifact ${path.relative(ROOT_DIR, artifactPath)}`);
	}
}

function addWinGameCaptureRuntimeArtifacts(artifacts, tag) {
	artifacts.push({
		label: '@aethernet/win-game-capture',
		relativePath: `win-game-capture.${tag}.node`,
	});
}

function copyRuntimeArtifactsToInstalledPackages({label, packageDir}) {
	const primaryNodeFileName = label === '@aethernet/win-game-capture' ? primaryWinGameCaptureNodeFileName() : null;
...
function buildNativeAddons() {
	if (skipNative) {
		console.log('Skipping native addons (AETHERNET_SKIP_NATIVE=true).');
		return;
	}
	buildNativeAddon({
		label: '@aethernet/webauthn',
