...
describe('OpenExternal URL validation', () => {
	test('allows standard and custom external protocols', () => {
		...
		assert.equal(shouldOpenExternalUrl('https://aethernet.app'), true);
		assert.equal(shouldOpenExternalUrl('http://aethernet.app'), true);
		assert.equal(shouldOpenExternalUrl('mailto:support@aethernet.app'), true);
		assert.equal(shouldOpenExternalUrl('tel:+15551234567'), true);
		assert.equal(shouldOpenExternalUrl('aethernet://invite/test'), true);
	});
	...
	test('opens normalized URLs once inside the dedupe window', async () => {
		...
		await openExternalDeduped('HTTPS://Aethernet.App/path');
		await openExternalDeduped('https://aethernet.app/path');

		assert.deepEqual(openExternalCalls, ['https://aethernet.app/path']);
	});
