...
import {removeAethernetVulkanLayerRegistrations} from '@electron/main/WindowsVulkanLayerCleanup';
...
if (launchConfigurationError) {
	console.error(`AETHERNET desktop launch configuration error: ${launchConfigurationError.message}`);
	...
	void getDesktopDebugInfo(userDataConfig.base, {nativeProbes: false})
		...
		.catch((error: unknown) => {
			...
			writeCliAndExit(process.stderr, `Failed to collect AETHERNET desktop debug info: ${message}`, 1);
		});
} else {
	...
	if (process.platform === 'linux') {
		process.env.AETHERNET_LINUX_DESKTOP_ENTRY_ID = LINUX_DESKTOP_ENTRY_ID;
	}
	...
	try {
		...
		try {
			dialog.showErrorBox('AETHERNET failed to start', message);
		} catch {}
		app.exit(1);
		process.exit(1);
	}
	...
	if (!gotTheLock) {
		...
		runStartupPhaseAsync('gpu-driver-workarounds', appendWindowsGpuDriverWorkaroundSwitches)
			...
			.then(async () => {
				...
				try {
					runStartupPhase('vulkan-layer-cleanup', removeAethernetVulkanLayerRegistrations);
				} catch (error: unknown) {
