...
const DEBUG_INFO_ARGS = new Set(['--aethernet-debug-info', '--aethernet-client-info', '--client-info']);
const DISABLE_HARDWARE_ACCELERATION_ARGS = new Set([
	'--aethernet-disable-gpu',
	'--aethernet-disable-hardware-acceleration',
]);
const OPEN_DEVTOOLS_ARGS = new Set(['--aethernet-devtools', '--aethernet-open-devtools']);
const RESET_WINDOW_STATE_ARGS = new Set(['--aethernet-reset-window-state']);
const SAFE_MODE_ARGS = new Set(['--aethernet-safe-mode']);
const RENDERER_CONSOLE_LOG_ARGS = new Set(['--aethernet-log-renderer-console']);
const NET_LOG_ARGS = new Set(['--aethernet-net-log']);
const APP_URL_ARGS = new Set(['--aethernet-app-url']);
...
export function getLaunchAppUrlOverride(argv: ReadonlyArray<string>): string | null {
	...
	if (!trimmed) {
		throw new Error('--aethernet-app-url requires a URL');
	}
	const candidate = /^[a-zA-Z][a-zA-Z0-9+\-.]*:\/\//.test(trimmed) ? trimmed : `https://${trimmed}`;
	const url = new URL(candidate);
	if (url.protocol !== 'https:' && url.protocol !== 'http:') {
		throw new Error('--aethernet-app-url must use http or https');
	}
	return url.toString();
}
...
function sanitizeLaunchArg(arg: string): string {
	if (/^aethernet:\/\//i.test(arg)) {
		return 'aethernet://<redacted>';
	}
	if (arg.startsWith('--aethernet-app-url=')) {
		return `--aethernet-app-url=${sanitizeUrlForDiagnostics(arg.slice('--aethernet-app-url='.length))}`;
	}
