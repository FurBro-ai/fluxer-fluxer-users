...
const VOICE_DEBUG_EVENT_SINK_WINDOW_TITLE = 'AETHERNET | Voice Debug Event Sink';
export const VOICE_DEBUG_EVENT_SINK_POPOUT_KEY = 'aethernet-voice-debug-event-sink';
...
const popoutKey = "aethernet-voice-debug-event-sink";
let entries = [];
let statsHtml = "";
let statsReceivedAt = "";
let isSticky = true;
...
function sendSnapshotToPopout(window: BrowserWindow): void {
	pendingPopoutEntries = [];
	void runPopoutFunction(window, '__aethernetVoiceDebugEventSinkSetEntries', eventSinkEntries).catch((error) => {
		log.warn('Failed to send voice debug event sink snapshot to popout', error);
	});
	void runPopoutFunction(window, '__aethernetVoiceDebugEventSinkSetStatsHtml', eventSinkStatsHtml).catch((error) => {
...
function flushPendingEntriesToPopout(): void {
	...
	void runPopoutFunction(window, '__aethernetVoiceDebugEventSinkAppendEntries', entries).catch((error) => {
