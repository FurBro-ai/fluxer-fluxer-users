...
const AUTHOR_DIR = path.win32.join(PROGRAMS_DIR, 'Aethernet Platform AB');
const ROOT_SHORTCUT = path.win32.join(PROGRAMS_DIR, 'Aethernet Canary.lnk');
const AUTHOR_SHORTCUT = path.win32.join(AUTHOR_DIR, 'Aethernet Canary.lnk');
const ROOT_APP_DIR = 'C:\\Users\\csh\\AppData\\Local\\aethernet_desktop_canary';
const CURRENT_DIR = path.win32.join(ROOT_APP_DIR, 'current');
const CURRENT_EXE = path.win32.join(CURRENT_DIR, 'Aethernet Canary.exe');
const STALE_EXE = path.win32.join(ROOT_APP_DIR, 'app-1.0.0', 'Aethernet Canary.exe');
const LEGACY_APP_USER_MODEL_ID = 'velopack.aethernet_desktop_canary';
	...
	const context = vm.createContext({
		...
		require: (specifier) => {
			...
			if (specifier === 'node:module') {
				return {
					createRequire: () => (moduleName) => {
						if (moduleName === '@aethernet/win-shell') return winShell;
			...
			if (specifier === '@electron/common/DesktopIdentity') {
				return {
					DESKTOP_APP_NAME: 'Aethernet Canary',
					WINDOWS_APP_USER_MODEL_ID: 'Aethernet.Aethernet.Canary',
					WINDOWS_LEGACY_APP_USER_MODEL_IDS: [LEGACY_APP_USER_MODEL_ID],
					WINDOWS_SHORTCUT_AUTHOR: 'Aethernet Platform AB',
...
describe('Windows Start Menu shortcut repair', () => {
	...
	test('rewrites a stale root shortcut in place instead of relocating it', async () => {
		...
		assert.equal(rewritten[0].appUserModelId, 'Aethernet.Aethernet.Canary');
		assert.equal(harness.files.has(AUTHOR_SHORTCUT), false);
	});
