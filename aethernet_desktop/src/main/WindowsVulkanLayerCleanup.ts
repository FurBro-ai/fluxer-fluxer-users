...
const AETHERNET_VULKAN_LAYER_MANIFEST_FILE_NAME = /^aethernet-vulkan-layer\.win32-(?:x64|ia32|arm64)-msvc\.json$/;
const AETHERNET_VULKAN_LAYER_PACKAGE_DIRECTORY_NAMES = new Set(['win-game-capture', 'win-screen-capture']);
...
function isAethernetVulkanLayerValue(valueName: string): boolean {
	const segments = normalizeVulkanLayerValueName(valueName).split('\\');
	const fileName = segments.at(-1) ?? '';
	const packageDirectoryName = segments.at(-2) ?? '';
	if (!AETHERNET_VULKAN_LAYER_MANIFEST_FILE_NAME.test(fileName)) return false;
	return AETHERNET_VULKAN_LAYER_PACKAGE_DIRECTORY_NAMES.has(packageDirectoryName);
}
...
export function removeAethernetVulkanLayerRegistrations(): void {
	...
	for (const root of VULKAN_REGISTRY_ROOTS) {
		...
		for (const valueName of valueNames) {
			if (!isAethernetVulkanLayerValue(valueName)) continue;
			try {
				deleteVulkanLayerRegistryValue(root, valueName);
			} catch (error) {
				log.warn('[VulkanLayerCleanup] Failed to remove Aethernet Vulkan layer registry value', {
					root,
					valueName,
					error,
				});
			...
			log.info('[VulkanLayerCleanup] Removed Aethernet Vulkan layer registry value', {root, valueName});
		}
	}
}
