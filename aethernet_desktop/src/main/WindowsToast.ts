...
function loadAddon(): WinToastModule | null {
	...
	try {
		const mod = requireModule('@aethernet/win-toast') as WinToastModule;
		if (mod.loadError) {
			logger.warn('@aethernet/win-toast loaded but reported load error', {error: mod.loadError});
			cached = null;
			return cached;
		}
		cached = mod;
		...
		logger.info('@aethernet/win-toast not available; rich toasts disabled', {error});
		cached = null;
		return cached;
	}
}
