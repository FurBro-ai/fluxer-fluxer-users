...
import {buildAethernetAudioExcludePatterns, isKnownAethernetAudioProcessPid} from './AethernetAudioIdentity';
...
function normalizeLinuxRoutingRule(rule: LinuxRoutingRule): LinuxRoutingRule {
	...
	for (const pattern of buildAethernetAudioExcludePatterns()) {
		appendUniquePattern(exclude, pattern);
	}
...
function loadNativeAudioAddon(): NativeAddonLoadResult {
	...
	if (process.platform === 'win32') {
		try {
			const addon = requireModule('@aethernet/win-process-loopback') as WindowsNativeAudioModule;
			if (addon.loadError) {
	...
	if (process.platform === 'darwin') {
		try {
			const addon = requireModule('@aethernet/mac-app-audio') as MacNativeAudioModule;
			if (addon.loadError) {
				const detail = describeAddonLoadError(addon.loadError);
	...
	if (process.platform === 'linux') {
		try {
			const addon = requireModule('@aethernet/linux-audio-capture') as LinuxNativeAudioModule;
...
async function resolveAudioRootPidForSource(sourceId: unknown): Promise<number | null> {
	...
	try {
		if (loadResult.platform === 'win32') {
			...
			if (isKnownAethernetAudioProcessPid(pid)) return null;
			const rootPid = addon.resolveAudioRootPid(pid);
			const resolved = Number.isFinite(rootPid) && rootPid > 0 ? rootPid : pid;
			return isKnownAethernetAudioProcessPid(resolved) ? null : resolved;
		}
		if (loadResult.platform === 'darwin') {
			const addon = loadResult.addon as MacNativeAudioModule;
			const pid = addon.pidFromWindowId(Number(windowToken));
			if (!Number.isFinite(pid) || pid <= 0) return null;
			return isKnownAethernetAudioProcessPid(pid) ? null : pid;
		}
...
async function startNativeAudioCapture(
	...
	if (
		...
		isKnownAethernetAudioProcessPid(targetPid)
	) {
		throw new Error('Refusing to capture native audio from AETHERNET process');
	}
	...
	if (requestedScope === 'system' && availability.capabilities?.systemExcludesSelf !== true) {
		throw new Error(`Native ${loadResult.platform} system audio capture does not guarantee AETHERNET self-exclusion`);
	}
