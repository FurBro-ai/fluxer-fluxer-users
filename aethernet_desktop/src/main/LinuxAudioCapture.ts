...
import {buildAethernetAudioExcludePatterns, isAethernetAudioNode} from './AethernetAudioIdentity';
import {
...
function mergeFreshExclusions(data: RoutingRule): Array<VirtmicNode> {
	...
	for (const pattern of buildAethernetAudioExcludePatterns()) append(pattern);
...
function loadAddon(): LoadResult {
	...
	try {
		mod = requireModule('@aethernet/linux-audio-capture') as AudioCaptureModule;
	} catch (error) {
...
function buildExclusions(
	...
	excludes.push(...buildAethernetAudioExcludePatterns());
...
function listVirtmicTargets(_options?: {granular?: boolean}): {
	...
	try {
		...
		const targets = raw.filter((node) => !isAethernetAudioNode(node));
...
async function resolveWindowPidViaX11(xid: string): Promise<number | null> {
	...
	const portals = requireModule('@aethernet/linux-portals') as {
		resolveX11WindowPid: ((token: string) => Promise<number | null>) | null;
	};
	if (!portals.resolveX11WindowPid) {
		logger.warn('@aethernet/linux-portals.resolveX11WindowPid not available; X11 pid lookup skipped', {xid});
		return null;
	}
	try {
...
async function resolveWindowPidViaKWin(token: string): Promise<number | null> {
	...
	if (!portals.resolveKwinWindowPid) {
		logger.warn('@aethernet/linux-portals.resolveKwinWindowPid not available; KWin pid lookup skipped', {token});
		return null;
	}
	try {
...
async function resolveWindowPidViaGnomeShell(token: string): Promise<number | null> {
	...
	if (!portals.resolveWindowPid) {
		logger.warn('@aethernet/linux-portals.resolveWindowPid not available; GNOME pid lookup skipped', {token});
		return null;
	}
	try {
