...
function loadAethernetWebAuthnAddon(): NativeWebAuthnAddon | null {
	try {
		const addon = requireModule('@aethernet/webauthn') as NativeWebAuthnAddon;
		logger.info('@aethernet/webauthn initialized', addon.getBackendInfo?.());
		return addon;
	} catch (error) {
		logger.warn(`@aethernet/webauthn failed to initialize on ${process.platform}/${process.arch}.`, error);
		return null;
	}
}
...
function createNativePasskeyProvider(): PasskeyProvider {
	const aethernetAddon = loadAethernetWebAuthnAddon();
	if (!aethernetAddon) {
		return createFailedNativePasskeyProvider();
	}
	return createNativePasskeyProviderForAddon(aethernetAddon, true);
}
