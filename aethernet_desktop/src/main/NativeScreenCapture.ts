...
function loadMacNativeScreenCaptureAddon(): NativeAddonLoadResult {
	try {
		const addon = requireModule('@aethernet/mac-screen-capture') as MacNativeScreenCaptureModule;
		if (addon.loadError) {
...
function loadLinuxNativeScreenCaptureAddon(): NativeAddonLoadResult {
	try {
		const addon = requireModule('@aethernet/linux-screen-capture') as LinuxNativeScreenCaptureModule;
		if (addon.loadError) {
			const detail = describeAddonLoadError(addon.loadError);
...
function loadWindowsNativeScreenCaptureAddon(): NativeAddonLoadResult {
	try {
		const addon = requireModule('@aethernet/win-game-capture') as WindowsNativeScreenCaptureModule;
		if (addon.loadError) {
			const detail = describeAddonLoadError(addon.loadError);
