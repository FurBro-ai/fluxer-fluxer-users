...
function loadAddon(): MacTccModule | null {
	...
	try {
		const mod = requireModule('@aethernet/mac-tcc') as MacTccModule;
		if (mod.loadError) {
			logger.info('@aethernet/mac-tcc reported load error', {error: mod.loadError});
			cached = null;
			return cached;
		}
		cached = mod;
		...
		logger.info('@aethernet/mac-tcc not available; TCC pre-flight disabled', {error});
		cached = null;
		return cached;
	}
}
