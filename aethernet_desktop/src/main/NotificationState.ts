...
function loadWinShellModule(): WinShellModule | null {
	...
	try {
		const mod = requireModule('@aethernet/win-shell') as WinShellModule;
		if (mod.loadError) {
			logger.warn('@aethernet/win-shell load error; Windows notification state unavailable', {error: mod.loadError});
			cachedWinShellModule = null;
			return cachedWinShellModule;
		}
		...
		logger.warn('Failed to load @aethernet/win-shell; Windows notification state unavailable', {error});
		cachedWinShellModule = null;
		return cachedWinShellModule;
	}
}
