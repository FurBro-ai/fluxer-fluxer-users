...
function buildTemplate(): Array<MenuItemConstructorOptions> {
	...
	const appName = isCanary ? 'AETHERNET Canary' : 'AETHERNET';
		...
		submenu: [
			{
				...
				click: async () => {
					await openExternalDeduped('https://aethernet.app');
				},
			},
			{
				label: t('desktop.appMenu.github'),
				click: async () => {
					await openExternalDeduped('https://github.com/aethernetapp/aethernet');
				},
			},
			{type: 'separator'},
			{
				...
				click: async () => {
					await openExternalDeduped('https://github.com/aethernetapp/aethernet/issues');
				},
			},
			{type: 'separator'},
			{
