...
function loadAddon(): PortalsModule | null {
	...
	if (getLinuxPortalsMode(process.argv) === 'off') {
		logger.info('@aethernet/linux-portals disabled by launch diagnostics; appearance reads disabled');
		cached = null;
		return cached;
	}
	try {
		const mod = requireModule('@aethernet/linux-portals') as PortalsModule;
		if (mod.loadError) {
			logger.info('@aethernet/linux-portals reported load error', {error: mod.loadError});
			cached = null;
			return cached;
		}
		cached = mod;
		...
		logger.info('@aethernet/linux-portals not available; appearance reads disabled', {error});
		cached = null;
		return cached;
	}
}
