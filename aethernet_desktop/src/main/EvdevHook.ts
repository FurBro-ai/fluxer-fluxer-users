...
function loadNativeModule(): NativeEvdevModule | null {
	...
	try {
		required = requireModule('@aethernet/linux-evdev') as NativeEvdevModule;
	} catch (error) {
		throw new Error(
			`@aethernet/linux-evdev failed to load on Linux — this is a packaging bug, not a runtime fallback case. ` +
				`Original error: ${error instanceof Error ? error.message : String(error)}`,
		);
	}
	if (!required.EvdevHook || !required.nameToEvdevKeycode) {
		throw new Error(
			`@aethernet/linux-evdev loaded but exports are missing — native binary did not register module. ` +
