...
function loadMacSysctl(): MacSysctlModule | null {
	...
	try {
		macSysctlCache = requireModule('@aethernet/mac-sysctl') as MacSysctlModule;
	} catch (error) {
		throw new Error(
			`@aethernet/mac-sysctl failed to load on macOS; native diagnostics cannot continue. Original error: ${
...
function loadNativePlatformInfo(): NativePlatformInfoModule | null {
	...
	try {
		nativePlatformInfoCache = requireModule('@aethernet/platform-info') as NativePlatformInfoModule;
	} catch {
