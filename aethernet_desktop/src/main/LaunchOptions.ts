...
const PORTABLE_MODE_ARGS = new Set(['--aethernet-portable']);
const SAFE_MODE_ARGS = new Set(['--aethernet-safe-mode']);
const DISABLE_INPUT_HOOK_ARGS = new Set([
	'--aethernet-disable-native-input',
	'--aethernet-disable-input-hooks',
	'--aethernet-disable-global-key-hook',
]);
const INPUT_HOOK_MODE_ARGS = new Set(['--aethernet-input-hook', '--aethernet-native-input']);
const DISABLE_LINUX_PORTALS_ARGS = new Set(['--aethernet-disable-linux-portals']);
const LINUX_PORTALS_MODE_ARGS = new Set(['--aethernet-linux-portals', '--aethernet-portals']);
const DISABLE_NATIVE_NOTIFICATIONS_ARGS = new Set(['--aethernet-disable-native-notifications']);
const NATIVE_NOTIFICATIONS_MODE_ARGS = new Set(['--aethernet-native-notifications', '--aethernet-notifications']);
const DISABLE_NATIVE_AUDIO_ARGS = new Set(['--aethernet-disable-native-audio']);
const NATIVE_AUDIO_MODE_ARGS = new Set(['--aethernet-native-audio']);
const DISABLE_SPELLCHECK_ARGS = new Set(['--aethernet-disable-spellcheck', '--aethernet-disable-linux-spellcheck']);
const SPELLCHECK_MODE_ARGS = new Set(['--aethernet-spellcheck']);
const DISABLE_V8_CODE_CACHE_ARGS = new Set(['--aethernet-disable-v8-code-cache']);
...
export function getLinuxInputHookMode(argv: ReadonlyArray<string> = process.argv): LinuxInputHookMode {
	if (isSafeModeLaunch(argv) || hasFlag(argv, DISABLE_INPUT_HOOK_ARGS) || truthyEnv('AETHERNET_DISABLE_NATIVE_INPUT')) {
		return 'off';
	}
	const argMode = normalizeMode(
		getArgValue(argv, INPUT_HOOK_MODE_ARGS),
		['auto', 'off', 'evdev', 'native'],
		'--aethernet-input-hook',
	);
	return argMode ?? envMode('AETHERNET_INPUT_HOOK', ['auto', 'off', 'evdev', 'native']) ?? 'auto';
}
...
export function getLinuxPortalsMode(argv: ReadonlyArray<string> = process.argv): LaunchToggleMode {
	if (
		...
		truthyEnv('AETHERNET_DISABLE_LINUX_PORTALS')
	) {
		return 'off';
	}
	const argMode = normalizeMode(
		getArgValue(argv, LINUX_PORTALS_MODE_ARGS),
		['auto', 'off'],
		'--aethernet-linux-portals',
	);
	return argMode ?? envMode('AETHERNET_LINUX_PORTALS', ['auto', 'off']) ?? 'auto';
}
...
export function getNativeNotificationsMode(argv: ReadonlyArray<string> = process.argv): NativeNotificationsMode {
	if (
		...
		truthyEnv('AETHERNET_DISABLE_NATIVE_NOTIFICATIONS')
	) {
		return 'off';
	}
	const argMode = normalizeMode(
		...
		'--aethernet-native-notifications',
	);
	return argMode ?? envMode('AETHERNET_NATIVE_NOTIFICATIONS', ['auto', 'off', 'native', 'electron']) ?? 'auto';
}

export function getNativeAudioMode(argv: ReadonlyArray<string> = process.argv): LaunchToggleMode {
	if (
		isSafeModeLaunch(argv) ||
		hasFlag(argv, DISABLE_NATIVE_AUDIO_ARGS) ||
		truthyEnv('AETHERNET_DISABLE_NATIVE_AUDIO')
	) {
		return 'off';
	}
	const argMode = normalizeMode(getArgValue(argv, NATIVE_AUDIO_MODE_ARGS), ['auto', 'off'], '--aethernet-native-audio');
	return argMode ?? envMode('AETHERNET_NATIVE_AUDIO', ['auto', 'off']) ?? 'auto';
}

export function getSpellcheckLaunchMode(argv: ReadonlyArray<string> = process.argv): SpellcheckLaunchMode {
	if (isSafeModeLaunch(argv) || hasFlag(argv, DISABLE_SPELLCHECK_ARGS) || truthyEnv('AETHERNET_DISABLE_SPELLCHECK')) {
		return 'off';
	}
	const argMode = normalizeMode(
		getArgValue(argv, SPELLCHECK_MODE_ARGS),
		['off', 'auto', 'hunspell', 'system'],
		'--aethernet-spellcheck',
	);
	return argMode ?? envMode('AETHERNET_SPELLCHECK', ['off', 'auto', 'hunspell', 'system']) ?? 'default';
}

export function shouldDisableV8CodeCache(argv: ReadonlyArray<string> = process.argv): boolean {
	return (
		isSafeModeLaunch(argv) || hasFlag(argv, DISABLE_V8_CODE_CACHE_ARGS) || truthyEnv('AETHERNET_DISABLE_V8_CODE_CACHE')
	);
}
