...
const GENERATED_MARKER = '# X-Generated-By=aethernet-desktop';
...
export function ensureLinuxProtocolDesktopEntry(): void {
	...
	if (process.env.AETHERNET_DISABLE_DESKTOP_FILE === '1') {
		logger.debug('Skipping .desktop entry creation; AETHERNET_DISABLE_DESKTOP_FILE=1');
		try {
			app.setAsDefaultProtocolClient(APP_PROTOCOL);
		} catch (error) {
			logger.warn('Failed to register protocol client', {error});
