...
function loadSystemHunspell(): SystemHunspellNative | null {
	...
	try {
		const mod = requireModule('@aethernet/system-hunspell') as SystemHunspellNative;
		if (!mod.Hunspell || !mod.discoverSystemDictionaries) {
			throw mod.loadError ?? new Error('@aethernet/system-hunspell exports missing');
		}
		systemHunspellModule = mod;
		return mod;
	} catch (error) {
		log.warn('[Spellcheck] @aethernet/system-hunspell unavailable; falling back to hunspell-asm on Linux', {error});
		return null;
	}
}
...
const STATIC_CDN_ENDPOINT = process.env.AETHERNET_STATIC_CDN_ENDPOINT?.replace(/\/+$/, '') ?? '';
const DICTIONARY_DOWNLOAD_BASE_URL =
	process.env.AETHERNET_SPELLCHECK_DICTIONARY_BASE_URL ||
...
const BUILTIN_ALLOWLIST: ReadonlyArray<string> = ['AETHERNET', 'Aethernet', 'Hampus'];
...
const dictionaryFileUrl = (dict: SpellcheckDictionaryCatalogEntry, fileName: 'index.aff' | 'index.dic'): string => {
	if (!DICTIONARY_DOWNLOAD_BASE_URL) {
		throw new Error(
			'Set AETHERNET_SPELLCHECK_DICTIONARY_BASE_URL or AETHERNET_STATIC_CDN_ENDPOINT to download dictionaries',
		);
	}
...
export const registerSpellcheck = (webContents: WebContents): void => {
	...
	(
		webContents as WebContents & {
			__aethernetSpellcheckSetState?: typeof setState;
		}
	).__aethernetSpellcheckSetState = setState;
};
