...
function isInAethernetRoot(candidate: string, rootDir: string): boolean {
...
function loadWinShell(): ShellLinkBinding | null {
	...
	try {
		cachedBinding = requireModule('@aethernet/win-shell') as ShellLinkBinding;
	} catch (error) {
		console.warn('[WindowsShortcuts] Failed to load @aethernet/win-shell', error);
		return null;
	}
	if (cachedBinding.loadError) {
		console.warn('[WindowsShortcuts] @aethernet/win-shell load error', cachedBinding.loadError);
	}
...
async function repairOneShortcut(
	...
	const pointsAtAethernet = lnkContainsString(shortcutPath, rootDir) || isInAethernetRoot(shortcutPath, rootDir);
	const hasLegacyAumid = WINDOWS_LEGACY_APP_USER_MODEL_IDS.some((legacyAumid) =>
		lnkContainsString(shortcutPath, legacyAumid),
	);
	if (!pointsAtAethernet) return;
...
async function repairWindowsShortcutsAsync(repairPaths: WindowsShortcutRepairPaths): Promise<void> {
	...
	if (!binding || !binding.createShortcut) {
		console.warn('[WindowsShortcuts] @aethernet/win-shell unavailable; skipping shortcut repair');
		return;
	}
...
export function repairWindowsShortcuts(): void {
	...
	repairWindowsShortcutsAsync(repairPaths).catch((error) => {
		console.warn('[WindowsShortcuts] Failed to repair Aethernet shortcuts', error);
	});
}
