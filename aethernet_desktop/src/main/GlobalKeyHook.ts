...
function nativeModuleNameForPlatform(): string | null {
	switch (process.platform) {
		case 'linux':
			return '@aethernet/linux-input-hook';
		case 'darwin':
			return '@aethernet/macos-input-hook';
		case 'win32':
			return '@aethernet/windows-input-hook';
		default:
			return null;
	}
}
...
function linuxUaccessGrantScript(): string {
	return [
		...
		"cat > /etc/udev/rules.d/70-aethernet-input.rules <<'EOF'",
		'# Grants the active local desktop user access to input devices for Aethernet system-wide shortcuts.',
...
async function grantLinuxEvdevAccess(): Promise<LinuxEvdevGrantResult> {
	...
	if (isFlatpakRuntime()) {
		return {success: false, needsRelogin: false, error: 'Enable Flatpak input device access, then restart Aethernet.'};
	}
	if (!findPkexec()) {
		return {
			success: false,
	...
	const uaccess = await runPkexec(['/bin/sh', '-c', linuxUaccessGrantScript(), 'aethernet-input-access', username]);
	if (!uaccess.success) {
