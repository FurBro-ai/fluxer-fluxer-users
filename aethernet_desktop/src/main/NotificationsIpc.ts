...
function assertLinuxNotificationsModule(
	...
	if (!mod.FreedesktopNotifications) {
		throw new Error(
			`@aethernet/linux-notifications loaded but exports no FreedesktopNotifications. ` +
				`Surface: ${JSON.stringify(describeLinuxNotificationsSurface(mod))}`,
		);
	}
	if (typeof mod.FreedesktopNotifications !== 'function') {
		throw new Error(
			`@aethernet/linux-notifications FreedesktopNotifications export is not constructable. ` +
				`Surface: ${JSON.stringify(describeLinuxNotificationsSurface(mod))}`,
		);
	}
}
...
function assertLinuxNotificationClient(
	...
	throw new Error(
		`@aethernet/linux-notifications FreedesktopNotifications ABI mismatch: missing instance method(s) ` +
...
function loadLinuxNotificationsModule(): NativeNotificationsModule | null {
	...
	try {
		mod = requireModule('@aethernet/linux-notifications') as NativeNotificationsModule;
	} catch (error) {
		throw new Error(
			`@aethernet/linux-notifications failed to load on Linux — this is a packaging bug, not a runtime fallback case. ` +
...
function ensureLinuxNativeClient(getMainWindow: () => BrowserWindow | null): NativeNotificationClient {
	...
	if (!linuxNotificationsModule?.FreedesktopNotifications) {
		throw new Error('@aethernet/linux-notifications is unavailable on Linux; refusing Electron Notification fallback.');
	}
