...
function loadWinShell(): WinShellBinding | null {
	...
	try {
		cachedWinShellBinding = requireModule('@aethernet/win-shell') as WinShellBinding;
	} catch (error) {
		log.warn('[Autostart] Failed to load @aethernet/win-shell:', error);
		cachedWinShellBinding = null;
	}
	if (cachedWinShellBinding?.loadError) {
		log.warn('[Autostart] @aethernet/win-shell load error:', cachedWinShellBinding.loadError);
	}
...
function loadLinuxPortals(): LinuxPortalsBinding | null {
	...
	try {
		cachedLinuxPortalsBinding = requireModule('@aethernet/linux-portals') as LinuxPortalsBinding;
	} catch (error) {
		log.warn('[Autostart] Failed to load @aethernet/linux-portals:', error);
		cachedLinuxPortalsBinding = null;
	}
	if (cachedLinuxPortalsBinding?.loadError) {
		log.warn('[Autostart] @aethernet/linux-portals load error:', cachedLinuxPortalsBinding.loadError);
	}
...
function getFlatpakAutostartCommandline(): Array<string> {
	const command = process.env.AETHERNET_FLATPAK_COMMAND?.trim() || 'aethernet';
