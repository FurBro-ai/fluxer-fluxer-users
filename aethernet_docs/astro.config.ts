...
import {aethernetDarkCodeTheme, aethernetLightCodeTheme} from './src/styles/CodeTheme';
import {rehypeTableColumnWidths} from './src/table/RehypeTableColumns.ts';

const docsEndpoint = new URL(docsPublicEndpoint());
const staticDirectory = fileURLToPath(new URL('../aethernet_static/', import.meta.url));
...
export default defineConfig({
	...
	integrations: [
		starlight({
			...
			description: 'Aethernet external API and protocol reference.',
			expressiveCode: {
				styleOverrides: {
				...
				themes: [aethernetDarkCodeTheme, aethernetLightCodeTheme],
			},
			...
			social: [
				{
					href: 'https://github.com/aethernetapp/aethernet',
					icon: 'github',
					label: 'GitHub',
				},
			],
