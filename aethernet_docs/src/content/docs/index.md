...
title: Aethernet API
description: Start using the Aethernet API.
---

Aethernet is a self-hostable chat platform.
...
Start with `GET /.well-known/aethernet` on the instance origin. It requires no authentication and allows cross-origin requests.

```text
GET https://example.com/.well-known/aethernet
```

For the hosted instance, use `https://aethernet.app/.well-known/aethernet`.

Read base URLs from the response's [endpoints](/http-api/instance/#instance-endpoints-object). Do not derive them from the origin or hard-code Aethernet domains.
