...
<sup>1</sup> Each entry is trimmed, and Aethernet skips one that is then empty and any repeat of an entry already seen, so neither appears in `processed` or `errors`. An empty array is accepted and purges nothing
