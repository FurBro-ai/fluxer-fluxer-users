...
The unit and the quantity together define the span a redeemed code grants. Aethernet adds the span to the redeemer's entitlement anchor at redemption time. Lifetime gifts are not supported.
...
Each code is 32 characters drawn from the uppercase letters, the lowercase letters, and the digits. Aethernet creates it unredeemed and records the system account with ID `0` as its creator, which is the creator the [gift object](/http-api/gifts/#gift-object) reports to the redeemer.
