...
The Media Proxy serves Aethernet attachments, image assets, themes, entrance sound audio, and signed external media. It also accepts uploads authorised by a capability, a signed grant in the upload URL.
...
| X-Aethernet-Version<sup>6</sup> | string | The serving Aethernet build identifier, or `dev` when none is configured |
