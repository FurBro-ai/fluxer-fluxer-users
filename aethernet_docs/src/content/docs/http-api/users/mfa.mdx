...
Multi-factor authentication asks for a second proof of identity after the account password. Aethernet accepts a TOTP authenticator app and WebAuthn credentials such as a passkey. A one-use backup code works instead of a TOTP code, and [sudo mode](#sudo-mode) reuses these factors to protect every security-sensitive operation in the API.

Every route here requires a non-bot user session. Aethernet rejects an account in suspicious activity state with 403 `ACCOUNT_SUSPICIOUS_ACTIVITY`, including on the sudo routes.

:::caution[A challenge is one-use and context-bound]
Aethernet consumes a backup code, a WebAuthn registration challenge, and a WebAuthn sudo challenge on first use. A challenge is bound to the issuing user and operation context and expires after five minutes, so a registration challenge cannot be redeemed as a sudo assertion.
:::

## TOTP parameters

Aethernet validates a TOTP code as a six-digit HMAC-SHA-1 one-time password over a 30-second time step counted from the Unix epoch. It accepts one time step of clock skew on either side. A submitted code is exactly six decimal digits, and Aethernet rejects any other value. The shared secret is Base32, and Aethernet ignores whitespace, hyphens, and trailing padding when it decodes one.

The client generates the secret, presents it to the user, and submits it with a code derived from it when calling [Enable TOTP MFA](#enable-totp-mfa). Aethernet never generates or returns a TOTP secret.

:::caution[A TOTP code can pass twice]
Aethernet rejects an accepted code for 30 seconds if it is sent again. The skew keeps a code valid up to 90 seconds, so it passes again once those 30 seconds lapse. Wait for the next time step.
:::
...
Aethernet lowercases the submitted value and each stored code, then removes every character outside `a-z` and `0-9`. `abcdefgh` and `AbCd EfGh` both match the issued code `abcd-efgh`.
:::
...
Aethernet trims the submitted value, uppercases it, and removes every hyphen before it compares. A wrong value fails with `INVALID_VERIFICATION_CODE` on the path `code`.
:::
...
Aethernet refuses a resend for 30 seconds after the previous send on the same ticket. That refusal is HTTP 429 with a `Retry-After` computed from the moment the next send becomes available. It is independent of the route bucket and of the 15-minute send controls.

## Sudo mode

Sudo mode is a short-lived proof that the human in front of the session is still the account holder. An operation that requires it accepts a valid `X-Aethernet-Sudo-Mode-JWT` request header or the [sudo verification object](#sudo-verification-object) fields inside the JSON body.
...
When Aethernet issues no fresh token it echoes the request's `X-Aethernet-Sudo-Mode-JWT` value back unchanged and never re-signs it, so the five-minute window never slides. Aethernet echoes the value even when it fails verification.
:::
...
Aethernet verifies every assertion against the relying party identifier and the allowed origins configured for the instance. The credential's signature counter strictly increases, unless both the stored and reported counters are zero, which is how an authenticator without a counter appears.
...
Aethernet never returns the credential's public key, signature counter, or reported transports.
...
Aethernet accepts additional extension result fields.
...
Aethernet accepts additional credential property fields.
...
Aethernet emits no other PublicKeyCredential request options member, so `hints` and `extensions` never appear on this object.
...
<sup>4</sup> Aethernet requests a preferred discoverable credential and preferred user verification, and requires neither, so `requireResidentKey` is `false`

<sup>5</sup> Aethernet requests `none`, so no attestation statement is retained

<sup>6</sup> Always present with `credProps` set to `true`, the only extension Aethernet requests
...
<sup>1</sup> Aethernet supplies the account username in both `name` and `displayName`
...
<sup>1</sup> The only member Aethernet ever sets, always `true` on [WebAuthn registration options](#webauthn-registration-options-object)
...
Aethernet checks sudo mode first, so an account that already holds a WebAuthn credential proves it with that credential.
...
The body extends the [sudo verification object](#sudo-verification-object) with the fields below, and an existing sudo proof is sent in the `X-Aethernet-Sudo-Mode-JWT` request header.
...
The body extends the [sudo verification object](#sudo-verification-object) with the field below, and an existing sudo proof is sent in the `X-Aethernet-Sudo-Mode-JWT` request header.
...
The body can be omitted, and any supplied body is an object with no fields. Aethernet strips unknown keys before it handles the request.
...
Aethernet stores a ticket holding the code and its expiry, and one verification email goes to the account address. No account field changes and no backup code is issued.
...
Aethernet clears the ticket's stored proof on every resend. A client that already holds a proof needs [Verify MFA backup codes challenge code](#verify-mfa-backup-codes-challenge-code) again before [Regenerate MFA backup codes](#regenerate-mfa-backup-codes) accepts it.
:::
...
Aethernet replaces the ticket's code, send time, and code expiry, which invalidates the previous code. The stored proof is cleared and one verification email goes to the account address.
...
Aethernet charges the per-ticket verification allowance before it reads the code, so a wrong code and a correct code both draw on it.
...
This operation sets no `X-Aethernet-Sudo-Mode-JWT` response header.
...
The body is a [sudo verification object](#sudo-verification-object). An existing sudo proof is sent in the `X-Aethernet-Sudo-Mode-JWT` request header.
...
Aethernet issues a registration challenge for the current user. It expires after five minutes and can be redeemed once.
...
The account needs a verified email and fewer than 10 registered credentials. This route accepts no sudo verification fields of its own. Aethernet verifies the attestation against the instance relying party identifier and its allowed origins, and does not require user verification.
...
Aethernet consumes the registration challenge before it re-checks the credential limit and before it verifies the attestation. Request fresh registration options after any failure.
:::
...
Aethernet issues a sudo challenge for the current user. It expires after five minutes and can be redeemed once by submitting it as `webauthn_challenge` alongside `webauthn_response` in a [sudo verification object](#sudo-verification-object).
