...
A submission may use either the standard or the URL-safe base64 alphabet, with or without padding. Aethernet always returns the standard alphabet with padding.
...
Aethernet applies no bound to any individual field of the message, so every length, range, and enumeration membership below describes what the first-party client writes and reads.

## Rejection grounds

Aethernet reports every entry in the resulting `errors` array against the path `synced_preferences`. It checks the encoded string first.
...
One entry names one grouping in the favourites view. The client chooses the identifier, so it is not a [snowflake](/snowflakes/) and Aethernet never resolves it.
...
The `privacy` field stores client privacy behaviour. Aethernet reads none of it, and none of it changes the privacy fields of the [user settings object](/http-api/users/#user-settings-object), which control what Aethernet itself discloses about the account.
...
The `local_spam_overrides` field stores client-local classifications that override the [SPAMMER public user flag](/http-api/users/#public-user-flags) in display only. Aethernet neither reads these lists nor changes any flag because of them.
...
Both maps are keyed by an arbitrary string, and Aethernet stores and returns any key unchanged. The first-party client uses the identifiers below, and a key outside this set has no defined playback effect.
...
The `search_engines` field stores the selected external providers. Every identifier names a provider the client defines, and Aethernet neither validates nor resolves one.
