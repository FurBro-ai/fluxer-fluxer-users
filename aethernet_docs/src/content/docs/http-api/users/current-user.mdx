...
<sup>1</sup> The value cannot be `everyone` or `here` and cannot contain `aethernet` or `system message`, on every instance including a self-hosted one
...
To skip the body proof fields, send the sudo token in the `X-Aethernet-Sudo-Mode-JWT` request header.
...
Aethernet rejects an account with an outstanding required action. The one exemption is a body that has `email_token` and nothing beyond the sudo verification fields. Any other defined field removes the exemption.
...
Aethernet consumes the avatar control before it detects identical content, and clearing the avatar consumes nothing. A clearing request consumes the banner control, and so does a request whose value is unchanged.
...
The route issues no sudo token and returns no `X-Aethernet-Sudo-Mode-JWT` response header.
...
Aethernet checks guild ownership before it accepts the deletion, so an owner of any guild transfers or deletes each guild first.
:::
