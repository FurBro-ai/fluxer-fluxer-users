...
Aethernet answers a field-level failure with 400 and a top-level `errors` array. Each element identifies one failed input field. The [validation error object](/http-api/#validation-error-object) documents the element shape.
...
The `path` of an element is the dot-joined position of the failed value, so a nested field appears as `embeds.0.title`. A failure with no field position uses the literal path `root`. Aethernet produces at most one element for each distinct pair of `path` and `code`.
...
Aethernet normalises empty values on all targets before validation runs. An empty string becomes `null` wherever it appears, including inside an array element. A nested object becomes `null` when it holds no members. It also becomes `null` when every one of its members is `null` after Aethernet has applied the same rule to each of them. The top-level object itself is never replaced, so a request that sends nothing still reaches the schema as an object and fails on the fields the schema requires.
...
A boundary schema constraint can name its own [validation code](#validation-error-code-registry). When it names none, Aethernet maps the failure to one of the codes below by the kind of constraint that failed.
...
A failure that has an HTTP status and no recognised Aethernet error code takes the fallback `code` its status selects below. Where an operation or one of the registries below names the error, Aethernet keeps that more specific code. An unclassified 401, 413, 422, or 429 falls back to `GENERAL_ERROR`.

Aethernet answers an unrecognised failure with 500 `INTERNAL_SERVER_ERROR` and a generic message that names no detail. A failure that has a registered API code but no status returns 400, except `GENERAL_ERROR`, which returns 500.
...
Your IP address {ipAddress} has been permanently blocked from the Aethernet API by platform administrators

### `GLOBAL_IP_TEMPORARILY_BANNED`

Your IP address {ipAddress} has been temporarily blocked from the Aethernet API for 24 hours because of abusive or unusual access patterns
...
### `NO_USERS_WITH_AETHERNETTAG_EXIST`
...
We don't send verification texts to this country. Use a mobile number from another country, or email support@aethernet.app and a person will review your account
...
Your carrier says this number isn't in service. Check the number and try again, or email support@aethernet.app if it's correct

### `PHONE_NUMBER_NOT_MOBILE`

This isn't a mobile number, so it can't receive our text. Use a mobile number, or email support@aethernet.app if you think that's wrong
...
We couldn't verify this number automatically. Email support@aethernet.app and a person will review your account
...
Username can't contain "aethernet" or "system message"
...
### `VANITY_URL_CODE_CANNOT_CONTAIN_AETHERNET`

Vanity URL code can't contain "aethernet"
