...
<sup>1</sup> Aethernet resolves the recurring pair and the gift pair separately. Each pair takes the first currency for which both of its prices are configured, so the pairs can resolve to different currencies
...
Aethernet selects the subscription by preferring an `active` or `trialing` mirror, then a `past_due`, `unpaid` or `incomplete` one, then anything else. Ties break on the latest current period end.

## Billing subscription object

One mirrored payment provider subscription. Aethernet passes every member through, so `status`, `plan_interval` and `plan_currency` are the provider's own values.
...
The checkout catalogue resolved for the request. A client can render prices without a second call to [get price IDs](#get-price-ids). Aethernet resolves one catalogue on every request, and a country with its own localised currency always resolves to that currency.
...
Aethernet reports an unresolvable catalogue as null here, and the request still succeeds. [Get price IDs](#get-price-ids) answers 400 `STRIPE_ERROR` for the same condition.
...
The deployment must configure a complete recurring pair and a complete gift pair for at least one currency that the requested country prefers. Aethernet answers 400 `STRIPE_ERROR` when it does not.
...
<sup>1</sup> Aethernet trims surrounding whitespace and requires exactly two characters, and any other length fails validation with `STRING_LENGTH_EXACT`. The request-time geolocation country wins, so this value is read only when Aethernet cannot geolocate the request, and the default currency preference order applies when neither resolves
...
The operation creates an externally hosted billing portal session that returns to the Aethernet premium callback when the user closes it. Nothing is written to Aethernet state, and any change the user makes in the portal reaches Aethernet through [receive Stripe webhook](/http-api/billing/#receive-stripe-webhook).
...
A subscription that has a schedule discards its pending billing cycle change and ends in cancellation at the current period end. Aethernet then rewrites the premium end from the refreshed subscription. Premium access continues through the recorded end, and [receive Stripe webhook](/http-api/billing/#receive-stripe-webhook) later applies the final downgrade.
...
Aethernet releases any subscription schedule, which discards a pending billing cycle change, then clears period end cancellation. The account's cancellation flag becomes false. Every account session receives [User Update](/gateway/events/#user-update).
...
The operation is idempotent. For an account that already belongs to the guild, Aethernet grants the Visionary role again and changes nothing else. The join bypasses the guild ban check and the join risk gate, so a banned account is added anyway.
