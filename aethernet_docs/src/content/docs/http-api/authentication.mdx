...
A successful sign-in creates one authentication session and issues its token. Aethernet sets no ceiling on live sessions and evicts none when a further session starts, so an account holds one session per sign-in until it revokes them through [terminate authentication sessions](#terminate-authentication-sessions).
...
There is no backup code completion route and `backup_code` never appears in `allowed_methods`. [Complete login with TOTP](#complete-login-with-totp) reads it from `code`, and only while the account holds the TOTP authenticator type. Aethernet never reports whether one remains.
:::
...
The approximate geolocation Aethernet derives from the IP address recorded for a session or a handoff.
...
<sup>1</sup> Aethernet sanitises the value before binding it to the state and discards a value that does not survive, which the [SSO completion response](#sso-completion-response-object) reports as the empty string. Sanitisation keeps the trimmed value only when it begins with a single `/`, is at most 2,048 characters, and contains no carriage return or line feed

<sup>2</sup> The accepted values are the instance default reported as `redirect_uri` by [get SSO status](#get-sso-status) and the mobile callback `aethernet://auth/sso/callback`, and any other value returns the field code `INVALID_URL_FORMAT`. The accepted value is bound to the state and reused at the token exchange
...
A provider subject is bound to exactly one Aethernet account, and an account accepts exactly one provider subject. A subject already linked to a different account, or a new subject for an account that already holds a different linked subject, returns the field code `SSO_IDENTITY_MISMATCH`.
:::

:::caution[A verified provider email adopts a local account]
When the subject is not yet linked, Aethernet adopts any existing account holding the verified email address in the provider claim and binds the subject to it, whether or not that account has a password or an authenticator. A provider that asserts an arbitrary verified address therefore takes over the matching local account. An operator MUST enable SSO only for a provider that controls the addresses it asserts.
:::

Aethernet refuses the adoption only when the account already holds a different provider subject.
...
Aethernet resolves the region from the client IP address, and an age below the minimum for that region returns the field code `MUST_BE_MINIMUM_AGE`. That minimum is 13 years unless account policy sets a different minimum for the region, and the applied minimum appears only in the localised message.
...
| X-Captcha-Token?<sup>1</sup> | string | The proof issued by the CAPTCHA provider |
| X-Captcha-Type?<sup>2</sup> | string | The provider that issued the proof, either `hcaptcha` or `turnstile` |
| X-Aethernet-Client-Properties?<sup>3</sup> | string | The base64-encoded JSON with the reporting client's `os` |
...
<sup>3</sup> Both values feed the [client info](#client-info-object) object recorded on the session, and `X-Aethernet-Client-Properties` is read only for a native Aethernet `User-Agent`
...
When the instance has disabled new-IP authorisation or sends no email, Aethernet authorises the client IP address silently and the login continues. An account that already has a second factor never enters IP authorisation.
...
Aethernet never compares the account's address against the one the token was issued for. An account that changes its address while a verification token is outstanding can therefore verify the new address with the old token.
:::
...
Password recovery verifies CAPTCHA when CAPTCHA is enabled. Aethernet consumes both the client IP address and email address allowances before it validates the address, and exhausting either returns 429.
...
Aethernet sends one 64-character reset token by email to an address that resolves to an account. The token is bound to that account and its current address, and it is valid for one hour. Nothing else changes. No Gateway Dispatch is emitted.
...
<sup>1</sup> The value becomes the account's new password, and Aethernet checks it against the public breached-password corpus, returning the field code `PASSWORD_IS_TOO_COMMON` on a match
...
Aethernet checks the replacement password against the same breached-password corpus described under [reset a password](#reset-a-password) before it spends the token.
...
The caller MUST send a valid sudo token in the `X-Aethernet-Sudo-Mode-JWT` header, or supply a password or MFA proof in the body.
...
<sup>1</sup> Each value is the base64url `id_hash` from [list authentication sessions](#list-authentication-sessions). Aethernet ignores an unknown identifier, and an empty array deletes nothing and still returns 204
...
An account that has neither a password nor a second factor satisfies sudo mode with no body proof and no sudo token, and Aethernet issues none. Every other account MUST present a valid sudo token, a password, or an MFA proof.
:::
...
Each named session is deleted and loses its Gateway session as [shared behaviour](#shared-behaviour) states. A fresh MFA proof issues a new sudo token, while an accepted incoming token is echoed unchanged. Aethernet sets whichever token results in the `X-Aethernet-Sudo-Mode-JWT` response header. A password proof on an account with no second factor issues no token, so the header is not set unless the request already had one. No Gateway Dispatch is emitted.
...
Aethernet sends the authorisation message to the address associated with the login attempt and marks the ticket's single resend used whether or not delivery succeeds. No account state changes. No Gateway Dispatch is emitted.
...
The request has no body. Aethernet derives the device metadata shown to the approving device from the request itself.
...
| X-Aethernet-Client-Properties? | string | The base64-encoded JSON with the reporting client's `os`, read only for a native Aethernet `User-Agent` |
...
Aethernet counts failed code attempts against the client IP address and blocks it after 5 failures in 15 minutes. A pending result records no failure, while an unknown or expired code records one.
...
<sup>1</sup> Aethernet normalises the value by removing hyphens and whitespace and upper-casing the rest, and the normalised result is exactly 12 characters from the handoff alphabet
...
Aethernet reads that token from the `Authorization` header, or from the body `token` field when the body supplies one.
