...
When an identifier names no stored expression, Aethernet returns 404 `UNKNOWN_EMOJI` or 404 `UNKNOWN_STICKER`. A stored expression whose guild no longer exists returns 404 `UNKNOWN_GUILD`.
...
<sup>4</sup> Aethernet detects this from the submitted image at creation, and the value never changes
