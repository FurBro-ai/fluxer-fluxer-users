...
Use the HTTP API to read and change resources. Discover the base URL through [`/.well-known/aethernet`](/http-api/instance/#get-instance-discovery). Third-party clients use `endpoints.api_public`, and the first-party web application uses `endpoints.api_client`.
...
Each direct file's `files[n]` index is also the `id` in its attachment metadata entry. Aethernet reads `n` from the field name, so part order sets no identity. The indices need not begin at zero and need not be consecutive.
:::
...
| X-Aethernet-Client-Properties?<sup>4</sup> | string | Base64-encoded JSON with the native client's `os`, read when an authentication session is created |
| X-Aethernet-Sudo-Mode-JWT?<sup>5</sup> | string | A sudo mode proof previously issued to the authenticated user |
...
<sup>4</sup> Read only for a native Aethernet `User-Agent`, at most 4096 characters, and only the `os` member is used
...
A client MUST NOT send an `X-Request-ID` it is not willing to see in logs, because Aethernet echoes and records the value without validating it.
...
| Field | Type | Description |
| --- | --- | --- |
| X-Aethernet-Version?<sup>1</sup> | string | The build version of the instance that served the request |
| X-Request-ID?<sup>2</sup> | string | The identifier assigned to the request |
| X-Aethernet-Sudo-Mode-JWT?<sup>3</sup> | string | The sudo mode token for the caller, present only on an operation that completes a sudo proof |
...
| Access-Control-Expose-Headers?<sup>5</sup> | string | The literal value `X-Aethernet-Version, ETag` |
...
A small set of routes exists only on the hosted Aethernet deployment. A self-hosted deployment answers one of them with 404 `NOT_FOUND`. [Deployment availability](/http-api/deployment-availability/) lists every hosted-only route and states how a client resolves the deployment kind before authenticating.
...
[Get instance discovery](/http-api/instance/#get-instance-discovery) on `/.well-known/aethernet`, [Get OpenAPI document](/http-api/instance/#get-openapi-document) on `/v1/openapi.json`, and [Get client geolocation](/http-api/instance/#get-client-geolocation) on `/v1/ip` set `Access-Control-Allow-Origin: *` in the operation itself. The wildcard stands for any origin outside the allow-list, and for an allowed origin the policy replaces it with that exact origin and sends `Vary: Origin`.

`Access-Control-Expose-Headers` is the value `X-Aethernet-Version, ETag`. Every other Aethernet response header, the rate limit headers and `X-Request-ID` included, is hidden from cross-origin script. `Access-Control-Allow-Headers` is `Content-Type, Authorization, X-Requested-With, Accept-Language, X-Request-ID, If-None-Match`, so a cross-origin client revalidates an [ETag](/http-api/experiments/#get-experiment-assignments) it was served.

:::caution[Same-host mutations require a matching origin]
A production deployment rejects a non-`GET` request whose `Host` is `web.aethernet.app` or `web.canary.aethernet.app` unless its `Origin` is exactly `https://` followed by that same host, returning 403 `INVALID_API_ORIGIN`. A request sent to the API host is unaffected.
:::
...
Aethernet produces at most one entry for each distinct pair of `path` and `code`, so a field that fails several equivalent constraints appears once.
