...
A user is a Aethernet account. Another account sees the [partial user object](#partial-user-object), and the owner sees the [user object](#user-object). Account mutation lives on [Current user](/http-api/users/current-user/).
...
| system? | boolean | Whether the account is a Aethernet system account, omitted when false |
...
<sup>11</sup> `premium_until` is the later of the subscription end and any stacked gift extension. Aethernet reports it even when premium entitlements are no longer active
...
Each entry is an exact string in the `required_actions` array of the [user object](#user-object), ordered by the registry order below. Aethernet removes a requirement the account already satisfies, and removes a requirement implied by a stricter retained one.
...
The complete account-wide settings record, with one field per setting. `synced_preferences` holds a base64-encoded Protobuf snapshot defined by the [user settings Protobuf reference](/http-api/users/settings-protobuf/). Aethernet stores that snapshot for clients and never interprets it.
...
<sup>6</sup> The message is `aethernet.user.preferences.v1.SyncedPreferences`. The empty string means nothing has been synced yet. The decoded message cannot exceed 262144 bytes, the 256 KiB ceiling
...
<sup>1</sup> The value cannot be `everyone` or `here` and cannot contain `aethernet` or `system message`, all compared case-insensitively
