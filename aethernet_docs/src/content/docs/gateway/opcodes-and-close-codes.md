...
<sup>1</sup> Aethernet never sends Opcode 9 with `d: true`, so an Invalid Session is never an instruction to resume

### Payload and when it is sent

| Op | Payload | When sent |
| --- | --- | --- |
| 0 | Event-specific data | Aethernet sends a named event with `t` and a session sequence with `s`<sup>1</sup> |
...
Aethernet resolves an inbound payload in this order.
...
A transport can end with no Aethernet application close code, as happens on a network failure, an intermediary reset, and an ordinary `1000` or `1001` close. An established session remains available for 60,000 ms after the transport ends, and a later transport end starts a new 60,000 ms window. Neither the window length nor the bounded replay history grows.
