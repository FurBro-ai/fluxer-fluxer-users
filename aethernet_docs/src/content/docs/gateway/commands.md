...
Aethernet charges every command that gets past those checks against the [source IP, session, and connection payload budgets](/gateway/limits-and-rate-limits/#connection-and-command-rate-limits) before it handles the opcode. Each session has its own session budget, so two sessions of one account never share one, and Aethernet skips that budget while the connection is unauthenticated.
...
When the session ends, Aethernet sends Opcode `9` with `d: false`, after which heartbeats are acknowledged again. A heartbeat that arrives in the short window between the session ending and that frame closes with `4007`.
...
`token` and `properties` are required. Aethernet ignores unknown fields. There is no `intents` field and no intent system.
...
Aethernet refuses a bot session that resolves to more than 2,500 guilds after any `shard` filter is applied. The connection closes with `4011` and reason `Sharding required`. A user session is never refused for its guild count.
...
`os`, `browser`, and `device` are required strings. The remaining fields are optional hints. Aethernet accepts and ignores unrecognised properties.
...
<sup>2</sup> The Snowflake names an emoji that exists. Aethernet drops the field from the published object, without failing the rest of it, when the account has no global expressions entitlement

<sup>3</sup> A single Unicode emoji. Aethernet strips the field before validation when `emoji_id` is supplied
...
Aethernet accepts a non-string `status`. Null and a Boolean publish the session as offline, and every other non-string value resolves to `online`.
...
<sup>4</sup> Only `true` and the string `"true"` set it, and Aethernet publishes `false` when the member lacks `STREAM` in the channel. The screenshare track uses this same connection, so setting the flag issues no grant and sends no [Voice Server Update](/gateway/events/#voice-server-update)
...
Every field is optional. A non-null `guild_id` or `channel_id` is a canonical decimal [snowflake](/snowflakes/) string, and a positive integer is also accepted. Every other value fails validation, and Aethernet drops the update with no close code and no event.
...
`latitude` and `longitude` accept a number or a string here, and Aethernet coerces both to a string.
...
Aethernet skips a guild this session is not connected to. A request that resolves to no connected guild produces no chunk.
...
<sup>1</sup> Aethernet keeps each guild's options for the session and applies them again when the session connects to that guild later
...
Aethernet applies each option only when its key is present, in the order `active`, `sync`, `member_list_channels`, `members`, `typing`.
...
`typing` is a Boolean, and Aethernet ignores any other value. It decides [Typing Start](/gateway/events/#typing-start) delivery for the guild independently of the active flag.
