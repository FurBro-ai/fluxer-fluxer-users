...
Read the release notes between the version you run and the one you are moving to, at [Releases](https://github.com/aethernetapp/aethernet/releases).

## Match the images to the stack files

The images come from `AETHERNET_IMAGE_TAG` in `.env`. The stack files come from a git ref, which is a branch or a tag in the Aethernet repository. The installer derives that ref from the image tag: `main` for `v1` or `latest`, and the tag string itself for anything else. `--ref` overrides the derivation. `docker compose pull` never updates a stack file, and refreshing a stack file never moves an image.

The repository holds no ref by the name of a pinned image tag. A release tags each image on its own, as `aethernet-api@2026.813.205040`, so pass `--ref` with that tag or with the commit it points at. Without that override the download fails with exit 4.
...
Upgrades change `AETHERNET_IMAGE_TAG` and add missing required secrets. Existing values are preserved, except for the unusable upload-relay placeholder described below.

The refreshed stack requires `AETHERNET_ERLANG_COOKIE`. `--update` generates a missing value as 64 hex characters. Without it, Compose commands fail with `set AETHERNET_ERLANG_COOKIE in .env`.

To write it by hand, run the generator in a shell:

```bash
printf '\nAETHERNET_ERLANG_COOKIE=%s\n' "$(openssl rand -hex 32)" >> .env
```

Do not paste `AETHERNET_ERLANG_COOKIE=$(openssl rand -hex 32)` into `.env`. Compose does not execute shell commands in that file.

`api` and `media-proxy` require `AETHERNET_MEDIA_PROXY_UPLOAD_RELAY_SECRET_BASE64` to decode to at least 32 bytes. `--update` replaces a missing or `CHANGE_ME` value with a generated secret. Both services must use the same value.

To write it by hand, run the generator in a shell the same way:

```bash
printf '\nAETHERNET_MEDIA_PROXY_UPLOAD_RELAY_SECRET_BASE64=%s\n' "$(openssl rand -base64 32)" >> .env
```

## The script is the reference

Read the [Linux and macOS installer](https://aethernet.dev/install.sh) or [Windows installer](https://aethernet.dev/install.ps1) before running it.
...
```
stat /srv/aethernet/docker-compose.proxy.yml: no such file or directory
```
...
curl -fsSL --proto '=https' --tlsv1.2 -o docker-compose.proxy.yml \
  https://raw.githubusercontent.com/aethernetapp/aethernet/main/deploy/self-hosting/docker-compose.proxy.yml
```

`main` is the ref for `AETHERNET_IMAGE_TAG=v1` and for `latest`. [Match the images to the stack files](#match-the-images-to-the-stack-files) has the ref a pinned tag needs.
...
`--dir` and `--ref` mean what they mean on an install, and the installer derives an omitted `--ref` from the `AETHERNET_IMAGE_TAG` line in `.env`. Every failure prints a sentence on standard error and exits non-zero, and the script header lists what each code means. A Postgres major version change is exit 3.
...
Both `https` and `wss` work. Update `AETHERNET_LIVEKIT_URL` in `.env` to the new path, or remove it to use the derived address, then run `docker compose up -d api`.
...
| --- | --- |
| `aethernet.dump` | Every account, message, guild and configuration row, in the Postgres custom format |
...
```cron
15 3 * * * cd /srv/aethernet && docker compose exec -T postgres pg_dump -U aethernet -d aethernet --format=custom > "backups/aethernet-$(date -u +\%Y\%m\%dT\%H\%M\%SZ).dump" && find backups -name 'aethernet-*.dump' -mtime +14 -delete
```

`/srv/aethernet` stands for the directory holding `.env`. Write it as an absolute path, because cron does not expand `~`. An unescaped `%` in a crontab is a newline, which is why every one above has a backslash.
...
```ini
AETHERNET_POSTGRES_HOST=db.example.com
AETHERNET_POSTGRES_PORT=5432
AETHERNET_POSTGRES_SSL=true
AETHERNET_S3_ENDPOINT=https://s3.eu-central-1.amazonaws.com
AETHERNET_S3_REGION=eu-central-1
AETHERNET_S3_FORCE_PATH_STYLE=false
```

`AETHERNET_S3_PUBLIC_ENDPOINT` follows `AETHERNET_S3_ENDPOINT` when it is not set on its own, and the credentials stay `AETHERNET_S3_ACCESS_KEY` and `AETHERNET_S3_SECRET_KEY`. `AETHERNET_S3_BUCKET_CDN`, `AETHERNET_S3_BUCKET_UPLOADS`, `AETHERNET_S3_BUCKET_DOWNLOADS`, `AETHERNET_S3_BUCKET_REPORTS` and `AETHERNET_S3_BUCKET_HARVESTS` name the buckets. The bundled object store creates whichever names they hold, and a store outside the stack needs those buckets to exist already.

`AETHERNET_KV_URL`, `AETHERNET_NATS_URL`, `AETHERNET_SEARCH_URL` and `AETHERNET_LIVEKIT_INTERNAL_URL` move the other bundled services the same way, and `AETHERNET_NATS_JETSTREAM_URL` and `AETHERNET_SVC_NATS_URL` follow `AETHERNET_NATS_URL` when they are not set on their own.
...
docker compose exec -T postgres pg_restore -U aethernet -d aethernet --clean --if-exists \
  < backups/record-20260831T120000Z/aethernet.dump
docker compose up -d
```
...
docker compose stop
docker compose up -d --wait postgres
docker compose cp backups\record-20260831T120000Z\aethernet.dump postgres:/tmp/aethernet.dump
docker compose exec -T postgres pg_restore -U aethernet -d aethernet --clean --if-exists /tmp/aethernet.dump
docker compose exec -T postgres rm /tmp/aethernet.dump
docker compose up -d
```
...
```bash
docker compose stop
docker run --rm -v aethernet_seaweedfs-data:/data \
...
`tar` extracts over whatever is already on the volume, so the `find` empties it first. In PowerShell write `${PWD}` instead of `$PWD`. Success is an existing attachment URL answering 200 again. The `aethernet_` prefix is the Compose project name, which `docker-compose.yml` sets to `aethernet`.
...
```bash
docker compose exec -T postgres pg_dump -U aethernet -d aethernet --format=custom \
  > backups/pre-major.dump
docker compose down
docker volume rm aethernet_postgres-data
```
...
`AETHERNET_IMAGE_TAG=v1` tracks the latest compatible release, so every upgrade can move the instance. Set it to a release tag to hold one version and upgrade on your own schedule.
...
The tag applies only to the Aethernet images. `caddy`, `postgres`, `valkey`, `nats`, `meilisearch`, `seaweedfs` and `livekit` are pinned in `docker-compose.yml`, and they move only when an upgrade refreshes that file.

## Change the domain or the passkey relying party

`AETHERNET_DOMAIN` supplies the default `AETHERNET_PASSKEY_RP_ID`, which is the domain a passkey is tied to. A passkey works only under the identifier it was created with.

:::danger[Changing either value invalidates every passkey]
Passkeys registered under the old `AETHERNET_PASSKEY_RP_ID` stop working and cannot be recovered. Every user has to register a new one, so keep another sign-in method working before you change it.
:::

Changing `AETHERNET_DOMAIN` also changes the certificate the edge requests, the CORS origins the API accepts, and the admin OAuth redirect URI.
...
`docker system prune -a` reclaims more. It also deletes every image that no container references, including ones unrelated to Aethernet.
