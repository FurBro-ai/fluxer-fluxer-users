...
title: Get started
description: Bring up a Aethernet instance with Docker Compose, from a fresh host to first login.
---
...
By the end you have a Aethernet instance on a hostname you own, running the web app, HTTP API, Gateway, admin dashboard, uploads, search and voice signalling in Docker containers on one machine. Allow about 30 minutes, most of it spent downloading images.
...
Choose the hostname the instance answers on. Step 4 writes it into `.env` as `AETHERNET_DOMAIN`, and every example below uses `chat.example.com` in its place.
...
```bash
curl -fsSLO https://aethernet.dev/install.sh
curl -fsSLO https://aethernet.dev/install.sh.sha256
...
```powershell
Invoke-WebRequest https://aethernet.dev/install.ps1 -OutFile install.ps1
Invoke-WebRequest https://aethernet.dev/install.ps1.sha256 -OutFile install.ps1.sha256
...
`~/aethernet` then holds `docker-compose.yml`, `docker-compose.proxy.yml`, `tunnel.compose.yml`, `Caddyfile` and `.env.example`, plus a `.env` readable only by you. Every value that ships as `CHANGE_ME` in `.env.example` is a fresh random value. Every command from here on runs in that directory.
...
`AETHERNET_PUBLIC_PORT` is the port of the public address, and every endpoint the services advertise follows it. It does not move what the host publishes, so `AETHERNET_HTTPS_PORT` has to name the same port. Move `AETHERNET_PUBLIC_SCHEME` with them when the new port serves plain HTTP, and set `AETHERNET_HTTP_PORT` instead of `AETHERNET_HTTPS_PORT`. `.env.example` ships beside `.env` and writes out both complete recipes.
...
Leave `AETHERNET_PUBLIC_ORIGIN` commented out, or keep it consistent with the public scheme, domain and port. Conflicting addresses can cause sign-in failures and setup loops. See [The public origin](/operator/configuration/#the-public-origin).
...
| `--email <address>` | Contact address written as `AETHERNET_VAPID_EMAIL`. Prompted when absent |
| `--dir <path>` | Working directory. Default `~/aethernet`, or the current directory when it holds an instance |
...
for p in /_health /api/_health /gateway/_health /media/_health /.well-known/aethernet /; do
...
```powershell
$base = 'https://chat.example.com'
foreach ($p in '/_health', '/api/_health', '/gateway/_health', '/media/_health', '/.well-known/aethernet', '/') {
  '{0} {1}' -f (Invoke-WebRequest "$base$p" -UseBasicParsing).StatusCode, $p
}
```

Every path returns 200. `/.well-known/aethernet` lists the endpoints clients use, with your own hostname in them.
...
The desktop client opens the hosted web app for its release channel, so reach your own instance in a browser. [Issue #1088](https://github.com/aethernetapp/aethernet/issues/1088) tracks that, and is being worked on.
...
docker compose exec -T postgres pg_dump -U aethernet -d aethernet --format=custom \
  > "backups/aethernet-$(date -u +%Y%m%dT%H%M%SZ).dump"
```
...
```powershell
mkdir -Force backups
docker compose exec -T postgres pg_dump -U aethernet -d aethernet --format=custom -f /tmp/aethernet.dump
docker compose cp postgres:/tmp/aethernet.dump backups/aethernet.dump
docker compose exec -T postgres rm /tmp/aethernet.dump
```

The uploads copy stops the stack, because a live copy can catch a file mid-write. Compose prefixes volume names with the project name, which the stack sets to `aethernet`:

```bash
docker compose stop
docker run --rm -v aethernet_seaweedfs-data:/data -v "$PWD/backups:/backup" alpine:3.22 \
...
File issues and follow development on [GitHub](https://github.com/aethernetapp/aethernet).
