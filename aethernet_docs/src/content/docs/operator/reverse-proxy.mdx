...
description: Serving a Aethernet instance from nginx, Caddy, Traefik, HAProxy, Apache, or a tunnel.
---
...
Aethernet runs behind any proxy that terminates TLS and forwards WebSocket upgrades.
...
On a first install, `sh install.sh --tls proxy` writes that line and `AETHERNET_EDGE_BIND` into `.env` for you, so there is nothing to add by hand. [Installer flags](/operator/get-started/#installer-flags) lists it with the rest.

The port binds `127.0.0.1:8080` by default. `AETHERNET_EDGE_BIND` sets both the interface and the host port. Move the port when something else on the host already holds `8080`:

```ini
AETHERNET_EDGE_BIND=127.0.0.1:8081
```
...
```ini
AETHERNET_EDGE_BIND=0.0.0.0:8080
```
...
With `AETHERNET_PUBLIC_SCHEME=https` the admin CSRF cookie is `__Host-csrf_token`, which browsers accept only over HTTPS.
...
The variables below tell the instance the address browsers use. Aethernet reads neither `X-Forwarded-Proto` nor `X-Forwarded-Host`, so you keep these correct by hand:

```ini
AETHERNET_DOMAIN=chat.example.com
...
Leave `AETHERNET_PUBLIC_ORIGIN` commented out, or keep it consistent with all three settings. Conflicting addresses can break sign-in and setup. See [The public origin](/operator/configuration/#the-public-origin).
...
Caddy trusts no proxy by default. If another proxy or CDN sits in front of it, configure Caddy's global `trusted_proxies` option and Aethernet's [trusted proxies](#trusted-proxies) so requests retain the real client address.
...
services:
  edge:
    ...
    labels:
      ...
      traefik.http.routers.aethernet.rule: Host(`chat.example.com`)
      traefik.http.routers.aethernet.entrypoints: websecure
      traefik.http.routers.aethernet.tls.certresolver: letsencrypt
      traefik.http.services.aethernet.loadbalancer.server.port: "8080"
...
frontend aethernet
    ...
    default_backend aethernet_edge

backend aethernet_edge
...
```yaml
tunnel: aethernet
credentials-file: /etc/cloudflared/aethernet.json
...
Nginx Proxy Manager appends to `X-Forwarded-For`, so the visitor's own value arrives in front of the real address. The edge takes the rightmost entry that is not in `AETHERNET_EDGE_TRUSTED_PROXIES`, which is the one Nginx Proxy Manager appended. Users on a LAN or a VPN inside the default `private_ranges` are the exception and resolve to the forged entry, so narrow `AETHERNET_EDGE_TRUSTED_PROXIES` to the proxy's address in that layout.
...
| `/.well-known/aethernet` | Instance discovery |
...
`AETHERNET_EDGE_TRUSTED_PROXIES` names the peer addresses whose `X-Forwarded-For` the edge believes. It defaults to `private_ranges`, which Caddy expands to `192.168.0.0/16`, `172.16.0.0/12`, `10.0.0.0/8`, `127.0.0.1/8`, `fd00::/8`, and `::1`, so a proxy on the same host or the same Docker network needs no change.
...
```ini
AETHERNET_EDGE_TRUSTED_PROXIES=private_ranges 203.0.113.10/32
```
...
```ini
AETHERNET_EDGE_TRUSTED_PROXIES=private_ranges 100.101.102.103/32
```
...
```bash
docker network inspect aethernet_aethernet -f '{{(index .IPAM.Config 0).Gateway}}'
```
...
`private_ranges` trusts the whole LAN. With `AETHERNET_EDGE_BIND=0.0.0.0:8080`, any host on that network can impersonate any client address. Firewall the port to the proxy host.
:::
...
Hosting LiveKit on a hostname other than `AETHERNET_DOMAIN` means widening the Content-Security-Policy the web app runs under. The line goes in `.env`, beside `AETHERNET_DOMAIN`:

```ini
AETHERNET_CSP_EXTRA_CONNECT_SRC=wss://livekit.example.com:7881
```
