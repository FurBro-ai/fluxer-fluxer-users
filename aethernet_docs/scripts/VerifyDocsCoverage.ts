...
import {extractRoutesFromControllers} from '@aethernet/openapi/src/extractors/RouteExtractor';
import type {OpenAPIDocument} from '@aethernet/openapi/src/OpenAPITypes';
...
const MAIN_SPEC = path.join(REPO_ROOT, 'aethernet_api/src/api/openapi/openapi.json');
const ADMIN_SPEC = path.join(REPO_ROOT, 'aethernet_admin/openapi-admin.json');
const MEDIA_PROXY_SERVER_DIR = path.join(REPO_ROOT, 'aethernet_media_proxy/src/server');

const BLUESKY_OAUTH_CONTROLLER = 'aethernet_api/src/api/bluesky/BlueskyOAuthController.ts';
const DOWNLOAD_CONTROLLER = 'aethernet_api/src/api/download/DownloadController.ts';

const MAIN_SPEC_EXEMPT = new Map<string, {file: string; anchor: string; reason: string}>([
	[
		'GET /oauth2/authorize',
		{
			file: 'aethernet_api/src/api/oauth/OAuth2Controller.ts',
	...
	[
		'GET /openapi.json',
		{
			file: 'aethernet_api/src/api/openapi/OpenAPIController.ts',
...
const OUT_OF_BAND_CREDENTIAL = new Map<string, OutOfBandRoute>([
	[
		...
		{
			reason:
				'x-aethernet-rpc-auth must timing-safe-equal Config.internal.gatewayRpcAuthToken, and an empty token rejects every caller. The caller is aethernet_gateway',
			documentedIn: null,
		},
	],
	[
	...
	[
		...
		{
			reason:
				'a Standard Webhooks delivery signed with AETHERNET_EMAIL_WEBHOOK_SECRET, which answers 404 Email not enabled while email is off',
...
const EXEMPTION_RULES: ReadonlyArray<ExemptionRule> = [
	{
		...
		anchors: [
			{
				file: 'aethernet_api/src/api/app/ControllerRegistry.ts',
				anchor: "if (config.dev.testModeEnabled || config.nodeEnv === 'development') {",
			},
			{file: 'aethernet_api/src/api/test/TestHarnessController.ts', anchor: 'function ensureHarnessAccess'},
		],
	...
	},
	{
		...
		anchors: [{file: 'aethernet_docs/src/content/docs/operator/reverse-proxy.mdx', anchor: '/_metrics'}],
	...
	},
	{
		...
		anchors: [{file: 'aethernet_api/src/api/app/ControllerRegistry.ts', anchor: 'if (config.sms.enabled) {'}],
...
const controllerRoutes = extractRoutesFromControllers(
	await registrationSources(path.join(REPO_ROOT, 'aethernet_api/src')),
);
...
for (const [route, anchor] of [...MEDIA_PROXY_ROUTES, ...MEDIA_PROXY_ASSET_PREFIXES]) {
	if (!mediaProxySource.includes(anchor)) {
		staleAnchors.push(`${route}: aethernet_media_proxy/src/server no longer contains ${anchor}`);
	}
}
...
console.log('main API and admin API (from route registration)');
{
	...
	const documentedNotRegistered = [...documentedMain, ...documentedAdmin]
		...
		.map(([shape, route]) => `${shape}  (${route.file}) is documented but aethernet_api registers no such route`)
		.sort();
	...
	const staleEntries = [
		...
		.filter(([shape]) => !registered.has(shape))
		.map(([shape, list]) => `${list} names ${shape}, which aethernet_api no longer registers`)
		.sort();
	...
	console.log('  aethernet_admin is a Rust crate and its 83 HTML panel routes are NOT checked here. It serves');
	console.log('  zero API operations, so the admin API above is the /admin routes aethernet_api registers.');
	console.log('  aethernet_media_proxy and aethernet_app_proxy are Rust too. The media proxy is checked against');
...
failures += section(
	...
	[...aliasShapes.values()]
		...
		.map((alias) => `${alias.shape}  (${alias.file}) names a route aethernet_api does not register`)
		.sort(),
);
failures += section(
...
failures += section('documented but not served by aethernet_media_proxy', mediaProxyProblems);

console.log('gateway dispatch events');
{
	const gatewayConstants = await readFile(path.join(REPO_ROOT, 'aethernet_api/src/api/constants/Gateway.ts'), 'utf8');
	...
	const gatewayDirectory = path.join(REPO_ROOT, 'aethernet_gateway/src');
	...
	console.log('  a documented event counts as real if it is in the union, or appears in aethernet_gateway');
...
{
	const erl = await readFile(path.join(REPO_ROOT, 'aethernet_gateway/src/utils/constants.erl'), 'utf8');
...
console.log('enum names and error codes');
{
	...
	for (const sub of [
		...
		'aethernet_api/src',
		'aethernet_admin/src',
		'aethernet_gateway/src',
		'aethernet_media_proxy/src',
	]) {
...
console.log('rate limit buckets, limits and windows');
{
	...
	const configDirectory = path.join(REPO_ROOT, 'aethernet_api/src/api/rate_limit_configs');
...
{
	const constants = await readFile(path.join(REPO_ROOT, 'aethernet_media_proxy/src/constants.rs'), 'utf8');
...
console.log('self-hosting guide against deploy/self-hosting');
{
	...
	const shellSecrets = parseShellKeys('aethernet_secret_keys', 'the install.sh secret list');
	const shellNonSecrets = parseShellKeys('aethernet_non_secret_keys', 'the install.sh non-secret list');
	const powershellSecrets = parsePowerShellKeys('AethernetSecretKeys', 'the install.ps1 secret list');
	const powershellNonSecrets = parsePowerShellKeys('AethernetNonSecretKeys', 'the install.ps1 non-secret list');
	...
	const shellUpgrade = parseShellKeys('aethernet_upgrade_secret_keys', 'the install.sh upgrade secret list');
	const powershellUpgrade = parsePowerShellKeys('AethernetUpgradeSecretKeys', 'the install.ps1 upgrade secret list');
	...
	const expectedSecretKind = (name: string): string => {
		if (name === 'AETHERNET_VAPID_PUBLIC_KEY') {
			return 'vapid_public';
		}
		if (name === 'AETHERNET_VAPID_PRIVATE_KEY') {
	...
	const shippedAssets = (await readdir(path.join(REPO_ROOT, 'deploy/self-hosting')))
		...
		.sort();
	const shellStackFiles = shellRows('aethernet_stack_files', 'FILES', 'the install.sh download list');
	const powershellStackFiles: Array<string> = [];
	for (const row of powershellRows('AethernetStackFiles', 'the install.ps1 download list')) {
	...
	const shellComposeNames = shellRows('aethernet_compose_names', 'NAMES', 'the install.sh compose name list');
	const powershellComposeNames: Array<string> = [];
	for (const row of powershellRows('AethernetComposeNames', 'the install.ps1 compose name list')) {
		const parsed = row.match(/^\s*'([^']+)'\s*$/u);
		if (parsed == null) {
	...
	const vapidEmailDerivation = 'AETHERNET_VAPID_EMAIL: ${AETHERNET_VAPID_EMAIL:-admin@${AETHERNET_DOMAIN}}';
	if (!compose.includes(vapidEmailDerivation)) {
		problems.push('docker-compose.yml no longer derives AETHERNET_VAPID_EMAIL from AETHERNET_DOMAIN');
	} else if (assigned.has('AETHERNET_VAPID_EMAIL')) {
		problems.push(
			'.env.example assigns AETHERNET_VAPID_EMAIL, so a copied .env defeats the admin@AETHERNET_DOMAIN derivation',
		);
	}
	...
	if (livekitConfig != null) {
		const coupled: ReadonlyArray<readonly [string, string]> = [
			['tcp_port', '${AETHERNET_LIVEKIT_TCP_PORT:-7881}'],
			['udp_port', '${AETHERNET_LIVEKIT_UDP_PORT:-7882}'],
		...
		for (const mapping of [
			'${AETHERNET_LIVEKIT_TCP_PORT:-7881}:${AETHERNET_LIVEKIT_TCP_PORT:-7881}',
			'${AETHERNET_LIVEKIT_UDP_PORT:-7882}:${AETHERNET_LIVEKIT_UDP_PORT:-7882}/udp',
		]) {
	...
	for (const candidate of namedServices) {
		if (!candidate.endsWith('-shard') && !candidate.startsWith('aethernet-') && !composeServices.has(candidate)) {
			continue;
		}
		...
		const isImage = candidate.startsWith('aethernet-');
		if (!isShard && !isImage) {
			continue;
		}
...
console.log('unthrottled routes and global bucket claims');
{
	...
	const middleware = await readFile(
		path.join(REPO_ROOT, 'aethernet_api/src/api/middleware/RateLimitMiddleware.ts'),
		'utf8',
	);
...
console.log('error registry and abuse signal weights');
{
	...
	const banner = await readFile(
		path.join(REPO_ROOT, 'aethernet_api/src/api/middleware/AbusiveIpAutoBanner.ts'),
		'utf8',
	);
...
{
	const erlConstants = await readFile(path.join(REPO_ROOT, 'aethernet_gateway/src/utils/constants.erl'), 'utf8');
	const sharding = await readFile(path.join(REPO_ROOT, 'aethernet_gateway/src/gateway/gateway_sharding.erl'), 'utf8');
	const abuse = await readFile(
		path.join(REPO_ROOT, 'aethernet_gateway/src/gateway/session_abuse_protection.erl'),
		'utf8',
	);
	const heartbeat = await readFile(
		path.join(REPO_ROOT, 'aethernet_gateway/src/gateway/gateway_handler_heartbeat.erl'),
		'utf8',
	);
...
console.log('snowflake layout');
{
	...
	const epochMatch = core.match(/export const AETHERNET_EPOCH = (\d+);/u);
	...
	if (epochMatch == null || workerBits == null || sequenceBits == null) {
		problems.push('could not read AETHERNET_EPOCH, WORKER_ID_BITS or SEQUENCE_BITS from source');
	} else {
...
console.log('OK: every registered aethernet_api route is documented or covered by an exemption rule, and the');
...
async function verifyInstallerExecution(installerRoot: string): Promise<Array<string>> {
	...
	const DOCKER_STUB = [
		...
		"	'compose config') echo 'ghcr.io/aethernetapp/aethernet-api:v1' ;;",
		'esac',
		'exit 0',
		'',
	].join('\n');
	const CURL_STUB = [
		...
		'	prev=$arg',
		'done',
		'[ -z "$out" ] || printf \'name: aethernet\\nservices:\\n  api:\\n    image: stub\\n\' > "$out"',
		'',
	].join('\n');

	const sandbox = await mkdtemp(path.join(tmpdir(), 'aethernet-installer-'));
	try {
		...
		await writeFile(path.join(instance, '.env'), 'AETHERNET_DOMAIN=x.example\nAETHERNET_IMAGE_TAG=2026.813.205040\n');
		await writeFile(path.join(instance, 'docker-compose.yml'), 'name: aethernet\nservices:\n  api:\n    image: stub\n');
		...
		await writeFile(
			...
			'AETHERNET_DOMAIN=x.example\nAETHERNET_IMAGE_TAG=2026.813.205040\n',
		);
		await writeFile(
			path.join(composeYmlInstance, 'compose.yml'),
			'name: aethernet\nservices:\n  api:\n    image: stub\n',
		);
