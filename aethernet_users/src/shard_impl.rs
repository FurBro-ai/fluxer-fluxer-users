...
use aethernet_svc::{postgres, postgres::KeyPart};
...
const AETHERNET_SYSTEM_DISCRIMINATOR: i32 = 0;
...
impl UsersShard {
    ...
    async fn get_full_user(&self, user_id: i64) -> anyhow::Result<Option<User>> {
        if user_id == AETHERNET_SYSTEM_USER_ID {
            return Ok(Some(aethernet_system_user()));
        }
        self.caches
    ...
    async fn get_partial_user(&self, user_id: i64) -> anyhow::Result<Option<UserPartial>> {
        if user_id == AETHERNET_SYSTEM_USER_ID {
            return Ok(Some(aethernet_system_user().to_partial()));
        }
        self.caches
    ...
    async fn get_partial_users(&self, mut user_ids: Vec<i64>) -> anyhow::Result<Vec<UserPartial>> {
        ...
        for user_id in user_ids {
            if user_id == AETHERNET_SYSTEM_USER_ID {
                partials.push(aethernet_system_user().to_partial());
                continue;
            }
...
fn aethernet_system_user() -> User {
    User {
        user_id: AETHERNET_SYSTEM_USER_ID,
        username: AETHERNET_SYSTEM_USERNAME.to_owned(),
        discriminator: AETHERNET_SYSTEM_DISCRIMINATOR,
        bot: Some(true),
        system: Some(true),
        email: None,
        email_verified: None,
...
impl ShardService for UsersShard {
    ...
    fn render_prometheus_metrics(&self, output: &mut String) {
        let _ = writeln!(
            output,
            "# TYPE aethernet_users_shard_cache_generation_bumps_total counter"
        );
        let _ = writeln!(
            output,
            "aethernet_users_shard_cache_generation_bumps_total {}",
            self.caches.generation_bumps.load(Ordering::Relaxed)
        );
        let _ = writeln!(
            output,
            "# TYPE aethernet_users_shard_cache_generation_stripes gauge"
        );
        let _ = writeln!(
            output,
            "aethernet_users_shard_cache_generation_stripes {}",
...
mod tests {
    ...
    fn test_user(user_id: i64) -> User {
        let mut user = aethernet_system_user();
    ...
    #[test]
    fn aethernet_system_user_partial_is_virtual_id_zero() {
        let partial = aethernet_system_user().to_partial();

        assert_eq!(partial.user_id, 0);
        assert_eq!(partial.username, "Aethernet");
    ...
    fn postgres_user_decoder_maps_tagged_kv_payload() {
        let user = decode_postgres_user(json!({
            "user_id": {"__aethernet_type": "bigint", "value": "42"},
            "username": "ada",
            "discriminator": 7,
            "authenticator_types": {"__aethernet_type": "set", "value": [1, 2]},
            "traits": {"__aethernet_type": "set", "value": ["founder"]},
            "acls": {"__aethernet_type": "set", "value": ["admin"]},
            "flags": {"__aethernet_type": "bigint", "value": "9007199254740991"},
            "date_of_birth": {"__aethernet_type": "local_date", "value": "1815-12-10"},
            "premium_since": {"__aethernet_type": "date", "value": "2026-06-15T12:34:56.789Z"},
            "version": 3
        }))
        .unwrap();
