...
#[cfg(test)]
use aethernet_common::user_flags::{USER_FLAG_PARTNER, USER_FLAG_STAFF_HIDDEN};
...
impl UserPartial {
    pub fn to_api_partial(&self) -> ApiUserPartial {
        if self.user_id == AETHERNET_SYSTEM_USER_ID {
...
fn aethernet_system_user() -> ApiUserPartial {
    ApiUserPartial {
...
mod tests {
    ...
    fn api_partial_maps_system_user_like_the_api_mapper() {
        let system = partial_with_flags(AETHERNET_SYSTEM_USER_ID, USER_FLAG_STAFF).to_api_partial();

        assert_eq!(system.username, AETHERNET_SYSTEM_USERNAME);
